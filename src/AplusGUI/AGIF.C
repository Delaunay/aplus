...
static A g_colorMapDefaultBg(MSDisplayServer* server_)
{
    A z = aplus_nl;
    if (server_ != 0)
        z = gi(server_->defaultBackground());
    return z;
}
...
static A g_colorMapDefaultFg(MSDisplayServer* server_)
{
    A z = aplus_nl;
    if (server_ != 0)
        z = gi(server_->defaultForeground());
    return z;
}
    ...
    if (QS(fc_)) {
        ...
        if (fc_->t == Ct) {
            ...
            if (QS(p.a[0])) {
                ...
                fmt->p[0] = (I)f;
                fmt->p[1] = (I)(fc_->n == 2) ? (int)p.a[1]->p[0] : OutputObject.defaultPrecision();
                AClientData* ac = new AClientData((A)aplus_nl, (A)fmt, (A)fc_);
                varData->outFunc((AFunc)cdipvSymbol, ac);
                ...
                showError("Invalid 'out' Function Specification");
        } else if (fc_->t == Et && fc_->n == 0)
            varData->outFunc((AFunc)0, 0);
        else
            showError("Invalid 'out' Function Specification");
    } else
        showError("Invalid 'out' Function Specification");
...
static A StringVectorToNestedArray(const MSStringVector& svec_)
{
    ...
    for (int i = 0; i < len; ++i)
        z->p[i] = (I)gsv(0, (char*)svec_(i).string());
    return z;
}
...
{
    if (w_->widgetType() == AplusArray::symbol() || w_->widgetType() == AplusTable::symbol() || w_->widgetType() == AplusView::symbol() || w_->widgetType() == AplusMatrix::symbol()) {
...
static void wDefaultMatrixColorChanged(MSWidgetView* w_, unsigned long new_)
{
    if (w_->widgetType() == AplusMatrix::symbol()) {
        ...
        if (am->rowIndexBg() == AVariableData::defaultMatrixColor() && am->colIndexBg() == AVariableData::defaultMatrixColor() && am->cornerIndexBg() == AVariableData::defaultMatrixColor()) {
            am->rowIndexBg(new_);
            am->colIndexBg(new_);
...
static A getCB(MSWidgetView* pWidgetView_, A cbname_)
{
    ...
    if (cbname_->t == Ct)
        cbnamep = (char*)cbname_->p;
    else if (cbname_->t == Et && QS(*cbname_->p))
        cbnamep = XS(*cbname_->p)->n;
...
static void s_varInFunc(A a_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        varData->inFunc((AFunc)0, 0);
    else
        showError("Invalid 'in' Function Specification");
}
...
static void s_varGeoFunc(A a_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        varData->geoFunc((AFunc)0, 0);
    else
        showError("Invalid 'geometry' Function Specification");
}
...
static void s_varDoneFunc(A a_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        varData->doneFunc((AFunc)0, 0);
    else
        showError("Invalid 'done' Function Specification");
}
...
static void s_varTitleFunc(A a_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        varData->titleFunc((AFunc)0, 0);
    else
        showError("Invalid 'title' Function Specification");
}
...
static void s_varFgFunc(A a_, A fc_)
{
    ...
    } else if (It == fc_->t && 1 == fc_->n || isNull(fc_) == MSTrue)
        varData->foreground(fc_);
    else
        showError("Invalid 'color' Function Specification");
}
...
static void s_varTitleColorFunc(A a_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        varData->titleColorFunc((AFunc)0, 0);
    else
        showError("Invalid 'titleColor' Function Specification");
}
...
static void s_varRoFunc(A a_, A fc_)
{
    ...
    } else if (It == fc_->t && 1 == fc_->n)
        varData->setReadOnly((int)fc_->p[0]);
    else if (isNull(fc_) == MSTrue)
        varData->setReadOnly(0);
    else
        showError("Invalid 'protect' Function Specification");
}
...
static void s_varFontFunc(A a_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        showError("Invalid 'font' Function Specification");
}
...
static void s_varTitleFontFunc(A a_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        varData->titleFontFunc((AFunc)0, 0);
    else
        showError("Invalid 'titleFont' Function Specification");
}
...
{
    (isNull(class_) == MSTrue || Et != class_->t || 1 > class_->n) ? getVarData(a_)->varClass((S)0) : getVarData(a_)->varClass(XS(*class_->p));
}
...
static A g_data(MSWidgetView* pWidgetView_)
{
    ...
    if (pWidgetView_->model() == 0) {
        if (dbg_tmstk)
            cout << "Widget with no model" << endl;
        arg = 0;
    } else {
...
void s_data(MSWidgetView* pWidgetView_, A aobject_)
{
    ...
    if (pWidgetView_->widgetType() == AplusShell::symbol() || pWidgetView_->widgetType() == AplusPopup::symbol()) {
...
I v_data(MSWidgetView* pWidgetView_, A a_)
{
    ...
    if (pWidgetView_ != 0 && pWidgetView_->model() != 0) {
        ...
        if (dbg_tmstk)
            cout << "No model defined in v_data" << endl;
        return 1;
    }
}
...
int verify(V v_, A a_)
{
    ...
    if (v_->attr != 0) {
        ...
        if (pWidgetView && pWidgetView->model()) {
            ...
            if (dbg_tmstk)
                cout << "Warning:  No model defined in verify" << endl;
            r = MSFalse;
        }
    }
...
{
    return (A)(pWidgetView_->server()->pasteBuffer().length() > 0) ? gsv(0, (char*)pWidgetView_->server()->pasteBuffer().string()) : gsv(0, "");
}
static void s_primary(MSWidgetView* pWidgetView_, A data_)
{
    if (QA(data_) && data_->t == Ct) {
        if (data_->r == 2) {
            ...
            (void)pWidgetView_->copyPrimary((char*)data_->p, (int)data_->n);
    }
}
...
{
    if (Ct == names_->t)
        sh_->workspacePresence(MSString((char*)names_->p));
    else if (Et == names_->t) {
...
static void s_shellIconData(MSShell* sh_, A a_)
{
    if (QA(a_) && a_->t == Et && a_->n == 3) {
        P p;
        p.i = a_->p;
        if (p.a[0]->t == It && p.a[0]->r == 0 && p.a[1]->t == It && p.a[1]->r == 0 && p.a[2]->t == It && p.a[2]->r == 1) {
            int width = (int)p.a[0]->p[0];
            int height = (int)p.a[1]->p[0];
            char* bits = new char[p.a[2]->n];
            for (int i = 0; i < p.a[2]->n; i++)
                bits[i] = (char)p.a[2]->p[i];
...
{
    if (server_ == 0)
        server_ = MSDisplayServer::defaultDisplayServer();
    return MSFalse;
}
...
A xwindowHashStat(MSDisplayServer* server_)
{
    ...
    for (int i = 0; i < size; i++)
        r->p[i] = (I)server_->widgetHashTable()->chainLength(i);
    return r;
}
...
A shadowHashStat(MSDisplayServer* server_)
{
    ...
    A r = gv(It, size);
    for (int i = 0; i < size; i++)
        r->p[i] = (I)server_->shadowHashTable()->chainLength(i);
    return r;
}
...
static A bXGCReferenceStat(void)
{
    ...
    for (int i = 0; i < MSGC::xgcList().count(); i++)
        r->p[i] = (I)MSGC::xgcList().data(i)->count();
    return r;
}
...
{
    if (dbg_tmstk)
        cout << "PrintLayoutInfo is currently unsupported." << endl;
    return;
}
