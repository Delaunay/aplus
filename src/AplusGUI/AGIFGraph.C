...
{
    if (fc_->t == Et && fc_->n == 2)
        tp_->lineStyleFunc((AFunc)cdipv, fc_);
    else if (isNull(fc_) == MSTrue)
        tp_->lineStyleFunc((AFunc)0, 0);
    else
        showError("Invalid 'lineStyle' Function Specification");
}
...
{
    if (fc_->t == Et && fc_->n == 2)
        tp_->lineWidthFunc((AFunc)cdipv, fc_);
    else if (isNull(fc_) == MSTrue)
        tp_->lineWidthFunc((AFunc)0, 0);
    else
        showError("Invalid 'lineWidth' Function Specification");
}
...
{
    if (fc_->t == Et && fc_->n == 2)
        tp_->lineColorFunc((AFunc)cdipv, fc_);
    else if (isNull(fc_) == MSTrue)
        tp_->lineColorFunc((AFunc)0, 0);
    else
        showError("Invalid 'lineColor' Function Specification");
}
...
{
    if (fc_->t == Et && fc_->n == 2)
        tp_->fillColorFunc((AFunc)cdipv, fc_);
    else if (isNull(fc_) == MSTrue)
        tp_->fillColorFunc((AFunc)0, 0);
    else
        showError("Invalid 'fillColor' Function Specification");
}
...
{
    if (fc_->t == Et && fc_->n == 2)
        tp_->legendFunc((AFunc)cdipv, fc_);
    else if (isNull(fc_) == MSTrue)
        tp_->legendFunc((AFunc)0, 0);
    else
        showError("Invalid 'legend' Function Specification");
}
...
{
    if (fc_->t == Et && fc_->n == 2)
        tp_->traceStyleFunc((AFunc)cdipv, fc_);
    else if (isNull(fc_) == MSTrue)
        tp_->traceStyleFunc((AFunc)0, 0);
    else
        showError("Invalid 'traceStyle' Function Specification");
}
...
{
    if (fc_->t == Et && fc_->n == 2)
        tp_->traceSymbolFunc((AFunc)cdipv, fc_);
    else if (isNull(fc_) == MSTrue)
        tp_->traceSymbolFunc((AFunc)0, 0);
    else
        showError("Invalid 'traceSymbol' Function Specification");
}
...
{
    if (fc_->t == Et && fc_->n == 2)
        tp_->traceSymbolSizeFunc((AFunc)cdipv, fc_);
    else if (isNull(fc_) == MSTrue)
        tp_->traceSymbolSizeFunc((AFunc)0, 0);
    else
        showError("Invalid 'traceSymbolSize' Function Specification");
}
...
{
    if (fc_->t == Et && fc_->n == 2)
        tp_->altYaxisFunc((AFunc)cdipv, fc_);
    else if (isNull(fc_) == MSTrue)
        tp_->altYaxisFunc((AFunc)0, 0);
    else
        showError("Invalid 'altYaxis' Function Specification");
}
...
{
    if (fc_->t == Et && fc_->n == 2)
        tp_->altXaxisFunc((AFunc)cdipv, fc_);
    else if (isNull(fc_) == MSTrue)
        tp_->altXaxisFunc((AFunc)0, 0);
    else
        showError("Invalid 'altXaxis' Function Specification");
}
...
{
    if (fc_->t == Et && fc_->n == 2)
        tp_->gradientFunc((AFunc)cdipv, fc_);
    else if (isNull(fc_) == MSTrue)
        tp_->gradientFunc((AFunc)0, 0);
    else
        showError("Invalid 'gradient' Function Specification");
}
...
static A g_tracePieAngle(AplusTraceSet* tp_)
{
    A r = gf(tp_->pieAngle());
    return r;
}
...
static void s_tracePieAspectRatio(AplusTraceSet* tp_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        tp_->pieAspectRatio(value_->t == Ft ? p.f[0] : (double)p.i[0]);
    }
}
static A g_tracePieAspectRatio(AplusTraceSet* tp_)
{
    A r = gf(tp_->pieAspectRatio());
    return r;
}
...
static void s_tracePieDepthFactor(AplusTraceSet* tp_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        tp_->pieDepthFactor(value_->t == Ft ? p.f[0] : (double)p.i[0]);
    }
}
static A g_tracePieDepthFactor(AplusTraceSet* tp_)
{
    A r = gf(tp_->pieDepthFactor());
    return r;
}
...
void graphYlabelFormatFunc(AplusGraph* gr_, A fc_, int axis_)
{
    if (QA(fc_) && fc_->t == Ct) {
        ...
        gr_->y_labelFormatFunc((AFunc)0, 0, axis_);
    else
        showError("Invalid 'y format label' Function Specification");
}
...
void graphXlabelFormatFunc(AplusGraph* gr_, A fc_, int axis_)
{
    if (QA(fc_) && fc_->t == Ct) {
        ...
        gr_->x_labelFormatFunc((AFunc)0, 0, axis_);
    else
        showError("Invalid 'x format label' Function Specification");
}
...
void graphYlabelFunc(AplusGraph* gr_, A fc_, int axis_)
{
    if (fc_->t == Et && fc_->n == 2) {
        ...
        gr_->y_labelFunc((AFunc)0, 0, axis_);
    else
        showError("Invalid 'y label' Function Specification");
}
...
void graphXsubLabelFunc(AplusGraph* gr_, A fc_, int axis_)
{
    if (fc_->t == Et && fc_->n == 2) {
        ...
        gr_->x_subLabelFunc((AFunc)0, 0, axis_);
    else
        showError("Invalid 'x label' Function Specification");
}
...
void graphXlabelFunc(AplusGraph* gr_, A fc_, int axis_)
{
    if (fc_->t == Et && fc_->n == 2) {
        ...
        gr_->x_labelFunc((AFunc)0, 0, axis_);
    else
        showError("Invalid 'x label' Function Specification");
}
...
static void s_graphXYcoordinate(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft) && value_->n == 2) {
        P p;
        p.i = value_->p;
        gr_->xCursorValue(MSBottom, value_->t == Ft ? p.f[0] : (double)p.i[0]);
...
static void s_graphXXYYcoordinate(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft) && value_->n == 2) {
        P p;
        p.i = value_->p;
        gr_->xCursorValue(MSTop, value_->t == Ft ? p.f[0] : (double)p.i[0]);
...
static void s_graphSessionPeriod(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->sessionPeriod(value_->t == Ft ? p.f[0] : (double)p.i[0]);
    }
}
static A g_graphSessionPeriod(AplusGraph* gr_)
{
    A r = gf(gr_->sessionPeriod());
    return r;
}
...
static void s_graphSessionOffset(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->sessionOffset(value_->t == Ft ? p.f[0] : (double)p.i[0]);
    }
}
static A g_graphSessionOffset(AplusGraph* gr_)
{
    A r = gf(gr_->sessionOffset());
    return r;
}
...
static void s_graphTpoPriceInc(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->tpoPriceInc(value_->t == Ft ? p.f[0] : (double)p.i[0]);
    }
}
static A g_graphTpoPriceInc(AplusGraph* gr_)
{
    A r = gf(gr_->tpoPriceInc());
    return r;
}
...
static void s_graphTpoPeriod(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->tpoPeriod(value_->t == Ft ? p.f[0] : (double)p.i[0]);
    }
}
static A g_graphTpoPeriod(AplusGraph* gr_)
{
    A r = gf(gr_->tpoPeriod());
    return r;
}
...
static void s_graphTpoOpen(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->tpoOpen(value_->t == Ft ? p.f[0] : (double)p.i[0]);
    }
}
static A g_graphTpoOpen(AplusGraph* gr_)
{
    A r = gf(gr_->tpoOpen());
    return r;
}
...
static void s_graphTpoClose(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->tpoClose(value_->t == Ft ? p.f[0] : (double)p.i[0]);
    }
}
static A g_graphTpoClose(AplusGraph* gr_)
{
    A r = gf(gr_->tpoClose());
    return r;
}
...
static A g_graphXminimum(AplusGraph* gr_)
{
    A r = gf(gr_->axisMinimum(MSBottom));
    return r;
}
...
{
    if (isNull(value_) == MSTrue) {
        gr_->axisMinimum(MSGraph::Unset, MSTop);
        return;
    }
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->axisMinimum(value_->t == Ft ? p.f[0] : (double)p.i[0], MSTop);
    }
}
static A g_graphXXminimum(AplusGraph* gr_)
{
    A r = gf(gr_->axisMinimum(MSTop));
    return r;
}
...
{
    if (isNull(value_) == MSTrue) {
        gr_->axisMaximum(MSGraph::Unset, MSBottom);
        return;
    }
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->axisMaximum(value_->t == Ft ? p.f[0] : (double)p.i[0], MSBottom);
    }
}
static A g_graphXmaximum(AplusGraph* gr_)
{
    A r = gf(gr_->axisMaximum(MSBottom));
    return r;
}
...
{
    if (isNull(value_) == MSTrue) {
        gr_->axisMaximum(MSGraph::Unset, MSTop);
        return;
    }
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->axisMaximum(value_->t == Ft ? p.f[0] : (double)p.i[0], MSTop);
    }
}
static A g_graphXXmaximum(AplusGraph* gr_)
{
    A r = gf(gr_->axisMaximum(MSTop));
    return r;
}
...
{
    if (isNull(value_) == MSTrue) {
        gr_->axisMinimum(MSGraph::Unset, MSLeft);
        return;
    }
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->axisMinimum(value_->t == Ft ? p.f[0] : (double)p.i[0], MSLeft);
    }
}
static A g_graphYminimum(AplusGraph* gr_)
{
    A r = gf(gr_->axisMinimum(MSLeft));
    return r;
}
...
{
    if (isNull(value_) == MSTrue) {
        gr_->axisMinimum(MSGraph::Unset, MSRight);
        return;
    }
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->axisMinimum(value_->t == Ft ? p.f[0] : (double)p.i[0], MSRight);
    }
}
static A g_graphYYminimum(AplusGraph* gr_)
{
    A r = gf(gr_->axisMinimum(MSRight));
    return r;
}
...
{
    if (isNull(value_) == MSTrue) {
        gr_->axisMaximum(MSGraph::Unset, MSLeft);
        return;
    }
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->axisMaximum(value_->t == Ft ? p.f[0] : (double)p.i[0], MSLeft);
    }
}
static A g_graphYmaximum(AplusGraph* gr_)
{
    A r = gf(gr_->axisMaximum(MSLeft));
    return r;
}
...
{
    if (isNull(value_) == MSTrue) {
        gr_->axisMaximum(MSGraph::Unset, MSRight);
        return;
    }
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->axisMaximum(value_->t == Ft ? p.f[0] : (double)p.i[0], MSRight);
    }
}
static A g_graphYYmaximum(AplusGraph* gr_)
{
    A r = gf(gr_->axisMaximum(MSRight));
    return r;
}
...
static void s_graphXincrement(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->axisLabelIncrement(value_->t == Ft ? p.f[0] : (double)p.i[0], MSBottom);
    }
}
static A g_graphXincrement(AplusGraph* gr_)
{
    A r = gf(gr_->axisLabelIncrement(MSBottom));
    return r;
}
...
static void s_graphXXincrement(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->axisLabelIncrement(value_->t == Ft ? p.f[0] : (double)p.i[0], MSTop);
    }
}
static A g_graphXXincrement(AplusGraph* gr_)
{
    A r = gf(gr_->axisLabelIncrement(MSTop));
    return r;
}
...
static void s_graphYincrement(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->axisLabelIncrement(value_->t == Ft ? p.f[0] : (double)p.i[0], MSLeft);
    }
}
static A g_graphYincrement(AplusGraph* gr_)
{
    A r = gf(gr_->axisLabelIncrement(MSLeft));
    return r;
}
...
static void s_graphYYincrement(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->axisLabelIncrement(value_->t == Ft ? p.f[0] : (double)p.i[0], MSRight);
    }
}
static A g_graphYYincrement(AplusGraph* gr_)
{
    A r = gf(gr_->axisLabelIncrement(MSRight));
    return r;
}
...
static void s_graphScreenLeftMargin(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->margin(value_->t == Ft ? p.f[0] : (double)p.i[0], MSLeft);
    }
}
static A g_graphScreenLeftMargin(const AplusGraph* gr_)
{
    A r = gf(gr_->margin(MSLeft));
    return r;
}
...
static void s_graphScreenRightMargin(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->margin(value_->t == Ft ? p.f[0] : (double)p.i[0], MSRight);
    }
}
static A g_graphScreenRightMargin(const AplusGraph* gr_)
{
    A r = gf(gr_->margin(MSRight));
    return r;
}
...
static void s_graphScreenTopMargin(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->margin(value_->t == Ft ? p.f[0] : (double)p.i[0], MSTop);
    }
}
static A g_graphScreenTopMargin(const AplusGraph* gr_)
{
    A r = gf(gr_->margin(MSTop));
    return r;
}
...
static void s_graphScreenBottomMargin(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->margin(value_->t == Ft ? p.f[0] : (double)p.i[0], MSBottom);
    }
}
static A g_graphScreenBottomMargin(const AplusGraph* gr_)
{
    A r = gf(gr_->margin(MSBottom));
    return r;
}
...
static void s_graphXleftMargin(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->bottomAxisDataMargin(value_->t == Ft ? p.f[0] : (double)p.i[0], MSLeft);
    }
}
static A g_graphXleftMargin(AplusGraph* gr_)
{
    A r = gf(gr_->bottomAxisDataMargin(MSLeft));
    return r;
}
...
static void s_graphXrightMargin(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->bottomAxisDataMargin(value_->t == Ft ? p.f[0] : (double)p.i[0], MSRight);
    }
}
static A g_graphXrightMargin(AplusGraph* gr_)
{
    A r = gf(gr_->bottomAxisDataMargin(MSRight));
    return r;
}
...
static void s_graphXXleftMargin(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->topAxisDataMargin(value_->t == Ft ? p.f[0] : (double)p.i[0], MSLeft);
    }
}
static A g_graphXXleftMargin(AplusGraph* gr_)
{
    A r = gf(gr_->topAxisDataMargin(MSLeft));
    return r;
}
...
static void s_graphXXrightMargin(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->topAxisDataMargin(value_->t == Ft ? p.f[0] : (double)p.i[0], MSRight);
    }
}
static A g_graphXXrightMargin(AplusGraph* gr_)
{
    A r = gf(gr_->topAxisDataMargin(MSRight));
    return r;
}
...
static void s_graphYtopMargin(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->leftAxisDataMargin(value_->t == Ft ? p.f[0] : (double)p.i[0], MSTop);
    }
}
static A g_graphYtopMargin(AplusGraph* gr_)
{
    A r = gf(gr_->leftAxisDataMargin(MSTop));
    return r;
}
...
static void s_graphYbottomMargin(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->leftAxisDataMargin(value_->t == Ft ? p.f[0] : (double)p.i[0], MSBottom);
    }
}
static A g_graphYbottomMargin(AplusGraph* gr_)
{
    A r = gf(gr_->leftAxisDataMargin(MSBottom));
    return r;
}
...
static void s_graphYYtopMargin(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->rightAxisDataMargin(value_->t == Ft ? p.f[0] : (double)p.i[0], MSTop);
    }
}
static A g_graphYYtopMargin(AplusGraph* gr_)
{
    A r = gf(gr_->rightAxisDataMargin(MSTop));
    return r;
}
...
static void s_graphYYbottomMargin(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->rightAxisDataMargin(value_->t == Ft ? p.f[0] : (double)p.i[0], MSBottom);
    }
}
static A g_graphYYbottomMargin(AplusGraph* gr_)
{
    A r = gf(gr_->rightAxisDataMargin(MSBottom));
    return r;
}
...
static void s_graphXlegend(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->xLegendPosition(value_->t == Ft ? p.f[0] : (double)p.i[0]);
    }
}
static A g_graphXlegend(AplusGraph* gr_)
{
    A r = gf(gr_->xLegendPosition());
    return r;
}
...
static void s_graphYlegend(AplusGraph* gr_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        gr_->yLegendPosition(value_->t == Ft ? p.f[0] : (double)p.i[0]);
    }
}
static A g_graphYlegend(AplusGraph* gr_)
{
    A r = gf(gr_->yLegendPosition());
    return r;
}
...
static A g_graphSubtitle(AplusGraph* gr_)
{
    A ap = gr_->subTitleA();
...
static A g_graphFootnote(AplusGraph* gr_)
{
    A ap = gr_->footnoteA();
    return (QA(ap) && isNull(ap) == MSFalse) ? (A)ic(ap) : ap;
}
...
static void s_graphYtitleAlign(AplusGraph* gr_, A just_)
{
    ...
    if (MSG::Vertical & oldAlign)
        newAlign |= MSG::Vertical; // Need to preserver the style bits
    else
        newAlign |= MSG::Horizontal;
