...
A getVarFunc(AClientData* ac_)
{
    A r = aplus_nl;
    if (ac_ != 0) {
        if (isNull(ac_->actualData()) == MSFalse)
            r = (A)ic(ac_->actualData());
        else {
            r = gv(Et, 2);
...
{
    if (p_ == 0) {
        MSShell* p = new AplusShell;
        (void)p->windowGroup(g_defaultLeader());
        p_ = p;
    } else if (p_ == (MSWidgetView*)-1) {
        MSPopup* p = new AplusPopup;
        (void)p->windowGroup(g_defaultLeader());
        p_ = p;
    }
...
static void s_resizeOptions(MSWidgetView* pWidgetView_, unsigned long opts_)
{
    ...
    if (opts_ & SmallWidth)
        constraint |= At::MinimizeWidth;
    if (opts_ & SmallHeight)
        constraint |= At::MinimizeHeight;
    if (opts_ & LikeWidth)
        constraint |= At::MaintainWidth;
    if (opts_ & LikeHeight)
        constraint |= At::MaintainHeight;
...
static unsigned long g_resizeOptions(MSWidgetView* pWidgetView_)
{
    ...
    if (constraint & At::Left)
        result |= Left;
    if (constraint & At::Right)
        result |= Right;
    if (constraint & At::Top)
        result |= Top;
    if (constraint & At::Bottom)
        result |= Bottom;
    if (constraint & At::MinimizeWidth)
        result |= SmallWidth;
    if (constraint & At::MinimizeHeight)
        result |= SmallHeight;
    if (constraint & At::MaintainWidth)
        result |= LikeWidth;
    if (constraint & At::MaintainHeight)
        result |= LikeHeight;
...
static void s_sflags(MSWidgetCommon* pWidgetView_, A flags_)
{
    if (pWidgetView_->model())
        ((AplusModel*)pWidgetView_->model())->sflags(flags_);
}
static A g_sflags(MSWidgetCommon* pWidgetView_)
{
    if (pWidgetView_->model())
        return ((AplusModel*)pWidgetView_->model())->sflags();
    else
        return aplus_nl;
}
...
{
    if (pWidgetView_ != 0)
        pWidgetView_->highlightColor(p_);
}
...
{
    if (pWidgetView_ != 0)
        pWidgetView_->highlightThickness(ht_);
}
...
{
    if (pWidgetView_ != 0)
        pWidgetView_->shadowThickness(st_);
}
        ...
        if (open_->t == Et && open_->n == childCount) {
            for (int i = 0; i < open_->n; i++) {
                ...
                if (wrk && QA(wrk) && wrk->t == It && wrk->n == 1)
                    wrk->p[0] ? cl_->open(cl_->children()(i)) : cl_->close(cl_->children()(i));
            }

        } else if (open_->t == It && open_->n == 1) {
            for (int i = 0; i < childCount; i++)
                open_->p[0] ? cl_->open(cl_->children()(i)) : cl_->close(cl_->children()(i));
        }
    }
}
...
static void s_treeOrientation(AplusTreeView* tv_, A sym_)
{
    if (!QS(sym_) && (sym_->t == Et && sym_->n > 0 && QS(*sym_->p))) {
        if (XS(*sym_->p) == si("vertical"))
            tv_->orientation(AplusTreeView::Vertical);
        else if (XS(*sym_->p) == si("horizontal"))
            tv_->orientation(AplusTreeView::Horizontal);
    }
}
...
static void s_pageIndexFunc(AplusPage* page_, A fc_)
{
    if (fc_->t == Et && fc_->n == 2) {
        ...
        page_->indexFunc((AFunc)0, 0);
    else
        showError("Invalid 'pageIndex' Function Specification");
}
...
static void s_pageBoldFunc(AplusPage* page_, A fc_)
{
    if (fc_->t == Et && fc_->n == 2) {
        ...
        page_->boldFunc((AFunc)0, 0);
    else
        showError("Invalid 'pageBold' Function Specification");
}
...
static void s_pageBlinkFunc(AplusPage* page_, A fc_)
{
    if (fc_->t == Et && fc_->n == 2) {
        ...
        page_->blinkFunc((AFunc)0, 0);
    else
        showError("Invalid 'pageBlink' Function Specification");
}
...
static void s_pageUnderlineFunc(AplusPage* page_, A fc_)
{
    if (fc_->t == Et && fc_->n == 2) {
        ...
        page_->underlineFunc((AFunc)0, 0);
    else
        showError("Invalid 'pageUnderline' Function Specification");
}
...
static void s_scaleValueMin(MSScale* sp_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        sp_->valueMin(value_->t == Ft ? p.f[0] : (double)p.i[0]);
    }
}
static A g_scaleValueMin(MSScale* sp_)
{
    A r = gf(sp_->valueMin());
    return r;
}
...
static void s_scaleValueMax(MSScale* sp_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        sp_->valueMax(value_->t == Ft ? p.f[0] : (double)p.i[0]);
    }
}
static A g_scaleValueMax(MSScale* sp_)
{
    A r = gf(sp_->valueMax());
    return r;
}
...
static void s_scaleValueInc(MSScale* sp_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        sp_->valueInc(value_->t == Ft ? p.f[0] : (double)p.i[0]);
    }
}
static A g_scaleValueInc(MSScale* sp_)
{
    A r = gf(sp_->valueInc());
    return r;
}
...
static void s_scaleValuePageInc(MSScale* sp_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        sp_->valuePageInc(value_->t == Ft ? p.f[0] : (double)p.i[0]);
    }
}
static A g_scaleValuePageInc(MSScale* sp_)
{
    A r = gf(sp_->valuePageInc());
    return r;
}
...
static void s_scaleSliderHeight(MSScale* sp_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        sp_->sliderSize((int)(value_->t == Ft ? p.f[0] : p.i[0]));
    }
}
static A g_scaleSliderHeight(MSScale* sp_)
{
    A r = gf(sp_->sliderSize());
    return r;
}
...
static A g_scaleSliderWidth(MSScale* sp_)
{
    A r = gf(sp_->sliderSize());
    return r;
}
...
static void s_scaleLabelInc(MSScale* sp_, A value_)
{
    if (!QS(value_) && (value_->t == It || value_->t == Ft)) {
        P p;
        p.i = value_->p;
        sp_->labelInc(value_->t == Ft ? p.f[0] : (double)p.i[0]);
    }
}
static A g_scaleLabelInc(MSScale* sp_)
{
    A r = gf(sp_->labelInc());
    return r;
}
...
static void s_fieldColumnAlignment(MSWidgetView* o_, A sym_)
{
    ...
    if (tc)
        tc->columnAlignment((MSAlignment)GUIEnum.alignFormat(sym_));
}
...
static A g_fieldColumnAlignment(MSWidgetView* o_)
{
    ...
    AplusTableColumn* tc = (AplusTableColumn*)o_;
    if (tc)
        result = GUIEnum.alignFormat((unsigned long)tc->columnAlignment());
    return result;
}
...
static void s_fieldCycleFunc(MSWidgetView* o_, A fc_)
{
    ...
    if (varData) {
        if (fc_->t == Et && fc_->n == 2) {
            ...
            varData->cycleFunc((AFunc)0, 0);
        else
            showError("Invalid 'array' Cycle Function Specification");
    }
}
...
static void s_fieldBgGrayScaleFunc(MSWidgetView* o_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        tc->bgGrayScaleFunc((AFunc)0, 0);
    else
        showError("Invalid 'field' BgGrayScale Function Specification");
}
...
static void s_fieldBreakBgGrayScaleFunc(MSWidgetView* o_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        showError("Invalid 'field' BreakBgGrayScale Function Specification");
}
...
static void s_fieldBreakFontFunc(MSWidgetView* o_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        tc->breakFontFunc((AFunc)0, 0);
    else
        showError("Invalid 'field' BreakFont Function Specification");
}
...
static void s_fieldBreakLeadingFunc(MSWidgetView* o_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        tc->breakLeadingFunc((AFunc)0, 0);
    else
        showError("Invalid 'field' BreakLeading Function Specification");
}
...
static void s_fieldBreakOffsetFunc(MSWidgetView* o_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        tc->breakOffsetFunc((AFunc)0, 0);
    else
        showError("Invalid 'field' BreakOffset Function Specification");
}
...
static void s_fieldBreakStyleFunc(MSWidgetView* o_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        tc->breakStyleFunc((AFunc)0, 0);
    else
        showError("Invalid 'field' BreakStyle Function Specification");
}
...
static void s_fieldFgGrayScaleFunc(MSWidgetView* o_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        tc->fgGrayScaleFunc((AFunc)0, 0);
    else
        showError("Invalid 'field' FgGrayScale Function Specification");
}
...
static void s_fieldBreakFgGrayScaleFunc(MSWidgetView* o_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        tc->breakFgGrayScaleFunc((AFunc)0, 0);
    else
        showError("Invalid 'field' BreakFgGrayScale Function Specification");
}
...
static void s_fieldBreakProcessFunc(MSWidgetView* o_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        tc->breakProcessFunc((AFunc)0, 0);
    else
        showError("Invalid 'field' BreakProcess Function Specification");
}
...
static void s_fieldFormatBreakFunc(MSWidgetView* o_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        tc->formatBreakFunc((AFunc)0, 0);
    else
        showError("Invalid 'field' FormatBreak Function Specification");
}
...
static void s_fieldBreakCriteriaFunc(MSWidgetView* o_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        tc->breakCriteriaFunc((AFunc)0, 0);
    else
        showError("Invalid 'field' FormatBreak Function Specification");
}
...
static void s_fieldReportFontFunc(MSWidgetView* o_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        tc->reportFontFunc((AFunc)0, 0);
    else
        showError("Invalid 'field' ReportFont Function Specification");
}
...
static void s_fieldStyleFunc(MSWidgetView* o_, A fc_)
{
    ...
    if (fc_->t == Et && fc_->n == 2) {
        ...
        tc->styleFunc((AFunc)0, 0);
    else
        showError("Invalid 'field' Style Function Specification");
}
...
static void s_matrixColSpaceFunc(AplusMatrix* o_, A fc_)
{
    if (fc_->t == Et && fc_->n == 2) {
        ...
        o_->spaceFunc((AFunc)0, 0);
    else
        showError("Invalid 'colSpace' Function Specification");
}
...
{
    if (MSFalse == o_->editing())
        o_->selectedRow(row_);
    else
        showError("Invalid row selection: array in edit mode");
}
...
static void s_arraySbSize(MSRowColumnView* o_, int size_)
{
    o_->vsbSize(size_);
    o_->hsbSize(size_);
}
...
{
    if (MSFalse == o_->editing())
        o_->selectedColumn(col_);
    else
        showError("Invalid column selection: array in edit mode");
}
...
static void s_tableGroupHeading(MSArrayView* o_, A x_)
{
    if (o_ && x_ && QA(x_))
        ...
        } else if (x_->t == Et && QS(x_->p[0])) // single (`groupheading;`a)
        {
            ...
            AplusParagraph* ap = ((AplusParagraph*)getVarData(x_)->pWidgetView());

            if (ap && ap->widgetType() == AplusParagraph::symbol()) {
                ((MSTable*)o_)->MSReportTable::addGroupHeading(*ap).text(AplusConvert::asMSStringVector(ap->model()->a())).column(ap->column()).columnSpan(ap->columnSpan());
            }
        } else // vector (`groupheading;(`a;`b;`c))
        {
            ...
            for (i = 0; i < x_->n; i++) {
                ...
                if (ax && QA(ax) && ax->t == Et && QS(ax->p[0])) {
                    AplusParagraph* ap = ((AplusParagraph*)getVarData(ax)->pWidgetView());
...
static I g_tableCurrentBreakColumn(MSArrayView* o_)
{
    ...
    if (t->breakColumn().length() > 0)
        return (I)t->breakColumn().lastElement();
    return -1;
}
...
static A g_tableOutputStyle(MSArrayView* o_)
{
    ...
    for (i = 2; i <= MSP::NoHeadings; i *= 2) {
        if (GUIEnum.reportStyleEnumHashTable()->lookup((unsigned long)(style & i)) != 0)
            count++;
    }
    if (count > 0) {
        r = gv(Et, count);
...
static void s_tableOutputStyle(MSArrayView* o_, A sym_)
{
    ...
    if (sym_ != 0) {
        ...
        for (int i = 0; i < (int)sym_->n; i++) {
            ...
            if (QS(p[i]))
                style |= (unsigned long)GUIEnum.reportStyleStringHashTable()->lookup(s);
            else {
                cerr << "ã ! ";
                if (s != 0)
                    cerr << s;
...
static void s_scalarCycleFunc(AplusEntryField* o_, A fc_)
{
    ...
    if (varData) {
        if (fc_->t == Et && fc_->n == 2) {
            ...
            showError("Invalid 'scalar' Cycle Function Specification");
    }
}
...
static void s_slotCycleFunc(AplusSlot* o_, A fc_)
{
    if (fc_->t == Et && fc_->n == 2) {
        ...
        o_->cycleFunc((AFunc)0, 0);
    else
        showError("Invalid 'slot' Cycle Function Specification");
}
...
static void s_layoutRowSpace(MSLayoutManager* tp_, A x_)
{
    if (!QS(x_) && (x_->t == It || x_->t == Ft)) {
        P p;
        p.i = x_->p;
        tp_->rowSpacing((int)(x_->t == Ft ? p.f[0] : p.i[0]));
    }
}
static A g_layoutRowSpace(MSLayoutManager* tp_)
{
    A r = gf(tp_->rowSpacing());
    return r;
}

static void s_layoutColSpace(MSLayoutManager* tp_, A x_)
{
    if (!QS(x_) && (x_->t == It || x_->t == Ft)) {
        P p;
        p.i = x_->p;
        tp_->columnSpacing((int)(x_->t == Ft ? p.f[0] : p.i[0]));
    }
}
static A g_layoutColSpace(MSLayoutManager* tp_)
{
    A r = gf(tp_->columnSpacing());
    return r;
}
...
static void s_notebookOrientation(AplusNotebook* nb_, A sym_)
{
    if (!QS(sym_) && (sym_->t == Et && sym_->n > 0 && QS(*sym_->p))) {
        if (XS(*sym_->p) == si("vertical"))
            nb_->orientation(MSNotebook::Vertical);
        else if (XS(*sym_->p) == si("horizontal"))
...
static AplusDisplayServer* c_XServer(char* dpyName_)
{
    ...
    if (dpyName_ != 0)
        server = new AplusDisplayServer(dpyName_);
    else
        server = new AplusDisplayServer;
    return server;
}
...
MSWidgetView* validateParent(MSWidgetView* parent_)
{
    ...
    if (pp == 0) {
        MSShell* sp = new AplusShell;
        (void)sp->windowGroup(g_defaultLeader());
        pp = sp;
    } else if (pp == (MSWidgetView*)-1) {
        MSPopup* po = new AplusPopup;
        (void)po->windowGroup(g_defaultLeader());
        pp = po;
    }
    return pp;
}
