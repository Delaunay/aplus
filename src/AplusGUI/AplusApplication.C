    ...
    struct timeval* timeout)
{
    ...
    if (nmsecs) {
        if (__winx_timer_ID != -1)
            KillWindowsTimer(__winx_timer_ID);
...
AplusApplication::AplusApplication(int argc_, char** argv_)
    : MSApplication(argc_, argv_)
{
    if (mainLoop() != 0) {
        delete mainLoop();
    }
...
AplusMainLoop::AplusMainLoop(MSApplication* pApplication_)
    : MSApplicationMainLoop(pApplication_)
{
}
...
{
    if (flush() == MSTrue || processAVariables() == MSTrue)
        zeroTimeOut(MSTrue);
    else
        zeroTimeOut(MSFalse);
}
...
void AplusMainLoop::selectAndProcess(void)
{
    ...
    else if ((tvpnext = MSTimer::nextTimeVal()) == (struct timeval*)(0) && (tvpnext = timernext()) == (struct timeval*)(0)) {
