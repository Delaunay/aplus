...
AplusArray::AplusArray(MSWidget* widget_)
    : MSArrayView(widget_)
{
...
MSBoolean AplusArray::validate(V v_, const char* string_, unsigned row_, unsigned col_)
{
    ...
    if (v_ != 0) {
        ...
        if ((inFunc = AplusModel::getInFunc(v_)) != 0)
            inData = inFunc->invoke(v_, (char*)string_, (int)row_, (int)col_);
        else
...
void AplusArray::receiveEvent(MSEvent& event_)
{
    ...
    if (eventType == AplusEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateEvent in AplusArray" << endl;
        ...
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusArray" << endl;
...
const char* AplusArray::formatOutput(MSString& str_, unsigned i, unsigned j)
{
    ...
    case It:
        if (outFunc != 0)
            outStr = outFunc->invoke(v, (int)p.i[offset], i, j);
        break;
    case Ft:
        if (outFunc != 0)
            outStr = outFunc->invoke(v, (double)p.f[offset], i, j);
        break;
    case Ct:
        n = charlength;
        offset = i * n;
        if (outFunc != 0)
            outStr = outFunc->invoke(v, p.c + (offset), n, i, j);
        break;
    case Et:
...
void AplusArray::update(V v_, int row_, int col_, UpdateType type_)
{
    ...
    } else if (type_ == ShapeUpdate)
        shapeUpdate();
    else if (type_ == AppendUpdate)
        appendUpdate();
}
