...
AplusActionButton::AplusActionButton(MSWidget* xwin_, const char* title_)
    : MSActionButton(xwin_, title_)
{
...
{
    if (buttonBox()->readOnly(buttonBox()->buttons().indexOf((unsigned long)(MSWidget*)this)) == MSFalse && sensitive() == MSTrue) {
        return MSFalse;
...
AplusButtonBox::AplusButtonBox(MSWidget* xwin_)
    : MSActionBox(xwin_)
{
    _selectedItem = -1;
    _geometry = aplus_nl;
...
void AplusButtonBox::receiveEvent(MSEvent& event_)
{
    ...
    if (eventType == AplusEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateEvent in AplusButtonBox" << endl;
        ...
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusButtonBox" << endl;
        ...
        if (dbg_tmstk)
            cout << "Received UpdateTitleEvent in AplusButtonBox" << endl;
        updateTitle();
    }
}
    ...
    if (pick_ != 0) {
        if (QA(pick_)) {
            ...
            if (pick != 0 && QA(pick) && pick->t == It && pick->r <= 1) {
                ...
                cerr << "action: pick assignment error in update." << endl;
            else
                cerr << "action: pick assignment error in update." << endl;
            if (pick != 0)
                dc(pick);
...
void AplusButtonBox::updateData(void)
{
    if (model() && ((AplusModel*)model())->aplusVar()) {
        ...
        for (int r = 0; r < nr; r++) {
            ...
            if (widgetType() == AplusCheckBox::symbol())
                ((MSToggleButton*)btn)->selectColor(color(r));
            ...
            btn->integerTag(r);
            if (doMap == MSTrue && btn->mapped() == MSFalse)
                btn->map();
        }
        if (change == MSTrue)
            naturalSize();
...
int AplusButtonBox::callAset(V v_, A d_, A i_, A p_)
{
    ...
    busyOn();
    d_ = (A)ic(d_);
    if ((r = safeAset(v_, d_, i_, p_)) == 0)
        showError(qs);
    else
        AplusModel::doneCB(v_, d_, i_, p_);
    dc(d_);
    busyOff();
    return r;
}
...
int lcm(A span_)
{
    ...
    for (i = 0; i < n - 1; i++)
        x = lcm(x, ((int)p.i[i + 1] > 0) ? (int)p.i[i + 1] : 1);
    return x;
}
            ...
            if (isNull(ag) == MSFalse && ag->t == It) {
                ...
                for (r = 0; r < nr; r++) {
                    ...
                    if (btn != 0) {
                        ...
                        } else if (btn->mapped() == MSFalse)
                            btn->map();
                        if (ax != btn->at().row() || ay != btn->at().column() || h_span != btn->at().columnSpan() || v_span != btn->at().rowSpan() || mstat != btn->mapped())
                            change = MSTrue;
                    }
                }
                if (ag->r <= 1)
