...
AplusCheckButton::AplusCheckButton(MSWidget* owner_)
    : MSCheckButton(owner_, "")
{
    dynamic(MSTrue);
...
void AplusCheckButton::up(void)
{
    if (checkBox())
        checkBox()->up();
}
void AplusCheckButton::down(void)
{
    if (checkBox())
        checkBox()->down();
}
void AplusCheckButton::left(void)
{
    if (checkBox())
        checkBox()->left();
}
void AplusCheckButton::right(void)
{
    if (checkBox())
        checkBox()->right();
}
...
{
    if (checkBox()->readOnly(checkBox()->buttons().indexOf((unsigned long)(MSWidget*)this)) == MSFalse && sensitive() == MSTrue) {
        return MSFalse;
...
AplusCheckBox::AplusCheckBox(MSWidget* owner_)
    ...
    if (isSlotFiller(a_) == MSTrue) {
        ...
        for (int row = 0; row < (int)val->n; row++) {
            if (QS(pv.a[row]))
                ...
                return MSFalse;
            else if ((int)*pv.a[row]->p < 0 || (int)*pv.a[row]->p > 1)
                return MSFalse;
        }
        return MSTrue;
    }
