...
AplusChoice::AplusChoice(MSWidget* widget_)
    : MSOptionMenu(widget_)
{
    _itemVal = aplus_nl;
    dynamic(MSTrue);
...
MSBoolean AplusChoice::verifyData(V, A a_)
{
    ...
    if (0 != a_ && QA(a_) && a_->t == Et && a_->n == 2 && a_->r == 1) {
        ...
        A attr = p.a[0];
        int row;
        if (val->t == Et && val->n == 0 && attr->t == Et && attr->n == 0)
            r = MSTrue;
            ...
            if (r == MSTrue) {
                ...
                for (row = 0; row < (int)val->n && r == MSTrue; row++) {
                    if (QS(pv.a[row]))
                        r = MSFalse;
                    else if (pv.a[row]->t != It || pv.a[row]->r != 0)
                        r = MSFalse;
                    else {
                        if ((int)*pv.a[row]->p == 1)
                            trueCount++;
                        else if ((int)*pv.a[row]->p != 0)
                            r = MSFalse;
                    }
                }
                if (trueCount == 0 || trueCount > 1)
                    r = MSFalse;
            } else if (val->t == Et && val->n == 0 && attr->t == Et && attr->n == 0)
                r = MSTrue;
            else
                r = MSFalse;
        }
...
void AplusChoice::receiveEvent(MSEvent& event_)
{
    ...
    if (eventType == AplusEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateEvent in AplusChoice" << endl;
        ...
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusChoice" << endl;
        ...
        if (dbg_tmstk)
            cout << "Received UpdateDataEvent in AplusChoice" << endl;
        setWidth();
    } else if (eventType == AplusUpdateTitleEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateTitleEvent in AplusChoice" << endl;
        updateTitle();
    }
}
    ...
    if (pick_ == 0)
        ...
        if (QA(pick_)) {
            ...
            if (pick != 0 && QA(pick) && pick->t == It && pick->r <= 1) {
                        ...
                        for (int i = 0; i < ni; i++) {
                            if (*pv.a[i]->p == 1) {
                                if (one == MSFalse)
                                    ...
                                    *un((A*)(un((A*)(un((A*)(&v->a))->p + 1))->p + i))->p = 0;
                            }
                        }
                    } else
                        updateData();
                ...
                cerr << "choice: pick assignment error in update." << endl;
            else
                cerr << "choice: pick assignment error in update." << endl;
            if (pick != 0)
                dc(pick);
...
void AplusChoice::updateOptions(void)
{
    ...
    if (v != 0 && n == 2 && optionMenu() != 0) {
        ...
        for (int i = 0; i < numberOfItems(); i++) {
            ...
            fg = (titleColorFunc != 0) ? titleColorFunc->invoke(v, (A)val, (A)attr) : foreground();
            mi->foreground(fg);
            if (outStr->t == Ct)
                mi->label((char*)outStr->p);
            if (isNull(outStr) == MSFalse)
                dc(outStr);
        }
        if (optionMenu()->font() != fid)
            optionMenu()->font(fid);
        else
            optionMenu()->naturalSize();
        setChoice();
    }
}
...
Font AplusChoice::dataFont(void)
{
    ...
    if (v) {
        ...
        if (fontFunc != 0)
            return (Font)fontFunc->invoke(v, a);
    }
    return font();
}
