...
AplusCollapsible::AplusCollapsible(MSWidget* xwin_)
    : MSCollapsibleLayout(xwin_)
{
}
...
void AplusCollapsible::receiveEvent(MSEvent& event_)
{
    if (event_.type() == MSIndexedEvent::symbol()) {
        ...
        if (dbg_tmstk)
            cout << "Received UpdateEvent in "
                 << "AplusCollapsible" << endl;
        redraw();
    }
    if (event_.type() == AplusVerifyEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusCollapsible" << endl;
