...
V getV(A a_)
{
    ...
    S s = (S)XS(*a_->p);
    return (a_->n <= 1) ? getVFromSym(Cx, s) : vi(XS(a_->p[a_->n - 1]), a_->n - 1 ? cxi(*s->n ? s : si("")) : Cx);
}
...
A getSymWithContext(V v_)
{
    ...
    if (v_ == 0)
        ...
        if (v_->cx == 0)
            ...
            ar->p[0] = (I)MS(v_->cx == cx("") ? si("") : v_->cx->s);
        // symbol
        if (v_->s == 0)
            ar->p[1] = (I)MS(si(""));
        else
...
AVariableData::~AVariableData(void)
{
    if (QA(titleA()) && isNl(titleA()) == MSFalse)
        dc(titleA());
    if (QA(titleAFg()) && isNl(titleAFg()) == MSFalse)
        dc(titleAFg());
    if (QA(titleAFont()) && isNl(titleAFont()) == MSFalse)
        dc(titleAFont());
    if (QA(bgA()) && isNl(bgA()) == MSFalse)
        dc(bgA());
    if (QA(fgA()) && isNl(fgA()) == MSFalse)
        dc(fgA());
    if (QA(fontA()) && isNl(fontA()) == MSFalse)
        dc(fontA());
}
...
{
    if (!QS(title_)) {
        if (QA(titleA()) && isNull(titleA()) == MSFalse)
            dc(titleA());
...
unsigned long AVariableData::titleFg(void)
{
    if (isNull(titleAFg()) == MSTrue) {
        if (pWidgetView() != 0)
            return pWidgetView()->server()->defaultForeground();
        else
            return MSDisplayServer::defaultDisplayServer()->defaultForeground();
    } else
        return (unsigned long)titleAFg()->p[0];
}
...
Font AVariableData::titleFont(void)
{
    if (isNull(titleAFont()) == MSTrue) {
        if (pWidgetView() != 0)
            return pWidgetView()->server()->defaultFont();
        else
            return MSDisplayServer::defaultDisplayServer()->defaultFont();
    } else
        return (Font)titleAFont()->p[0];
}
...
void AVariableData::foreground(A fg_)
{
    ...
    if (pWidgetView() != 0) {
        if (QS(fg_))
            fg = gi((I)pWidgetView()->server()->pixel((char*)XS(fg_)->n));
        else if (fg_->t == It)
            fg = gi((I)fg_->p[0]);
        else if (fg_->t == Ct)
        ...
        fg = gi((I)fg_->p[0]);
    if (isNull(fgA()) == MSFalse)
        dc(fgA());
    _fg = fg;
    if (pWidgetView() != 0)
        pWidgetView()->foreground((isNull(fg) == MSTrue) ? MSDisplayServer::defaultDisplayServer()->defaultForeground() : foreground());
    fgFunc((AFunc)0, 0);
}
...
void AVariableData::background(A bg_)
{
    ...
    if (pWidgetView() != 0) {
        if (QS(bg_))
            bg = gi((I)pWidgetView()->server()->pixel((char*)XS(bg_)->n));
        else if (bg_->t == It)
            bg = gi((I)bg_->p[0]);
        else if (bg_->t == Ct)
    ...
    } else if (QA(bg_) && bg_->t == It)
        bg = gi((I)bg_->p[0]);
    if (isNull(bgA()) == MSFalse)
        dc(bgA());
    _bg = bg;
    if (pWidgetView() != 0)
        pWidgetView()->background(
            (isNull(bg) == MSTrue) ? MSDisplayServer::defaultDisplayServer()->defaultBackground() : background());
    bgFunc((AFunc)0, 0);
}
...
void AVariableData::font(A fa_)
{
    ...
    if (pWidgetView() != 0) {
        if (QS(fa_))
            afid = gi((I)pWidgetView()->server()->fontID((char*)XS(fa_)->n));
        else if (It == fa_->t)
            afid = gi((I)fa_->p[0]);
        else if (Ct == fa_->t)
            afid = gi((I)pWidgetView()->server()->fontID((char*)fa_->p));
    } else if (QA(fa_) && It == fa_->t)
        afid = gi((I)fa_->p[0]);
    if (isNull(fontA()) == MSFalse)
        dc(fontA());
    _font = afid;
    if (0 != pWidgetView())
        pWidgetView()->font((isNull(afid) == MSTrue) ? MSDisplayServer::defaultDisplayServer()->defaultFont() : font());
    fontFunc((AFunc)0, 0);
}
...
unsigned long AVariableData::defaultRowColor(void)
{
    if (_defaultRowColor == ULONG_MAX) {
        _defaultRowColor = MSDisplayServer::defaultDisplayServer()->pixel("lightsteelblue3");
    }
...
unsigned long AVariableData::defaultMatrixColor(void)
{
    if (_defaultMatrixColor == ULONG_MAX) {
        _defaultMatrixColor = MSDisplayServer::defaultDisplayServer()->pixel("mediumaquamarine");
    }
