...
AplusEntryField::AplusEntryField(MSWidget* widget_)
    : MSEntryFieldPlus(widget_)
{
...
void AplusEntryField::receiveEvent(MSEvent& event_)
{
    ...
    if (eventType == AplusEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateEvent in AplusEntryField" << endl;
        update(MSIndexVector::nullVector());
    } else if (eventType == AplusVerifyEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusEntryField" << endl;
        ...
        if (dbg_tmstk)
            cout << "Received UpdateDataEvent in AplusEntryField" << endl;
        setClipMode();
...
MSBoolean AplusEntryField::isProtected(void) const
{
    ...
    if (v) {
        ...
        if (roFunc != 0)
            ro = (MSBoolean)roFunc->invoke(v, a);
    }
    return ro;
}
