...
AplusFormatter::OutputFormat AplusFormatter::format(const char* str_)
{
    if (str_ != 0) {
        if (str_[0] == '\0')
            ...
            return (OutputFormat)(unsigned long)enumHashTable()->lookup(str_);
    } else
        return BadFormat;
}
...
{
    if (QS(str_))
        return format((char*)XS(str_)->n);
    else if (str_->t == Ct)
        return format((char*)str_->p);
    else if (str_->t == Et && str_->n > 0 && QS(*str_->p))
        return format((char*)XS(*str_->p)->n);
    else
        return BadFormat;
}
...
A AplusFormatter::sfmt(A fmt_, A data_)
{
    if (QA(fmt_)) {
        if (fmt_->t == Ct)
            ...
            P p;
            p.i = fmt_->p;
            if (QS(p.a[0]) || (QA(p.a[0]) && (p.a[0]->t == Et && p.a[0]->n > 0 && QS(*p.a[0]->p)))) {
                A r = 0;
                if (QA(data_) && data_->t <= Ft) {
                    P pp;
                    pp.i = data_->p;
                    double value = (data_->t == Ft) ? pp.f[0] : (double)pp.i[0];
                    int pr = (fmt_->n == 2 && QA(p.a[1]) && It == p.a[1]->t) ? (int)p.a[1]->p[0] : defaultPrecision();
...
struct tm* AplusFormatter::parseDate(double ymd_)
{
    ...
    if ((yr % 4 == 0 && yr % 100 != 0) || yr % 400 == 0)
        leapyr = MSTrue;
    int century, wday;
    if (mmx >= 1 && mmx <= 12 && dd >= 1 && dd <= ((leapyr == MSTrue && mmx == 2) ? 29 : mdays[mmx - 1])) {
        for (int i = 0; i < mmx - 1; i++)
            yday += mdays[i];
        if (leapyr == MSTrue && mmx > 2)
            yday++;
        yday += dd;
