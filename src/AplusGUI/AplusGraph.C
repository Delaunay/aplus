...
AplusGraph::AplusGraph(MSWidget* owner_)
    : MSGraph(owner_)
{
...
void AplusGraph::y_labelFormat(::A sym_, int axis_, int precision_)
{
    ...
    if ((k = outFmt()->format(sym_)) != AplusFormatter::BadFormat) {
        ...
        enumError()->showError(((AplusModel*)model())->aplusVar(), (char*)XS(sym_->p[0])->n);
}

::A AplusGraph::y_labelFormatSym(int axis_)
{
    if (y_labelFormat(axis_) == YFunc || y_labelFormat(axis_) == YYFunc)
        return generateFuncSym();
    else
        return generateSym(outFmt()->formatEnumString(y_labelFormat(axis_)),
...
{
    if (x_subLabelFormat(axis_) == XFunc)
        return generateFuncSym();
    else
        return generateSym(outFmt()->formatEnumString(x_subLabelFormat(axis_)),
...
{
    if (x_labelFormat(axis_) == XFunc || x_labelFormat(axis_) == XXFunc)
        return generateFuncSym();
    else
        return generateSym(outFmt()->formatEnumString(x_labelFormat(axis_)),
...
::A AplusGraph::enumSymbols(const char* str_)
{
    ...
    if ((k = (GraphEnum)(unsigned long)enumHashTable()->lookup(str_)) > 0 && (k >= _GraphMode && k <= _DisplayStyle)) {
        switch (k) {
        case _GraphStyle:
            r = gv(Et, 2);
            r->p[0] = MS(si("ver"));
...
::A AplusGraph::generateGraphModeSym(unsigned long mode_)
{
    ...
    if (n > 0) {
        ::A a = gv(Et, n);
        if (mode_ == Standard)
            a->p[i++] = MS(si((char*)stringEnumHashTable()->lookup(mode_ & Standard)));
        if (mode_ & Normalize)
            a->p[i++] = MS(si((char*)stringEnumHashTable()->lookup(mode_ & Normalize)));
        if (mode_ & Optimize)
            a->p[i++] = MS(si((char*)stringEnumHashTable()->lookup(mode_ & Optimize)));
        if (mode_ & PieChart)
            a->p[i++] = MS(si((char*)stringEnumHashTable()->lookup(mode_ & PieChart)));
        return a;
    }
    return aplus_nl;
}
...
void AplusGraph::graphAMode(::A sym_) // Need to look up symbol in hashtable and call graphUIMode()
{
    if (!QS(sym_) && (sym_->t == Et && sym_->n > 0 && QS(*sym_->p))) {
        ...
        for (unsigned i = 0; i < sym_->n; i++) {
            if ((k = (unsigned long)enumHashTable()->lookup((char*)XS(sym_->p[i])->n)) > 0)
                mode += k;
            else
                enumError()->showError(((AplusModel*)model())->aplusVar(), (char*)XS(sym_->p[i])->n);
        }
        graphMode(mode);
    }
}
...
void AplusGraph::graphAUIMode(::A sym_) // Need to look up symbol in hashtable and call graphUIMode()
{
    if (!QS(sym_) && (sym_->t == Et && sym_->n > 0 && QS(*sym_->p))) {
        GraphUIMode k;
        if ((k = (GraphUIMode)(unsigned long)enumHashTable()->lookup((char*)XS(sym_->p[0])->n)) > 0 && (k >= MSGraph::Normal && k <= MSGraph::AddTextTrace)) {
            graphUIMode(k);
        } else
            enumError()->showError(((AplusModel*)model())->aplusVar(), (char*)XS(sym_->p[0])->n);
    }
...
void AplusGraph::axisAMode(::A sym_) // Need to look up symbol in hashtable and call axisMode()
{
    ...
    if (!QS(sym_) && (sym_->t == Et && sym_->n > 0 && QS(*sym_->p))) {
        ...
        if (mode == Std) {
            ...
            if (strpbrk(style, "x") != 0)
                mode |= MSBottom;
            if (strpbrk(style, "X") != 0)
                mode |= MSTop;
            if (strpbrk(style, "y") != 0)
                mode |= MSLeft;
            if (strpbrk(style, "Y") != 0)
                mode |= MSRight;
            axis(mode);
...
void AplusGraph::gridA(::A sym_) // Need to look up symbol in hashtable and call grid()
{
    if (!QS(sym_) && (sym_->t == Et && sym_->n > 0 && QS(*sym_->p))) {
        unsigned long k;
        if ((k = (unsigned long)enumHashTable()->lookup((char*)XS(sym_->p[0])->n)) >= 0 && (k == MSNone || k & (MSTop | MSBottom | MSRight | MSLeft))) {
            grid(k);
        } else
            enumError()->showError(((AplusModel*)model())->aplusVar(), (char*)XS(sym_->p[0])->n);
    }
...
void AplusGraph::gridAStyle(::A sym_)
{
    if (!QS(sym_) && (sym_->t == Et && sym_->n > 0 && QS(*sym_->p))) {
        ...
        if ((k = (LineStyle)(unsigned long)enumHashTable()->lookup((char*)XS(sym_->p[0])->n)) > 0 && styleConvert(k, lineStyle, weight) == MSTrue) {
            freeze();
            gridStyle(lineStyle);
            unfreeze();
            gridWeight(weight);
...
void AplusGraph::legendAStyle(::A sym_)
{
    if (!QS(sym_) && (sym_->t == Et && sym_->n > 0 && QS(*sym_->p))) {
        unsigned long k;
        if ((k = (unsigned long)enumHashTable()->lookup((char*)XS(sym_->p[0])->n)) > 0 && (k >= Vertical || k <= LastValue)) {
            legendStyle(k);
        } else
            enumError()->showError(((::AplusModel*)model())->aplusVar(), (char*)XS(sym_->p[0])->n);
    }
...
void AplusGraph::yTitleAStyle(::A sym_, MSAlignment axis_)
{
    ...
    if (!QS(sym_) && (sym_->t == Et && sym_->n > 0 && QS(*sym_->p))) {
        unsigned long k;
        if ((k = (unsigned long)enumHashTable()->lookup((char*)XS(sym_->p[0])->n)) > 0 && (k == MSG::Horizontal || k == MSG::Vertical)) {
            ...
            enumError()->showError(v, (char*)XS(sym_->p[0])->n);
    }
}
...
void AplusGraph::tickStyleA(::A sym_, MSAlignment axis_)
{
    if (!QS(sym_) && (sym_->t == Et && sym_->n > 0 && QS(*sym_->p))) {
        unsigned long k;
        if ((k = (unsigned long)enumHashTable()->lookup((char*)XS(sym_->p[0])->n)) > 0 && (k == MSG::Inside || k == MSG::Outside)) {
            tickStyle(k, axis_);
        } else
            enumError()->showError(((AplusModel*)model())->aplusVar(), (char*)XS(sym_->p[0])->n);
    }
    ...
    if (v != 0) {
        ...
        for (i = 0; i < traceSetList().count(); i++) {
            ...
            for (j = 0; j < numTraceSets; j++) {
                if (((AplusModel*)traceSet->model())->aplusVar() == vt[j]) {
                    found = MSTrue;
                    if (traceSet->trace(0) == selectTrace())
                        selectTraceFound = MSTrue;
                }
            }
            if (found == MSFalse) {
        ...
        for (i = 0; i < numTraceSets; i++) {
            ...
            for (j = 0; j < traceSetList().count(); j++) {
                ...
                if (((AplusModel*)traceSet->model())->aplusVar() == vt[i])
                    break;
                else
                    traceSet = 0;
            }
...
void AplusGraph::printDebugInfo(const char* str_)
{
    V v = (model() != 0) ? ((AplusModel*)model())->aplusVar() : 0;
    if (v)
        cout << "ã    " << (char*)v->cx->s->n << "." << (char*)v->s->n << ": " << str_ << endl;
}
