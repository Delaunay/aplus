...
AplusLabel::AplusLabel(MSWidget* widget_)
    : MSLabel(widget_)
{
...
const char* AplusLabel::formatOutput(MSString& str_, int row_)
{
    ...
    if (a != 0 && row_ < numRows()) {
        ...
        case Ct:
            if (rank <= 1)
                str_ = MSString((char*)p.c, len);
            else if (rank == 2)
                str_ = MSString(((char*)p.c + (row_ * len)), len);
            break;
        case Et:
...
void AplusLabel::receiveEvent(MSEvent& event_)
{
    if (event_.type() == AplusEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateEvent in AplusLabel" << endl;
        MSLabel::updateData();
    }
    if (event_.type() == AplusVerifyEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusLabel" << endl;
        ...
        if (a_->t == Et) {
            ...
            for (int i = 0; i < a_->n; i++) {
                if (checkNull(p.a[i]) == MSFalse) {
                    if (!QA(p.a[i]) || p.a[i]->r > 1 || p.a[i]->t != Ct)
                        return MSFalse;
                }
            }
            return MSTrue;
        } else if (a_->t == Ct && a_->r <= 2)
            return MSTrue;
    }
    return MSFalse;
}
