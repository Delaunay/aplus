...
AplusLabelOut::AplusLabelOut(void)
    ...
    , _v(0)
    , _precision(-1)
    , _format(AplusFormatter::BadFormat)
    , _a(0)
    , _tick(0)
    , _grid(0)
    , _value(0)
...
double AplusLabelOut::tickSize(unsigned index_) const
{
    ...
    int i;
    if (_tick != 0) {
        i = (int)(index_ < _tick->n ? index_ : index_ % _tick->n);
        p.i = _tick->p;
    }
...
unsigned AplusLabelOut::gridWidth(unsigned index_) const
{
    ...
    int i;
    if (_grid != 0) {
        i = (int)(index_ < _grid->n ? index_ : index_ % _grid->n);
        p.i = _grid->p;
    }
...
AplusFuncLabel::AplusFuncLabel(A a_, AplusLabelOut* alo_)
    : AplusLabelOut()
{
