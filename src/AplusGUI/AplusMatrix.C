...
AplusMatrix::AplusMatrix(MSWidget* w_)
    : AplusArray(w_)
{
...
Font AplusMatrix::labelFont(int row_)
{
    ...
    if (v != 0) {
        ...
        if (titleFontFunc != 0)
            return titleFontFunc->invoke(v, (A)aplus_nl, row_, -1);
    }
    return font();
}
...
unsigned long AplusMatrix::labelColor(int row_)
{
    ...
    if (v != 0) {
        ...
        if (titleColorFunc != 0)
            return titleColorFunc->invoke(v, (A)aplus_nl, row_, -1);
    }
    return foreground();
}
...
A AplusMatrix::heading(int col_)
{
    ...
    if (v != 0) {
        AOutFunction* titleFunc = AplusModel::getTitleFunc(v);
        if (titleFunc != 0)
            outStr = (A)titleFunc->invoke(v, (A)aplus_nl, -1, col_);
        else {
...
Font AplusMatrix::headingFont(int col_)
{
    ...
    if (v != 0) {
        AFontFunction* titleFontFunc = AplusModel::getTitleFontFunc(v);
        if (titleFontFunc != 0)
            return titleFontFunc->invoke(v, (A)aplus_nl, -1, col_);
    }
    return font();
}
...
unsigned long AplusMatrix::headingColor(int col_)
{
    ...
    if (v != 0) {
        AColorFunction* titleColorFunc = AplusModel::getTitleColorFunc(v);
        if (titleColorFunc != 0)
            return titleColorFunc->invoke(v, (A)aplus_nl, -1, col_);
    }
    return foreground();
}
...
unsigned AplusMatrix::columnLength(unsigned col_)
{
    ...
    if (v != 0 && col_ >= 0) {
        if (isNull(spaceVector()) == MSFalse) {
            if (col_ + 1 < spaceVector()->n)
                return (int)spaceVector()->p[col_ + 1];
            else
                return labelLength();
        }
    }
...
void AplusMatrix::rowIndex(A sv_)
{
    if ((sv_->t == It && sv_->r <= 1) || (sv_->t == Et && sv_->n == 0)) {
        ...
        for (i = 0; i < sv->n; i++) {
            r = (int)sv->p[i];
            if (isRowIndexed(r) == MSFalse)
                drawLabels(panner()->window(), r, r);
        }
        dc(sv);
        sv = rowIndex();
        if (rowIndexXGC() == 0 && sv->n > 0)
            createRowIndexXGC();
        for (i = 0; i < sv->n; i++)
...
void AplusMatrix::colIndex(A sv_)
{
    if ((sv_->t == It && sv_->r <= 1) || (sv_->t == Et && sv_->n == 0)) {
        ...
        for (i = 0; i < sv->n; i++) {
            c = (int)sv->p[i];
            if (isColIndexed(c) == MSFalse)
                drawHeadings(panner()->window(), c, c);
        }
        dc(sv);
        sv = colIndex();
        if (colIndexXGC() == 0 && sv->n > 0)
            createColIndexXGC();
        for (i = 0; i < sv->n; i++)
...
void AplusMatrix::rowIndexBg(unsigned long bg_)
{
    if (rowIndexBg() != bg_) {
        ...
        if (rowIndexXGC() == 0)
            ...
            rowIndexXGC()->foreground(rowIndexBg());
        A sv = rowIndex();
        for (int i = 0; i < sv->n; i++)
            drawLabels(panner()->window(), (int)sv->p[i], (int)sv->p[i]);
    }
...
void AplusMatrix::colIndexBg(unsigned long bg_)
{
    if (colIndexBg() != bg_) {
        ...
        if (colIndexXGC() == 0)
            ...
            colIndexXGC()->foreground(colIndexBg());
        A sv = colIndex();
        for (int i = 0; i < sv->n; i++)
            drawHeadings(panner()->window(), (int)sv->p[i], (int)sv->p[i]);
    }
...
void AplusMatrix::cornerIndexBg(unsigned long bg_)
{
    if (cornerIndexBg() != bg_) {
        ...
        if (cornerIndexXGC() == 0)
            ...
            cornerIndexXGC()->foreground(cornerIndexBg());
        drawLabel(panner()->window());
    }
}
