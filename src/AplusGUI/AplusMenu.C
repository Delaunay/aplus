...
AplusMenu::AplusMenu(MSWidget* widget_, AplusMenu::Orientation orientation_)
    : MSMenuBar(widget_)
{
...
void AplusMenu::receiveEvent(MSEvent& event_)
{
    if (event_.type() == AplusEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateEvent in AplusMenu" << endl;
    ...
    if (event_.type() == AplusVerifyEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusMenu" << endl;
