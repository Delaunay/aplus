...
{
    if (dbg_tmstk)
        cout << "Warning: AplusModel::compare called" << endl;
    return 1;
}
...
{
    if (dbg_tmstk)
        cout << "Warning: AplusModel::set called" << endl;
...
{
    if (dbg_tmstk)
        cout << "Warning: AplusModel::setFromMSF called" << endl;
    return MSError::MSSuccess;
}
...
{
    if (dbg_tmstk)
        cout << "WARNING:  AplusModel::operator= called" << endl;
    return *this;
}
...
void AplusModel::unbindWidgetView(void)
{
    if (_aplusVar != 0) {
        if (dbg_tmstk)
            cout << "Removing Object Association" << endl;
        _aplusVar->o = 0;
...
MSBoolean AplusModel::coupleWidgetView(MSWidgetView* pWidgetView_)
{
    ...
    if (_aplusVar == 0 || _aplusVar->a == 0) {
        if (dbg_tkerr)
            showError("Null V or A in AplusModel::coupleWidgetView");
        return MSFalse;
    }

    if (evaluate() == MSFalse) {
        if (dbg_tkerr)
            showError("Error during dependency evaluation in AplusModel::coupleWidgetView");
        return MSFalse;
    }
    ...
    if (ave.result() == MSTrue) {
        ...
        if (dbg_tmstk)
            cout << "Verify failed in AplusModel::setData" << endl;
...
AInFunction* AplusModel::getInFunc(V v_)
{
    if (v_ != 0) {
        ...
        if (pVarData != 0 && pVarData->inFunc()->func() != 0)
            return pVarData->inFunc();
    }
