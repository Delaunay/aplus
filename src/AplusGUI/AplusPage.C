...
const unsigned long AplusPageEventMask = (ExposureMask | ButtonPressMask | ButtonReleaseMask | Button1MotionMask);
...
AplusPage::AplusPage(MSWidget* parent_)
    : MSCompositeText(parent_)
{
...
AplusPage::~AplusPage(void)
{
    ...
    if (isNull(boxMatrix()) == MSFalse)
        dc(boxMatrix());
    if (isNull(lineMatrix()) == MSFalse)
        dc(lineMatrix());
    if (isNull(boxColors()) == MSFalse)
        dc(boxColors());
    if (isNull(keyBuffer()) == MSFalse)
        dc(keyBuffer());
}
...
void AplusPage::colorTable(A ct_)
{
    if (QA(ct_)) {
        ...
        if (ct_->t == It && ct_->r == (I)2 && ct_->d[1] == (I)2) {
            ...
            P p;
            p.i = ct_->p;
            for (int i = 0; i < (int)ct_->d[0]; i++)
                addColor(i, (unsigned long)p.i[2 * i],
...
void AplusPage::updateFont(Font oldfid_)
{
    ...
    if (oldfid_ != font()) {
        ...
        if (fontString != 0) {
            ...
            if (pos == fs.length()) {
                int ipos = fs.length();
                if ((pos = fs.indexOf('-', 0)) < fs.length())
                    ipos = pos;
                fs.insert("-bold", ipos);
                Font fsid = server()->fontID(fs);
                if (fsid != server()->defaultFont())
                    boldFontID(fsid);
                else
                    boldFontID(0);
            } else
...
char* AplusPage::string(int row_)
{
    ...
    if (a != 0 && row_ < numRows()) {
        P p;
        p.i = (model() != 0) ? ((AplusModel*)model())->data() : 0;
        int len = numCols();
        cp = new char[len + 1];
        int offset = row_ * len;
...
{
    if (index_ == (A)MP(22))
        update(v_, -1, -1, AppendUpdate);
    else if (!index_)
        update(v_, -1, -1, ShapeUpdate);
...
void AplusPage::update(V v_, A index_, A pick_, I ravel_)
{
    if (ravel_) {
        ...
        if (av->r == 2 && index_->r == 1) {
            ...
            for (row = 0; row < (int)av->d[0]; row++) {
                for (i = 0; i < index_->n; i++) {
                    if (co <= index_->p[i] && index_->p[i] < co + nc)
                        cc++;
                }
                if (cc > 0) {
                    index = gv(Et, 2);
        ...
        commonUpdate(v_, index_, pick_, ravel_);
    else
        screenUpdate(index_);
}
...
MSBoolean AplusPage::verifyA(A a_)
{
    ...
    return (0 != a_ && QA(a_) && a_->t == It && a_->r == rank && a_->n == n && a_->d[0] == numRows() && a_->d[1] == numCols()) ? MSTrue : MSFalse;
}

MSBoolean AplusPage::verifyA(A a_, A r_, A c_)
{
    if (isNull(r_) == MSTrue && isNull(c_) == MSTrue)
        return verifyA(a_);
    else if (isNull(r_) == MSTrue) {
        if (0 != a_ && QA(a_) && a_->n == c_->n * numRows() && a_->d[0] == numRows())
            return MSTrue;
    } else if (isNull(c_) == MSTrue) {
        if (0 != a_ && QA(a_) && a_->n == r_->n * numCols() && a_->d[0] == numCols())
            return MSTrue;
    } else if (0 != a_ && QA(a_) && a_->n == r_->n * c_->n)
        return MSTrue;
    return MSFalse;
}
        ...
        for (int i = 0; i < nr; i++) {
            for (int j = 0; j < nc; j += len) {
                ...
                while (j + len < nc) {
                    ...
                    if (nextColor == color && nextBold == bold && nextUline == uline)
                        len++;
                    else
                        break;
                }
...
void AplusPage::screenUpdate(A index_)
{
    if (mapped() == MSTrue) {
        ...
        if (isNull(r) == MSTrue && isNull(c) == MSTrue)
            drawAllRowsCols(index_);
        else if (isNull(r) == MSTrue)
            drawAllRows(index_);
        else if (isNull(c) == MSTrue)
            drawAllCols(index_);
        else
            drawIndexed(index_);
        drawLines(MSFalse);
    ...
    int index_, MSBoolean bold_, BlinkPhase phase_, MSBoolean underline_)
{
    if (str_ != 0) {
        ...
        if (bold_ == MSTrue && boldFontID() == 0)
            XDrawString(display(), window(), textGC(), fs, x + 1, y, str_, len_);
...
void AplusPage::buttonPress(const XEvent* event_)
{
    ...
    if (sensitive() == MSTrue && event_->xbutton.button == Button1 && acceptFocus() == MSTrue && traverseFocus(this) == MSTrue) {
        unsigned int keys;
        ...
        while (keys & mask) {
            if (abs(ex - ix) > 5 && abs(ey - iy) > 5)
                moved = MSTrue;
            if (ix != ox || iy != oy) {
                if (ox > ex) {
                    (oy > ey) ? drawRect(gc, ex, ey, (ox - ex), (oy - ey)) : drawRect(gc, ex, oy, (ox - ex), (ey - oy));
                } else {
                    (oy > ey) ? drawRect(gc, ox, ey, (ex - ox), (oy - ey)) : drawRect(gc, ox, oy, (ex - ox), (ey - oy));
                }
                ox = ix;
                oy = iy;
                if (ox > ex) {
...
void AplusPage::drawBoxes(MSBoolean clear_)
{
    if (isNull(boxMatrix()) == MSFalse) {
        ...
        for (int i = 0; i < boxMatrix()->d[0]; i++) {
            ...
            (clear_ == MSTrue) ? XSetForeground(display(), textGC(), background()) : XSetForeground(display(), textGC(), boxColor(i));
            k += (int)boxMatrix()->d[1];
...
void AplusPage::drawLines(MSBoolean clear_)
{
    if (isNull(lineMatrix()) == MSFalse && lineMatrix()->d[0] > 0) {
        ...
        for (int i = 0; i < lineMatrix()->d[0]; i++) {
            ...
            (clear_ == MSTrue) ? XSetForeground(display(), textGC(), background()) : XSetForeground(display(), textGC(), foreground());
...
void AplusPage::boxColorVector(A colors_)
{
    if (QA(colors_)) {
        ...
        if (colors_->t == It && colors_->r == 1) {
            ...
            cerr << "Page Widget: invalid bounding box color vector specified." << endl;
        drawBoxes(MSFalse);
    }
}
...
void AplusPage::lines(A lines_)
{
    if (QA(lines_)) {
        if (lines_->t == It && lines_->r == 2 && lines_->d[1] == 4) {
            ...
            cerr << "Page Widget: invalid line specified." << endl;
        redraw();
    }
}
...
void AplusPage::boxes(A boxes_)
{
    if (QA(boxes_)) {
        if (boxes_->t == It && boxes_->r == 2 && boxes_->d[1] == 4) {
            ...
            cerr << "Page Widget: invalid bounding box specified." << endl;
        redraw();
    }
}
...
void AplusPage::receiveEvent(MSEvent& event_)
{
    if (event_.type() == AplusEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateEvent in AplusPage" << endl;
    ...
    if (event_.type() == AplusVerifyEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusPage" << endl;
