...
AplusParagraph::AplusParagraph(MSWidget* owner_)
    : MSWidgetView(owner_)
    , MSParagraph()
{
...
void AplusParagraph::receiveEvent(MSEvent& event_)
{
    if (event_.type() == AplusEvent::symbol()) {
        if (dbg_tmstk)
            showError("Received UpdateEvent in AplusParagraph", 2); // print info message
    }
    if (event_.type() == AplusVerifyEvent::symbol()) {
        if (dbg_tmstk)
            showError("Received VerifyEvent in AplusParagraph", 2); // print info message
