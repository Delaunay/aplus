...
PasswordEditor::PasswordEditor(MSWidget* owner_)
    : MSEntryField::FieldEditor(owner_)
{
    _fillChar = '*';
}
...
AplusPassword::AplusPassword(MSWidget* owner_)
    : AplusEntryField(owner_)
{
    _valid = MSFalse;
    if (passwordEditor() != 0)
        delete _editor;
...
MSBoolean AplusPassword::checkPassword(const char* username_, const char* password_)
{
    ...
    if (strcmp(pwd->pw_passwd, "*") == 0) {
        if (kerberosCheck(username_, password_) != 0)
            return MSFalse;
    } else {
