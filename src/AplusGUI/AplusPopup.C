...
AplusPopup::AplusPopup(MSDisplayServer* server_)
    : MSPopup(server_)
{
    AplusModel* am = new AplusModel(0);
    INTERNAL_COUPLE(am);
...
void AplusPopup::receiveEvent(MSEvent& event_)
{
    if (event_.type() == AplusEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateEvent in AplusPopup" << endl;
        redraw();
    }
    if (event_.type() == AplusVerifyEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusPopup" << endl;
...
MSBoolean AplusPopup::traverseFocus(MSWidget* newFocusWidget_)
{
    ...
    if (AplusShell::allowNestedTraversal()) {
        ...
        return MSPopup::traverseFocus(newFocusWidget_);
}
