...
AplusPrintColumn::AplusPrintColumn(MSWidget* owner_)
    : MSWidgetView(owner_)
    , MSPrintColumn(0)
{
    destroyable(MSFalse);
...
void AplusPrintColumn::receiveEvent(MSEvent& event_)
{
    if (event_.type() == AplusEvent::symbol()) {
        if (dbg_tmstk)
            showError("Received UpdateEvent in AplusPrintColumn", 2); // print info message
    }
    if (event_.type() == AplusVerifyEvent::symbol()) {
        if (dbg_tmstk)
            showError("Received VerifyEvent in AplusPrintColumn", 2); // print info message
...
void AplusPrintColumn::constructColumn(void)
{
    ...
    if (verifyData(model()->aplusVar(), aItems) == MSFalse) {
        if (dbg_tmstk)
            showError("Invalid `reportcolumn format", 1); // print warning message
        return;
    }
