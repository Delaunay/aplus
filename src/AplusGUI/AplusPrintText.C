...
::A AplusPrintText::convertMode(unsigned long mode_)
{
    ...
    for (i = 2; i <= MSBottom; i *= 2) {
        if (stringEnumHashTable()->lookup((unsigned long)(mode_ & i)) != 0)
            count++;
    }
    if (count > 0) {
        r = gv(Et, count);
        for (j = 0, i = 2; i <= MSBottom; i *= 2) {
            if ((str = (char*)stringEnumHashTable()->lookup((unsigned long)(mode_ & i))) != 0)
                r->p[j++] = MS(si(str));
        }
    }
    return r;
...
unsigned long AplusPrintText::convertMode(::A sym_)
{
    ...
    if (sym_ != 0) {
        ...
        for (int i = 0; i < (int)sym_->n; i++) {
            if (QS(p[i]))
                mode |= (unsigned long)enumHashTable()->lookup((char*)XS(sym_->p[i])->n);
        }
    }
    return mode;
}
