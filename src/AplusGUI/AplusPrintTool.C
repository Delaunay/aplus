...
void AplusPrintTool::receiveEvent(MSEvent& event_)
{
    if (event_.type() == AplusEvent::symbol()) {
        if (dbg_tmstk)
            showError("Received UpdateEvent in AplusPrintTool", 2); // print info message
    }
    if (event_.type() == AplusVerifyEvent::symbol()) {
        if (dbg_tmstk)
            showError("Received VerifyEvent in AplusPrintTool", 2); // print info message
...
void AplusPrintTool::constructBody(void)
{
    ...
    if (verifyData(v, a) == MSFalse) {
        if (dbg_tmstk)
            showError("Invalid report format"); // print error message
        return;
    }
...
void AplusPrintTool::constructPrintItem(MSPrintManager* pManager_, ::A aItem_)
{
    ...
    case Et:
        ...
        } else // this is a print column
        {
            if (AplusPrintColumn::verifyData(0, aItem_) == MSFalse) {
                if (dbg_tmstk)
                    showError("Invalid print column format in report", 1); // print warning message
                return;
            }
    ...
    default:
        if (dbg_tmstk)
            showError("Invalid print item format in report", 1); // print warning message
        break;
    }
}
...
void AplusPrintTool::constructHeaders(void)
{
    ...
    {
        if (dbg_tmstk)
            showError("Invalid header format in report", 1); // print warning message
        return;
    }
...
void AplusPrintTool::constructFooters(void)
{
    ...
    {
        if (dbg_tmstk)
            showError("Invalid footer format in report", 1); // print warning message
        return;
    }
...
void AplusPrintTool::constructBanner(void)
{
    ...
    {
        if (dbg_tmstk)
            showError("Invalid banner format in report", 1); // warning
        return;
    }
...
void AplusPrintTool::constructPageNumber(void)
{
    ...
    {
        if (dbg_tmstk)
            showError("Invalid page number format in report", 1);
        return;
    }
