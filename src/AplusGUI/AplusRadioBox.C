...
AplusRadioButton::AplusRadioButton(MSWidget* owner_)
    : MSRadioButton((MSRadioBox*)owner_, "")
{
    dynamic(MSTrue);
...
void AplusRadioButton::up(void)
{
    if (radioBox())
        radioBox()->up();
}
void AplusRadioButton::down(void)
{
    if (radioBox())
        radioBox()->down();
}
void AplusRadioButton::left(void)
{
    if (radioBox())
        radioBox()->left();
}
void AplusRadioButton::right(void)
{
    if (radioBox())
        radioBox()->right();
}
...
{
    if (radioBox()->readOnly(radioBox()->buttons().indexOf((unsigned long)(MSWidget*)this)) == MSFalse && sensitive() == MSTrue) {
        return MSFalse;
...
AplusRadioBox::AplusRadioBox(MSWidget* owner_)
    : AplusButtonBox(owner_)
{
    _armedButton = 0;
}
...
void AplusRadioBox::setButtonState(MSActionButton* btn_, unsigned long val_)
{
    ...
    if (abtn)
        (val_ == 0) ? abtn->setDisarmState() : abtn->setArmState();
}
