...
AplusRulePrintItem::AplusRulePrintItem(MSWidget* owner_)
    : MSWidgetView(owner_)
    , MSRulePrintItem()
{
...
AplusRulePrintItem::AplusRulePrintItem(const AplusRulePrintItem& rule_)
    : MSWidgetView()
    , MSRulePrintItem(rule_)
{
...
void AplusRulePrintItem::receiveEvent(MSEvent& event_)
{
    if (event_.type() == AplusEvent::symbol()) {
        if (dbg_tmstk)
            showError("Received UpdateEvent in AplusRulePrintItem", 2); // print info message
    }
    if (event_.type() == AplusVerifyEvent::symbol()) {
        if (dbg_tmstk)
            showError("Received VerifyEvent in AplusRulePrintItem", 2); // print info message
