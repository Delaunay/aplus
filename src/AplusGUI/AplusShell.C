...
AplusShell::AplusShell(MSDisplayServer* server_)
    : MSShell(server_)
{
    AplusModel* am = new AplusModel(0);
    INTERNAL_COUPLE(am);
...
void AplusShell::receiveEvent(MSEvent& event_)
{
    if (event_.type() == AplusEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateEvent in AplusShell" << endl;
        redraw();
    }
    if (event_.type() == AplusVerifyEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusShell" << endl;
...
void AplusShell::insertFocusAfter(MSWidgetView* from_, MSWidgetView* to_)
{
    ...
    if (fromIndex != _traversalList.length() && toIndex != _traversalList.length()) {
...
MSBoolean AplusShell::traverseFocus(MSWidget* newFocusWidget_)
{
    ...
    if (AplusShell::allowNestedTraversal()) {
        MSBoolean lf = MSTrue;
        if (focusWidget() != 0 && focusWidget() != newFocusWidget_)
            lf = loseFocusNotify(focusWidget());
        if (lf == MSTrue) {
        ...
        return MSShell::traverseFocus(newFocusWidget_);
}
