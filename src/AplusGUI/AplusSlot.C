...
AplusSlot::AplusSlot(MSWidget* owner_)
    : MSCompositeFieldBox(owner_)
{
...
{
    removeAllCycles();
    if (isNull(_cycleColors) == MSFalse)
        dc(_cycleColors);
    if (isNull(geometry()) == MSFalse)
        dc(geometry());
}
...
void AplusSlot::updateData(void)
{
    ...
    if (v) {
        ...
        for (int r = 0; r < nr; r++) {
            ...
            if (doMap == MSTrue && fld->mapped() == MSFalse)
                fld->map();
        }
        if (change == MSTrue)
            naturalSize();
...
void AplusSlot::highlightThickness(int t_)
{
    if (fields().length() > 0) {
        if (highlightThickness() != t_) {
            ...
            for (int i = 0; i < fields().length(); i++) {
                fld = (AplusSlotEntryField*)(MSWidget*)fields()(i);
                if (fld->highlightThickness() != t_)
                    fld->highlightThickness(t_);
            }
            unfreezeIfUnfrozen(wasFrozen);
            if (MSFalse == wasFrozen)
                redraw();
        }
    }
}
...
void AplusSlot::shadowThickness(int t_)
{
    if (fields().length() > 0) {
        if (shadowThickness() != t_) {
            ...
            for (int i = 0; i < fields().length(); i++) {
                fld = (AplusSlotEntryField*)(MSWidget*)fields()(i);
                if (fld->shadowThickness() != t_)
                    fld->shadowThickness(t_);
            }
            unfreezeIfUnfrozen(wasFrozen);
            if (MSFalse == wasFrozen)
                redraw();
...
void AplusSlot::highlightColor(unsigned long hlc_)
{
    if (fields().length() > 0) {
        if (highlightColor() != hlc_) {
            ...
            for (int i = 0; i < fields().length(); i++) {
                fld = (AplusSlotEntryField*)(MSWidget*)fields()(i);
                if (fld->highlightColor() != hlc_)
                    fld->highlightColor(hlc_);
            }
            unfreezeIfUnfrozen(wasFrozen);
            if (MSFalse == wasFrozen)
                redraw();
...
int AplusSlot::highlightThickness(void)
{
    if (fields().length() > 0) {
        ...
        return MSCompositeFieldBox::highlightThickness();
}
...
int AplusSlot::shadowThickness(void)
{
    if (fields().length() > 0) {
        ...
        return MSCompositeFieldBox::shadowThickness();
}
            ...
            if (isNull(ag) == MSFalse && ag->t == It) {
                ...
                for (r = 0; r < nr; r++) {
                    ...
                    if (fld != 0) {
                        ...
                        } else if (fld->mapped() == MSFalse)
                            fld->map();
                        if (ax != fld->at().row() || ay != fld->at().column() || h_span != fld->at().columnSpan() || v_span != fld->at().rowSpan() || mstat != fld->mapped())
                            change = MSTrue;
                    }
                }
                if (ag->r <= 1)
    ...
    if (v != 0 && row != fields().length()) {
        ...
        *pick->p = (I)attr;
        ...
        if ((inFunc = AplusModel::getInFunc(v)) != 0)
            ...
            r = defaultInFunc(string_, row);
        if (r != 0 && isNull(r) == MSFalse) {
            r = (A)ic(r);
            if (safeAset(v, r, 0, pick) == 0)
                showError(qs);
            else {
    ...
    if (pick_ != 0) {
        if (QA(pick_)) {
            ...
            if (pick != 0 && QA(pick) && pick->t == It && pick->r <= 1) {
                ...
                cerr << "slot: pick assignment error in update." << endl;
            else
                cerr << "slot: pick assignment error in update." << endl;
            if (pick != 0)
                dc(pick);
...
{
    if (dbg_tmstk)
        cout << "cycleInterval(" << interval_ << ")" << endl;
...
void AplusSlot::receiveEvent(MSEvent& event_)
{
    ...
    if (eventType == AplusEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateEvent in AplusSlot" << endl;
        ...
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusSlot" << endl;
    ...
    {
        if (dbg_tmstk)
            cout << "Received UpdateDataEvent in AplusArray" << endl;
        setClipMode();
        updateData();
    } else if (eventType == AplusUpdateTitleEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateTitleEvent in AplusArray" << endl;
        updateTitle();
    }
}
