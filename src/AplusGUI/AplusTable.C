...
AplusTable::AplusTable(MSWidget* widget_)
    : MSTable(widget_)
{
    if (dbg_tmstk)
        cout << "Creating AplusTable" << endl;
...
void AplusTable::receiveEvent(MSEvent& event_)
{
    ...
    if (eventType == AplusEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateEvent in AplusTable" << endl;
        ...
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusTable" << endl;
        ...
        if (dbg_tmstk)
            cout << "Received UpdateDataEvent in AplusTable" << endl;
        updateData();
    } else if (eventType == AplusProtectEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received an AplusProtectEvent in AplusTable" << endl;
        V v = ((AplusModel*)model())->aplusVar();
...
{
    if (dbg_tmstk)
        cout << "AplusTable::addSenderNotify" << endl;
    ...
    if (var) {
        ...
        if (nVars == 0)
            ...
            for (int i = 0; i < nVars; i++)
                v[i] = (V)getVFromSym(var->cx, (S)XS((::A)p[i]));
            variables(v, nVars);
            delete[] v;
        }
...
{
    if (r_ == -1 && c_ == -1 && t_ == ValueUpdate)
        redrawImmediately();
    else if (t_ == ShapeUpdate || t_ == AppendUpdate)
        updateData();
}
...
Font AplusTable::getVFont(V v_)
{
    ...
    if (varData && varData->pWidgetView())
        fid = varData->pWidgetView()->font();
    else
        fid = server()->defaultFont();
...
void AplusTable::updateBackground(unsigned long oldbg_)
{
    ...
    for (i = 0; i < n; i++)
        ((AplusTableColumn*)tableColumn(i))->setBg(background());
...
void AplusTable::updateFont(Font oldfont_)
{
    ...
    unsigned i, n = numColumns();
    for (i = 0; i < n; i++)
        ((AplusTableColumn*)tableColumn(i))->setFont(font());
    n = hiddenColumnList()->count();
    for (i = n - 1; i < n; i--) {
        MSTableColumn* tc = (MSTableColumn*)hiddenColumnList()->array(i);
