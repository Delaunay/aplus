...
AplusTableColumn::AplusTableColumn(AplusTable* owner_)
    : MSTableColumn((MSTable*)owner_)
    , _breakText(aplus_nl)
{
...
void AplusTableColumn::receiveEvent(MSEvent& event_)
{
    if (event_.type() == AplusEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateEvent in AplusTableColumn" << endl;
    ...
    if (event_.type() == AplusVerifyEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusTableColumn" << endl;
    ...
    {
        if (dbg_tmstk)
            cout << "Received UpdateDataEvent in AplusTableColumn" << endl;
        V v = ((AplusModel*)model())->aplusVar();
...
MSBoolean AplusTableColumn::verifyData(V, A a_)
{
    if (a_ != 0 && QA(a_)) {
        if (a_->t == Et)
            return MSTrue;
        else if (a_->r == 1 && a_->t != Ct)
            return MSTrue;
        else if (a_->t == Ct && a_->r >= 1 && a_->r <= 2)
            return MSTrue;
    }
    return MSFalse;
}
...
{
    if (dbg_tmstk)
        cout << "AplusTableColumn::addSenderNotify" << endl;
...
MSBoolean AplusTableColumn::validate(V v_, const char* string_, unsigned row_)
{
    ...
    if (v_ != 0) {
        ...
        if ((inFunc = AplusModel::getInFunc(v_)) != 0)
            inData = inFunc->invoke(v_, (char*)string_, (int)row_, 0);
        else
            inData = defaultInFunc(v_, string_);
...
{
    if (v_ != 0) {
        if (t_ == ShapeUpdate || t_ == AppendUpdate)
            ((AplusTable*)table())->columnUpdate(column());
        else if (t_ == ValueUpdate) {
            if (column() == 0 && numRows() >= ((AplusTable*)table())->numRows())
                ((AplusTable*)table())->appendUpdate();
            if (r_ == -1)
                ((AplusTable*)table())->cycleColumn(column());
            else
                ((AplusTable*)table())->cycleRowColumn(r_, column());
        }
    }
}
...
Font AplusTableColumn::cellFont(unsigned row_)
{
    ...
    if (fontFunc != 0 && row_ < numRows()) {
        ...
        fid = ((AplusTable*)table())->getVFont(v);
    return fid;
}
...
Font AplusTableColumn::titleFont(void)
{
    if (model() && ((AplusModel*)model())->aplusVar() != 0) {
        ...
        if (avar && isNull(avar->titleAFont()) == MSFalse)
            return avar->titleFont();
        else
            return table()->titleFont();
    }
    return font();
}
...
{
    if (isNull(getVarData()->fontA()) == MSTrue)
        font(fid_);
}
...
MSBoolean AplusTableColumn::breakCriteria(unsigned row_) // FIX THIS
{
    if (breakCriteriaFunc()->func() != 0) {
        ...
        if (isNumericColumn())
            ...
            // else, don't break
            return (row_ != 0) ? (isDuplicate(row_) == MSFalse) || row_ == ((numRows()) - 1) ? MSTrue : MSFalse : MSFalse;
        }
    }
}
