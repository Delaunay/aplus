...
AplusTrace::AplusTrace(MSTraceSet* traceSet_, int col_, const MSSymbol& tag_)
    : MSTrace(traceSet_, col_, tag_)
{
    _gradient = MSFalse;
}
...
V AplusTrace::aplusVar(void)
{
    ...
    if (ats)
        v = ((AplusModel*)ats->model())->aplusVar();
    return v;
}

AplusTraceSet::AplusTraceSet(MSGraph* owner_)
    : MSTraceSet(owner_)
{
...
A AplusTraceSet::enumSymbols(const char* str_)
{
    ...
    if ((k = (TraceEnum)(unsigned long)enumHashTable()->lookup(str_)) >= 0 && (k >= _TraceStyle && k <= _Constraint)) {
        switch (k) {
        case _TraceStyle:
            r = gv(Et, 20);
        ...
        graph()->enumError()->showError(((AplusModel*)model())->aplusVar(), str_);
    return r;
}
...
void AplusTraceSet::lineStyleFuncInvoke(void)
{
    if (lineStyleFunc()->func() != 0) {
        ...
        if (rank == 1) {
            ...
            if ((s = (AplusGraph::LineStyle)(unsigned long)graph()->enumHashTable()->lookup((char*)XS(*sym->p)->n)) > 0 && graph()->styleConvert(s, style, weight) == MSTrue) {
                styleVector << (unsigned long)style;
                weightVector << (unsigned)weight;
            } else
                graph()->enumError()->showError(((AplusModel*)model())->aplusVar(), (char*)XS(sym->p[0])->n);
            dc(sym);
        } else {
...
void AplusTraceSet::fillColorFuncInvoke(void)
{
    ...
    if (f->func() != 0) {
        ...
        if (rank != 1) {
            ...
            uv << (unsigned long)f->invoke(v, a, -1, -1, aplus_nl);
        if (uv.length() > 0)
            MSTraceSet::fillColor(uv);
    }
}
...
void AplusTraceSet::traceSymbolSizeFuncInvoke(void)
{
    if (traceSymbolSizeFunc()->func() != 0) {
        ...
        if (sizeVector.length() > 0)
            symbolSize(sizeVector);
    }
}
...
void AplusTraceSet::altXaxisFuncInvoke(void)
{
    if (altXaxisFunc()->func() != 0) {
        ...
        if (rank == 1) {
            ...
            if (convertXAxis(sym, axis) == MSTrue)
                axisVector << axis;
        } else {
            for (int i = 0; i < traceList().count(); i++) {
...
void AplusTraceSet::altYaxisFuncInvoke(void)
{
    if (altYaxisFunc()->func() != 0) {
        ...
        if (rank == 1) {
            ...
            if (convertYAxis(sym, axis) == MSTrue)
                axisVector << axis;
        } else {
            for (int i = 0; i < traceList().count(); i++) {
...
void AplusTraceSet::constraint(A sym_)
{
    if (!QS(sym_) && (sym_->t == Et && sym_->n > 0 && QS(*sym_->p))) {
        unsigned long k;
        if ((k = (Constraint)(unsigned long)enumHashTable()->lookup((char*)XS(sym_->p[0])->n)) >= 0 && (k >= MSNone && k <= MSG::HoldY)) {
            MSTraceSet::constraint(k);
        } else
            graph()->enumError()->showError(((AplusModel*)model())->aplusVar(), (char*)XS(sym_->p[0])->n);
    }
}
    ...
    else if (ravel_) // ravel update
    {
        ...
        if (isNull(c) == MSTrue) // c is aplus_nl-all cols are updated
        {
            if (isNull(r) == MSTrue)
                ...
                update(v_, (int)r->p[r->n - 1], -1, ValueUpdate);
        } else if (isNull(r) == MSTrue)
            update(v_, -1, -1, ValueUpdate);
        else if (r->n > 0)
            update(v_, (int)r->p[r->n - 1], (int)c->p[c->n - 1], ValueUpdate);
    }
}
...
void AplusTraceSet::receiveEvent(MSEvent& event_)
{
    if (event_.type() == AplusEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateEvent in AplusTraceSet" << endl;
    ...
    if (event_.type() == AplusVerifyEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusTraceSet" << endl;
...
{
    for (int i = 0; i < traceList().count(); i++)
        trace(i)->gradient(x_(i % x_.length()) == 1 ? MSTrue : MSFalse);
    graph()->updateLegendStatus(MSTrue);
    graph()->redrawImmediately();
}
