...
AplusTreeView::AplusTreeView(MSWidget* pOwner_)
    : MSTreeView<AplusTreeItem>(pOwner_)
{
    _margin = 0;
    readOnly(MSTrue);
...
void AplusTreeView::receiveEvent(MSEvent& event_)
{
    if (event_.type() == AplusEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateEvent in AplusTreeView" << endl;
        ...
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusTreeView" << endl;
...
{
    if (r_ == -1 && c_ == -1 && type_ == ValueUpdate)
...
void AplusTreeView::updateFont(Font oldfid_)
{
    ...
    if (v != 0) {
        ...
        if (fid != oldfid_)
            MSTreeView<AplusTreeItem>::updateFont(oldfid_);
    }
}
...
void AplusTreeView::updateForeground(unsigned long oldfg_)
{
    ...
    if (v != 0) {
        ...
        if (fg != oldfg_)
            MSTreeView<AplusTreeItem>::updateForeground(oldfg_);
    }
}
...
void AplusTreeView::updateBackground(unsigned long oldbg_)
{
    ...
    if (v != 0) {
        ...
        if (bg != oldbg_)
            MSTreeView<AplusTreeItem>::updateBackground(oldbg_);
    }
}
        ...
        if (!QS(aNode_) && aNode_->t == Et && aNode_->n > 0) // if this is a symbol vector
        {
            ...
            for (int i = 0; i < pathLen; ++i) {
                ...
                if (cursor.isValid() == MSFalse || cursor.setToFirstExistingChild() == MSFalse) {
                    if (dbg_tmstk)
                        showError("Node with the given name not found in the tree!", 2);
                    return;
                }
                ...
                {
                    if (dbg_tmstk)
                        showError("Incorrect format for the node path - it must be a symbol vector", 2);
                    return;
                }
            ...
            if (cursor.isValid() == MSTrue) {
                ...
                if (dbg_tmstk)
                    showError("Node with the given name not found in the tree!", 2); // print a warning message
                return;
            }
        } else {
            if (dbg_tmstk)
