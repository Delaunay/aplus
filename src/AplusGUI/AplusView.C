...
AplusView::AplusView(MSWidget* widget_)
    : MSList(widget_)
{
...
void AplusView::receiveEvent(MSEvent& event_)
{
    if (event_.type() == AplusEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received UpdateEvent in AplusView" << endl;
    ...
    if (event_.type() == AplusVerifyEvent::symbol()) {
        if (dbg_tmstk)
            cout << "Received VerifyEvent in AplusView" << endl;
            ...
            if (isNull(r) == MSTrue)
                ...
                for (i = 0; i < (int)r->n; i++)
                    if ((int)r->p[i] >= vsb()->max())
                        appendUpdate(MSIndexVector::nullVector());
                for (i = 0; i < (int)r->n; i++)
