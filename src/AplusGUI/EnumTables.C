...
EnumTables::~EnumTables(void)
{
    if (_printEnumHashTable) {
        delete _printEnumHashTable;
        _printEnumHashTable = 0;
    }
    if (_stringEnumHashTable) {
        delete _stringEnumHashTable;
        _stringEnumHashTable = 0;
    }
    if (_pageSizeEnumHashTable) {
        delete _pageSizeEnumHashTable;
        _pageSizeEnumHashTable = 0;
    }
    if (_pageSizeStringHashTable) {
        delete _pageSizeStringHashTable;
        _pageSizeStringHashTable = 0;
    }
    if (_alignStringHashTable) {
        delete _alignStringHashTable;
        _alignStringHashTable = 0;
    }
    if (_alignEnumHashTable) {
        delete _alignEnumHashTable;
        _alignEnumHashTable = 0;
    }
    if (_styleEnumHashTable) {
        delete _styleEnumHashTable;
        _styleEnumHashTable = 0;
    }
    if (_styleStringHashTable) {
        delete _styleStringHashTable;
        _styleStringHashTable = 0;
    }
    if (_reportStyleEnumHashTable) {
        delete _reportStyleEnumHashTable;
        _reportStyleEnumHashTable = 0;
    }
    if (_reportStyleStringHashTable) {
        delete _reportStyleStringHashTable;
        _reportStyleStringHashTable = 0;
    }
    if (_reportCompModeEnumHashTable) {
        delete _reportCompModeEnumHashTable;
        _reportCompModeEnumHashTable = 0;
    }
    if (_reportCompModeStringHashTable) {
        delete _reportCompModeStringHashTable;
        _reportCompModeStringHashTable = 0;
    }
}
...
unsigned long EnumTables::alignFormat(A sym_) /* This originated from AXAlign::format */
{
    ...
    if ((align & MSLeft) && (align & MSRight))
        align -= MSRight;
    if ((align & MSTop) && (align & MSBottom))
        align -= MSBottom;
    return align;
}
...
A EnumTables::formatStyle(unsigned long x_)
{
    ...
    for (i = 2; i <= MSP::PosBelow; i *= 2) {
        if (styleEnumHashTable()->lookup((unsigned long)(x_ & i)) != 0)
            count++;
    }
    if (count > 0) {
        r = gv(Et, count);
        for (j = 0, i = 2; i <= MSP::PosBelow; i *= 2) {
            if ((str = (char*)styleEnumHashTable()->lookup((unsigned long)(x_ & i))) != 0)
                r->p[j++] = MS(si(str));
        }
    }
    return r;
