...
{
    if (MSFalse == isNoDelay() && (-1 == fd() || 0 == setNoDelayOnFd(handle(), wrnlvl(), fd(), 1)))
        _attrs.noDelay(MSTrue);
}
void AipcConnection::turnNoDelayOff(void)
{
    if (MSTrue == isNoDelay() && 0 == (-1 == fd() || setNoDelayOnFd(handle(), wrnlvl(), fd(), 0)))
...
A AipcConnection::getAttr(C* attr_)
{
    ...
    if (-1 != idx) {
        ...
        if (-1 != idx) {
            ...
            R aplus_nl; /* subclasses call parent class here */
    }
}
...
{
    if (isNoDelay())
        setNoDelayOnFd(handle(), wrnlvl(), fd(), 1);
...
{
    if (errstate_ == MSProtocolConnection<A>::Read)
        resetNotify("read");
    else if (errstate_ == MSProtocolConnection<A>::Write)
        resetNotify("write");
    else
        resetNotify("unknownState");
}
...
int AipcConnection::openSync(int timeout_)
{
    ...
    if (fd() < 0) {
        ...
        if (remoteName() == (struct sockaddr*)(0)) {
            close();
            return -1;
        }
