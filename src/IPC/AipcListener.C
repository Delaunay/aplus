    ...
    int socknamelen)
{
    ...
    if (0)
        ipcWarn(0, "", peername, peernamelen, sockname, socknamelen);
}
...
int AipcListener::getListenPort(void)
{
    ...
    if (0 != (getsockname(fd(), localName(), (socklen_t*)&_localNamelen)))
        R - 1;
#else
    if (0 != (getsockname(fd(), localName(), &_localNamelen)))
        R - 1;
#endif
