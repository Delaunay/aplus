...
void TimrConnection::goOff(void)
{
    _timer = 0;
    int myhandle = handle();
    if (Interval == _onExpire) {
        open();
    }
    A data = _aEventData ? getEventData() : getExpiry();
    ACallback(_eventSymbol->n, data);
    dc(data);
    if (this != AipcService::lookup(myhandle, TimrConnection::ServiceType))
        return;
    if (Destroy == _onExpire) {
        delete this;
        return;
    } else if (Restart == _onExpire) {
        open();
    }
}
...
MSBoolean TimrConnection::setExpiry(A aeobj_)
{
    ...
    if (isAbsolute() && (Restart == _onExpire || Interval == _onExpire))
        _onExpire = Hold;
    return MSTrue;
}
...
MSBoolean TimrConnection::setOnExpire(A aOnExpire)
{
    ...
    if (1 != aOnExpire->n || !sym(aOnExpire))
        return MSFalse;
    S oesym = XS(aOnExpire->p[0]);

    if (OnExpireSymbols[0] == oesym)
        _onExpire = Destroy;
        ...
        if (isAbsolute())
            ...
            _onExpire = Restart;
    } else if (OnExpireSymbols[2] == oesym)
        _onExpire = Hold;
...
MSBoolean TimrConnection::setEventSymbol(A newEventSymbol_)
{
    ...
    if (1 != newEventSymbol_->n || !sym(newEventSymbol_))
        return MSFalse;
    _eventSymbol = XS(newEventSymbol_->p[0]);
    return MSTrue;
}

A TimrConnection::getEventSymbol(void)
{
    A z = gs(Et);
    *z->p = MS(_eventSymbol);
    return z;
...
void TimrConnection::open(void)
{
    ...
    if (isPending())
        return;
    if (isAbsolute())
        _timer = new MSAbsoluteTimer(_secs, _usecs,
            new MSMethodCallback<TimrConnection>(this, &TimrConnection::goOff));
    else
        _timer = new MSRegularTimer(_secs, _usecs,
            new MSMethodCallback<TimrConnection>(this, &TimrConnection::goOff));
}
...
int TimrConnection::timrSetAttrIndex(C* attr_)
{
    ...
    for (idx = 0; idx < attrs->n; ++idx)
        if (attrsym == attrs->p[idx])
            break;
...
A TimrConnection::getAttr(C* attr_)
{
    ...
    if (-1 != idx) {
        ...
        if (-1 != idx) {
            ...
            return aplus_nl; /* subclasses call parent class here */
    }
}
