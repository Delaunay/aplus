...
A pA_Connection::getAobjFromBuffer(MSBuffer* bb)
{
    ...
    if (0 > bufftobuff(bb, db, db->maxofbuffer() - db->put()))
        R(A)
        0;
...
A pA_Connection::readBurst(void)
{
    ...
    bbuff.reset();
    if (0 > (n = readTheBuffer(&bbuff, slen1))) {
        mfbuffer(&bbuff);
        R(A)
        0;
    }
...
A pA_Connection::readOne(void)
{
    ...
    if (0 > readTheBuffer(db, db->maxofbuffer() - db->put()))
        R(A)
        0;
    if (db->put() == db->maxofbuffer()) {
        d = (A)(db->minofbuffer());
...
A pA_Connection::setableAttrlist(void)
{
    ...
    int i, idx = 0;
    for (i = 0; i < fromParent->n; ++i)
        z->p[idx++] = fromParent->p[i];
    for (i = 0; i < fromHere->n; ++i)
        z->p[idx++] = fromHere->p[i];
    dc(fromParent);
    return z;
}
...
A pA_Connection::getableAttrlist(void)
{
    ...
    for (i = 0; i < fromParent->n; ++i)
        z->p[idx++] = fromParent->p[i];
    for (i = 0; i < sattrs->n; ++i)
        z->p[idx++] = sattrs->p[i];
    for (i = 0; i < nsattrs->n; ++i)
        z->p[idx++] = nsattrs->p[i];
    dc(fromParent);
    return z;
}
...
MSBoolean pA_Connection::setAttr(C* attr_, A aval_)
{
    ...
    default:
        return AipcConnection::setAttr(attr_, aval_);
    }
    return MSTrue;
}
...
A pA_Connection::getAttr(C* attr_)
{
    ...
    if (-1 != idx) {
        ...
        if (-1 != idx) {
            ...
            return AipcConnection::getAttr(attr_);
    }
}

// M:Syncronous Send and Read

#define LOCALCHANENBL(fds, chan) \
    if (chan)                    \
    Syncfds.fdsset(fds, chan->fd())
...
A pA_Connection::syncSendCover(A msg_, A aTimeout)
{
    ...
    if (writeChannel() == 0)
        return syncErrorResult("nochan", "channel is null");

    /* put stuff on queue */
    MSBuffer* sb = createBufferFromAobj(msg_);
    if (NULL == sb)
        return syncErrorResult("export", "Export routine failed.");
    sendTheBuffer(sb);

    /* while loop on select() until timeout or write queue empty */
    rc = syncWriteLoop(tvp);
    if (0 > rc)
        return syncErrorResult(errorSymbol, errorMessage);
    else
        return gvi(Et, 3, gsym("OK"), gi(rc), writeQueueStatus());
}
...
A pA_Connection::syncReadCover(A aTimeout)
{
    ...
    if (dataobj)
        return gvi(Et, 3, gsym("OK"), dataobj, aplus_nl);
    else
        return syncErrorResult(errorSymbol, errorMessage);
}
