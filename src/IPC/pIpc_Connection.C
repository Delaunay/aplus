...
int pIpc_Connection::send(const A& msg_)
{
    ...
    if (isInReset() || readChannel() == 0)
        return -1;
    if ((msg_->t != Et) || (msg_->n != 2))
        R - 1;

    rhdr = (A)(msg_->p[0]);
    if ((rhdr->t != It) || (2 != rhdr->n))
        R - 1;
    rdat = (A)(msg_->p[1]);
    if ((rdat->t != Ct) || (rdat->r != 1))
        R - 1;
    MSBuffer* bp = new MSBuffer(16 + rdat->n);

    bp->stuff("MGMG", 4); /* magic cookie */
    t = todsec();
    bp->stuff((char*)&t, 4); /* time of day */
    s = rhdr->p[0];
    bp->stuff((char*)&s, 2); /* sequence number */
    s = rhdr->p[1];
    bp->stuff((char*)&s, 2); /* subject */
