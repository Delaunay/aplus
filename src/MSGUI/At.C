...
{
    return MSBoolean(_row == at_.row() && _column == at_.column() && _rowSpan == at_.rowSpan() && _columnSpan == at_.columnSpan() && _constraints == at_.constraints());
}
...
unsigned long At::parseConstraints(const char* constraints_) const
{
    ...
    if (len > 0) {
        ...
        for (int j = 1, k = 1; j <= MaintainHeight && len > 0; j <<= 1, k++) {
            if ((cp = strchr((char*)(void*)constraints_, options[k])) != 0)
                r |= j, len--;
        }
    }
    return r;
