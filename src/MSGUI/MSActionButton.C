...
static const unsigned long MSActionButtonEventMask = (ExposureMask | ButtonPressMask | ButtonReleaseMask | Button1MotionMask);
...
{
    if (parentWidgetType() == MSActionBox::symbol())
        return (MSActionBox*)owner();
    else
        return 0;
}
...
void MSActionButton::up(void)
{
    if (actionBox())
        actionBox()->up();
}
void MSActionButton::down(void)
{
    if (actionBox())
        actionBox()->down();
}
void MSActionButton::left(void)
{
    if (actionBox())
        actionBox()->left();
}
void MSActionButton::right(void)
{
    if (actionBox())
        actionBox()->right();
}
...
void MSActionButton::set(MSAttrValueList& avList_)
{
    ...
    for (unsigned i = 0; i < avList_.length(); i++) {
        if (avList_[i].attribute() == "tag") {
            ...
            index << i;
        } else if (avList_[i].attribute() == "integerTag")
            integerTag(avList_[i].value().asInt()), index << i;
    }
