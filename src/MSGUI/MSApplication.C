...
MSBoolean ServerList::flushServers(void)
{
    ...
    if (_serverList.length() > 0) {
        ...
        for (unsigned i = 0; i < n; i++) {
            MSDisplayServer* pServer = (MSDisplayServer*)_serverList(i);
            if (pServer != 0 && pServer->flush() == MSTrue)
                didWork = MSTrue;
        }
    }
    return didWork;
...
MSDisplayServer* ServerList::serverFromDisplay(Display* pDisplay_)
{
    if (_serverList.length() > 0) {
        ...
        for (unsigned i = 0; i < n; i++) {
            MSDisplayServer* pServer = (MSDisplayServer*)_serverList(i);
            if (pServer != 0 && pServer->display() == pDisplay_)
                return pServer;
        }
    }
    return 0;
...
void MSApplication::init(int argc_, char** argv_)
{
    if (_application == 0) {
        _application = this;
        for (unsigned i = 0; i < argc_; i++)
            _argumentList.append(argv_[i]);
    }
...
MSApplication::~MSApplication(void)
{
    ...
    if (_serverList != 0)
        delete _serverList;
    if (_widgetDestructionQueue != 0)
        delete _widgetDestructionQueue;
...
{
    if (_mainLoop == 0)
        _mainLoop = new MSApplicationMainLoop(this);
    if (_application != 0 && _mainLoop != 0)
        _mainLoop->loop();
    else {
...
MSString MSApplication::version(void)
{
    ...
    if (index2 < aString.length())
        return aString.subString(index2 + 6, index - index2 - 6);
    else
        return MSString("unknown");
}
...
MSString MSApplication::argumentString(void)
{
    ...
    MSString aString;
    for (unsigned i = 0; i < aStringVector.length(); i++)
        aString << aStringVector(i) << " ";
    return aString;
}

MSBoolean MSApplication::addServer(MSDisplayServer* pServer_)
{
    if (_serverList != 0 && pServer_ != 0)
        return _serverList->add(pServer_);
    return MSFalse;
}
...
{
    if (_serverList != 0)
        return _serverList->numberOfServers();
    return 0;
}
...
{
    if (_serverList != 0)
        return _serverList->serverFromDisplay(pDisplay_);
    return 0;
}
...
{
    if (_serverList != 0)
        return _serverList->flushServers();
    return MSFalse;
}
...
{
    if (_mainLoop != 0)
        _mainLoop->flushAndProcess(blocking_);
}
...
{
    if (_mainLoop != 0)
        _mainLoop->continueLoop(continueLoop_);
}
...
MSWidget* MSApplication::findWidget(Window win_)
{
    ...
    for (unsigned i = 0; i < _serverList->_serverList.length(); i++) {
        ...
        if ((unsigned long)pWidget != hashTable->notFound())
            break;
    }
    return pWidget;
}
