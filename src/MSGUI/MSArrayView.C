...
MSArrayView::MSArrayView(MSWidget* owner_, const char* title_)
    : MSRowColumnView(owner_, title_)
{
    init();
}
MSArrayView::MSArrayView(MSWidget* owner_, const MSStringVector& title_)
    : MSRowColumnView(owner_, title_)
{
    init();
}
...
unsigned long MSArrayView::cellForeground(unsigned row_, unsigned)
{
    if (foregroundColors().length() == 0)
        ...
        return foregroundColors()(row_ % foregroundColors().length());
}
unsigned long MSArrayView::cellBackground(unsigned row_, unsigned)
{
    if (backgroundColors().length() == 0)
        ...
        return backgroundColors()(row_ % backgroundColors().length());
}
...
void MSArrayView::adjustFirstColumn(void)
{
    ...
    if (firstColumn() < fixedColumns())
        _firstColumn = fixedColumns();
    else if (firstColumn() > fixedColumns() && firstColumn() + columns() - fixedColumns() >= numColumns()) {
        _firstColumn = (numColumns() > columns()) ? (numColumns() - columns() + fixedColumns()) : fixedColumns();
        _firstColumn = (firstColumn() >= fixedColumns()) ? firstColumn() : fixedColumns();
    }
    if (oldFirstColumn != firstColumn())
        firstColumnChangeNotify();
}
...
void MSArrayView::adjustSelection(void)
{
    ...
    if (selectedRow() >= 0) {
        ...
        } else if (selectedRow() >= numRows())
            _selectedRow = numRows() - 1;
        if (sr != selectedRow() && editor()->mapped() == MSTrue)
            unmapEditor();
    }
}
    ...
    if (numRows() == 0)
        ...
        if (selectedRow() >= 0) {
            if (selectedRow() >= numRows() && selectedColumn() >= numColumns()) {
                ...
                _selectedRow = numRows() - 1;
            else if (selectedColumn() >= numColumns())
                _selectedColumn = numColumns() - 1;
        ...
        if (selectionMode() == MSMultiple && selectedRow() != -1 && selectionVector().indexOf(selectedRow()) == selectionVector().length()) {
...
void MSArrayView::updateVsb(void)
{
    ...
    if (isVsbEnabled() == MSTrue) {
        if (rows() < numRows() && vsb()->width() > 1) {
            ...
            if (vsb()->mapped() == MSFalse)
                vsb()->map();
        } else if (vsb()->mapped() == MSTrue)
            vsb()->unmap();
    }
}
...
void MSArrayView::vsbValueUpdate(void)
{
    if (editorActivate() == MSTrue) {
        if (vsb()->value() < firstRow())
            scrollDown(firstRow() - vsb()->value(), selectedRow());
        else if (vsb()->value() > firstRow())
            scrollUp(vsb()->value() - firstRow(), selectedRow());
    } else
        updateScrollBars();
}
...
{
    cleanUpBottom(window_);
    cleanUpRight(window_);
}

// used for cell updates only
void MSArrayView::drawRowColumn(int row_, int column_)
{
    if (row_ < numRows() && column_ < numColumns() && inColRange(column_) == MSTrue && inRowRange(row_) == MSTrue) {
...
void MSArrayView::drawSelectedRow(Window window_, int row_, MSBoolean select_)
{
    if (row_ < numRows() && inRowRange(row_) == MSTrue) {
        ...
        for (j = 0; j < n && j < fixedColumns(); j++) {
            if (j != selectedColumn()) {
                if (columnBackground(j) != selectedRowBackground())
                    drawCell(window_, x, y, row_, j);
            }
...
void MSArrayView::drawSelectBevel(MSRect& r_, MSBoolean select_)
{
    ...
    (select_ == MSTrue) ? drawRaised(panner()->window(), r_, columnSpacing()) : undrawBevel(panner()->window(), r_, columnSpacing());
}
...
void MSArrayView::updateSelectedRow(int row_)
{
    ...
    if (row_ >= 0 && row_ < numRows()) {
        ...
        if (inRowRange(row_) == MSTrue)
            moveSelection(row_, selectedColumn());
        else if (row_ > lastRow())
            scrollUp(row_ - (firstRow() + rows()) + 1, row_);
        else if (row_ < firstRow())
            scrollDown(firstRow() - row_, row_);
    } else if (row_ < 0)
        moveSelection(-1, -1);
    rowColumnSelection();
}
    ...
    if (row_ != selectedRow() || column_ != selectedColumn()) {
        ...
        if (column_ == -1 || row_ == -1)
            ...
            if (column_ >= fixedColumns() && column_ < numColumns() && row_ >= 0 && row_ < numRows()) {
                if (inColRange(column_) == MSTrue && inRowRange(row_) == MSTrue)
                    moveSelection(row_, column_);
                else {
                    if (inColRange(column_) == MSTrue)
                        _selectedColumn = column_;
                    else {
                        _firstColumn = (column_ > numColumns() - columns() + fixedColumns()) ? numColumns() - columns() + fixedColumns() : column_;
                        _selectedColumn = column_;
                    }
                    if (inRowRange(row_) == MSTrue)
                        _selectedRow = row_;
                    else {
                        _firstRow = (row_ > numRows() - rows()) ? numRows() - rows() : row_;
                        _selectedRow = row_;
                    }
                    redrawImmediately();
                }
...
void MSArrayView::scrollRight(int count_)
{
    if (lastColumn() < numColumns() - 1 && count_ > 0) {
        ...
        if (count_ >= columns() - fixedColumns()) {
            _firstColumn += count_;
            _columns = computeNumVisibleColumns();
            _firstColumn = (firstColumn() > numColumns() - columns() + fixedColumns()) ? numColumns() - columns() + fixedColumns() : firstColumn();
            updateView();
            adjustView();
            ...
            int i;
            for (i = 0; i < count_; i++)
                delta += columnPixelWidth(firstColumn() + i);
            ...
            for (i = firstColumn() + count_; i <= lastColumn(); i++)
            ...
            if (columnSeparator() > 0) {
                ...
                if (n > 0)
                    XFillRectangles(display(), panner()->window(),
                        panner()->backgroundShadowGC(), rects, n);
                delete[] rects;
            }
            int cw = 0;
            for (i = cs; i <= lastColumn() && i < numColumns(); i++)
            ...
            if (dest_x + w + cw < panner()->width() - offset) {
                ...
                for (int r = firstRow(); r <= lastRow() && r < nr; r++) {
                    if (inc > 0)
                        hh = (r % inc == inc - 1 || r == nr - 1) ? rowHeight() - rowSpacing() : rowHeight();
                ...
                if (b > 0)
                    XFillRectangles(display(), panner()->window(), backgroundShadowGC(), back, b);
                if (sb > 0)
                    XFillRectangles(display(), panner()->window(), selectionVectorGC(), sback, sb);
                delete[] back;
                delete[] sback;
            }
    ...
    if (firstColumn() > fixedColumns() && count_ > 0) {
        ...
        if (count_ >= columns() - fixedColumns()) {
            ...
            if (columnSeparator() > 0) {
                ...
                if (n > 0)
                    XFillRectangles(display(), panner()->window(), backgroundShadowGC(), rects, n);
                delete[] rects;
            }
...
void MSArrayView::tab(void)
{
    if (editorActivate() == MSTrue) {
        ...
        int sCol, sRow = selectedRow();
        int i;
        if (sRow == numRows() - 1 && selectedColumn() == nCols - 1)
            return;
        for (i = 0, sCol = selectedColumn() + 1; i < nCols; i++, sCol++) {
            ...
            if (numRows() > 0 && numColumns() > 0) {
                ...
                if (editorActivate() == MSTrue) {
                    ...
                    if (pEvent->xbutton.y < headingsHeight()) {
                        if (pEvent->xbutton.x < labelWidth())
                            headingLabelAreaSelection(pEvent);
                        else
                            headingAreaSelection(pEvent);
                    } else {
                        if (pEvent->xbutton.x < labelWidth())
                            labelAreaSelection(pEvent);
                        else
...
void MSArrayView::headingAreaSelection(const XEvent* pEvent_)
{
    ...
    if (inColRange(col) == MSTrue) {
        if (selectedColumn() != col) {
            int row;
            if (selectedRow() < 0 && numRows() > 0)
                row = 0;
            else
                row = selectedRow();
            if (row >= 0) {
                if (hasCallback(MSWidgetCallback::columnselection) == MSTrue || hasCallback(MSWidgetCallback::columnmenubutton) == MSTrue) {
...
void MSArrayView::labelAreaSelection(const XEvent* pEvent_)
{
    ...
    if (inRowRange(row) == MSTrue) {
        ...
        if (selectedRow() != row) {
            int col;
            if (selectedColumn() < 0 && numColumns() > 0)
                col = 0;
            else
                col = selectedColumn();
            if (row >= 0) {
                if (callback(MSWidgetCallback::rowselection) == 0)
                    selectedRowColumn(row, col);
                else
                    setSelection(row, col);
            }
...
void MSArrayView::dataAreaSelection(const XEvent* pEvent_)
{
    ...
    if (row < numRows() && col < numColumns() && inRowRange(row) == MSTrue && inColRange(col) == MSTrue) {
        if (selectedColumn() != col || selectedRow() != row) {
            ...
            if (pEvent_->xbutton.button == Button1) {
                if (selectionMode() == MSMultiple) {
                    ...
                    if (pEvent_->xbutton.state & ControlMask) {
                        ...
                        // track selection
                        if (selectionVector().indexOf(row) != selectionVector().length())
                            trackUnselection(row, col);
                        else
                            trackSelection(row, col, MSFalse, MSFalse);
                    } else {
                        if (pEvent_->xbutton.state & ShiftMask)
                            trackSelection(row, col, MSFalse, MSTrue);
                        else
                            trackSelection(row, col, MSTrue, MSFalse);
                    }
            ...
            if (pEvent_->xbutton.button == Button1) {
                if (isDoubleClick(pEvent_) == MSTrue)
                    defaultDoubleClickBehavior(pEvent_);
        ...
        if (pEvent_->xbutton.button == Button1)
            defaultButton1Behavior(pEvent_);
        else if (pEvent_->xbutton.button == Button2)
            defaultButton2Behavior(pEvent_);
        else if (pEvent_->xbutton.button == Button3)
            defaultButton3Behavior(pEvent_);
    }
}
...
void MSArrayView::update(const MSIndexVector& aIndexVector_)
{
    if (aIndexVector_.length() == 0) {
        if (vsb()->max() != numRows() || hsb()->max() != numColumns())
            shapeUpdate();
        else
            redrawImmediately();
    } else {
        ...
        if (numColumns() > 1) {
            ...
            for (unsigned i = 0; i < aIndexVector_.length(); i++)
                cycleRowColumn(aIndexVector_(i), 0);
        }
    }
}
...
int MSArrayView::drawWidth(void)
{
    ...
    int nc = numColumns();
    if (nc == 0)
        sum = columns() * columnPixelWidth(0);
    else {
        int lc = lastColumn();
        sum = labelWidth() + fixedColumnPixelWidth();
        for (int i = firstColumn(); i <= lc && i < nc; i++)
...
int MSArrayView::computeXCoord(int column_)
{
    ...
    int i;
    if (column_ < fixedColumns())
        for (i = 0; i < column_; i++)
            xr += columnPixelWidth(i);
    else {
        xr += fixedColumnPixelWidth();
        for (i = firstColumn(); i < column_; i++)
            xr += columnPixelWidth(i);
    }
    return xr;
}
    ...
    MSBoolean rowSelected_, MSBoolean cellSelected_, MSBoolean cellValid_)
{
    ...
    if (fontObj.font() != fid_)
        fontObj.fontStruct(server()->fontStruct(fid_));
    ...
    if (columns() == 1 && x_ + ww > panner()->width() - po)
        ww = panner()->width() - po - x_;
    ...
    if (column_ == lastColumn() || (columns() <= fixedColumns() && column_ == columns() - 1)) {
        if (column_ == numColumns() - 1)
            gc = backgroundShadowGC();
        else {
    ...
    if (viewFormatOutput(buffer, row_, column_) != 0 && buffer.length() > 0) {
        ...
        if (cplen <= columnLength(column_) || w <= acpw) {
            ...
            if (cplen > 0 && utw > acpw)
                cplen = computeMaxTextLength(fontObj.fontStruct(), buffer.string(), acpw, cplen);
        }
        if (cellValid_ == MSFalse) {
            XSetFillStyle(display(), textGC(), FillStippled);
            XSetTSOrigin(display(), textGC(), x_, y_);
        } else
            XSetFillStyle(display(), textGC(), FillSolid);
...
void MSArrayView::drawCell(Window window_, int x_, int y_, int row_, int column_)
{
    ...
    if (frozen() == MSFalse && cl != 0 && row_ < nRows && row_ < numRows() && column_ < numColumns()) {
        ...
        else if (rowSelected == MSTrue)
            bg = selectedRowBackground();
        else
            bg = viewCellBackground(row_, column_);
    ...
    if (rows() > 0) {
        ...
        if (w > 0 && thickness > 0) {
            ...
            if (inc > 0 && lastColumn() >= numColumns() - 1) {
                ...
                if (n > 0)
                    XFillRectangles(display(), window_, backgroundShadowGC(), back, n);
                delete[] back;
            } else {
                ...
                for (; i <= lastRow() && i < nRows; i++) {
                    ...
                    if (selected(i) == MSTrue) {
                        ...
                        XFillRectangle(display(), window_, selectionVectorGC(), x, y, w, dh);
                    y += rh;
                }
                if (k > 0) {
                ...
                if (n > 0)
                    XFillRectangles(display(), window_, selectionVectorGC(), back, n);
                delete[] top;
                delete[] bottom;
                delete[] back;
            }
    ...
    if (inc > 0 && mapped() != MSFalse && frozen() != MSTrue) {
        ...
        if (thickness > 0) {
            ...
            if (count >= 1) {
                ...
                if (re_ == lastRow() && re_ != nRows - 1)
                    h = panner()->height() - y - offset;
                else {
                    for (int r = rs_; r <= re_; r++)
                        h += rowHeight();
                    if (re_ == nRows - 1)
                        h -= rowSpacing();
                }
                if (columns() == 1 && x > panner()->width() - offset)
                    x = panner()->width() - offset;
                    ...
                    if ((c % inc) == (inc - 1) || c == nCols - 1) {
                        ...
                        if (re_ == nRows - 1) {
                            if (cs_ + c == nCols - 1) {
                                ...
                                bottom[n].height = top[n].height = h - rowSpacing();
                        }
    ...
    if (ce_ >= cs_) {
        ...
        if (mapped() != MSFalse && frozen() != MSTrue && inc > 0) {
            ...
            if (thickness > 0) {
                ...
                if (ce_ == lastColumn() && ce_ != numColumns() - 1)
                    w = panner()->width() - x - offset;
                else
                    for (int c = cs_; c <= ce_; c++)
                        w += columnPixelWidth(c);
...
void MSArrayView::drawCycle(int x_, int y_, int row_, int column_, unsigned long color_, MSCycleColorMode mode_)
{
    ...
    if (cl != 0 && row_ < nRows && row_ < numRows() && column_ < numColumns()) {
        ...
        if (mode_ == MSReverseVideo) {
            ...
            if (mode_ == MSBackground)
                bg = color_;
            else if (cellSelected == MSTrue)
                bg = selectedCellBackground();
            else if (rowSelected == MSTrue)
                bg = selectedRowBackground();
            else
...
void MSArrayView::cycleCell(MSColorCycle* cycle_)
{
    if (cycle_->count() == cycle_->numCycles()) {
        if (cycle_->column() == -1)
            drawRow(cycle_->row());
        else if (cycle_->row() == -1)
            drawColumn(cycle_->column());
        else
            drawRowColumn(cycle_->row(), cycle_->column());
    } else
        drawCycle(cycle_);
}
...
void MSArrayView::trackSelection(int startRow_, int startCol_, MSBoolean clear_, MSBoolean shifted_)
{
    ...
    if (shifted_ == MSTrue && lastBlock() != -1 && previousSelectedRow != -1) {
        ...
        if (previousSelectedRow > lastBlock())
            inside = (startRow_ >= lastBlock() && startRow_ <= previousSelectedRow) ? MSTrue : MSFalse;
        else
            inside = (startRow_ <= lastBlock() && startRow_ >= previousSelectedRow) ? MSTrue : MSFalse;

        if (inside == MSTrue) {
            if (startRow_ < previousSelectedRow)
                unfillSelection(startRow_ + 1, previousSelectedRow);
            else
                unfillSelection(previousSelectedRow, startRow_ - 1);
        } else {
            if (startRow_ < lastBlock()) {
                fillSelection(startRow_ + 1, lastBlock() - 1);
                if (previousSelectedRow > lastBlock())
                    unfillSelection(lastBlock() + 1, previousSelectedRow);
            } else {
                fillSelection(lastBlock() + 1, startRow_ - 1);
                if (previousSelectedRow < lastBlock())
                    unfillSelection(previousSelectedRow, lastBlock() - 1);
            }
        }
    ...
    for (i = fr; i <= lr; i++) {
        if (selected(i) == MSTrue || (i >= startSelect && i <= endSelect))
            drawnVector << i;
    }
    ...
    while (keys & Button1Mask) {
        if (sameScreen == True) {
            ix -= panner()->x_origin();
            iy -= panner()->y_origin();
            lastSelectedCol = (ix < (fixedColumnPixelWidth() + labelWidth())) ? xToColumn(ix - labelWidth()) : xToColumn(ix - labelWidth()) + firstColumn();
            if (inColRange(lastSelectedCol) == MSFalse)
                lastSelectedCol = lastColumn();
            if (iy < headingsHeight()) {
                ...
                if (row >= numRows())
                    row = numRows() - 1;
                else if (inRowRange(row) == MSFalse)
                    row--;
            }
            if (row != lastSelectedRow) {
                ...
                for (i = fr; i <= lr; i++) {
                    ...
                    if (i < startSelect || i > endSelect) {
                        ...
                        if (selected(i) == MSTrue) {
                            dv << i;
                            if (drawnVector.indexOf(i) == drawnVector.length())
                                drawSelected(i);
                        } else {
                            if (drawnVector.indexOf(i) != drawnVector.length())
                                undrawSelected(i);
            ...
            if (row != lastSelectedRow) {
                ...
                for (i = fr; i <= lr; i++) {
                    ...
                    if (i < startSelect || i > endSelect) {
                        ...
                        // draw it.
                        if (selected(i) == MSTrue) {
                            if (undrawnVector.indexOf(i) != undrawnVector.length())
                                drawSelected(i);
                        }
                    } else {
                        if (selected(i) == MSTrue) {
...
{
    if (fixedColumns() > 0)
        drawRows(window_, rs_, re_, 0, fixedColumns() - 1);
}
...
{
    int sum = 0;
    for (unsigned i = 0; i < fixedColumns(); i++)
        sum += columnPixelWidth(i);
    return sum;
}
...
{
    int col = (pEvent_->xbutton.x < (fixedColumnPixelWidth() + labelWidth())) ? xToColumn(pEvent_->xbutton.x - labelWidth()) : xToColumn(pEvent_->xbutton.x - labelWidth()) + firstColumn();
    return col;
}
...
{
    if (editorActivate() == MSTrue) {
        if (hsb()->value() < firstColumn())
            scrollLeft(firstColumn() - hsb()->value());
...
void MSArrayView::updateSelectedColumn(int column_)
{
    ...
    if (column_ >= fixedColumns() && column_ < numColumns()) {
        if (inColRange(column_) == MSTrue)
            moveSelection(selectedRow(), column_);
    ...
    if (col != selectedColumn() && selectedColumn() != -1)
        rowColumnSelection();
}
...
void MSArrayView::updateFirstColumn(int column_)
{
    if (column_ < 0)
        column_ = 0;
    else if (column_ >= numColumns())
        column_ = numColumns() - 1;
    ...
    if (column_ >= fixedColumns() && column_ < numColumns() && column_ != firstColumn()) {
        ...
        adjustFirstColumn();
        redrawImmediately();
        if (selectedColumn() >= 0 && selectedColumn() >= numColumns())
            selectedColumn(numColumns() - 1);
        if (oldFirstColumn == firstColumn())
            firstColumnChangeNotify(); // otherwise called in adjustFirstColumn
    }
}
...
{
    if (selectedRow() < numRows() && selectedColumn() < numColumns() && inRowRange(selectedRow()) == MSTrue && inColRange(selectedColumn()) == MSTrue) {
        if (sensitive() == MSTrue) {
    ...
    if (editor()->mapped() == MSTrue) {
        ...
        if (cl != 0) {
            ...
            if (alignment != MSLeft) {
                ...
                MSFontObject fontObj(fontStruct);
                if (fontObj.font() != fid)
                    fontObj.fontStruct(server()->fontStruct(fid));
                int cpw = columnPixelWidth(selectedColumn());
                int slen = string.length();
                int w = fontObj.textWidth(string, slen);
                xoff = alignment == MSCenter ? (cpw - w) / 2 : alignment == MSRight ? cpw - 2 * columnSpacing() - w : 0;
                if (xoff < 0)
                    xoff = 0;
            }
            ...
            if (pEvent_->xbutton.button == Button2)
                mode = MSTextField::OverstrikeMode;
            else
                mode = MSTextField::InsertMode;
...
{
    if (rowSeparator() != interval_) {
        _rowSeparator = interval_;
        redrawImmediately();
    }
}
...
{
    if (columnSeparator() != cs_) {
        _columnSeparator = cs_;
        redrawImmediately();
    }
}
