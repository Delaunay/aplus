...
void MSArrowButton::button1Press(const XEvent*)
{
    ...
    if (repeatInterval() > 0) {
        ...
        if (repeatThreshold() == 0)
            arrowTimer()->expirationInterval(repeatInterval());
        else
...
{
    owner()->activate();
    if (owner()->repeatInterval() != 0)
        expirationInterval(owner()->repeatInterval());
}
...
void MSArrowButton::set(MSAttrValueList& avList_)
{
    ...
    for (unsigned i = 0; i < avList_.length(); i++) {
        if (avList_[i].attribute() == "type") {
            ...
            if (t != typeVector.length())
                type((MSArrow::ArrowType)t);
            index << i;
