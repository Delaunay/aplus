...
unsigned long MSAttrValue::stringToAlignment(const MSString& aString_)
{
    ...
    for (unsigned j = 0; j < alignmentVector.length(); j++) {
        if (alignmentVector(j) == "MSCenter")
            alignment |= MSCenter;
        else if (alignmentVector(j) == "MSTop")
            alignment |= MSTop;
        else if (alignmentVector(j) == "MSBottom")
            alignment |= MSBottom;
        else if (alignmentVector(j) == "MSLeft")
            alignment |= MSLeft;
        else if (alignmentVector(j) == "MSRight")
            alignment |= MSRight;
    }
    return alignment;
}
...
MSShadowStyle MSAttrValue::stringToShadowStyle(const MSString& string_)
{
    if (string_ == "MSRaised")
        return MSRaised;
    else if (string_ == "MSSunken")
        return MSSunken;
    else if (string_ == "MSEtchedIn")
        return MSEtchedIn;
    else if (string_ == "MSEtchedOut")
        return MSEtchedOut;
    else
        return MSFlat;
}
...
{
    if (lineStyle_ == MSDot)
        return MSString("MSDot");
    else if (lineStyle_ == MSDash)
        return MSString("MSDash");
    else
        return MSString("MSSolid");
}
