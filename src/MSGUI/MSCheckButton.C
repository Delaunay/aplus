...
void MSCheckButton::drawCheckButton(void)
{
    if (mapped() == MSTrue && owner()->mapped() == MSTrue) {
        if (armed() == MSTrue)
            selectMSGC().foreground(selectColor());
...
{
    if (parentWidgetType() == MSCheckBox::symbol())
        return (MSCheckBox*)owner();
    else
        return 0;
}
...
void MSCheckButton::disarm(void)
{
    if (armed() == MSTrue) {
        setDisarmState();
        if (activateCallback(MSWidgetCallback::disarm) == MSFalse)
            checkBoxDisarm();
    }
}

void MSCheckButton::up(void)
{
    if (checkBox())
        checkBox()->up();
}
void MSCheckButton::down(void)
{
    if (checkBox())
        checkBox()->down();
}
void MSCheckButton::left(void)
{
    if (checkBox())
        checkBox()->left();
}
void MSCheckButton::right(void)
{
    if (checkBox())
        checkBox()->right();
}
