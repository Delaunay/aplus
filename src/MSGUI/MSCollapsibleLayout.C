...
MSCollapsibleLayout::MSCollapsibleLayout(MSWidget* owner_)
...
MSWidgetVector MSCollapsibleLayout::children(void)
{
    ...
    while ((np = np->next()) != hp)
        vector.append(((MSCollapsibleEntry*)np->data())->widget());
    return vector;
}
    ...
    int& hiddenWidgets_, int& unconstrainedWidgets_)
{
    ...
    while ((np = np->next()) != hp) {
        ...
        if (entry->state() == MSCollapsibleEntry::OPEN) {
            ...
            if (!(entry->resizeConstraints() & At::MinimizeHeight) && !(entry->resizeConstraints() & At::MaintainHeight))
...
void MSCollapsibleLayout::doShownPlacement(int& height_, int& hiddenWidgets_, int& unconstrainedWidgets_)
{
    ...
    while ((np = np->next()) != hp) {
        ...
        if (entry->state() == MSCollapsibleEntry::OPEN) {
            ...
            if (!(entry->resizeConstraints() & At::MinimizeWidth) && !(entry->resizeConstraints() & At::MaintainWidth))
                newWidth = width;
...
void MSCollapsibleLayout::set(MSAttrValueList& avList_)
{
    ...
    for (unsigned i = 0; i < avList_.length(); i++) {
        if (avList_[i].attribute() == "handleBackground")
            handleBackground(avList_[i].value()), index << i;
        else if (avList_[i].attribute() == "handleForeground")
            handleForeground(avList_[i].value()), index << i;
...
void MSCollapsibleLayout::childConfigure(MSWidget* widget_)
{
    ...
    if (internalEvent() == MSFalse) {
        ...
        if (entry != 0 && widget_->mapped() == MSTrue && entry->state() == MSCollapsibleEntry::NOTREADY) {
            entry->setEntry();
            computeSize();
        } else if (entry != 0 && widget_->mapped() == MSTrue) {
            entry->setEntry();
            computeSize();
        }
    }
...
void MSCollapsibleEntry::moveTo(int x_, int y_)
{
    ...
    if (state() == OPEN)
        _widget->moveTo(_owner->_handleSize, y_);
}
