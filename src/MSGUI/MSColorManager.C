...
void MSColorManager::init(void)
{
    ...
    if (MSDefaultBackground == 0)
        MSDefaultBackground = defaultBg();
    if (MSDefaultForeground == 0)
        MSDefaultForeground = defaultFg();
}
...
unsigned long MSColorManager::addColor(const char* color_)
{
    if (color_ != 0) {
        ...
        if (s != 0)
            s = XAllocColor(display(), colormap(), &hardware_def);
        else {
...
unsigned long MSColorManager::pixel(const char* color_)
{
    ...
    if (pixel == colorPixelHashTable().notFound())
        return addColor(color_);
    return pixel;
}
...
static unsigned mergeSortUp(unsigned n_, long* sp_, unsigned* p_, unsigned low_, unsigned high_)
{
    ...
    if (indexCompareUp(sp_, high_, low_)) {
        m = low_;
        low_ = high_;
        high_ = m;
    }
...
MSBoolean MSColorManager::matchColor(XColor& color_)
{
    ...
    for (int i = 1; i <= bestMatchIteration; i++) {
        ...
        for (int j = 0; j < ncols; j++) {
            colorIndex[j] = j;
            closeness[j] = colorFactor * (MSUtil::abs((long)color_.red - (long)colors[j].red) + MSUtil::abs((long)color_.green - (long)colors[j].green) + MSUtil::abs((long)color_.blue - (long)colors[j].blue)) + brightnessFactor * (MSUtil::abs((long)color_.red + (long)color_.green + (long)color_.blue) - ((long)colors[j].red + (long)colors[j].green + (long)colors[j].blue));
        }
        ...
        while ((long)colors[cIndex].red >= (long)color_.red - (long)redCloseness() && (long)colors[cIndex].red <= (long)color_.red + (long)redCloseness() && (long)colors[cIndex].green >= (long)color_.green - (long)greenCloseness() && (long)colors[cIndex].green <= (long)color_.green + (long)greenCloseness() && (long)colors[cIndex].blue >= (long)color_.blue - (long)blueCloseness() && (long)colors[cIndex].blue <= (long)color_.blue + (long)blueCloseness()) {
            if (XAllocColor(display(), colormap(), &colors[cIndex])) {
                if (i == bestMatchIteration)
