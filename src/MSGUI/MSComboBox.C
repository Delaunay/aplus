...
MSComboList::MSComboList(MSComboListShell* owner_)
    : MSStringList(owner_)
{
...
void MSComboList::buttonPress(const XEvent* pEvent_)
{
    if (pEvent_->xbutton.subwindow == 0) {
        ...
        if (child == comboBox()->fieldEditor()->window())
            activate();
        else
            escape();
    } else {
        defaultButtonBehavior(pEvent_);
        if (pEvent_->xbutton.subwindow == panner()->window() && comboBox()->selectionMode() == MSSingle)
            doubleClick();
    }
}
...
void MSComboList::keyPressEvent(const XEvent* pEvent_)
{
    ...
    default:
        ...
        if (MSView::model() != 0 && selectionMode() != MSMultiple) {
            if (selectedText() == MSTrue)
                searchStart = 0;
            else
                searchStart = selectedRow();
            if (keysym == XK_BackSpace)
                selectString(searchStart > 0 ? searchStart - 1 : 0, comboBox()->editString());
            else
                selectString(searchStart, comboBox()->editString());
...
unsigned MSComboList::selectString(int startRow_, const char* pString_)
{
    if (MSView::model() != 0) {
        if (pString_ != 0 && strlen(pString_) > 0) {
            if (startRow_ < 0 || startRow_ >= list().length())
                startRow_ = 0;
            if (startRow_ >= 0) {
...
MSComboListShell::MSComboListShell(MSComboBox* box_)
    : MSWidget(box_->server())
{
    _comboBox = box_;
...
{
    if (listShell() == 0)
        _listShell = new MSComboListShell(this);
}
...
MSAttrValueList& MSComboBox::get(MSAttrValueList& avList_)
{
    avList_ << MSAttrValue("selectionMode",
        (selectionMode() == MSBrowse ? "MSBrowse" : (selectionMode() == MSSingle ? "MSSingle" : "MSMultiple")),
