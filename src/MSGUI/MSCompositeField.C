        ...
        if (pString != 0) {
            ...
            if (len > 0) {
                ...
                if (labelAlignment() == MSLeft)
                    lw = _fieldLabel->width();
                else
                    lw = _fieldValue->width();
...
void MSCompositeField::drawFieldValue(unsigned long fg_, unsigned long bg_)
{
    if (frozen() == MSFalse && owner()->mapped() == MSTrue && mapped() == MSTrue) {
        ...
        if (pString != 0 && buffer.length() > 0) {
            ...
            if (dw > 0) {
                while (_fieldValue->textWidth(pString, len) > dw)
                    len--;
                if (len > 0) {
...
void MSCompositeField::drawFieldValueShadow(void)
{
    ...
    if (st > 0) {
        ...
        case MSRaised:
            drawBevelShadow(id, aRect, st, topGC, bottomGC);
            break;
        case MSSunken:
            drawBevelShadow(id, aRect, st, bottomGC, topGC);
            break;
        case MSEtchedIn:
            drawEtchedShadow(id, aRect, MSEtchedIn, st, bottomGC, topGC);
            break;
        case MSEtchedOut:
            drawEtchedShadow(id, aRect, MSEtchedOut, st, topGC, bottomGC);
            break;
        case MSFlat:
            drawFlatShadow(id, aRect, st, bottomGC);
            break;
        }
    }
}
...
void MSCompositeField::marginHeight(unsigned marginHeight_)
{
    if (marginHeight() != marginHeight_) {
        ...
        if (firstMap() == MSTrue || dynamic() == MSTrue)
            computeSize();
    }
}
...
void MSCompositeField::labelSpacing(unsigned labelSpacing_)
{
    if (labelSpacing_ != labelSpacing()) {
        ...
        if (firstMap() == MSTrue && labelAlignment() == MSTop)
            adjustSize();
    }
}
...
void MSCompositeField::updateBackground(unsigned long oldbg_)
{
    ...
    if (_fieldValue->background() == oldbg_)
        _fieldValue->background(background());
...
void MSCompositeField::updateForeground(unsigned long oldfg_)
{
    ...
    if (_fieldValue->foreground() == oldfg_)
        _fieldValue->foreground(foreground());
    if (_fieldLabel->foreground() == oldfg_)
        _fieldLabel->foreground(foreground());
    if (clipIndicatorForeground() == oldfg_)
        _clipIndicatorForeground = foreground();
    redraw();
}
...
void MSCompositeField::set(MSAttrValueList& avList_)
{
    ...
    for (unsigned i = 0; i < avList_.length(); i++) {
        ...
        if (avList_[i].attribute() == "label")
            ...
            if (avList_[i].value() == "MSLeft")
                valueAlignment(MSLeft);
            else if (avList_[i].value() == "MSCenter")
                valueAlignment(MSCenter);
            else
                valueAlignment(MSRight);
            index << i;
            ...
            if (avList_[i].value() == "MSTop")
                labelAlignment(MSTop);
            else
                labelAlignment(MSLeft);
            index << i;
        } else if (avList_[i].attribute() == "labelJustification") {
            if (avList_[i].value() == "MSLeft")
                labelJustification(MSLeft);
            else if (avList_[i].value() == "MSRight")
                labelJustification(MSRight);
            else if (avList_[i].value() == "MSCenter")
                labelJustification(MSCenter);
            else
                labelJustification(MSLeft);
            index << i;
            ...
            if (avList_[i].value() == "MSClipIndicator")
                clipMode(MSClipIndicator);
            else if (avList_[i].value() == "MSClipStars")
...
MSAttrValueList& MSCompositeField::get(MSAttrValueList& avList_)
{
    ...
    switch (clipMode()) {
    case MSClipIndicator:
        value = "MSClipIndicator";
        break;
    case MSClipStars:
        value = "MSClipStars";
        break;
