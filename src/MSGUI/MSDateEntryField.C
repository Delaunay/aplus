    ...
    KeySym keysym_, unsigned int state_, const char* string_)
{
    ...
    if (keyTranslate(keyPress) == MSFalse)
        MSMonthView::keyPress(ev, keysym_, state_, string_);
}
...
void MSDateEntryField::DateMonthView::buttonPress(const XEvent* pEvent_)
{
    ...
    if (win == 0) {
        ...
        if (child == window())
            MSMonthView::buttonPress(pEvent_);
        else
            escape();
    } else if (win == _leftArrow->window())
        buttonPressNotify(_leftArrow, pEvent_);
    else if (win == _rightArrow->window())
        buttonPressNotify(_rightArrow, pEvent_);
    else
        MSMonthView::buttonPress(pEvent_);
}
...
void MSDateEntryField::DateMonthView::buttonRelease(const XEvent* pEvent_)
{
    ...
    if (_leftArrow != 0)
        buttonReleaseNotify(_leftArrow, pEvent_);
    if (_rightArrow != 0)
        buttonReleaseNotify(_rightArrow, pEvent_);
}
...
void MSDateEntryField::monthDropDown(MSBoolean dropDown_)
{
    ...
    if (dropDown_ == MSTrue && dateShell() == 0)
        _dateShell = new DateShell(this);
}
...
void MSDateEntryField::showMonthView(void)
{
    ...
    if (value().isSet() == MSFalse)
        monthView()->viewDate(MSDate::today());
    else
        monthView()->viewDate(value());
    clearEditor();
    mapEditor();
    MSString buffer;
    ...
    if (x < 0)
        x = 0;
    y += height();
    if (y + dateShell()->height() > server()->height())
        y -= height() + dateShell()->height();
...
void MSDateEntryField::updateForeground(unsigned long oldFg_)
{
    ...
    if (monthView() != 0)
        monthView()->foreground(foreground());
}
...
void MSDateEntryField::generateInputMask(void)
{
    ...
    case MSDate::Slash4:
        mask = "--/--/----";
        break;
    case MSDate::EuropeanDot:
        mask = "--.--.--";
        break;
    case MSDate::EuropeanDot4:
        mask = "--.--.----";
        break;
    default:
        mask = "";
    ...
    if (inputMaskCharacter() != '-' && mask != "")
        mask.change('-', inputMaskCharacter());
