...
void MSDelimiterList::calculateHeadingsHeight(void)
{
    if (delimiterTitle().length() == 0)
        _headingsHeight = rowHeight() / 2 + MSDelimiterPadding;
    else
        _headingsHeight = (int)(rowHeight() * 1.5);
}
...
void MSDelimiterList::drawDelimiterHeaders(Window window_)
{
    ...
    if (delimiterTitle().length() != 0) {
        ...
        arrowHeight = headingsHeight() - MSDelimiterPadding;
    XPoint points[3];
    points[0].y = 0;
    ...
    for (unsigned i = 0; i < delimiterVector().length(); i++) {
        ...
        if (index >= start && index <= end) {
            ...
            if (delimiterEdit() == MSFalse)
                XSetFillStyle(display(), delimiterGC().gc(), FillStippled);
            XBFillPolygon(display(), window_, delimiterGC().gc(), points, 3, Convex, CoordModeOrigin);
            if (delimiterEdit() == MSFalse)
                XSetFillStyle(display(), delimiterGC().gc(), FillSolid);
            XDrawLine(display(), window_, delimiterGC().gc(), x, y0, x, y1);
        }
    }
}
        ...
        if (selected_ == MSTrue) {
            ...
            if (selectedDelimiter() != -1) {
                ...
                if (selected >= firstColumn()) {
                    int x, w;
                    if (selectedDelimiter() == 0)
                        x = offset;
                    else
                        x = computeDelimiterXCoord(delimiterVector()(selectedDelimiter() - 1));
                    x = (x < offset ? offset : x);
...
{
    if (listEdit() == MSTrue)
        MSList::dataAreaSelection(pEvent_);
    else
        server()->bell();
}
...
void MSDelimiterList::dragDelimiter(const XEvent* pEvent_)
{
    ...
    if (delimiterVector().indexOf(startCol) != delimiterVector().length()) {
        ...
        int y0, y1;
        if (delimiterTitle().length() != 0)
            y0 = rootY + headingsHeight() - rowHeight();
        else
            y0 = rootY + headingsHeight() - MSDelimiterPadding;
        ...
        while (keys & Button1Mask || keys & Button2Mask) {
            ...
            if (!sameScreen || ix < 0 || ix >= panner()->width() || ix > last || iy < -20 || iy >= panner()->height() + 20 || newCol > lastColumn()) {
                if (lineDrawn == MSTrue) {
                    XDrawLine(display(), server()->root(), trackGC().gc(), x, y0, x, y1);
                    lineDrawn = MSFalse;
                    col = -1;
        ...
        if (col == -1) {
            ...
            if (selectedDelimiter() >= delimiterVector().length())
                _selectedDelimiter = -1;
...
int MSDelimiterList::xToColumn(int x_)
{
    ...
    if (x_ < 0)
        ...
        col = (x_ - panner()->shadowThickness() - panner()->highlightThickness() - columnSpacing()) / charWidth() + firstColumn();
    int max = actualNumColumns();
...
void MSDelimiterList::delimiterButton2Event(const XEvent* pEvent_)
{
    if (delimiterSelection() == MSTrue) {
        if (delimiterEdit() == MSFalse && delimiterVector().length() == 0)
            server()->bell();
        else
            selectDelimiter(pEvent_);
    } else
        server()->bell();
}
