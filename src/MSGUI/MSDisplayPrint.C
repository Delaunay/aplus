...
void MSDisplayPrint::originInc(MSWidget* widget_)
{
    ...
    if (outputMode() != PPM)
        setClipWindow(widget_->width(), widget_->height());
}
...
void MSDisplayPrint::printInit(MSWidget* owner_)
{
    ...
    if (defaultFontID() == 0)
        defaultFont(owner()->server()->fontID(defaultFontString().string()));
    x_org(0);
    y_org(0);
}
...
#endif
        pout << " - "
             << "MStk Release " << applicationVersionString() << endl;
    ...
    if (outputMode() == EPS) {
        ...
        if (MSRight & pageLayout()) {
            if (pageOrientation() == Landscape) {
                ...
                x_trans = MSPageSizeXTable[pagesize] - rightPixel() - psWidth;
        }
        if (MSTop & pageLayout()) {
            if (pageOrientation() == Landscape) {
                x_trans = topPixel() + psHeight;
            } else
                y_trans = MSPageSizeYTable[pagesize] - topPixel() - psHeight;
        }
...
{
    return defaultFontID() == 0 ? (owner() == 0 ? MSDisplayServer::defaultDisplayServer()->fontID(fontString()) : owner()->server()->fontID(fontString())) : defaultFontID();
}
...
void MSDisplayPrint::updateFont(void)
{
    ...
    if (gcValues().font != 0) {
        ...
        if (tempStr != 0) {
            ...
            _fontInfo = (XFontStruct*)_owner->server()->fontStruct(defaultFontID());
    }
}
...
void MSDisplayPrint::setAttributes(void)
{
    ...
    if (setForeground() == MSTrue)
        update = MSTrue;
    if (setLineAttributes() == MSTrue)
        update = MSTrue;
    if (update == MSTrue)
        pout << endl;
}
    ...
    if (lineStyle() != gcValues().line_style || updateDash() == MSTrue) {
        ...
        case LineDoubleDash:
            if (updateDash() == MSTrue && activeDash() != 0) {
                pout << "[ ";
                for (int i = 0; i < activeDash()->dashCt(); i++)
                    pout << activeDash()->dash(i) << " ";
                pout << "]";
                pout << " ";
                pout << dashOffset();
                pout << " ";
    ...
    if (mode_ == CoordModeOrigin) {
        ...
        for (int i = count = 0; i < n_; i++) {
            ...
            if (i == j - 1 && j != n_ - 1) {
                ...
                pout << "M";
                pout << " ";
                if (gcValues().line_style == LineSolid)
                    i--;
                count = 0;
            }
        }
...
void MSDisplayPrint::printImageString(GC gc_, int x_, int y_, const char* string_, int n_)
{
    if (n_ != 0) {
        updateGC(gc_);
        setFontAttributes();
        if (printMode() == Colorfg)
            gcValues().background = whitePixel();
        if (printMode() == Color || printMode() == Colorfg)
            setBackground(-1);
        else
            setBackground();
...
void MSDisplayPrint::printFillPolygon(GC gc_, XPoint* points_, int n_, int, int mode_, MSBoolean fill_)
{
   ...
   */
    if (n_ > 1) {
        ...
        updateGC(gc_);
        if (printMode() == Colorfg && fill_ != MSTrue)
...
void MSDisplayPrint::updateGC(GC gc_)
{
    ...
    mask = GCLineWidth | GCLineStyle | GCForeground | GCBackground | GCFont | GCCapStyle | GCJoinStyle | GCFillStyle | GCFillRule | GCDashOffset | GCArcMode;
...
void MSDisplayPrint::special(MSBoolean mode_)
{
    if (special() != mode_) {
        if (mode_ == MSTrue)
            pout << "0 w [1 1] 0 d " << endl;
        else
            pout << lineWidth() << " w [] 0 d " << endl;
        _special = mode_;
    }
}
