...
MSDisplayServer::MSDisplayServer(void)
{
    ...
    if (_dpy == 0)
        _name = getenv("DISPLAY");
    else
        _name = DisplayString(_dpy);
    init();
}
...
MSDisplayServer::~MSDisplayServer(void)
{
    ...
    if (_fontManager != 0)
        delete _fontManager;
    if (_shadowHashTable != 0)
        delete _shadowHashTable;
    if (_widgetHashTable != 0)
        delete _widgetHashTable;
    ...
    if (_watchCursor != 0)
        delete _watchCursor;
    if (_menuGrabCursor != 0)
        delete _menuGrabCursor;
    if (_defaultDisplayServer == this)
        _defaultDisplayServer = 0;
    if (_channel != 0)
        delete _channel;
    _channel = 0;
#ifdef MS_KEYPAD_BUG
    if (_keypadKeycodes != 0)
        delete[] _keypadKeycodes;
#endif
...
void MSDisplayServer::dispatchEvent(XEvent& aEvent_)
{
    if (aEvent_.xany.type == MappingNotify) {
        if (aEvent_.xmapping.request == MappingModifier || aEvent_.xmapping.request == MappingKeyboard) {
...
#endif
        if ((unsigned long)pWidget != _widgetHashTable->notFound())
            pWidget->event(&aEvent_);
    }
}
...
{
    if (pServer_ == 0)
        pServer_ = MSDisplayServer::defaultDisplayServer();
...
void MSDisplayServer::addPassiveGrab(MSWidget* pWidget_)
{
    if (pWidget_ != 0) {
        ...
        if (index == _passiveGrabList->length())
            _passiveGrabList->append(pWidget_);
    }
}
    ...
    Window confineTo_, Cursor cursor_, Time time_, MSBoolean revertBack_)
{
    ...
    if (ret == GrabSuccess) {
        ...
        if (revertBack_ == MSTrue) {
            PointerGrabber* pGrabber = new PointerGrabber(grabWindow_, ownerEvent_, eventMask_, pointerMode_,
    ...
    int keyboardMode_, Time time_, MSBoolean revertBack_)
{
    ...
    if (ret == GrabSuccess) {
        ...
        if (revertBack_ == MSTrue) {
            KeyboardGrabber* pGrabber = new KeyboardGrabber(grabWindow_, ownerEvent_, pointerMode_,
                keyboardMode_, time_);
...
void MSDisplayServer::ungrabPointer(Window window_, Time time)
{
    ...
    if (pGrabList->length() > 0) {
            ...
            if (pGrabWidget->_grabWindow == window_) {
                ...
                if (index - 1 >= 0) {
                    ...
                    if (ret != GrabSuccess) {
                        ...
                        _pointerGrabber = pGrabber->_grabWindow;
                } else {
        ...
        if (found == MSFalse) {
            PointerGrabber* pGrabber = (PointerGrabber*)((const MSWidgetVector*)pGrabList)->lastElement();
            int ret = XGrabPointer(_dpy, pGrabber->_grabWindow, pGrabber->_ownerEvents,
...
void MSDisplayServer::ungrabKeyboard(Window window_, Time time)
{
    ...
    if (pGrabList->length() > 0) {
            ...
            if (pGrabWidget->_grabWindow == window_) {
                ...
                if (index - 1 >= 0) {
                    ...
                    if (ret != GrabSuccess) {
                        ...
                        _keyboardGrabber = pGrabber->_grabWindow;
                } else {
        ...
        // revert back to the first one on the list
        if (found == MSFalse) {
            KeyboardGrabber* pGrabber = (KeyboardGrabber*)((const MSWidgetVector*)pGrabList)->lastElement();
            int ret = XGrabKeyboard(_dpy, pGrabber->_grabWindow, pGrabber->_ownerEvents,
                pGrabber->_pointerMode, pGrabber->_keyboardMode, pGrabber->_time);
            if (ret != GrabSuccess) {
                fprintf(stderr, "Warning : Unable to revert keyboard grab to Window %ld\n", pGrabber->_grabWindow);
...
{
    if (_workspaceChangedFunction != 0)
        _workspaceChangedFunction(currentWorkspaceAtom_);
}

MSBoolean MSDisplayServer::isCDERunning(void) const
{
    if (_windowManager == MWM && _mwmWindow != 0)
        return MSTrue;
    return MSFalse;
}
...
MSStringVector MSDisplayServer::workspaceNames(void) const
{
    if (isCDERunning() == MSTrue)
        return _pMWMWidget->workspaceNames();
...
{
    if (isCDERunning() == MSTrue)
        return _pMWMWidget->currentWorkspaceName();
    return MSString();
}
...
{
    if (isCDERunning() == MSTrue)
        return _pMWMWidget->currentWorkspaceAtom();
    return 0;
}
...
{
    if (isCDERunning() == MSTrue)
        return _pMWMWidget->numberOfWorkspaces();
    return 0;
}
...
{
    if (isCDERunning() == MSTrue)
        return _pMWMWidget->workspaceName(wsAtom_);
    return MSString();
}
...
{
    if (isCDERunning() == MSTrue)
        return _pMWMWidget->workspaceAtom(wsName_);
    return 0;
}
...
void MSDisplayServer::setWindowManager(void)
{
    ...
    if ((actualType != mwmInfoAtom) || (actualFormat != 32) || (numItems < 2))
        _windowManager = Other;
    else {
    ...
    if (pMWMInfo != 0)
        XFree((char*)pMWMInfo);
    if (isCDERunning() == MSTrue)
        _pMWMWidget = new MWMWidget(this, _mwmWindow);
}
...
MSString MWMWidget::getWorkspaceName(Atom workspaceAtom_) const
{
    ...
    if (_mwmWindow != 0) {
        ...
        if (status == Success && actualType == XA_STRING)
            theName = (char*)data;
        if (data != 0)
            XFree((char*)data);
        if (pString != 0)
...
void MWMWidget::propertyNotify(const XEvent* pEvent_)
{
    ...
    if (property != _wmStateAtom) {
        if (property == _workspaceCurrentAtom)
            updateCurrentWorkspaceName();
        else if (property == _workspaceListAtom)
            updateWorkspaceList();
        else
            updateWorkspaceInfo(property);
    }
}
...
void MSDisplayServer::correctKeypadKeys(const XEvent* pEvent_, KeySym& keysym_, unsigned int state_, char* pString_)
{
    ...
    if (state_ == _numLockModifier) {
        for (int i = 0; i < keypadDualModeElements; i++) {
