...
MSEntryField::FieldEditor::FieldEditor(MSWidget* owner_)
    : MSTextField(owner_)
{
...
void MSEntryField::updateBackground(unsigned long oldbg_)
{
    ...
    if (oldbg_ == editorForeground())
        editorForeground(background());
}

void MSEntryField::updateForeground(unsigned long oldfg_)
{
    MSCompositeField::updateForeground(oldfg_);
    if (oldfg_ == editorBackground())
        editorBackground(foreground());
}
...
void MSEntryField::configureEditor(void)
{
    ...
    _editor->shadowThickness((sht < MSEntryFieldEditorShadowThickness) ? sht : MSEntryFieldEditorShadowThickness);

    if (_editor->inputMaskMode() == MSFalse)
        _editor->maxLength(editWidth());
    ...
    if (sensitive() == MSTrue && keyTranslate(keyPress) == MSFalse) {
        if (_editor->mapped() == MSTrue)
            ...
            if (keysym_ == XK_Return)
                ...
                if (keysym_ == XK_KP_Add)
                    increment();
                else if (keysym_ == XK_KP_Subtract || keysym_ == XK_F24)
                    decrement();
                else if (keysym_ == XK_Up)
                    up();
                else if (keysym_ == XK_Down)
                    down();
                else if (keysym_ == XK_Left)
                    left();
                else if (keysym_ == XK_Right)
                    right();
                    ...
                    if (_editor->inputMaskMode() == MSFalse) {
                        ...
                        if (_editor->text()(pos) != inputMask()(pos))
                            mapEditor();
                    }
                }
            }
...
{
    if (cycleColorMode() == MSReverseVideo || cycleColors().length() > 0)
        startCycle(cycleColors());
    else
        drawFieldValue();
}
...
void MSEntryField::cycleColors(const MSStringVector& colors_)
{
    ...
    for (int i = 0; i < colors_.length(); i++)
        colors[i] = (server()->pixel(colors_(i)));
    cycleColors(colors);
}
...
{
    if (autoMaskInput() == MSFalse)
        _editor->inputMask(mask_);
}
    ...
    if (string_.length() > 0) {
        ...
        if (nCharToDisplay > 0) {
            ...
            if (dfw > 0) {
                ...
                while (_fieldValue->textWidth(pString, nCharDisplayable) > dfw)
                    nCharDisplayable--;
...
void MSEntryField::clearSelection(MSBoolean redrawAlways_)
{
    ...
    if (redrawAlways_ == MSTrue && redrawn == MSFalse)
        drawFieldValue();
}
...
void MSEntryField::drawFieldValue(unsigned long fg_, unsigned long bg_)
{
    ...
    if (sensitive() == MSFalse)
        MSCompositeField::drawFieldValue(fg_, bg_);
...
int MSEntryField::locateCursorPosition(int x_, const MSString& string_)
{
    ...
    if (dw > 0) {
        ...
        if (len > 0) {
            ...
            if (x_ > xx) {
                int diff = x_ - xx;
                while (_fieldValue->textWidth(pString, len) > diff)
                    len--;
...
void MSEntryField::button1Release(const XEvent* pEvent_)
{
    if (inputFocus() == this && hadFocus() == MSTrue) {
        if (hasCallback(MSWidgetCallback::reference) == MSTrue)
            reference();
        else if (isProtected() == MSFalse)
            startEditing(pEvent_);
    }
            ...
            while (keys & Button1Mask) {
                if (sameScreen == True) {
                    ...
                    if (winx < _fieldValue->x()) {
                        ...
                        if (scrollRight(buffer, cursor) == MSFalse)
