...
{
    if (_direction == Up)
        _entryField->increment();
    else
        _entryField->decrement();
}
...
void MSEntryFieldCombo::buttonColor(ButtonFlag flag_, unsigned long color_)
{
    if (color_ != buttonColor(flag_)) {
        ...
        case ComboButton:
            if (_comboArrow != 0)
                _comboArrow->color(color_);
            break;

        case UpDownArrows:
            if (_upArrow != 0)
                _upArrow->color(color_);
            if (_downArrow != 0)
                _downArrow->color(color_);
            break;
...
unsigned long MSEntryFieldCombo::buttonColor(ButtonFlag flag_) const
{
    ...
    case ComboButton:
        if (_comboArrow != 0)
            return _comboArrow->color();
        break;
    case UpDownArrows:
        if (_upArrow != 0)
            return _upArrow->color();
        break;
    case TextButton:
    default:
        break;
...
{
    if (button_ == TextButton)
        return; // use comboButtonLabel instead
    if (bitState(button_) != flag_) {
        ...
        case UpDownArrows:
            if (_upArrow == 0)
                _upArrow = new MSArrow(this, MSArrow::Up);
            if (_downArrow == 0)
                _downArrow = new MSArrow(this, MSArrow::Down);
            break;
        case ComboButton:
            if (_comboArrow == 0)
                _comboArrow = new MSArrow(this, MSArrow::Down);
        default:
            break;
        }
    }
            ...
            if (_editor->mapped() == MSFalse) {
                ...
                if (focusOK == MSTrue) {
                    if (pEvent_->xbutton.button == Button1 && _buttonFlags != 0) {
                        if (comboOrTextButton() == MSTrue && pEvent_->xbutton.x >= buttonRect().x() && pEvent_->xbutton.y >= buttonRect().y()) {
...
{
    if (buttonState(UpDownArrows) == MSTrue && (_upArrow->selected() == MSTrue || _downArrow->selected() == MSTrue)) {
        if (_upArrow->selected())
            _upArrow->select(MSFalse);
        else if (_downArrow->selected())
            _downArrow->select(MSFalse);
        stopTimers();
    } else if (comboOrTextButton() == MSTrue && _buttonSelected == MSTrue) {
        _buttonSelected = MSFalse;
        drawTextButton(MSFalse);
        if (pEvent_->xbutton.x >= buttonRect().x() && pEvent_->xbutton.x <= buttonRect().x() + buttonRect().width() && pEvent_->xbutton.y >= buttonRect().y() && pEvent_->xbutton.y <= buttonRect().y() + buttonRect().height()) {
            textButtonActivate();
        }
    } else
        MSEntryField::buttonRelease(pEvent_);
}
...
void MSEntryFieldCombo::redraw(void)
{
    if (frozen() == MSFalse && mapped() == MSTrue) {
        ...
        else if (buttonState(TextButton) == MSTrue)
            drawTextButton(_buttonSelected);
        drawShadow();
...
unsigned MSEntryFieldCombo::computeDecorPixelWidth(void)
{
    ...
    } else if (comboOrTextButton() == MSTrue)
        width = computeButtonPixelWidth();
...
{
    if (_buttonFlags == 0)
        return (MSEntryField::computeValuePixelWidth());
    else
        return valueWidth() * _fieldValue->charWidth() + 2 * _fieldValue->offset() + computeDecorPixelWidth() + buttonSpacing();
}
...
void MSEntryFieldCombo::placement(void)
{
    if (_buttonFlags == 0)
        ...
        int vh = _fieldValue->textHeight() + 2 * (marginHeight() + _fieldValue->shadowThickness() + _fieldValue->highlightThickness());
        int lh = _fieldLabel->textHeight() + 2 * (_fieldLabel->shadowThickness() + _fieldLabel->highlightThickness());
...
void MSEntryFieldCombo::labelPixelWidth(unsigned labelPixelWidth_)
{
    if (_fieldLabel != 0) {
        ...
        if (_fieldLabel->width() != labelPixelWidth_) {
            ...
            if (labelAlignment() == MSTop) {
                unsigned lh = _fieldLabel->textHeight() + 2 * (_fieldLabel->shadowThickness() + _fieldLabel->highlightThickness());
...
{
    if (_repeatTimer == 0)
        _repeatTimer = new RepeatTimer(this, MSEntryFieldComboRepeatInterval, dir_);
    else {
