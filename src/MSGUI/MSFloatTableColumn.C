...
{
    return (MSView::model() != 0 && row_ != 0) ? row_ == vector().length() || vector()(row_) != vector()(row_ - 1) ? MSTrue : MSFalse : MSFalse;
}
...
{
    if (MSView::model() != 0)
        return ((MSFloatVector*)_model)->gradeUp();
    else
...
{
    if (MSView::model() != 0)
        return ((MSFloatVector*)_model)->gradeDown();
    else
        return MSIndexVector::nullVector();
}
...
{
    if (MSView::model() != 0)
        ((MSFloatVector*)_model)->permute(indexVector_);
}
...
void MSFloatTableColumn::moveRow(int from_, int to_)
{
    if (MSView::model() != 0) {
        ...
        if (from_ < to_) {
            if (to_ >= vectorModel.length())
                vectorModel.append(item);
            else
