...
{
    if (fontStruct() != 0)
        XFreeFontInfo(0, _fontStruct, 1);
}

MSFontManager::MSFontManager(MSDisplayServer& server_)
    : _fontIDHashTable(MSFontManagerTableSize)
    , _fontDataHashTable(MSFontManagerTableSize)
{
    _server = &server_;
    init();
}
...
void MSFontManager::addFont(const char* fontString_)
{
    ...
    if (fontString_ != 0 && server() != 0) {
        ...
        if (nfs == 0) {
            ...
            if (isoName.length() > 0 && isoName != fontString_)
                nfs = XLoadQueryFont(display(), isoName.string());
            if (nfs == 0) {
                MSMessageLog::warningMessage("Warning - loading font '%s' failed - not available on this system.\n",
                    fontString_);
                return;
            } else
                fontID = XLoadFont(display(), isoName.string());
        } else
            fontID = XLoadFont(display(), fontString_);
        if (nfs != 0 && fontID != 0) {
            MSFontData* data = 0;
            if (fontID != 0) {
        ...
        if (pString == 0)
            ...
            for (i = 0; name_[i] != '\0'; i++)
                if (name_[i] == '-')
                    count++;
            if (count == 1) {
                if (pString[1] >= '1' && pString[1] <= '9')
                    sscanf(pString + 1, "%d", &size);
                else
                    weight = (pString + 1);
            } else if (count == 2) {
                ...
                if (cpnext[1] >= '1' && cpnext[1] <= '9')
                    sscanf(cpnext + 1, "%d", &size);
            }
        }
