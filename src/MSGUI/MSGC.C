    ...
    , _data(0)
{
    ...
    if (msgc_._data->shared() == MSTrue) {
        ...
        create(MSFalse, (XGCValues*)&msgc_.values(), msgc_.mask());
}
...
MSGC::Data* MSGC::findMatch(XGCValues* v_, unsigned long mask_) const
{
    for (int i = MSGC::xgcList().count() - 1; i >= 0; i--) {
        if (isMatch(xgcList().data(i), v_, mask_) == MSTrue)
            return xgcList().data(i);
    }
    return 0;
}
...
MSBoolean MSGC::isMatch(Data* data_, XGCValues* v_, unsigned long m_) const
{
    if (m_ == data_->mask() && server() == data_->server() && data_->shared() == MSTrue) {
        ...
        if (m_ & GCForeground)
            if (v.foreground != v_->foreground)
                return MSFalse;
        if (m_ & GCBackground)
            if (v.background != v_->background)
                return MSFalse;
        if (m_ & GCFont)
            if (v.font != v_->font)
                return MSFalse;
        if (m_ & GCFunction)
            if (v.function != v_->function)
                return MSFalse;
        if (m_ & GCGraphicsExposures)
            if (v.graphics_exposures != v_->graphics_exposures)
                return MSFalse;
        if (m_ & GCLineWidth)
            if (v.line_width != v_->line_width)
                return MSFalse;
        if (m_ & GCLineStyle)
            if (v.line_style != v_->line_style)
                return MSFalse;
        if (m_ & GCCapStyle)
            if (v.cap_style != v_->cap_style)
                return MSFalse;
        if (m_ & GCJoinStyle)
            if (v.join_style != v_->join_style)
                return MSFalse;
        if (m_ & GCFillStyle)
            if (v.fill_style != v_->fill_style)
                return MSFalse;
        if (m_ & GCFillRule)
            if (v.fill_rule != v_->fill_rule)
                return MSFalse;
        if (m_ & GCArcMode)
            if (v.arc_mode != v_->arc_mode)
                return MSFalse;
        ...
        if (m_ & GCTile)
            if (v.tile != v_->tile)
                return MSFalse;
        if (m_ & GCStipple)
            if (v.stipple != v_->stipple)
                return MSFalse;
        if (m_ & GCTileStipXOrigin)
            if (v.ts_x_origin != v_->ts_x_origin)
                return MSFalse;
        if (m_ & GCTileStipYOrigin)
            if (v.ts_y_origin != v_->ts_y_origin)
                return MSFalse;
        if (m_ & GCSubwindowMode)
            if (v.subwindow_mode != v_->subwindow_mode)
                return MSFalse;
        if (m_ & GCClipXOrigin)
            if (v.clip_x_origin != v_->clip_x_origin)
                return MSFalse;
        if (m_ & GCClipYOrigin)
            if (v.clip_y_origin != v_->clip_y_origin)
                return MSFalse;
        if (m_ & GCClipMask)
            if (v.clip_mask != v_->clip_mask)
                return MSFalse;
        if (m_ & GCPlaneMask)
            if (v.plane_mask != v_->plane_mask)
                return MSFalse;
        if (m_ & GCDashOffset)
            if (v.dash_offset != v_->dash_offset)
                return MSFalse;
        if (m_ & GCDashList)
            if (v.dashes != v_->dashes)
                return MSFalse;
