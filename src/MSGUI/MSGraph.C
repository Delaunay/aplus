...
MSGraphStatusWin::MSGraphStatusWin(MSWidget* owner_)
    : MSPrimitive(owner_)
{
...
MSGraphLegend::MSGraphLegend(MSWidget* owner_)
    : MSGraphStatusWin(owner_)
{
    _firstDraw = MSTrue;
 ...
 *
 */
MSGraphEditor::MSGraphEditor(MSWidget* owner_)
    : MSText(owner_)
{
    highlightThickness(1);
    shadowThickness(1);
...
void MSGraphEditor::stringVector(const MSStringVector& aStringVector_)
{
    ...
    for (unsigned i = 0; i < aStringVector_.length(); i++)
        aString << aStringVector_(i) << '\n';
    if (aString.length() > 0)
        aString.drop(-1);
    string(aString);
}

MSGraphRepeatTimer::MSGraphRepeatTimer(MSGraph* g_, unsigned long i_)
    : MSIntervalTimer(i_)
{
    _graph = g_;
}
...
MSGraphDoubleClickTimer::MSGraphDoubleClickTimer(MSGraph* g_, unsigned long i_)
    : MSRegularTimer(i_, 0)
{
    _graph = g_;
}
...
MSGraph::~MSGraph(void)
{
    ...
    if (axisGC() != 0)
        XFreeGC(display(), axisGC());
    if (gridGC() != 0)
        XFreeGC(display(), gridGC());
    if (zeroGC() != 0)
        XFreeGC(display(), zeroGC());
    if (traceGC() != 0)
        XFreeGC(display(), traceGC());
    if (windowGC() != 0)
        XFreeGC(display(), windowGC());
    if (axisTitleGC() != 0)
        XFreeGC(display(), axisTitleGC());
    if (subtitleGC() != 0)
        XFreeGC(display(), subtitleGC());
    if (footnoteGC() != 0)
        XFreeGC(display(), footnoteGC());
    if (editor() != 0)
        safeDestroy(editor());
    if (legend() != 0)
        safeDestroy(legend());
    ...
    if (_xGrid != 0)
        delete[] _xGrid;
    if (printManager() != 0)
        printManager()->removePrintItem(this);
    clearPieData();
}
...
const MSTraceSet* MSGraph::graphTraceSet(const MSSymbol& tag_) const
{
    ...
    for (unsigned i = 0; i < ct; i++)
        if (graphTraceSet(i)->tag() == tag_)
            return graphTraceSet(i);
...
void MSGraph::insertChild(MSWidget* widget_)
{
    if (editor() != 0 && dataWin() != 0 && legend() != 0) {
        ...
        if (visible() != ts->visible()) {
            if (visible() == MSTrue)
                visibilityUnobscuredNotify(ts);
            else
                visibilityObscuredNotify(ts);
        }
    }
}
...
void MSGraph::axisMinimum(double x_, unsigned long i_)
{
    ...
    if (x < DBL_MAX && x > DBL_MIN || x == 0) {
        if (i_ & MSLeft && fabs(_yMin[0] - x_) > DBL_MIN) {
            _yMin[0] = _yMinSet[0] = x_;
            changed = MSTrue;
        }
        if (i_ & MSRight && fabs(_yMin[1] - x_) > DBL_MIN) {
            _yMin[1] = _yMinSet[1] = x_;
            changed = MSTrue;
        }
        if (i_ & MSBottom && fabs(_xMin[0] - x_) > DBL_MIN) {
            _xMin[0] = _xMinSet[0] = x_;
            changed = MSTrue;
        }
        if (i_ & MSTop && fabs(_xMin[1] - x_) > DBL_MIN) {
            _xMin[1] = _xMinSet[1] = x_;
            changed = MSTrue;
        }
        if (changed == MSTrue)
            redrawImmediately();
...
void MSGraph::axisMaximum(double x_, unsigned long i_)
{
    ...
    double x = fabs(x_);
    if (x < DBL_MAX && x > DBL_MIN || x == 0) {
        if (i_ & MSLeft && fabs(_yMax[0] - x_) > DBL_MIN) {
            _yMax[0] = _yMaxSet[0] = x_;
            changed = MSTrue;
        }
        if (i_ & MSRight && fabs(_yMax[1] - x_) > DBL_MIN) {
            _yMax[1] = _yMaxSet[1] = x_;
            changed = MSTrue;
        }
        if (i_ & MSBottom && fabs(_xMax[0] - x_) > DBL_MIN) {
            _xMax[0] = _xMaxSet[0] = x_;
            changed = MSTrue;
        }
        if (i_ & MSTop && fabs(_xMax[1] - x_) > DBL_MIN) {
            _xMax[1] = _xMaxSet[1] = x_;
            changed = MSTrue;
        }
        if (changed == MSTrue)
            redrawImmediately();
...
void MSGraph::axisLabelIncrement(double x_, unsigned long i_)
{
    ...
    if (x < DBL_MAX && x > DBL_MIN) {
        if (i_ & MSLeft && fabs(_yLabelInc[0] - x_) > DBL_MIN) {
            _yLabelInc[0] = x_;
            changed = MSTrue;
        }
        if (i_ & MSRight && fabs(_yLabelInc[1] - x_) > DBL_MIN) {
            _yLabelInc[1] = x_;
            changed = MSTrue;
        }
        if (i_ & MSBottom && fabs(_xLabelInc[0][0] - x_) > DBL_MIN) {
            _xLabelInc[0][0] = x_;
            changed = MSTrue;
        }
        if (i_ & MSTop && fabs(_xLabelInc[1][0] - x_) > DBL_MIN) {
            _xLabelInc[1][0] = x_;
            changed = MSTrue;
        }
        if (changed == MSTrue)
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSBottom && fabs(x_ - _xLabelInc[0][1]) > DBL_MIN) {
        _xLabelInc[0][1] = x_;
        changed = MSTrue;
    }
    if (i_ & MSTop && fabs(x_ - _xLabelInc[1][1]) > DBL_MIN) {
        _xLabelInc[1][1] = x_;
        changed = MSTrue;
    }
    if (changed == MSTrue)
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSLeft && str_ != _yTitle[0]) {
        _yTitle[0] = str_;
        changed = MSTrue;
    }
    if (i_ & MSRight && str_ != _yTitle[1]) {
        _yTitle[1] = str_;
        changed = MSTrue;
    }
    if (i_ & MSBottom && str_ != _xTitle[0]) {
        _xTitle[0] = str_;
        changed = MSTrue;
    }
    if (i_ & MSTop && str_ != _xTitle[1]) {
        _xTitle[1] = str_;
        changed = MSTrue;
    }
    if (changed == MSTrue)
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSLeft && _yTitleFg[0] != fg_) {
        _yTitleFg[0] = fg_;
        changed = MSTrue;
    }
    if (i_ & MSRight && _yTitleFg[1] != fg_) {
        _yTitleFg[1] = fg_;
        changed = MSTrue;
    }
    if (i_ & MSBottom && _xTitleFg[0] != fg_) {
        _xTitleFg[0] = fg_;
        changed = MSTrue;
    }
    if (i_ & MSTop && _xTitleFg[1] != fg_) {
        _xTitleFg[1] = fg_;
        changed = MSTrue;
    }
    if (changed == MSTrue)
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSLeft && _yTitleFont[0] != font_) {
        _yTitleFont[0] = font_;
        changed = MSTrue;
    }
    if (i_ & MSRight && _yTitleFont[1] != font_) {
        _yTitleFont[1] = font_;
        changed = MSTrue;
    }
    if (i_ & MSBottom && _xTitleFont[0] != font_) {
        _xTitleFont[0] = font_;
        changed = MSTrue;
    }
    if (i_ & MSTop && _xTitleFont[1] != font_) {
        _xTitleFont[1] = font_;
        changed = MSTrue;
    }
    if (changed == MSTrue) {
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSLeft && _yTitleAlign[0] != align_) {
        _yTitleAlign[0] = align_;
        changed = MSTrue;
    }
    if (i_ & MSRight && _yTitleAlign[1] != align_) {
        _yTitleAlign[1] = align_;
        changed = MSTrue;
    }
    if (i_ & MSBottom && _xTitleAlign[0] != align_) {
        _xTitleAlign[0] = align_;
        changed = MSTrue;
    }
    if (i_ & MSTop && _xTitleAlign[1] != align_) {
        _xTitleAlign[1] = align_;
        changed = MSTrue;
    }
    if (changed == MSTrue)
        redrawImmediately();
...
void MSGraph::axisTitleAlignment(unsigned long align_, unsigned long i_)
{
    MSBoolean changed = MSFalse;
    if (i_ & MSLeft && _yTitleAlign[0] != align_) {
        _yTitleAlign[0] = formatAlignment(align_);
        changed = MSTrue;
    }
    if (i_ & MSRight && _yTitleAlign[1] != align_) {
        _yTitleAlign[1] = formatAlignment(align_);
        changed = MSTrue;
    }
    if (changed == MSTrue)
        redrawImmediately();
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSLeft && _yLabelAlign[0] != align_) {
        _yLabelAlign[0] = align_;
        changed = MSTrue;
    }
    if (i_ & MSRight && _yLabelAlign[1] != align_) {
        _yLabelAlign[1] = align_;
        changed = MSTrue;
    }
    if (i_ & MSBottom && _xLabelAlign[0] != align_) {
        _xLabelAlign[0] = align_;
        changed = MSTrue;
    }
    if (i_ & MSTop && _xLabelAlign[1] != align_) {
        _xLabelAlign[1] = align_;
        changed = MSTrue;
    }
    if (changed == MSTrue)
        redrawImmediately();
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSBottom && _xSubLabelAlign[0] != align_) {
        _xSubLabelAlign[0] = align_;
        changed = MSTrue;
    }
    if (i_ & MSTop && _xSubLabelAlign[1] != align_) {
        _xSubLabelAlign[1] = align_;
        changed = MSTrue;
    }
    if (changed == MSTrue)
        redrawImmediately();
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSLeft && _yLabelOut[0] != out_) {
        _yLabelOut[0] = out_;
        changed = MSTrue;
    }
    if (i_ & MSRight && _yLabelOut[1] != out_) {
        _yLabelOut[1] = out_;
        changed = MSTrue;
    }
    if (i_ & MSBottom && _xLabelOut[0] != out_) {
        _xLabelOut[0] = out_;
        changed = MSTrue;
    }
    if (i_ & MSTop && _xLabelOut[1] != out_) {
        _xLabelOut[1] = out_;
        changed = MSTrue;
    }
    if (changed == MSTrue) {
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSBottom && _xSubLabelOut[0] != x_) {
        _xSubLabelOut[0] = x_;
        changed = MSTrue;
    }
    if (i_ & MSTop && _xSubLabelOut[1] != x_) {
        _xSubLabelOut[1] = x_;
        changed = MSTrue;
    }
    if (changed == MSTrue) {
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSLeft && _yAxisFg[0] != fg_) {
        _yAxisFg[0] = fg_;
        changed = MSTrue;
    }
    if (i_ & MSRight && _yAxisFg[1] != fg_) {
        _yAxisFg[1] = fg_;
        changed = MSTrue;
    }
    if (i_ & MSBottom && _xAxisFg[0] != fg_) {
        _xAxisFg[0] = fg_;
        changed = MSTrue;
    }
    if (i_ & MSTop && _xAxisFg[1] != fg_) {
        _xAxisFg[1] = fg_;
        changed = MSTrue;
    }
    if (changed == MSTrue)
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSLeft && _yLabelFont[0] != font_) {
        _yLabelFont[0] = font_;
        changed = MSTrue;
    }
    if (i_ & MSRight && _yLabelFont[1] != font_) {
        _yLabelFont[1] = font_;
        changed = MSTrue;
    }
    if (i_ & MSBottom && _xLabelFont[0] != font_) {
        _xLabelFont[0] = font_;
        changed = MSTrue;
    }
    if (i_ & MSTop && _xLabelFont[1] != font_) {
        _xLabelFont[1] = font_;
        changed = MSTrue;
    }
    if (changed == MSTrue) {
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSLeft && _yTickStyle[0] != x_) {
        _yTickStyle[0] = x_;
        changed = MSTrue;
    }
    if (i_ & MSRight && _yTickStyle[1] != x_) {
        _yTickStyle[1] = x_;
        changed = MSTrue;
    }
    if (i_ & MSBottom && _xTickStyle[0] != x_) {
        _xTickStyle[0] = x_;
        changed = MSTrue;
    }
    if (i_ & MSTop && _xTickStyle[1] != x_) {
        _xTickStyle[1] = x_;
        changed = MSTrue;
    }
    if (changed == MSTrue)
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSLeft && _yNumMinorTicks[0] != x_) {
        _yNumMinorTicks[0] = x_;
        changed = MSTrue;
    }
    if (i_ & MSRight && _yNumMinorTicks[1] != x_) {
        _yNumMinorTicks[1] = x_;
        changed = MSTrue;
    }
    if (i_ & MSBottom && _xNumMinorTicks[0] != x_) {
        _xNumMinorTicks[0] = x_;
        changed = MSTrue;
    }
    if (i_ & MSTop && _xNumMinorTicks[1] != x_) {
        _xNumMinorTicks[1] = x_;
        changed = MSTrue;
    }
    if (changed == MSTrue)
        redrawImmediately();
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSLeft && _yMajorTickSize[0] != x_) {
        _yMajorTickSize[0] = x_;
        changed = MSTrue;
    }
    if (i_ & MSRight && _yMajorTickSize[1] != x_) {
        _yMajorTickSize[1] = x_;
        changed = MSTrue;
    }
    if (i_ & MSBottom && _xMajorTickSize[0] != x_) {
        _xMajorTickSize[0] = x_;
        changed = MSTrue;
    }
    if (i_ & MSTop && _xMajorTickSize[1] != x_) {
        _xMajorTickSize[1] = x_;
        changed = MSTrue;
    }
    if (changed == MSTrue)
        redrawImmediately();
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSLeft && _yMinorTickSize[0] != x_) {
        _yMinorTickSize[0] = x_;
        changed = MSTrue;
    }
    if (i_ & MSRight && _yMinorTickSize[1] != x_) {
        _yMinorTickSize[1] = x_;
        changed = MSTrue;
    }
    if (i_ & MSBottom && _xMinorTickSize[0] != x_) {
        _xMinorTickSize[0] = x_;
        changed = MSTrue;
    }
    if (i_ & MSTop && _xMinorTickSize[1] != x_) {
        _xMinorTickSize[1] = x_;
        changed = MSTrue;
    }
    if (changed == MSTrue)
        redrawImmediately();
...
void MSGraph::grid(unsigned long x_)
{
    if (grid() != x_) {
        _grid = x_;
        if ((_grid & MSLeft) && (_grid & MSRight))
            _grid -= MSRight;
        if ((_grid & MSTop) && (_grid & MSBottom))
            _grid -= MSTop;
        redrawImmediately();
    }
}
...
void MSGraph::zeroAxis(unsigned long x_)
{
    if (zeroAxis() != x_) {
        _zeroAxis = x_;
        if ((_zeroAxis & MSLeft) && (_zeroAxis & MSRight))
            _zeroAxis -= MSRight;
        if ((_zeroAxis & MSTop) && (_zeroAxis & MSBottom))
            _zeroAxis -= MSTop;
        redrawImmediately();
    }
}
...
void MSGraph::topAxisDataMargin(double x_, unsigned long i_)
{
    MSBoolean changed = MSFalse;
    if (x_ >= 0. && x_ < 100.) {
        if (i_ & MSLeft && fabs(x_ - _xMargin[1][0]) > DBL_MIN) {
            _xMargin[1][0] = x_ < 1. ? x_ : x_ / 100.;
            changed = MSTrue;
        }
        if (i_ & MSRight && fabs(x_ - _xMargin[1][1]) > DBL_MIN) {
            _xMargin[1][1] = x_ < 1. ? x_ : x_ / 100.;
            changed = MSTrue;
        }
        if (changed == MSTrue)
            redrawImmediately();
...
void MSGraph::bottomAxisDataMargin(double x_, unsigned long i_)
{
    MSBoolean changed = MSFalse;
    if (x_ >= 0. && x_ < 100.) {
        if (i_ & MSLeft && fabs(x_ - _xMargin[0][0]) > DBL_MIN) {
            _xMargin[0][0] = x_ < 1. ? x_ : x_ / 100.;
            changed = MSTrue;
        }
        if (i_ & MSRight && fabs(x_ - _xMargin[0][1]) > DBL_MIN) {
            _xMargin[0][1] = x_ < 1. ? x_ : x_ / 100.;
            changed = MSTrue;
        }
        if (changed == MSTrue)
            redrawImmediately();
...
void MSGraph::leftAxisDataMargin(double x_, unsigned long i_)
{
    ...
    if (x_ >= 0. && x_ < 100.) {
        x_ = x_ > 1 ? x_ / 100. : x_;
        if (i_ & MSBottom && fabs(x_ - _yMargin[0][0]) > DBL_MIN) {
            _yMargin[0][0] = x_;
            changed = MSTrue;
        }
        if (i_ & MSTop && fabs(x_ - _yMargin[0][1]) > DBL_MIN) {
            _yMargin[0][1] = x_;
            changed = MSTrue;
        }
        if (changed == MSTrue)
...
void MSGraph::rightAxisDataMargin(double x_, unsigned long i_)
{
    ...
    x_ = x_ > 1 ? x_ / 100. : x_;
    if (x_ >= 0. && x_ < 1.) {
        if (i_ & MSBottom && fabs(x_ - _yMargin[1][0]) > DBL_MIN) {
            _yMargin[1][0] = x_;
            changed = MSTrue;
        }
        if (i_ & MSTop && fabs(x_ - _yMargin[1][1]) > DBL_MIN) {
            _yMargin[1][1] = x_;
            changed = MSTrue;
        }
        if (changed == MSTrue)
...
{
    MSBoolean changed = MSFalse;
    if (i_ & MSLeft && _yMode[0] != x_) {
        _yMode[0] = x_;
        changed = MSTrue;
    }
    if (i_ & MSRight && _yMode[1] != x_) {
        _yMode[1] = x_;
        changed = MSTrue;
    }
    if (i_ & MSBottom && _xMode[0] != x_) {
        _xMode[0] = x_;
        changed = MSTrue;
    }
    if (i_ & MSTop && _xMode[1] != x_) {
        _xMode[1] = x_;
        changed = MSTrue;
    }
    if (changed == MSTrue)
...
void MSGraph::margin(double x_, unsigned long i_)
{
    if ((x_ = x_ > 1 ? x_ / 100. : x_) >= 0 && x_ < 1.) {
        MSBoolean changed = MSFalse;
        if (i_ & MSLeft && fabs(x_ - _margin[0]) > DBL_MIN) {
            _margin[0] = x_;
            changed = MSTrue;
        }
        if (i_ & MSRight && fabs(x_ - _margin[1]) > DBL_MIN) {
            _margin[1] = x_;
            changed = MSTrue;
        }
        if (i_ & MSTop && fabs(x_ - _margin[2]) > DBL_MIN) {
            _margin[2] = x_;
            changed = MSTrue;
        }
        if (i_ & MSBottom && fabs(x_ - _margin[3]) > DBL_MIN) {
            _margin[3] = x_;
            changed = MSTrue;
        }
        if (changed == MSTrue)
...
void MSGraph::pieOffsetMargin(double x_)
{
    if ((x_ = x_ > 1 ? x_ / 100. : x_) >= 0 && x_ < 1.) {
        ...
        MSMessageLog::warningMessage("MSGraph pie margin value out of range\n");
}
...
void MSGraph::legendAlignment(unsigned long align_)
{
    if ((align_ = formatAlignment(align_)) != _legendAlign) {
        if (_legendAlign == MSNone)
            legend()->map();
        else if (align_ == MSNone)
            legend()->unmap();
...
void MSGraph::updateFont(Font oldfont_)
{
    ...
    if (fid != oldfont_) {
        dataWin()->font(fid);
        for (int i = 0; i < 2; i++) {
            if (oldfont_ == yTitleFont(i))
                _yTitleFont[i] = fid;
            if (oldfont_ == xTitleFont(i))
                _xTitleFont[i] = fid;
            if (oldfont_ == yLabelFont(i))
                _yLabelFont[i] = fid;
            if (oldfont_ == xLabelFont(i))
                _xLabelFont[i] = fid;
        }
...
void MSGraph::updateForeground(unsigned long oldfg_)
{
    ...
    if (pixel != oldfg_) {
        ...
        for (int i = 0; i < 2; i++) {
            if (oldfg_ == yTitleForeground(i))
                _yTitleFg[i] = pixel;
            if (oldfg_ == xTitleForeground(i))
                _xTitleFg[i] = pixel;
            if (oldfg_ == yAxisForeground(i))
                _yAxisFg[i] = pixel;
            if (oldfg_ == xAxisForeground(i))
                _xAxisFg[i] = pixel;
        }
        ...
        if (oldfg_ == legend()->foreground())
            legend()->foreground(pixel);
        updateLegendStatus(MSTrue);
        redrawImmediately();
    }
}
...
void MSGraph::updateBackground(unsigned long oldbg_)
{
    ...
    if (pixel != oldbg_) {
        if (legendBackground() == oldbg_)
            legendBackground(pixel);
        if (editor()->background() == oldbg_)
            editor()->background(pixel);
...
void MSGraph::redrawImmediately(MSBoolean update_, MSBoolean append_)
{
    if ((mapped() == MSTrue && frozen() != MSTrue && update_ != MSTrue) || outputMode() == Print) {
        ...
        busyOn();
        if (debug() == MSTrue)
            printDebugInfo(outputMode() == Draw ? "redraw" : "print");
        ...
        if (graphUIMode() == AddTrace || graphUIMode() == MoveTrace) {
            ...
            XUndefineCursor(display(), window());
        if (updateLegendStatus() == MSTrue)
            drawLegends();
        _graphPixmap->unlock();
    } else if (update_ == MSTrue) {
        _graphPixmap->lock();
        if (debug() == MSTrue)
            printDebugInfo(append_ == MSTrue ? "append" : "update");
        XFillRectangle(display(), graphPixmap(), clearGC(),
            offset(), offset(), width() - 2 * offset(), height() - 2 * offset());
        drawGrid();
        drawRule();
...
MSBoolean MSGraph::longPress(const XEvent* event_)
{
    ...
    if (event_->xbutton.type == ButtonRelease) {
        if ((event_->xbutton.time - time) >= longPressTime() && (event_->xbutton.time - time) < 2000 && abs(event_->xbutton.x - x) <= MotionThreshold && abs(event_->xbutton.y - y) <= MotionThreshold)
            return MSTrue;
    }
    return MSFalse;
}
...
void MSGraph::buttonPress(const XEvent* event_)
{
    if (isProtected() == MSFalse) {
        ...
        if (sw == editor()->window()) {
            ...
            if (event_->xbutton.button == Button1)
                button1Press(event_);
            else if (event_->xbutton.button == Button2)
                button2Press(event_);
    ...
    case Normal:
        if (graphUIMode() == AddTrace) {
            ...
            for (int i = 0; i < newtraceCt(); i++) {
                ...
                if (nt(i)->pointCount() > 1)
                    if (activateCallback(MSWidgetCallback::addtrace) != MSTrue) {
            ...
            newtraceDealloc();
            unfreeze();
        } else if (graphUIMode() == AddTextTrace)
            editorActivate();
        break;
...
void MSGraph::button1Press(const XEvent* event_)
{
    if (graphUIMode() == Normal && (event_->xbutton.state & ControlMask)) {
    ...
    } else if (editor()->mapped() == MSTrue)
        editorEscape();
    else if (doubleClick(event_) == MSTrue) {
        if (highlightLegendStatus() == MSTrue)
            highlightLegendStatus(MSFalse);
        ...
        if (selectTrace() == findSelectableTrace(event_))
            moveTrace(event_);
        else if (selectTrace() == findTextTrace(event_))
            moveTextTrace(event_);
...
void MSGraph::buttonRelease(const XEvent* event_)
{
    ...
    if (longPress(event_) == MSTrue && (tp = findDataPoint(event_)) != 0) {
        ...
        activateCallback(MSWidgetCallback::graphreference);
}
...
void MSGraph::button2Press(const XEvent* event_)
{
    ...
    if (editor()->mapped() == MSTrue)
        ...
        if ((selectTrace() != 0 && selectTrace() == findTextTrace(event_)) || (selectTrace() == 0 && (_selectTrace = findTextTrace(event_)) != 0)) {
            editTextTrace();
        } else {
            enterTextTrace();
        }
    ...
    if (isProtected() == MSFalse && keyTranslate(keyPress) == MSFalse) {
        if (keysym_ == XK_Tab) {
            if (state_ & (ControlMask | Mod1Mask)) {
                ...
                acceptTab(MSTrue);
            } else if (state_ & ShiftMask)
                traverseToPrevious();
            else if (state_ & Mod1Mask || editor()->mapped() != MSTrue)
                traverseToNext();
        } else if (editor()->mapped() == MSTrue)
            keyTranslateNotify(editor(), event_, keysym_, state_, str_);
        else if (keysym_ == XK_Escape) {
            if (graphUIMode() == AddTrace) {
                selectInput(MSGraphEventMask);
                graphUIModeSet(Normal);
                newtraceDealloc();
                if (graphZoomStatus() == MSTrue)
                    XDefineCursor(display(), window(), zoomCursor());
                else
                    XUndefineCursor(display(), window());
                redrawImmediately();
                ...
                graphUIModeSet(Normal);
                unHighlightTrace();
            } else if (marketProfileStatus() == MSTrue)
                tpoBreakChar(65);
        } else if ((keysym_ == XK_Delete || keysym_ == XK_KP_Decimal) && (event_->xkey.state & Mod1Mask) == Mod1Mask) {
            if (selectTrace() != 0)
                deleteTrace();
        } else if (keysym_ == XK_Left || keysym_ == XK_Right || keysym_ == XK_Up || keysym_ == XK_Down) {
        ...
        } else if (marketProfileStatus() == MSTrue && keysym_ > 65 && keysym_ < 125)
            tpoBreakChar((int)keysym_);
    }
}
...
void MSGraph::update(MSTraceSet* ts_, const MSIndexVector& i_)
{
    ...
    if (mapped() == MSTrue && graphUIMode() != AddTrace && (ts_->lastDataCount() > 0 && row >= ts_->lastDataCount() - 1)) {
        for (int i = 0; i < ts_->traceList().count(); i++) {
            if ((trace = ts_->trace(i)) != 0 && trace->dataCount() > 2 && trace->traceSet()->overlap() == MSFalse) {
                ys = trace->yAxis();
                xs = trace->xAxis();
                if (trace->style() >= Close) {
                    if (trace->y(high + trace->offset(), row) <= yMaxData(ys) && trace->y(high + trace->offset(), row) >= yMinData(ys) && trace->y(low + trace->offset(), row) <= yMaxData(ys) && trace->y(low + trace->offset(), row) >= yMinData(ys)) {
                        update = MSTrue;
                        append = row >= ts_->lastDataCount() ? MSTrue : MSFalse;
                    } else {
                        update = append = MSFalse;
                        break;
                    }
    ...
    if (update == MSTrue) {
        ...
        if (updateRect()->x() > x_end() || updateRect()->width() == 0 || updateRect()->height() == 0)
            update = MSFalse;
    }
    if (i_.length() == 0 || ts_->lastDataCount() == 0 || legendStyle() == LastValue)
        legend()->redraw();
    if (legendStyle() == LastValue)
        updateLegendStatus(MSTrue);
    if (ts_->lastDataCount() < 2)
        update = MSFalse;
    computeNormalizedOffsets();
...
void MSGraph::computeScales(void)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (showXaxis(i)) {
            ...
            xRuleHt[i] = rule_width + (xTickStyle(i) == Inside ? xLabelHt[i] : (xMinorTickSize(i) > xMajorTickSize(i) + xLabelHt[i] ? xMinorTickSize(i) : xMajorTickSize(i) + xLabelHt[i]));
        } else if (showXrule(i) == MSTrue || axisRule() == MSG::Box)
            xRuleHt[i] = rule_width;
        if (showYaxis(i)) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (showYaxis(i) == MSTrue) {
            ...
            _yRuleWidth[i] = rule_width + spacing[i] + (yTickStyle(i) == Inside || MSLeft & yLabelAlign(i) || MSRight & yLabelAlign(i) ? yTitleWidth[i] : (yMinorTickSize(i) > yMajorTickSize(i) + yTitleWidth[i] ? yMinorTickSize(i) : yMajorTickSize(i) + yTitleWidth[i]));
        } else if (showYrule(i) == MSTrue || axisRule() == MSG::Box)
            _yRuleWidth[i] = rule_width;
    }
    x = offset() + (showYaxis(0) != MSTrue ? xMinLabelWidth : (_yRuleWidth[0] > xMinLabelWidth ? _yRuleWidth[0] : xMinLabelWidth));
    ...
    xx = offset() + (showYaxis(1) != MSTrue ? xMaxLabelWidth + spacing[1] : (_yRuleWidth[1] < xMaxLabelWidth ? xMaxLabelWidth : _yRuleWidth[1]));
...
void MSGraph::computeUpdateRegion(MSTrace* trace_)
{
    ...
    for (i = 0; i < traceList().count(); i++) {
        if ((trace = graphTrace(i)) != 0 && trace->dataCount() > 2 && trace->style() != MSG::Text && trace->style() != Fill && trace->traceSet()->overlap() == MSFalse) {
            x = xPixelToValue(xx, trace->xAxis());
            for (k = trace->dataCount() - 1; k > 0; k--) {
                double xval = xValue(trace, k);
                if ((xval > 0 && xval <= x) || (xval < 0 && xval >= x))
                    break;
            }
...
double MSGraph::estimateNormalizedLabelValue(int inc_, double value_)
{
    if (graphMode() & Normalize) {
        ...
        if (index < 0) {
            ...
            for (i = 0; i < traceSetList().count(); i++) {
                MSTraceSet* traceSet = graphTraceSet(i);
                if (index < traceSet->dataCount() && traceSet->dataCount() > 1)
                    return traceSet->x(index);
    ...
    for (i = 0; i < traceList().count(); i++) {
        ...
        if ((trace = graphTrace(i)) != 0 && trace->dataCount() > 0) {
            if (orientation() == Horizontal && trace->style() != Bar && trace->style() != Stack
                && trace->style() != MSG::Text)
                continue;
            if (trace->style() == Pie)
                trace->barCount(_pieCount++);
            if (trace->style() == Bar)
                trace->barCount(_barCount++);
                ...
                if (trace->style() == Bar || trace->style() == Stack) {
                    if (trace->barCount() == 0 && (xValue(trace, 0) != -DBL_MAX || xValue(trace, trace->dataCount() - 1) != DBL_MAX)) {
                        if (orientation() == Horizontal) {
                            ...
                            _yMaxData[ys] += yValue(trace, trace->dataCount() - 1) == _yMaxData[ys] ? trace->traceSet()->xDelta() / 2 : 0;
                            ...
                            _xMaxData[xs] += xValue(trace, trace->dataCount() - 1) == _xMaxData[xs] ? trace->traceSet()->xDelta() / 2 : 0;
    ...
    if (showYaxis(axis_) == MSTrue) {
        ...
        if ((tickPositionLength = yLabelOut(axis_).tickPositionLength()) != 0) {
            ...
            for (int i = 0; i < tickPositionLength; i++) {
                if (yLabelOut(axis_).tickPosition(i) < yMinData(axis_) || yLabelOut(axis_).tickPosition(i) > yMaxData(axis_))
                    continue;
                if (labelLength > 0) {
                    if (i < labelLength)
                        yLabelOut(axis_).label(formatBuffer.removeAll(), i);
                } else
                    formatAxisLabel(formatBuffer.removeAll(), yLabelOut(axis_), yLabelOut(axis_).tickPosition(i));
...
void MSGraph::computeXincrement(int level_)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (xLabelIncrement(0) <= 0.0) {
            ...
            if (min_inc == 0.)
                min_inc = range / plotAreaRect()->width();
            scale = 1 / min_inc;
            ...
            if (fabs(xMinData(i) / tempInc) > MSGraph::_PrecisionRange)
            ...
            while (evaluate == MSTrue) {
                ...
                while (r <= xMaxData(i)) {
                    ...
                    xp = xv;
                    if (fabs(xMaxData(i) / inc) > MSGraph::_PrecisionRange)
                        break;
                    r += inc;
                }
                ...
                } else if (scale * inc > 3. * size) // too small?
                {
                    ...
                    if (r > min_inc && r < (1 / MSGraph::_PrecisionRange))
                        inc = r;
                    else
                        evaluate = MSFalse;
                } else
            ...
            _xIncData[i][level_] = xLabelIncrement(i);
        if (level_ == 0) {
            _xBase[i] = xMinData(i);
...
void MSGraph::drawYaxes(int axis_)
{
    ...
    if ((tickPositionLength = yLabelOut(axis_).tickPositionLength()) != 0)
        bufSize = 10 + tickPositionLength;
    else
        bufSize = int(10 + (yMaxData(axis_) - yMinData(axis_)) / yIncData(axis_) * (1 + yNumMinorTicks(axis_)));
    ...
    if (tickPositionLength != 0) {
        ...
        for (unsigned i = 0; i < tickPositionLength; i++) {
            ...
            if (formatBuffer.length() > 0) {
                ...
                xx = MSLeft & yLabelAlign(axis_) || MSRight & yLabelAlign(axis_) ? x + (axis_ == 1 ? 1 : -1) * (axisInfo->descent / 4 + axisRuleWidth()) : xx;
                xx -= axis_ == 1 ? 0 : XTextWidth(axisInfo, formatBuffer, formatBuffer.length());
                y += MSBottom & yLabelAlign(axis_) ? -axisRuleWidth() - axisInfo->descent / 2 : MSTop & yLabelAlign(axis_) ? axisInfo->ascent : axisInfo->ascent / 2;
        ...
        while (r <= yMaxData(axis_) && ct < bufSize) {
            ...
            if (majorTick % minorTick == 0) {
                if ((axis_ == 0 && MSLeft & grid()) || (axis_ == 1 && MSRight & grid())) {
                    if (y != y_zero || showYrule(axis_) == MSFalse && yGridCount() < bufSize)
                        yGrid(_yGridCount++, y);
                }
                if (yMajorTickSize(axis_) > 0) {
                    ...
                    segments[ct].y1 = segments[ct].y2 = y;
                    tickSize = (MSBottom & yLabelAlign(axis_) || MSTop & yLabelAlign(axis_)) ? _yLabelWidth[axis_] : yMajorTickSize(axis_);
                    segments[ct++].x2 = x - int(axisRuleWidth() + tickSize * factor) * sign;
                }
...
void MSGraph::drawXaxes(int axis_)
{
    ...
    if ((tickPositionLength = xLabelOut(axis_).tickPositionLength()) != 0)
        bufSize = 10 + tickPositionLength;
    else
        bufSize = (int)(10 + (xMaxData(axis_) - xMinData(axis_)) / xIncData(axis_) * (1 + xNumMinorTicks(axis_)));
    bufSize = bufSize > MSGraph::_MaxBufSize ? MSGraph::_MaxBufSize : bufSize;
    ...
    if (tickPositionLength != 0) {
        ...
        for (unsigned i = 0; i < tickPositionLength; i++) {
            if (xLabelOut(axis_).tickPosition(i) < xMinData(axis_) || xLabelOut(axis_).tickPosition(i) > xMaxData(axis_))
                continue;
            ...
            if (labelLength > 0) {
                if (i < labelLength)
                    xLabelOut(axis_).label(formatBuffer.removeAll(), i);
            } else {
            ...
            if (formatBuffer.length() > 0) {
                ...
                yy = y + (axisRuleWidth() + (sign == 1 ? axisInfo->ascent : axisInfo->descent) + (xTickStyle(axis_) & Inside ? 0 : xMajorTickSize(axis_))) * sign;
        ...
        if (xLabelOut(axis_).formatType() == MSFormat::Time && xMaxData(axis_) > 0 && !(graphMode() & Normalize)) {
            ...
            r = xMinData(axis_) - fmod(xMinData(axis_), xIncData(axis_));
        int majorTick = 0;
        ...
        while (r <= xMaxData(axis_) && ct < bufSize) {
            ...
            if (majorTick % minorTick == 0) {
                if ((axis_ == 0 && MSBottom & grid()) || (axis_ == 1 && MSTop & grid())) {
                    if (x != x_zero || showXrule(axis_) == MSFalse && xGridCount() < bufSize)
                        xGrid(_xGridCount++, x);
                }
                if (xMajorTickSize(axis_) > 0) {
                    segments[ct].x1 = segments[ct].x2 = x;
                    segments[ct].y1 = y - (int)(xTickStyle(axis_) & Inside && xTickStyle(axis_) & Outside ? xMajorTickSize(axis_) : 0) * sign;
                ...
                if (xMinorTickSize(axis_) > 0) {
                    segments[ct].x1 = segments[ct].x2 = x;
                    segments[ct].y1 = y - (int)(xTickStyle(axis_) & Inside && xTickStyle(axis_) & Outside ? xMinorTickSize(axis_) : 0) * sign;
...
void MSGraph::drawXsubLabels(int axis_)
{
    ...
    if (_xSubLabelOut[axis_] && xSubLabelOut(axis_).formatType() != MSFormat::NoFormat) {
        ...
        axisLabelHeight = axisRuleWidth() + (xTickStyle(axis_) == Inside ? 0 : xMajorTickSize(axis_)) + axisInfo->ascent + axisInfo->descent;
        y = axis_ == 0 ? (y_org() + axisLabelHeight + axisInfo->ascent) : (y_end() - axisLabelHeight - axisInfo->descent);
        if ((tickPositionLength = xSubLabelOut(axis_).tickPositionLength()) != 0) {
            int labelLength = xSubLabelOut(axis_).labelLength();
            for (int i = 0; i < tickPositionLength; i++) {
                if (xSubLabelOut(axis_).tickPosition(i) < xMinData(axis_) || xSubLabelOut(axis_).tickPosition(i) > xMaxData(axis_))
                    continue;
                x = xValueToPixel(xSubLabelOut(axis_).tickPosition(i), axis_);
                if (labelLength > 0) {
                    if (i < labelLength)
                        xSubLabelOut(axis_).label(formatBuffer.removeAll(), i);
                } else {
            ...
            if ((xSubLabelOut(axis_).formatType() == MSFormat::Time) && xMaxData(axis_) > 0) {
                ...
                r = xMinData(axis_) - fmod(xMinData(axis_), xIncData(axis_, 1));
            majorTick = 0;
...
void MSGraph::drawTitle(Window xwin_)
{
    ...
    if (mapped() == MSTrue && title().maxLength() > 0) {
        ...
        for (unsigned i = 0; i < title().length(); i++) {
            ...
            int x = MSLeft & titleAlignment() ? leftMargin() : (MSRight & titleAlignment() ? width() - rightMargin() - w : (width() - w) / 2);
...
void MSGraph::drawSubtitle(Window xwin_)
{
    ...
    if (mapped() == MSTrue && subtitle().maxLength() != 0) {
        ...
        for (unsigned i = 0; i < subtitle().length(); i++) {
            ...
            int w = XTextWidth(fi, cp, len);
            int x = MSLeft & subtitleAlignment() ? leftMargin() : (MSRight & subtitleAlignment() ? width() - rightMargin() - w : (width() - w) / 2);
...
void MSGraph::drawFootnote(Window xwin_)
{
    ...
    if (mapped() == MSTrue && footnote().maxLength() > 0) {
        ...
        for (int i = footnote().length() - 1; i >= 0; i--) {
            ...
            int w = XTextWidth(fi, cp, len);
            int x = MSLeft & footnoteAlignment() ? leftMargin() : (MSRight & footnoteAlignment() ? width() - rightMargin() - w : (width() - w) / 2);
...
void MSGraph::drawXtitle(void)
{
    for (int i = 0; i < 2; i++) {
        if (showXaxis(i) == MSTrue && xTitle(i).maxLength() != 0) {
            ...
            int labelHeight = (showXrule(i) == MSTrue ? axisRuleWidth() : 0) + (xTickStyle(i) == Inside ? 0 : xMajorTickSize(i)) + (xLabelOut(i).formatType() == MSFormat::NoFormat ? 0 : fi->ascent + fi->descent) + (_xSubLabelOut[i] == 0 || xSubLabelOut(i).formatType() == MSFormat::NoFormat ? 0 : fi->ascent + fi->descent);
            fi = (XFontStruct*)server()->fontStruct(xTitleFont(i));
            int y = i == 0 ? y_org() + labelHeight + fi->ascent : y_end() - labelHeight - fi->descent - ((xTitle(i).length() - 1) * (fi->ascent + fi->descent));
            for (unsigned j = 0; j < xTitle(i).length(); j++) {
                int w = XTextWidth(fi, xTitle(i)(j).string(), xTitle(i)(j).length());
                int x = MSLeft & xTitleAlign(i) ? x_org() : MSRight & xTitleAlign(i) ? x_end() - w : x_org() + (plotAreaRect()->width() - w) / 2;
...
void MSGraph::drawYtitle(void)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (showYaxis(i) == MSTrue && yTitle(i).maxLength() != 0) {
            ...
            if (i == 0)
                x = x_org() - _yRuleWidth[i];
            else
                x = x_end() + _yRuleWidth[i];
            if (yTitleAlign(i) & Vertical) {
                if (i == 0)
                    x += fit->max_bounds.width;
                else
                    x -= (yTitle(i).length() + 2) * fit->max_bounds.width;
                for (unsigned j = 0; j < yTitle(i).length(); j++) {
                    int ht = yTitle(i)(j).length() * (fit->ascent + fit->descent);
                    y = yTitleAlign(i) & MSTop ? y_end() : MSBottom & yTitleAlign(i) ? y_org() - ht : y_end() + (plotAreaRect()->height() - ht) / 2;
                ...
                int h = showXaxis(1) != MSTrue ? 0 : ((showXrule(1) == MSTrue ? axisRuleWidth() : 0) + (xTickStyle(1) == Inside ? 0 : xMajorTickSize(1)) + (xLabelOut(1).formatType() == MSFormat::NoFormat ? 0 : fil->ascent + fil->descent) + (_xSubLabelOut[1] != 0 && xSubLabelOut(1).formatType() == MSFormat::NoFormat ? 0 : fil->ascent + fil->descent));
                for (unsigned j = 0; j < yTitle(i).length(); j++) {
                    int w = XTextWidth(fit, yTitle(i)(j).string(), yTitle(i)(j).length());
                    x = MSLeft & yTitleAlign(i) ? i == 0 ? x_org() : (width() - offset() - x_end()) > w ? x_end() : width() - w : MSRight & yTitleAlign(i) ? i == 0 ? x_org() > w ? x_org() - w : offset() : x_end() - w : i == 0 ? x_org() : x_end() - w;
...
void MSGraph::computeLegendSize(void)
{
    ...
    if (mapped() == MSTrue && legendAlignment() != MSNone && frozen() != MSTrue) {
        ...
        for (unsigned i = 0; i < traceList().count(); i++) {
            if ((trace = graphTrace(i)) != 0 && trace->dataCount() > 0 && trace->style() != 0 && trace->style() != MSG::Text && trace->legend() != 0 && strlen(trace->legend()) > 0) {
                if (Pie & trace->style()) {
                ...
                if (size > legend()->textFieldWidth())
                    legend()->textFieldWidth(size);
            }
        }
        if (itemCount > 0) {
            rowCount = itemCount;
...
void MSGraph::drawLegends(void)
{
    ...
    if ((mapped() == MSTrue && frozen() != MSTrue) || outputMode() == Print) {
        if (legendAlignment() == MSNone || legendHeight() == 0) {
            ...
            legend()->selectInput();
            if (highlightLegendStatus() == MSTrue)
                legend()->background(legend()->foreground());
            legend()->clear();
            ...
            for (unsigned i = 0; i < traceList().count(); i++) {
                ...
                } else if ((trace = graphTrace(i)) != 0 && trace->style() != 0 && trace->dataCount() > 0 && trace->style() != MSG::Text && trace->legend() != 0 && strlen(trace->legend()) > 0) {
                    ++itemCount;
                    XClearArea(display(), legend()->window(), x, y + legend()->textHeight() / 2,
                        legend()->symbolWidth(), legend()->textHeight(), False);
...
void MSGraph::drawLegendSymbols(MSTrace* trace_, int x_, int y_, int index_)
{
    ...
    case Stack:
        ...
        if (trace_->stipple() != 0)
            XSetFillStyle(display(), legend()->gc(), FillSolid);
...
void MSGraph::positionLegend(unsigned long align_)
{
    ...
    if (legend()->mapped() == MSTrue) {
        if (align_ & Outside) {
            ...
            x = align_ & MSLeft ? space : align_ & MSRight ? width() - legend()->width() - space : align_ & MSCenter ? align_ & Vertical ? ((width() - legend()->width()) / 2) : space : space;
            y = align_ & MSTop ? top : align_ & MSBottom ? bot : align_ & MSCenter ? align_ & Vertical ? top : ((height() - legend()->height()) / 2) : top;
            ...
            x = align_ & MSLeft ? x_org() + space : align_ & MSRight ? x_end() - legend()->width() - space : (x_org() + (plotAreaRect()->width() - legend()->width()) / 2);
            y = align_ & MSTop ? y_end() + space : align_ & MSBottom ? y_org() - legend()->height() - space : (y_end() + (plotAreaRect()->height() - legend()->height()) / 2);
            legend()->moveTo(x, y);
        }
    }
}
...
void MSGraph::visibilityObscured(void)
{
    ...
    for (int i = 0; i < n; i++)
        visibilityObscuredNotify(graphTraceSet(i));
}
...
void MSGraph::visibilityUnobscured(void)
{
    ...
    int n = traceSetList().count();
    for (int i = 0; i < n; i++)
        visibilityUnobscuredNotify(graphTraceSet(i));
}
...
void MSGraph::axis(unsigned long mode_)
{
    ...
    if (mode_ == 0) {
        ...
        if (l == MSFalse && r == MSFalse && t == MSFalse && b == MSFalse)
            return;
    }
    if (axisRule() == Axis) {
        _showXrule[0] = b;
...
void MSGraph::axisRule(unsigned long mode_)
{
    ...
    if (_showXrule[0] != b || _showXrule[1] != t || _showYrule[0] != l || _showYrule[1] != r || axisRule() == Axis) {
        _showXrule[0] = b;
        _showXrule[1] = t;
        _showYrule[0] = l;
        _showYrule[1] = r;
...
MSWidgetVector MSGraph::children(void)
{
    ...
    for (int i = 0; i < traceSetList().count(); i++)
        vector.append(graphTraceSet(i));
    return vector;
}
...
MSAttrValueList& MSGraph::get(MSAttrValueList& avList_)
{
    ...
    if (_xMaxSet[1] == (double)unsetValue())
        axisValueResults << "MSGraph::Unset";
    else
        axisValueResults << MSString(_xMaxSet[1]);
    if (_xMaxSet[0] == (double)unsetValue())
        axisValueResults << "MSGraph::Unset";
    else
        axisValueResults << MSString(_xMaxSet[0]);
    if (_yMaxSet[0] == (double)unsetValue())
        axisValueResults << "MSGraph::Unset";
    else
        axisValueResults << MSString(_yMaxSet[0]);
    if (_yMaxSet[1] == (double)unsetValue())
        axisValueResults << "MSGraph::Unset";
    else
        axisValueResults << MSString(_yMaxSet[1]);
    ...
    if (_xMinSet[1] == (double)unsetValue())
        axisValueResults << "MSGraph::Unset";
    else
        axisValueResults << MSString(_xMaxSet[1]);
    if (_xMinSet[0] == (double)unsetValue())
        axisValueResults << "MSGraph::Unset";
    else
        axisValueResults << MSString(_xMaxSet[0]);
    if (_yMinSet[0] == (double)unsetValue())
        axisValueResults << "MSGraph::Unset";
    else
        axisValueResults << MSString(_yMaxSet[0]);
    if (_yMinSet[1] == (double)unsetValue())
        axisValueResults << "MSGraph::Unset";
    else
        axisValueResults << MSString(_yMaxSet[1]);
    ...
    for (i = 0; i < 4; i++) {
        if (axisMode((MSAlignment)axisValues(i)) == Ascending)
            index = 0;
        else
            index = 1;
    ...
    if (legendStyle() == MSG::Vertical)
        value = "MSG::Vertical";
    else if (legendStyle() == MSG::Horizontal)
        value = "MSG::Horizontal";
    else
        value = "MSG::LastValue";
...
void MSGraph::set(MSAttrValueList& avList_)
{
    ...
    for (int i = 0; i < avList_.length(); i++) {
        if ("axis" == avList_[i].attribute()) {
            ...
            if (value.indexOf("MSG::Std") != value.length())
                axis(MSG::Std);
            else if (value.indexOf("MSG::Box") != value.length())
                axis(MSG::Box);
            else
                axis(MSAttrValue::stringToAlignment(value));
            index << i;
            ...
            convertAxisAttribute(avList_[i].value(), value, axisChoice);
            if (value.length() == 0 || value == "MSGraph::Unset")
                axisMaximum(Unset, axisChoice);
            else
                axisMaximum(value.asInt(), axisChoice);
            index << i;
        } else if ("axisMinimum" == avList_[i].attribute()) {
            convertAxisAttribute(avList_[i].value(), value, axisChoice);
            if (value.length() == 0 || value == "MSGraph::Unset")
                axisMinimum(Unset, axisChoice);
            else
                axisMinimum(value.asInt(), axisChoice);
            index << i;
            ...
            convertAxisAttribute(avList_[i].value(), value, axisChoice);
            if (value.indexOf("MSG::Horizontal") != value.length())
                align |= MSG::Horizontal;
            if (value.indexOf("MSG::Vertical") != value.length())
                align |= MSG::Vertical;
            ...
            convertAxisAttribute(avList_[i].value(), value, axisChoice);
            if (value == "MSG::Ascending")
                newAxisMode = Ascending;
            else
                newAxisMode = Descending;
            ...
            if (value == "MSG::Inside")
                newTickStyle = Inside;
            else if (value == "MSG::Outside")
                newTickStyle = Outside;
            else
                newTickStyle = Inside | Outside;
            ...
            const MSString& value = avList_[i].value();
            if (value.indexOf("MSG::Axis") != value.length())
                axisRule(MSG::Axis);
            else
                axisRule(MSAttrValue::stringToAlignment(value));
            index << i;
            ...
            if (avList_[i].value() == "MSSolid")
                newLineStyle = MSSolid;
            else if (avList_[i].value() == "MSDash")
                newLineStyle = MSDash;
            else if (avList_[i].value() == "MSDot")
                newLineStyle = MSDot;
            else
                newLineStyle = MSDash | MSDot;
            gridStyle(newLineStyle);
            index << i;
        } else if (avList_[i].attribute() == "orientation") {
            if (avList_[i].value() == "MSG::Horizontal")
            ...
            const MSString& value = avList_[i].value();
            unsigned long align = 0;
            if (value.indexOf("MSG::Outside") != value.length())
                align |= MSG::Outside;
            if (value.indexOf("MSG::Vertical") != value.length())
                align |= MSG::Vertical;
            align |= MSAttrValue::stringToAlignment(value);
            ...
            if (avList_[i].value() == "MSG::Horizontal")
                legendStyle(MSG::Horizontal);
            else if (avList_[i].value() == "MSG::Vertical")
                legendStyle(MSG::Vertical);
            else
                legendStyle(MSG::LastValue);
            index << i;
...
{
    MSPrintItem::printRow(x_);
    return *this;
}
MSGraph& MSGraph::printColumn(int x_)
{
    MSPrintItem::printColumn(x_);
    return *this;
}
...
int MSGraph::computePrintSize(MSReport* report_, int, int y_, int w_, int, int topMargins_, int margins_)
{
    reset();
    if (leftPixel() < 0)
        leftPixel(report_->leftPixel());
    if (rightPixel() < 0)
        rightPixel(report_->rightPixel());
    ...
    if (resid != 0) {
        resid += topPixel();
        if (resid + bottomPixel() < remainingHeight)
            resid += bottomPixel();
        else {
            resid = 0;
            _pageCount++;
...
int MSGraph::print(MSReport* report_, int x_, int y_, int, int, int bottomIndent_, int margin_)
{
    ...
    if (printHeight() >= remainingHeight || (pageBreakRow() == -1 && pageAlignment() & MSTop && y_ != report_->bodyTop(report_->pageCount()))) {
        pageBreakRow(0);
        _currentPage++;
    ...
    x = justification() & MSRight ? x + pageWidth() - printWidth() : justification() & MSCenter ? x + (pageWidth() - printWidth()) / 2 : x;
    int wScreen = width();
    int hScreen = height();
...
int MSGraph::printGraph(MSReport* report_, int x_, int y_)
{
    ...
    if (displayPrintStream().fail() != ios::failbit) {
        ...
        MSMessageLog::errorMessage("Error: Unable to attach stream to graph %s\n", tag().symbolName());
    return h;
}
