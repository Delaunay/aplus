...
{
    if (_nt != 0) {
        for (int i = 0; i < _newtraceAllocCt; i++)
            delete _nt[i];
        delete[] _nt;
        _nt = 0;
    }
...
MSBoolean MSGraph::findLineHandle(int x_, int y_)
{
    ...
    for (int i = 0; i < newtraceCt(); i++) {
        for (int j = 0; j < nt(i)->pointCount(); j++) {
            if (((abs(x_ - nt(i)->points(j)->x) < size) && (abs(y_ - nt(i)->points(j)->y) < size))) {
                selectPoint(j);
                selectLine(i);
                return MSTrue;
            }
...
MSTraceSet* MSGraph::createTextTraceSet(MSFloatMatrix& aFloatMatrix_, MSStringVector& aStringVector_)
{
    ...
    if (orientation() == Horizontal) {
        ...
        ts->moveTo(aFloatMatrix_(0, 0), aFloatMatrix_(0, 1));
    ts->internalModel(MSTrue);
    unfreeze();
    return ts;
}
    ...
    for (int i = 0; i < traceList().count(); i++) {
        if ((trace = graphTrace(i)) != 0 && trace->traceSet()->selectable() == MSTrue) {
            for (int k = 0; k < trace->dataCount() - 1; k++) {
                ...
                if ((sa == 0 && abs(x - x1) < 3 && y <= top && y >= bot || (sb == 0 && abs(y - y1) < 3 && x <= right && x >= left))) {
                    return trace;
...
MSBoolean MSGraph::findLineSegment(int x_, int y_)
{
    ...
    for (int i = 0; i < newtraceCt(); i++) {
        for (int j = 0; j < nt(i)->pointCount() - 1; j++) {
            if ((nt(i)->sa(j) == 0 && abs(x_ - nt(i)->points(j)->x) < 3 && y_ <= nt(i)->top(j) && y_ >= nt(i)->bot(j)) || (nt(i)->sb(j) == 0 && abs(y_ - nt(i)->points(j)->y) < 3 && x_ <= nt(i)->right(j) && x_ >= nt(i)->left(j))) {
                selectPoint(j);
                selectLine(i);
                return MSTrue;
            } else if (nt(i)->sa(j) != 0 && nt(i)->sb(j) != 0 && x_ <= nt(i)->right(j) && x_ >= nt(i)->left(j) && y_ >= nt(i)->top(j) && y_ <= nt(i)->bot(j)) {
...
void MSGraph::moveLegend(const XEvent* event_)
{
    ...
    x = legend()->x_origin() + xoffset;
    y = legend()->y_origin() + yoffset;
...
{
    if (trace_->style() == Segment)
        plotSegmentTrace(trace_, window(), windowGC());
    else
        drawMoveLineTrace(trace_);
}
...
void MSGraph::drawMoveLineTrace(MSTrace* trace_)
{
    ...
    for (i = 0; i < trace_->traceSet()->traceList().count(); i++) {
        if ((trace = trace_->traceSet()->trace(i)) != 0) {
            ...
            if (inc != 1 && (datacount - 1) % inc != 0) { // make sure to draw last point
                k = datacount - 1;
                x = xValueToPixel(xValue(trace, k), xs) + trace_->xOffset();
                y = yValueToPixel(trace->y(k), ys) + trace_->yOffset();
                points[pointCount].x = x;
...
void MSGraph::drawLineHandles(MSTrace* trace_, MSTrace* refTrace)
{
    ...
    if (inc != 1 && (bufSize - 1) % inc != 0) { // make sure that handle is drawn for the last point
        i = bufSize - 1;
        x = (int)(xValueToPixel(xValue(trace_, i), xs) + xoffset);
        y = (int)(yValueToPixel(trace_->y(i), ys) + yoffset);
        rects[recs].x = x - size / 2;
...
MSTrace* MSGraph::findDataPoint(const XEvent* event_)
{
    ...
    int stackCt = 0;
    MSTrace** stackTrace = new MSTrace*[traceList().count() < MSGraph::_MaxBufSize ? traceList().count() : MSGraph::_MaxBufSize];
    ...
    for (i = 0; i < traceList().count(); i++) {
        if ((trace = graphTrace(i)) != 0 && trace->dataCount() > 1 && (trace->style() == Stack || trace->style() == Area)) {
            stackTrace[stackCt++] = trace;
        }
    }
    for (i = 0; i < traceList().count(); i++) {
        if ((trace = graphTrace(i)) != 0 && trace->dataCount() > 1 && trace->style() != 0 && trace->traceSet()->isProtected() == MSFalse) {
            ...
            for (k = 0; k < trace->dataCount(); k++) {
                ...
                if (event_->xbutton.button == 1 && trace->style() >= Close) {
                    ...
                    if (abs(ix - xxx) <= (selectDistance() / 2) && yy <= trace->y(k, 1 + trace->offset()) && yy >= trace->y(k, 1 + trace->offset())) {
                ...
                if (orientation() == Horizontal && (trace->style() == Bar || trace->style() == Stack)) {
                    ...
                    if (trace->style() >= HLC)
                        r = ((rr = fabs(yy - trace->y(k, j + 2)))) < r ? rr : r;
                    r *= yScale(ys);
                } else
                    r = (yy - y) * yScale(ys);
    ...
    start_again:
        ...
        if (orientation() == Horizontal) {
            ...
            if (trace->style() == Stack || trace->style() == Area) {
                ...
                if (x > 0 && (x + xOffset) < 0)
                    x = xPixelToValue(ox, xs);
                else if (x < 0 && (x + xOffset) > 0)
                    x = xPixelToValue(ox, xs);
                else
                    x -= xOffset;
            } else
...
void MSGraph::drawScanXYvalueBox(int x_, int y_)
{
    ...
    int x = (x_ + xOffset + dataWin()->width()) > width() - offset() ? width() - offset() - dataWin()->width() : x_ + xOffset < offset() ? offset() : x_ + xOffset;
    int y = (y_ + yOffset + dataWin()->height()) > height() - offset() ? height() - dataWin()->height() - offset() : y_ + yOffset < offset() ? offset() : y_ + yOffset;
    dataWin()->moveTo(x, y);
#ifdef MS_WINDOWS
    ...
    for (int i = 0; i < traceList().count(); i++) {
        if ((trace = graphTrace(i)) != 0 && trace->traceSet()->textLength() != 0 && trace->dataCount() == 1) {
            ...
            for (int j = 0; j < lines; j++)
                aStringVector << trace->traceSet()->formatText(formatBuffer.removeAll(), j);
            ...
            if ((button_x >= x && button_x <= (x + w)) && (button_y >= y && button_y <= (y + h)))
                return trace;
        }
    }
    return 0;
...
void MSGraph::editTextTrace(void)
{
    ...
    if (selectTrace() != 0 && selectTrace()->traceSet()->isProtected() == MSFalse) {
        ...
        MSString formatBuffer;
        for (int j = 0; j < lines; j++)
            aStringVector << selectTrace()->traceSet()->formatText(formatBuffer.removeAll(), j);
...
void MSGraph::moveTextTrace(const XEvent* event_)
{
    ...
    while (keys & mask) {
        ...
        if (ix != x || iy != y) {
            ...
            if ((ix - xoffset) > (x_end() - offset))
                ix = x_end() - offset + xoffset;
            if ((iy - yoffset) > (y_org() - h - offset))
                iy = y_org() - h - offset + yoffset;
            if (keys & ControlMask) {
...
void MSGraph::scanXY(const XEvent* event_)
{
    ...
    if ((bs = backingStore()) == MSFalse) {
        redrawImmediately();
        r.x(dataWin()->x_origin());
        r.y(dataWin()->y_origin());
    }
    ...
    if (bs == MSFalse) {
        r.width(dataWin()->width());
        r.height(dataWin()->height());
    ...
    while (keys & mask) {
        ...
        if (update == MSTrue) {
            if (cursorType() == Xcursor)
                drawScanXvalues(ix, iy, axis, lastIndex);
            else if (cursorType() == XYcursor)
                drawScanXYvalues(xPixelToValue(ix, axis), yPixelToValue(iy, axis), axis, axis);
        }
    }
    if (cursorType() != Xcursor)
        XDrawLine(display(), window(), windowGC(), x_org(), y, x_end(), y);
    ...
    if (legendMapped == MSTrue)
        legendAlignment(MSNone);
    XEvent ev;
...
int MSGraph::findYvalue(MSTrace* trace_, double x_)
{
    ...
    if (trace_->traceSet()->overlap() == MSTrue)
        return -1;
    if (xValue(trace_, 0) < xValue(trace_, trace_->dataCount() - 1)) {
        // ascending X values
        if (x_ < xValue(trace_, 0) - trace_->xShift())
            return (-1);
        ...
        if (x_ > xValue(trace_, 0) - trace_->xShift() || x_ < xValue(trace_, trace_->dataCount() - 1)) {
            if (x_ > xValue(trace_, 0) + trace_->traceSet()->xDelta())
                return (0);
            else
                return (-1);
        }
...
MSBoolean MSGraph::drawZoomRegion(const XEvent* event_)
{
    ...
    if (graphMode() & PieChart || traceList().count() == pieCount())
        return MSFalse;
    if (event_->xbutton.button == 1)
        mask = Button1Mask;
    else if (event_->xbutton.button == 2)
    ...
    while (keys & mask) {
        ...
        if (ix != x || iy != y) {
            if (x > bx) {
                if (y > by)
                    XDrawRectangle(display(), window(), windowGC(), bx, by, x - bx, y - by);
                else
                    XDrawRectangle(display(), window(), windowGC(), bx, y, x - bx, by - y);
            } else {
                if (y > by)
                    XDrawRectangle(display(), window(), windowGC(), x, by, bx - x, y - by);
                else
                    XDrawRectangle(display(), window(), windowGC(), x, y, bx - x, by - y);
            }
            x = ix;
            y = iy;
            if (x > bx) {
    ...
    if (abs(bx - x) > 5 && abs(by - y) > 5 && ix < width() && iy < height()) {
        ...
        yMinZoom(by < y ? y : by);
        yMaxZoom(by < y ? by : y);
        if (activateCallback(this, MSWidgetCallback::graphzoom) != MSTrue)
            zoom();
    }
    return moved;
}
