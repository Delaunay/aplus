...
MSHScale::HSlider::HSlider(MSWidget* owner_)
    : MSScale::Slider(owner_)
{
}
...
void MSHScale::computeSize(void)
{
    ...
    if (mintitle().maxLength() > 0) {
        ...
        minh = MSTop & mintitleAlignment() ? mintitleHeight() : MSBottom & mintitleAlignment() ? mintitleHeight() > slider()->height() ? (slider()->height() - mintitleHeight() / 2) : 0 : 0;
    ...
    if (maxtitle().maxLength() > 0) {
        ...
        maxh = MSTop & maxtitleAlignment() ? maxtitleHeight() : MSBottom & maxtitleAlignment() ? maxtitleHeight() > slider()->height() ? (slider()->height() - maxtitleHeight() / 2) : 0 : 0;
...
{
    int x = x_ < x_org() + slider()->offset() ? x_org() + slider()->offset() : x_ > x_end() - slider()->width() - slider()->offset() ? x_end() - slider()->width() - slider()->offset() : x_;
    int y = y_org() + SliderAreaShadowThickness;
    if (x != slider()->x_origin() || y != slider()->y_origin())
        slider()->moveTo(x, y);
}
...
void MSHScale::drawSliderTitles(void)
{
    ...
    if (mintitle().maxLength() > 0) {
        ...
        y = MSTop & mintitleAlignment() ? y_org() - mintitleHeight() - (MSTop & labelAlignment() ? labelHeight : 0) : MSBottom & mintitleAlignment() ? y_end() + (MSBottom & labelAlignment() ? labelHeight : 0) : center - mintitleHeight() / 2;
        y += fontInfo->ascent;
        for (int i = 0; i < mintitle().length(); i++) {
            len = mintitle()[i].length();
            const char* cp = mintitle()[i].string();
            w = XTextWidth(fontInfo, cp, len);
            x = MSCenter & mintitleAlignment() ? x_org() - w - labelSpacing() : MSLeft & mintitleAlignment() ? x_org() : MSRight & mintitleAlignment() ? x_org() - w : (x_org() - w / 2);
    ...
    if (maxtitle().maxLength() != 0) {
        ...
        y = MSTop & maxtitleAlignment() ? y_org() - maxtitleHeight() : MSBottom & maxtitleAlignment() ? y_end() : center - maxtitleHeight() / 2;
        y += fontInfo->ascent;
        for (int i = 0; i < maxtitle().length(); i++) {
            len = maxtitle()[i].length();
            const char* cp = maxtitle()[i].string();
            w = XTextWidth(fontInfo, cp, len);
            x = MSCenter & maxtitleAlignment() ? x_end() + labelSpacing() : MSLeft & maxtitleAlignment() ? x_end() : MSRight & maxtitleAlignment() ? x_end() - w : (x_end() - w / 2);
            XDrawString(display(), window(), gc, fontInfo, x, y, cp, len);
            y += fontInfo->ascent + fontInfo->descent;
        }
    }
...
void MSHScale::drawTickLabels(void)
{
    if (labelAlignment() != MSNone) {
        ...
        if (tickPositionLength != 0)
            bufSize = tickPositionLength + 10;
        else
            bufSize = (int)(10 + (valueMax() - valueMin()) / incData() * (1 + minorTickCount()));
        ...
        if (tickPositionLength != 0) {
            ...
            for (unsigned i = 0; i < tickPositionLength; i++) {
                if (labelOut()->tickPosition(i) < valueMin() || labelOut()->tickPosition(i) > valueMax())
                    continue;
                ...
                if (labelLength > 0) {
                    if (i < labelLength)
                        labelOut()->label(buffer.removeAll(), i);
                } else {
            ...
            if ((labelOut()->formatType() == MSFormat::Date || labelOut()->formatType() == MSFormat::Time) && valueMax() > 0) {
                r = outFmt().snapTime(valueMin(), incData());
            } else
                r = valueMin() - fmod(valueMin(), incData());
