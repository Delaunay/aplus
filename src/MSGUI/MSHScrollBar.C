...
MSHScrollBar::HElevator::HElevator(MSHScrollBar* owner_)
...
void MSHScrollBar::configureForMotif(void)
{
    if (_elevator != 0) {
        ...
        w = h = height() - 2 * offset;
        if (width() < 2 * (w + offset) + MinScrollBarLength + 2)
            w = (width() - (MinScrollBarLength + 2 + 2 * offset)) / 2;
...
void MSHScrollBar::motionLoop(void)
{
    ...
    while (keys & mask) {
        ...
        if (newX < sliderAreaRect().x())
            newX = sliderAreaRect().x();
...
void MSHScrollBar::openlookButton1Press(const XEvent* pXEvent_)
{
    ...
    //  Calculate whether the selection point is in the slider
    if ((_elevator->width() == DefaultElevatorWidth) && (xx >= _elevator->x() + DefaultElevatorBoxWidth) && (xx <= _elevator->x() + _elevator->width() - DefaultElevatorBoxWidth) && (yy >= _elevator->y()) && (yy <= _elevator->y() + _elevator->height())) {
        _separation_x = xx - _elevator->x();
        _initial_x = _elevator->x();
        _slidingOn = MSTrue;
        _changeType = Drag;
    ...
    // ... in arrow 1
    else if ((yy >= _elevator->_arrow1->y()) && (xx >= _elevator->x()) && (yy <= _elevator->_arrow1->y() + _elevator->_arrow1->height()) && (xx <= _elevator->x() + DefaultElevatorBoxWidth)) {
        _changeType = Dec;
        _elevator->_arrow1->select(MSTrue);
    }
    // ... in arrow 2
    else if ((yy >= _elevator->_arrow2->y()) && (xx >= _elevator->x() + _elevator->width() - DefaultElevatorBoxWidth) && (yy <= _elevator->_arrow2->y() + _elevator->_arrow2->height()) && (xx <= _elevator->x() + _elevator->width())) {
        _changeType = Inc;
        _elevator->_arrow2->select(MSTrue);
    }
    // ... in top marker
    else if ((yy >= sliderAreaRect().y()) && (xx >= ht) && (yy <= sliderAreaRect().y() + sliderAreaRect().height()) && (xx <= ht + MSHScrollBarDefaultMarkerWidth)) {
        _changeType = Home;
    }
    // ... in bottom marker
    else if ((yy >= sliderAreaRect().y()) && (xx >= width() - ht - MSHScrollBarDefaultMarkerWidth) && (yy <= sliderAreaRect().y() + sliderAreaRect().height()) && (xx <= width() - ht)) {
        _changeType = End;
    }
    // ... in the trough (i.e. slider area)...
    else if ((xx >= sliderAreaRect().x()) && (yy >= sliderAreaRect().y()) && (xx <= sliderAreaRect().x() + sliderAreaRect().width()) && (yy <= sliderAreaRect().y() + sliderAreaRect().height())) {
        /* Page the slider up or down */
        if (xx < _elevator->x() + DefaultElevatorBoxWidth)
            _changeType = PageUp;
        else
            _changeType = PageDown;
    } else
