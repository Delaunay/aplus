...
int MSIcon::computePixmapYCoord(const MSPixmap* pixmap_)
{
    ...
    if (alignment() & MSTop) {
        if (labelAlignment() == MSTop)
            r = offset + labelH + labelSp;
        else if (labelAlignment() == MSBottom)
            r = offset;
        else {
            if (labelH < pixmapH)
                r = offset;
            else
                r = offset + (labelH - pixmapH) / 2;
        }
    } else if (alignment() & MSBottom) {
        if (labelAlignment() == MSTop)
            r = height() - pixmapH - offset;
        else if (labelAlignment() == MSBottom)
            r = height() - labelH - pixmapH - labelSp - offset;
        else {
            if (labelH < pixmapH)
                r = height() - pixmapH - offset;
            else
                r = height() - labelH - offset + (labelH - pixmapH) / 2;
        }
    } else {
        if (labelAlignment() == MSTop)
            r = (height() - (pixmapH + labelH + labelSp)) / 2 + labelH + labelSp;
        else if (labelAlignment() == MSBottom)
            r = (height() - (pixmapH + labelH + labelSp)) / 2;
        else
            r = (height() - pixmapH) / 2;
    }
    return r;
...
int MSIcon::computeYCoord(int row_)
{
    ...
    if (showPixmap() == MSTrue && pixmap() != 0) {
        if (sensitive() == MSTrue)
            ...
            pixmapH = insensitivePixmap()->height();
    }
...
int MSIcon::computeXCoord(int row_, int column_, const char* string_, int len_)
{
    ...
    if (showPixmap() == MSTrue && pixmap() != 0) {
        if (sensitive() == MSTrue)
            ...
            pixmapW = insensitivePixmap()->width();
    }
...
int MSIcon::computePixmapXCoord(const MSPixmap* pixmap_)
{
    ...
    int r = 0;
    if (alignment() & MSLeft) {
        if (labelAlignment() == MSLeft)
            r = offset + labelW + labelSp;
        else if (labelAlignment() == MSRight)
            r = offset;
        else {
            if (labelW < pixmapW)
                r = offset;
            else
                r = offset + (labelW - pixmapW) / 2;
        }
    } else if (alignment() & MSRight) {
        if (labelAlignment() == MSLeft)
            r = width() - pixmapW - offset;
        else if (labelAlignment() == MSRight)
            r = width() - labelW - pixmapW - labelSp - offset;
        else {
            if (labelW < pixmapW)
                r = width() - pixmapW - offset;
            else
                r = width() - labelW - offset + (labelW - pixmapW) / 2;
        }
    } else {
        if (labelAlignment() == MSLeft)
            r = (width() - (pixmapW + labelW + labelSp)) / 2 + labelW + labelSp;
        else if (labelAlignment() == MSRight)
            r = (width() - (pixmapW + labelW + labelSp)) / 2;
        else
            r = (width() - pixmapW) / 2;
    }
    return r;
...
int MSIcon::computeXCoordinate(int, int, int pixmapW_, const char* string_, int len_)
{
    ...
    if (alignment() & MSLeft) {
        if (labelAlignment() == MSLeft)
            r = offset + labelW - textW;
        else if (labelAlignment() == MSRight)
            r = offset + pixmapW_ + labelSp;
        else {
            if (labelW < pixmapW_)
                r = offset + (pixmapW_ - textW) / 2;
            else
                r = offset + (labelW - textW) / 2;
        }
    } else if (alignment() & MSRight) {
        if (labelAlignment() == MSLeft)
            r = width() - pixmapW_ - textW - labelSp - offset;
        else if (labelAlignment() == MSRight)
            r = width() - offset - labelW;
        else {
            if (labelW < pixmapW_)
                r = width() - offset - pixmapW_ + (pixmapW_ - textW) / 2;
            else
                r = width() - offset - textW - (labelW - textW) / 2;
        }
    } else {
        if (labelAlignment() == MSLeft)
            r = (width() - (pixmapW_ + labelW + labelSp)) / 2 + labelW - textW;
        else if (labelAlignment() == MSRight)
            r = (width() - (pixmapW_ + labelW + labelSp)) / 2 + pixmapW_ + labelSp;
        else {
            if (textW < pixmapW_)
                r = (width() - pixmapW_) / 2 + (pixmapW_ - textW) / 2;
            else
                r = (width() - textW) / 2;
        }
    }
...
int MSIcon::computeYCoordinate(int row_, int pixmapH_)
{
    ...
    if (alignment() & MSTop) {
        if (labelAlignment() == MSTop)
            r = offset + row_ * textHeight();
        else if (labelAlignment() == MSBottom)
            r = offset + pixmapH_ + labelSp + row_ * textHeight();
        else {
            if (labelH < pixmapH_)
                r = offset + (pixmapH_ - labelH) / 2 + row_ * textHeight();
            else
                r = offset + row_ * textHeight();
        }
    } else if (alignment() & MSBottom) {
        if (labelAlignment() == MSTop)
            r = height() - pixmapH_ - labelSp - offset - (rows() - row_) * textHeight();
        else if (labelAlignment() == MSBottom)
            r = height() - offset - (rows() - row_) * textHeight();
        else {
            if (labelH < pixmapH_)
                r = height() - (pixmapH_ - labelH) / 2 - offset - (rows() - row_) * textHeight();
            else
                r = height() - offset - (rows() - row_) * textHeight();
        }
    } else {
        if (labelAlignment() == MSTop)
            r = (height() - (pixmapH_ + labelH + labelSp)) / 2 + row_ * textHeight();
        else if (labelAlignment() == MSBottom)
            r = (height() - (pixmapH_ + labelH + labelSp)) / 2 + pixmapH_ + labelSp + row_ * textHeight();
        else {
            if (labelH < pixmapH_)
                r = (height() - pixmapH_) / 2 + (pixmapH_ - labelH) / 2 + row_ * textHeight();
            else
                r = (height() - labelH) / 2 + row_ * textHeight();
        }
    }
    return r;
}
...
void MSIcon::button1Press(const XEvent* event_)
{
    ...
    if (acceptFocus() == MSTrue)
        focus = traverseFocus(this);
    if (focus == MSTrue && doubleClick().isDoubleClick(event_) == MSTrue)
        doubleClickNotify();
    else
        buttonSelectionNotify();
}
...
{
    if (showLabel() == MSFalse)
        return 0;
    else if (showPixmap() == MSFalse)
        return 0;
    else if (pixmap() == 0)
        return 0;
    else
