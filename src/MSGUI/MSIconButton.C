...
static const unsigned long MSIconButtonEventMask = (ExposureMask | ButtonPressMask | ButtonReleaseMask | Button1MotionMask);
...
void MSIconButton::drawPixmap(void)
{
    if (mapped() == MSTrue) {
        ...
        if (pmap != 0) {
            ...
            if (armed() == MSTrue)
                XSetBackground(display(), gc, selectShadowColor());
            else
...
void MSIconButton::updatePixmap(void)
{
    if (pixmap() == 0) {
        ...
        else if (armedPixmap() != 0)
            _pixmap = new MSPixmap(*armedPixmap());
    }
    if (insensitivePixmap() == 0) {
        if (pixmap() != 0)
            _insensitivePixmap = new MSPixmap(*pixmap());
        else if (armedPixmap() != 0)
            _insensitivePixmap = new MSPixmap(*armedPixmap());
    }
    if (armedPixmap() == 0) {
        if (pixmap() != 0)
            _armedPixmap = new MSPixmap(*pixmap());
        else if (insensitivePixmap() != 0)
            _armedPixmap = new MSPixmap(*insensitivePixmap());
    }
