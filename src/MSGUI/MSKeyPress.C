    ...
    if (pString_ != 0) {
        ...
        for (int i = 0; i < NumTokens; i++) {
            ...
            if ((pString = strstr((char*)(void*)pString_, keyTokens[i])) != 0) {
                mask_ += keyMasks[i];
                if (pString == pString_)
                    flag_ += keyMasks[i]; // check for beginning of string
                else if (*(pString - 1) != '~')
                    flag_ += keyMasks[i]; // check for negation character
            }
        }
        if (pString_[0] == '!')
            mask_ = ExactMask; // require an exact match
        // cast is for bug in Borland
...
{
    if (mask_ & ExactMask)
        return (state_ == (flag_ & ~(ALL)) && ((ALL & mask_) || keysym1_ == keysym2_)) ? MSTrue : MSFalse;
    else
        return ((state_ & mask_) == flag_ && ((ALL & mask_) || keysym1_ == keysym2_)) ? MSTrue : MSFalse;
}
