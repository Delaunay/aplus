...
{
    return MSKeyPress::isMatch(keyTranslation_.keysym(), keyTranslation_.state(), keysym(), mask(), flag());
}
...
MSBoolean MSKeyTableData::process(const MSKeyPress& keyTranslation_, MSWidget* widget_) const
{
    ...
    for (int i = 0; i < _ktList._count; i++) {
        tp = _ktList._array[i];
        if (tp->isMatch(keyTranslation_) == MSTrue && tp->callback()->process(widget_, keyTranslation_) == MSTrue)
            return MSTrue;
    }
    return MSFalse;
}
...
MSBoolean MSKeyTableData::hasMatch(const MSKeyPress& keyTranslation_) const
{
    ...
    for (int i = 0; i < _ktList._count; i++) {
        tp = _ktList._array[i];
        if (tp->isMatch(keyTranslation_) == MSTrue)
            return MSTrue;
    }
    return MSFalse;
}
