...
{
    if (_pHashTable == 0)
        _pHashTable = new MSHashTable(KTableSize);
}
...
MSKeyTranslationTable::~MSKeyTranslationTable(void)
{
    if (_pListHead != 0) {
        ...
        while ((np = hp->next()) != hp) {
            ...
            if (tp != 0) {
                if (tp->referenceCount() == 1)
                    _pHashTable->remove((char*)tp->name());
                tp->removeReference();
            }
            delete np;
        }
...
void MSKeyTranslationTable::addKeyTableData(const char* newKeyTableDataName_)
{
    ...
    if (newKeyTableData == 0)
        newKeyTableData = new MSKeyTableData(newKeyTableDataName_);
    add(newKeyTableData);
}
...
MSBoolean MSKeyTranslationTable::hasMatch(const MSKeyPress& keyTranslation_) const
{
    if (_pListHead != 0) {
        ...
        if (hp != hp->next()) {
            ...
            while ((np = np->next()) != hp) {
                data = (MSKeyTableData*)np->data();
                if (data->hasMatch(keyTranslation_) == MSTrue)
                    return MSTrue;
            }
        }
    }
...
void MSKeyTranslationTable::removeAllCallbacksThat(const char* specificationString_)
{
    ...
    if (_pListHead != 0) {
        ...
        while ((np = np->next()) != hp) {
            ...
            if (pKeyTableData->referenceCount() == 1)
                pKeyTableData->remove(specificationString_);
        }
    }
}
...
void MSKeyTranslationTable::removeCallback(unsigned long identifier_, const char* keyTableDataName_)
{
    ...
    if (keyTableDataName_ != 0) {
        ...
        if (pKeyTableData != 0 && pKeyTableData->referenceCount() == 1)
            pKeyTableData->remove(identifier_);
    } else {
