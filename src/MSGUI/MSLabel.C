...
{
    if (outputMode() == DrawPPM)
        resetClipRegion(MSTrue);
    if (resetClipRegion() == MSTrue) {
        setClipRegion();
        if (outputMode() == Draw)
            resetClipRegion(MSFalse);
    }
...
int MSLabel::computeYCoord(int row_)
{
    ...
    if (alignment() & MSTop)
        ...
        r = height() - offset - (numRows() - row_) * textHeight();
    else {
...
int MSLabel::computeXCoord(int, int column_, const char* pString_, int len_)
{
    ...
    if (pString_ != 0) {
        if (alignment() & MSLeft)
            r = offset + textWidth(pString_, column_);
        else if (alignment() & MSRight) {
...
void MSLabel::updatePixmap(void)
{
    ...
    if (insensitivePixmap() == 0 && pixmap() != 0)
        _insensitivePixmap = new MSPixmap(*pixmap());
    if (pixmap() == 0 && insensitivePixmap() != 0)
        _pixmap = new MSPixmap(*insensitivePixmap());
    if (firstMap() == MSTrue) {
        if (dynamic() == MSTrue)
            computeSize();
        else
...
void MSLabel::displayToolTip(MSBoolean displayToolTip_)
{
    if (_displayToolTip != displayToolTip_) {
        ...
        if (displayToolTip() == MSTrue)
            mask = eventMask() | EnterWindowMask | LeaveWindowMask;
        else
            mask = (eventMask() & ~EnterWindowMask) & ~LeaveWindowMask;
        selectInput(mask);
    }
...
void MSLabel::toolTipXY(int& x, int& y)
{
    ...
    if (x + w > server()->width()) {
        ...
        tailPosition = MSToolTip::Left;
    if (y + h > server()->height()) {
        y = rooty - h;
        tailPosition |= MSToolTip::Bottom;
    } else
        tailPosition |= MSToolTip::Top;
...
const MSStringVector& MSLabel::tip(void) const
{
    ...
    MSStringVector* tip = (MSStringVector*)hashTbl->lookup((unsigned long)this);
    if ((unsigned long)tip == hashTbl->notFound())
        return (*(MSStringVector*)hashTbl->notFound());
    else
        return *tip;
}
...
void MSLabel::set(MSAttrValueList& avList_)
{
    ...
    for (unsigned i = 0; i < avList_.length(); i++) {
        if (avList_[i].attribute() == "label")
            label(MSAttrValue::stringToStringVector(avList_[i].value())), index << i;
        else if (avList_[i].attribute() == "alignment")
            alignment(MSAttrValue::stringToAlignment(avList_[i].value())), index << i;
        else if (avList_[i].attribute() == "margin")
            margin(avList_[i].value().asInt()), index << i;
        else if (avList_[i].attribute() == "toolTip")
            toolTip(MSAttrValue::stringToStringVector(avList_[i].value())), index << i;
        else if (avList_[i].attribute() == "displayToolTip")
            displayToolTip(avList_[i].value().asBoolean()), index << i;
    }
