...
unsigned long MSLabelFormat::nextWeekday(unsigned long now_)
{
    ...
    if (tm->tm_wday == 0)
        now += secondsPerDay; // Sunday so move up one day
    else if (tm->tm_wday == 6)
        now += 2 * secondsPerDay; // Saturday so move up two days
...
double MSLabelFormat::snapTime(double start_, double increment_)
{
    ...
    if (fabs(start_) < LONG_MAX) {
        ...
        {
            newTime = start - (secondsPerDay * tm->tm_wday) - start % secondsPerDay + secondsPerDay;
        } else {
...
const double** MSLabelFormat::findProtocol(const MSLabelOut& out_)
{
    ...
    if (out_.format().formatType() == MSFormat::Time)
...
double MSLabelFormat::snapNumber(double in_, const MSLabelOut& out_)
{
    ...
    if (protocol != 0) {
        ...
        for (p = 0; protocol[p] != 0; p++) {
            ...
            if (best <= 0. || (out >= 0. && out < best))
                best = out;
        }
        if (best > 0.0)
            return (best * sign);
        else
            return (in_ * sign); // Couldn't find a nice values so return the old one
    }
    return in_;
}
