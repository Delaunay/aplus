...
MSLabelOut::MSLabelOut(const MSStringVector& aStringVector_)
    : _labels(aStringVector_)
    , _format(MSFloat::Default)
{
    init();
}

MSLabelOut::MSLabelOut(const MSFormat& aFormat_)
    : _format(aFormat_)
{
    init();
}
...
{
    if (index_ < labels().length())
        buffer_ = labels()(index_);
    return buffer_.string();
}
