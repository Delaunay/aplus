...
MSLayout::MSLayout(MSWidget* owner_, const char* title_)
    : MSLayoutManager(owner_)
{
...
MSLayout::MSLayout(MSWidget* owner_, const MSStringVector& title_)
    : MSLayoutManager(owner_)
{
    _label = 0; // prevent childCreate from adding this as a managed child
    _label = new MSLabel(this, title_);
...
{
    if (label() != 0 && widget_ != label())
        MSLayoutManager::childCreate(widget_);
}
void MSLayout::childDestroy(MSWidget* widget_)
{
    if (widget_ != label())
        MSLayoutManager::childDestroy(widget_);
}
void MSLayout::childMap(MSWidget* widget_)
{
    if (widget_ != label())
        MSLayoutManager::childMap(widget_);
}
void MSLayout::childUnmap(MSWidget* widget_)
{
    if (widget_ != label())
        MSLayoutManager::childUnmap(widget_);
}

void MSLayout::childConfigure(MSWidget* widget_)
{
    if (widget_ != label())
        MSLayoutManager::childConfigure(widget_);
    else if (label()->mapped() == MSTrue && label()->frozen() == MSFalse)
        adjustSize();
}
