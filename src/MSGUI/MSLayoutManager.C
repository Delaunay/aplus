...
MSLayoutManager::MSLayoutManager(MSWidget* owner_)
    : MSManager(owner_)
{
    init();
}
...
{
    if (_geometry == 0)
        _geometry = new MSIndexVector(geometry_);
    else
        *_geometry = geometry_;
    if (setPositions() == MSTrue)
        adjustSize();
...
void MSLayoutManager::extraSpace(int num_, MSLayoutVector* vec_, int size_, MSBoolean uniform_)
{
    ...
    if (diff != 0) {
        ...
        if (uniform_ == MSTrue)
            ...
            ndist = findDistribution(0, num_, vec_, dist, MSTrue);
...
void MSLayoutManager::recomputeVectors(void)
{
    ...
    if (columns() > 0) {
        ...
        if (uniformColumns() == MSTrue) {
            int max = 0;
            for (i = 0; i < columns(); i++)
                max = (max < _columnWidths[i].value()) ? _columnWidths[i].value() : max;
            for (i = 0; i < columns(); i++)
                _columnWidths[i]._value = max;
        }
    } else
        _columnWidths = 0;
    for (i = 0; i < columns(); i++)
        _vectorWidth += _columnWidths[i].value();
    ...
    if (rows() > 0) {
        ...
        if (uniformRows() == MSTrue) {
            int max = 0;
            for (i = 0; i < rows(); i++)
                max = (max < _rowHeights[i].value()) ? _rowHeights[i].value() : max;
            for (i = 0; i < rows(); i++)
                _rowHeights[i]._value = max;
        }
    } else
        _rowHeights = 0;
    for (i = 0; i < rows(); i++)
        _vectorHeight += _rowHeights[i].value();
}
...
void MSLayoutManager::doPlacement(MSLayoutVector* cvec_, MSLayoutVector* rvec_, int vp_, int hp_, int rs_, int cs_)
{
    ...
    while ((np = np->next()) != hp) {
        ...
        if (entry->at().constraints() & At::MaintainWidth)
            nw = entry->widget()->width();
        if (entry->at().constraints() & At::MaintainHeight)
...
void MSLayoutManager::placement(void)
{
    if (mapped() == MSTrue && rows() > 0 && columns() > 0) {
        ...
        for (i = 0; i < rows(); i++)
            lrows[i] = _rowHeights[i];
        for (i = 0; i < columns(); i++)
            lcols[i] = _columnWidths[i];
...
At MSLayoutManager::childPosition(MSWidget* widget_)
{
    ...
    if (entry != 0)
        ...
        return At(0, 0, 1, 1, widget_->resizeConstraints());
}
...
MSLayoutEntry* MSLayoutManager::getEntry(int row_, int col_)
{
    ...
    while ((np = np->next()) != hp) {
        entry = (MSLayoutEntry*)np->data();
        if (entry->at().column() == col_ && entry->at().row() == row_)
            return entry;
    }
    return 0;
}
...
void MSLayoutManager::visibilityUnobscured(void)
{
    ...
    while ((np = np->next()) != hp) {
        entry = (MSLayoutEntry*)np->data();
        if (entry != 0 && entry->mapped() == MSTrue)
            visibilityUnobscuredNotify(entry->widget());
    }
}
...
static int lcm(const MSIndexVector& span_)
{
    ...
    if (n > 0) {
        ...
        for (i = 0; i < n - 1; i++)
            x = lcm(x, ((int)span_(i + 1) > 0) ? (int)span_(i + 1) : 1);
        return x;
    }
    return 0;
}
        ...
        if (geometry().length() > 0) {
            ...
            } else if (columns > 1) // layout in column major order, row major not yet implemented
            {
                for (r = 0; r < rows; r++) {
                    ...
                    for (c = 0; c < columns; c += span) {
                        for (i = 0; i < span; i++)
                            matrix.set(r * w + (c + i), index);
                        index++;
                    }
                }
            } else
                matrix.series(matrix.length());
            ...
            for (r = 0, np = hp; (np = np->next()) != hp && r < nr; r++) {
                ...
                if (entry != 0) {
                    ...
                    if (ax != entry->at().column() || ay != entry->at().row() || h_span != entry->at().columnSpan() || v_span != entry->at().rowSpan() || mstat != entry->widget()->mapped())
                        change = MSTrue;
                }
            }
        } else
            change = setDefaultPositions();
    }
    return change;
}
...
MSAttrValueList& MSLayoutManager::get(MSAttrValueList& avList_)
{
    ...
    avList_ << MSAttrValue("orientation",
        (orientation() == Vertical ? "Vertical" : (orientation() == Horizontal ? "Horizontal" : "Unspecified")),
...
void columnSort(MSLayoutEntry** a_, int n_)
{
    ...
    for (i = 0; i < n; i++) {
        min = i;
        for (j = i + 1; j < n; j++) {
            if (a_[j]->at().column() < a_[min]->at().column())
                min = j;
        }
        swap(a_, min, i);
    }
...
MSLayoutEntry* MSLayoutManager::getNextVerticalEntry(MSLayoutEntry* entry_)
{
    ...
    if (rows() > 1) {
        ...
        while ((np = np->next()) != hp) {
            entry = (MSLayoutEntry*)np->data();
            if (entry != entry_ && entry->at().column() == c)
                entries[i++] = entry;
        }
        numrows = i;
        ...
        for (i = 0; i < numrows; i++) {
            ...
            if (r >= entry->at().row() && r <= entry->at().row() + entry->at().rowSpan()) {
                if (entry->widget()->isProtected() == MSFalse) {
                    next = entry;
                    break;
                } else {
                    r += entry->at().rowSpan();
...
MSLayoutEntry* MSLayoutManager::getNextHorizontalEntry(MSLayoutEntry* entry_)
{
    ...
    if (columns() > 1) {
        ...
        while ((np = np->next()) != hp) {
            entry = (MSLayoutEntry*)np->data();
            if (entry != entry_ && entry->at().row() == r)
                entries[i++] = entry;
        }
        numcols = i;
