...
void MSList::computeSize(void)
{
    ...
    if (editor() != 0 && vsb() != 0 && hsb() != 0 && label() != 0) {
        ...
        if (label()->mapped() == MSTrue)
            h += label()->height();
        if (dynamic() == MSTrue) {
            if (isHsbEnabled() == MSTrue && columns() < actualNumColumns())
                h += (hsb()->height() + spacing());
            if (isVsbEnabled() == MSTrue && rows() < numRows())
                w += (vsb()->width() + spacing());
        } else {
...
void MSList::adjustNumVisible(void)
{
    if ((sizeState() & AdjustSize) == AdjustSize) {
        ...
        if (isVsbEnabled() == MSTrue) {
            if (dynamic() == MSFalse || vsb()->mapped() == MSTrue)
                pW -= (vsb()->width() + spacing());
        }
        if (isHsbEnabled() == MSTrue) {
            if (dynamic() == MSFalse || hsb()->mapped() == MSTrue)
                pH -= (hsb()->height() + spacing());
        }
        panner()->resize(pW, pH);
...
void MSList::drawRow(int row_)
{
    if (row_ < numRows() && inRowRange(row_) == MSTrue)
        drawRows(panner()->window(), row_, row_);
}
...
void MSList::drawRow(Window window_, int row_, const char* pString_, int slen_)
{
    ...
    if (row_ == selectedRow())
        selectOutline = highlighted();
    else
        selectOutline = MSFalse;
    fid = rowFont(row_);
    pmap = rowPixmap(row_);
...
void MSList::drawSelectOutline(Window window_, int row_, MSBoolean select_)
{
    if (row_ >= firstRow() && row_ < firstRow() + rows()) {
        ...
        if (select_ == True)
            XSetForeground(display(), textGC(), selectedRowBackgroundColor(row_));
...
void MSList::keyPress(const XEvent* pEvent_, KeySym keysym_, unsigned int state_, const char* pString_)
{
    ...
    if (sensitive() == MSTrue && hasModel() == MSTrue) {
        if (isProtected() == MSFalse)
            MSRowColumnView::keyPress(pEvent_, keysym_, state_, pString_);
        else {
            int slen = (pString_ != 0) ? strlen(pString_) : 0;
            if (slen == 0 && (keysym_ == XK_Control_L || keysym_ == XK_Control_R || keysym_ == XK_Meta_L || keysym_ == XK_Meta_R || keysym_ == XK_Shift_L || keysym_ == XK_Shift_R))
                return;
            if (pEvent_->xkey.state & ControlMask && keysym_ == XK_s && isearchString().length() > 0) {
                ...
                if (isearchVector().length() > 0)
                    selectedRow(isearchVector().lastElement());
                    ...
                    if (selectionMode() == MSMultiple) {
                        ...
                        if (pEvent_->xbutton.state & ControlMask) {
                            ...
                            if (selectionVector().indexOf(row) != selectionVector().length())
                                trackUnselection(row, 0);
                            else
                                trackSelection(row, 0, MSFalse, MSFalse);
                        } else {
                            // if Shift key is held down
                            if (pEvent_->xbutton.state & ShiftMask)
                                trackSelection(row, 0, MSFalse, MSTrue);
                            else
                                trackSelection(row, 0, MSTrue, MSFalse);
                        }
                    } else {
                        clearSelection();
                        selectedRow(row);
...
int MSList::dragRowFromEvent(const XEvent* pEvent_)
{
    ...
    if (selectionMode() == MSToggle) {
        ...
        if (row < 0 || row >= numRows() || row >= firstRow() + rows())
            return -1;
    } else
        row = selectedRow();
    return row;
...
void MSList::moveRow(int from_, int to_)
{
    ...
    if (from_ < len && to_ < len) {
        ...
        if (from_ < to_) {
            if (to_ >= pixmapVector().length())
                pixmapVector().append(element);
            else
    ...
    _selectedRow = to_;
    if (selectionMode() == MSMultiple)
        (_selectionVector.removeAll()) << to_;
}
...
{
    if (row_ >= firstRow() && (selectedRow() == -1 || selectedRow() < numRows()) && row_ < numRows() && row_ < firstRow() + rows()) {
        MSString buffer;
        const char* pString = formatOutput(buffer, row_);
        int r = selectedRow();
        if (select_ == MSFalse && row_ == selectedRow())
            _selectedRow = -1;
...
void MSList::drawCycle(MSColorCycle* cycle_)
{
    ...
    if (row == -1) {
        ...
        for (int i = first; i <= last; i++)
            drawCycle(i, fg);
    } else if (row < numRows() && inRowRange(row) == MSTrue)
        drawCycle(row, fg);
}
...
void MSList::trackSelection(int startRow_, int, MSBoolean clear_, MSBoolean shifted_)
{
    ...
    if (lastSelectedRow != previousSelectedRow)
    ...
    if (editor()->mapped() == MSFalse) {
        if (selectedRow() < numRows() && inRowRange(selectedRow()) == MSTrue) {
            ...
            if (ro != MSTrue) {
                ...
                moveEditorToSelection(string);
                if (len > columns())
                    editor()->scrollIndex(len - columns());
                else
                    editor()->scrollIndex(0);
            }
        }
    }
}
...
{
    if (firstRow() != vsb()->value())
        vsb()->valueChange(firstRow());
    if (firstColumn() != hsb()->value())
...
void MSList::hsbValueUpdate(void)
{
    if (editorActivate() == MSTrue) {
        if (hsb()->value() < firstColumn())
            ...
            scrollRight(hsb()->value() - firstColumn());
    } else
        updateScrollBars();
}
...
void MSList::updateFirstColumn(int column_)
{
    ...
    if (column_ < 0)
        column_ = 0;
    else if (column_ >= actualNumColumns())
        column_ = actualNumColumns() - 1;
...
const MSPixmap* MSList::rowPixmap(unsigned row_)
{
    if (selectionMode() == MSToggle) {
        if (selected(row_) == MSTrue) {
            if (pixmapVector().length() > 0)
                return pixmapVector()(0);
        } else if (pixmapVector().length() > 1)
            return pixmapVector()(1);
        return 0;
    } else {
        if (row_ < pixmapVector().length())
            return pixmapVector()(row_);
        else
            return 0;
    }
}
    ...
    MSBoolean selected_, MSBoolean selectOutline_)
{
    if (row_ >= firstRow() && row_ < firstRow() + rows()) {
        ...
        if (pmap_ != 0) {
            ...
            int starty = yy;
            if (tHeight > pHeight)
                starty += ((tHeight - pHeight) / 2);
            else
                yy += ((pHeight - tHeight) / 2);
        ...
        if (pString_ != 0 && slen_ > 0) {
            ...
            const XFontStruct* fontStruct = (font_ == font() ? textFontStruct() : server()->fontStruct(font_));
            drawString(display(), window_, textGC(), fontStruct,
                xx, yy + textAscent(), pString_, slen_);
        }
        if (selectOutline_ == MSTrue)
            drawSelectOutline(window_, row_, highlighted());
        else if (selected_ == MSTrue)
            drawSelectOutline(window_, row_, MSFalse);
    }
}
...
{
    if (oldBg_ == selectedRowForeground())
        selectedRowForeground(background());
...
{
    if (selectedRowForeground() != background())
        return selectedRowForeground();
    else
        return rowBackground(row_);
}
...
int MSList::clearSelection(void)
{
    if (selectionMode() == MSToggle)
        ...
        return MSRowColumnView::clearSelection();
}
