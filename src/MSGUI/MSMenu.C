...
{
    if (radioBehavior() != radioBehavior_)
        _radioBehavior = radioBehavior_;
}
...
void MSMenu::calculateNaturalSize(int& naturalWidth_, int& naturalHeight_)
{
    ...
    for (int i = 0; i < columns(); i++) {
        ...
        for (j = 0; j < rows; j++) {
            ...
            if ((indent = mi->computeIndentation()) > maxIndent)
                maxIndent = indent;
            index++;
        }
        index = startIndex;
...
void MSMenu::drawItem(int item_, MSBoolean select_)
{
    if (mapped() == MSTrue && item_ >= 0) {
        ...
        if (mi != 0) {
            (select_ == MSTrue) ? mi->select() : mi->unselect();
        }
    }
}
...
{
    MSMenu* menu;
    if (event_->xbutton.same_screen && (menu = menuList().findMenu(server(), event_->xbutton.x_root, event_->xbutton.y_root)) != 0) {
        MSMenuItem* mi = menu->findItem(event_->xbutton.x_root, event_->xbutton.y_root);
...
MSMenuItem* MSMenu::findItem(int rootX_, int rootY_)
{
    ...
    for (i = 0; i < n; i++) {
        mi = (MSMenuItem*)itemVector()(i);
        if (xcoord >= mi->x() && ycoord >= mi->y() && xcoord <= mi->x() + mi->width() && ycoord <= mi->y() + mi->height()) {
...
void MSMenu::childCreate(MSWidget* widget_)
{
    if (widget_ != 0) {
        ...
        if (_itemVector.indexOf((unsigned long)widget_) != _itemVector.length())
            found = MSTrue;
        else if (_hiddenItemVector.indexOf((unsigned long)widget_) != _hiddenItemVector.length())
            found = MSTrue;
        if (found != MSTrue) {
...
void MSMenu::childDestroy(MSWidget* widget_)
{
    ...
    if (index != itemVector().length()) {
        ...
        if (index != hiddenItemVector().length())
            hiddenItemVector().removeAt(index);
    }
}
...
MSMenuItem* MSMenu::nextRightItem(void)
{
    ...
    if (itemCount() > 0) {
        ...
        for (unsigned j = 0; j < length; j++) {
            ...
            if (mi == ni || (ni != 0 && ni->sensitive() == MSTrue))
                return ni;
        }
    }
    return 0;
...
MSIntVector MSMenu::rowItems(int item_)
{
    ...
    for (i = 0; i < columns() - 1; i++) {
        ...
        if (insideColumn(i + 1, firstItem) == MSTrue)
            vector << firstItem;
        else
            break;
    }
