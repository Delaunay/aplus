    ...
    : MSWidgetOutput(owner_)
{
    ...
    _mnemonic = mnemonic_;
    _tag = tag_;
    if (mnemonic() == 0 && label().length() > 1 && defaultMnemonic() == MSTrue)
        _mnemonic = label().first();
    init();
    _showState = ShowLabel;
}
...
void MSMenuItem::indent(int indent_)
{
    if (indent() != indent_) {
        _indent = indent_;
        if (dynamic() == MSTrue && firstMap() == MSTrue)
            computeSize();
    }
}
...
{
    if (sensitive() == MSTrue)
        textMSGC().fillStyle(FillSolid);
    else
        textMSGC().fillStyle(FillStippled);
    if (mapped() == MSTrue)
        redraw();
}
...
*/
    if (showState() == ShowPixmap)
        myWidth = width() - widthOffset * 2 - indent();
    else if (showState() == ShowBoth)
        myWidth = indent() - spacing();
...
void MSMenuItem::drawLabel(void)
{
    if (owner()->mapped() == MSTrue && owner()->frozen() == MSFalse) {
        if (label().length() > 0) {
            ...
            int dw = drawWidth();
            if (dw > 0)
                while (fontObject()->textWidth(pString, len) > dw)
                    len--;
            if (len > 0) {
...
void MSMenuItem::activate(void)
{
    if (menu() != 0) {
        menu()->releaseGrab();
        if (changeState() == MSTrue)
            state((state() == MSTrue) ? MSFalse : MSTrue);
