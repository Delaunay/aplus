...
void MSMenuShell::adjustChildSize(void)
{
    ...
    if (menuBar() != 0)
        menuBar()->width(width());
    if (child() != 0)
        child()->resize(width(), height() - menuBarHeight());
    _placementFlag = MSFalse;
}
    ...
    unsigned int state_, const char* pString_)
{
    if (menuBar() != 0) {
        ...
        return MSShell::processAltKey(pEvent_, keysym_, state_, pString_);
}
    ...
    unsigned int state_, const char* pString_)
{
    if (menuBar() != 0 && keysym_ == XK_F10) {
        ...
        return MSShell::processFunctionKey(pEvent_, keysym_, state_, pString_);
}
