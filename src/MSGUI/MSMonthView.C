...
void MSMonthView::arrowButtons(MSBoolean arrowButtons_)
{
    if (_arrowButtons != arrowButtons_) {
        ...
        if (arrowButtons() == MSTrue) {
            if (_leftArrow == 0)
                _leftArrow = new MonthViewArrow(this, MSArrow::Left);
            if (_rightArrow == 0)
                _rightArrow = new MonthViewArrow(this, MSArrow::Right);
            _leftArrow->show();
            _rightArrow->show();
            configure();
        } else {
...
void MSMonthView::drawTitle(void)
{
    ...
    if (titleFormat() == MonthYear)
        aString << " " << MSString(viewDate().year());
...
{
    if (_leftArrow != 0)
        _leftArrow->arrowColor(pixel_);
    if (_rightArrow != 0)
        _rightArrow->arrowColor(pixel_);
}
...
void MSMonthView::updateBackground(unsigned long oldbg_)
{
    ...
    if (_leftArrow != 0)
        _leftArrow->background(background());
    if (_rightArrow != 0)
        _rightArrow->background(background());
    redraw();
}
...
void MSMonthView::buttonPress(const XEvent* pEvent_)
{
    ...
    if (showSelection() == MSFalse) {
        ...
        if (px > aRect.x() && py > aRect.y() && px < aRect.x() + aRect.width() && py < aRect.y() + aRect.height()) {
            px -= offset;
            ...
            day += 1;
            day -= _firstDayOffset;
            if (isDoubleClick(pEvent_) == MSTrue && day == selectedDay())
                activate();
            else
                selectedDay(day);
        }
