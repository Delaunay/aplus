...
MSNotebook::~MSNotebook(void)
{
    ...
    if (bindingPixmap() != 0)
        XFreePixmap(display(), _bindingPixmap);
    if (upLeftArrow() != 0)
        upLeftArrow()->destroy();
    if (downRightArrow() != 0)
        downRightArrow()->destroy();
    if (label() != 0)
        label()->destroy();
    if (_popupMenu != 0)
        _popupMenu->destroy();
    if (_applicationPopup != 0)
        _applicationPopup->destroy();
...
void MSNotebook::orientation(Orientation orientation_)
{
    if (_orientation != orientation_) {
        ...
        if (orientation() == Horizontal) {
            if (tabAlignment() == MSTop || tabAlignment() == MSBottom)
                _tabAlignment = MSRight;
            upLeftArrow()->type(MSArrow::Up);
            downRightArrow()->type(MSArrow::Down);
        } else {
            if (tabAlignment() == MSLeft || tabAlignment() == MSRight)
                _tabAlignment = MSBottom;
...
const char* MSNotebook::currentTitle(void) const
{
    static MSString title;
    if (currentEntry() != 0)
        title = currentEntry()->title().asString('\n');
    else
        title.removeAll();
    return title.string();
}
    ...
    if (tabAttr_.notebook() == this) {
        ...
        if (entry != 0) {
            ...
            if (entry->managed() == MSTrue) {
                ...
                if (firstMap() == MSTrue && (tabAttr_.isSet(MSNotebookTabAttribute::TabFont) || tabAttr_.isSet(MSNotebookTabAttribute::TabLabel))) {
                    adjustSize();
                }
        ...
        while ((np = np->next()) != hp) {
            ...
            if (entry->tab()->isModified(MSNotebookTabAttribute::TabBackground) == MSFalse) {
                ...
                if (entry != currentEntry())
                    entry->tab()->background(backpageBackground());
            }
        }
        redraw();
    }
    ...
    while ((np = np->next()) != hp) {
        ...
        if (entry->widget() != 0) {
            ...
            if (entry == currentEntry()) {
                ...
                entry->widget()->show();
            } else if (entry->widget()->mapped() == MSTrue)
                entry->widget()->unmap();
        }
    }
    ...
    if (showTabs() == MSTrue) {
        ...
        if (orientation() == Horizontal) {
            ...
            if (firstEntry() != 0 && managed != 0 && numDrawn != managed) {
                int arrowX;
                if (tabAlignment() == MSLeft)
                    arrowX = startX - arrowWidth();
                else
                    arrowX = startX + arrowOffset();
                upLeftArrow()->moveTo(arrowX, height() - HeightOffset - arrowHeight() * 2 - arrowOffset());
                upLeftArrow()->map();
                if (firstEntry() == firstItem())
                    upLeftArrow()->sensitive(MSFalse);
                else
                    upLeftArrow()->sensitive(MSTrue);
                downRightArrow()->moveTo(arrowX, height() - HeightOffset - arrowHeight());
                downRightArrow()->map();
                if (managedVector.indexOf((unsigned long)firstEntry()->widget()) + numDrawn == managed)
                    downRightArrow()->sensitive(MSFalse);
                else
                    downRightArrow()->sensitive(MSTrue);
            } else {
                upLeftArrow()->unmap();
                downRightArrow()->unmap();
            }
            ...
            if (firstEntry() != 0 && managed != 0 && numDrawn != managed) {
                int arrowY;
                if (tabAlignment() == MSTop)
                    arrowY = startY - arrowHeight();
                else
                    arrowY = startY + arrowOffset();
...
void MSNotebook::childMap(MSWidget* widget_)
{
    if (internalEvent() == MSFalse) {
        ...
        if (entry != 0 && currentEntry() != entry)
            widget_->moveTo(server()->width(), server()->height());
    }
}
...
void MSNotebook::visibilityObscured(void)
{
    ...
    while ((np = np->next()) != hp) {
        entry = (NotebookEntry*)np->data();
        if (entry != 0)
            visibilityObscuredNotify(entry->widget());
    }
}
...
MSNotebook::NotebookEntry* MSNotebook::getEntry(const char* title_) const
{
    ...
    while ((np = np->next()) != hp) {
        entry = (NotebookEntry*)np->data();
        if (entry->title() == MSStringVector(title))
            return entry;
    }
    return 0;
}
        ...
        if (totalSpace >= totalTabHeight())
            ...
            if (count > 1) {
                ...
                if (index < 0)
                    ...
                    firstEntry(getEntry(managedVector(index)));

            } else
                firstEntry(lastEntry_);
        }
    } else {
        int totalSpace = availableWidthForTabs();
        if (totalSpace >= totalTabWidth())
...
MSNotebook::NotebookEntry* MSNotebook::nextEntry(void)
{
    ...
    if (firstEntry() != 0) {
        while ((np = np->next()) != hp) {
            if (firstEntry() == (NotebookEntry*)np->data())
                break;
        }
        if (np == hp)
            return firstEntry();
...
MSNotebook::NotebookEntry* MSNotebook::nextBrowseEntry(MSBoolean cycle_)
{
    ...
    if (currentEntry() != 0) {
        while ((np = np->next()) != hp) {
            if (currentEntry() == (NotebookEntry*)np->data())
                break;
        }
        if (np == hp)
            return currentEntry();
...
void MSNotebook::updateTitle(void)
{
    ...
    positionLabel();
    label()->unfreeze();
    if (label()->height() != h || label()->width() != w || doPlacement == MSTrue)
        adjustSize();
    else if (label()->mapped() == MSTrue)
        label()->redraw();
    internalEvent(MSFalse);
}
...
void MSNotebook::updateBackground(unsigned long oldBG_)
{
    ...
    if (selectedPageBackground() == oldBG_)
        selectedPageBackground(background());
...
void MSNotebook::updateForeground(unsigned long oldFG_)
{
    ...
    if (selectedPageForeground() == oldFG_)
        selectedPageForeground(foreground());
    MSNodeItem* hp = childListHead();
    MSNodeItem* np = hp;
    while ((np = np->next()) != hp) {
        ...
        if (entry->tab()->isModified(MSNotebookTabAttribute::TabForeground) == MSFalse) {
            ...
            if (entry != currentEntry())
                entry->tab()->foreground(foreground());
        }
    }
    if (label()->foreground() == oldFG_)
    ...
    if (showBinding() == MSTrue) {
        ...
        if (w >= MSNotebookMinimumBindingWidth && h >= MSNotebookMinimumBindingWidth) {
            ...
            for (i = 0; i < count; i++) {
                ...
                XSetLineAttributes(display(), notebookGC(), 1, LineSolid, CapRound, JoinMiter);
                for (int i = 1; i < sd; i++)
                    XDrawArc(display(), dest, notebookGC(), sx + i + xoff, sy + i + yoff, sw - i, sh - i, a1 * 64, a2 * 64);
            ...
            if (outputMode() < Print) {
                if (bindingPixmap() != 0)
                    XFreePixmap(display(), bindingPixmap());
                ...
                if (remainder > 0) {
                    if (orientation() == Horizontal)
                        XCopyArea(display(), dest, bindingPixmap(), notebookGC(), 0, 0, pw, remainder, xoff, yoff);
                    else
                        XCopyArea(display(), dest, bindingPixmap(), notebookGC(), 0, 0, remainder, ph, xoff, yoff);
                }
                xoff = 0;
                yoff = 0;
                ...
                if (remainder > 0) {
                    if (orientation() == Horizontal)
                        XDrawLine(display(), dest, notebookGC(), lx1 + xoff, ly1 + yoff, lx2 + xoff, ly1 + remainder + yoff - 1);
                    else
                        XDrawLine(display(), dest, notebookGC(), lx1 + xoff, ly1 + yoff, lx1 + remainder + xoff - 1, ly2 + yoff);
                }
                xoff = oldXoff;
                yoff = oldYoff;
            }
...
void MSNotebook::updatePopupMenu(void)
{
    if (_popupMenu != 0) {
        ...
        while ((np = np->next()) != hp) {
            NotebookEntry* entry = (NotebookEntry*)np->data();
            if (entry->managed() == MSTrue) {
                if (entry->tab()->sensitive() == MSTrue)
                    _popupMenu->taggedMenuItem(i++)->sensitive(MSTrue);
                else
                    _popupMenu->taggedMenuItem(i++)->sensitive(MSFalse);
            }
        }
    }
}
...
void MSNotebook::permuteTitles(const MSStringVector& aStringVector_)
{
    ...
    firstEntry(firstItem());
    if (currentEntry() != 0) {
        if (currentEntry()->managed() == MSTrue)
            resetFirstEntry();
        else
            currentEntry(0);
    }
...
MSBoolean MSNotebook::okToSwitch(void)
{
    ...
    if (focus != 0) {
        ...
        for (cursor.setToFirst(); cursor.isValid() == MSTrue; cursor.setToNext()) {
            if (focus == cursor.widget())
                return top()->traverseFocus(0);
        }
        return MSTrue;
    } else
        return MSTrue;
...
void MSNotebook::NotebookTab::buttonPress(const XEvent* event_)
{
    if (sensitive() == MSTrue && event_->xbutton.button == Button1) {
        MSNotebook* notebook = (MSNotebook*)owner();
        if (notebook->isProtected() == MSFalse && notebook->okToSwitch() == MSTrue)
            arm();
        else
            server()->bell();
...
void MSNotebook::NotebookTab::motionNotify(const XEvent* event_)
{
    ...
    if (sensitive() == MSTrue && notebook->isProtected() == MSFalse && notebook->okToSwitch() == MSTrue) {
        if (event_->xmotion.is_hint == NotifyNormal && (event_->xmotion.state & Button1Mask) == Button1Mask) {
            if (event_->xmotion.x < 0 || event_->xmotion.x > width() || event_->xmotion.y < 0 || event_->xmotion.y > height()) {
                disarm();
...
void MSNotebook::NotebookTab::tabBackground(unsigned long bg_, MSBoolean modified_)
{
    _tabBackground = bg_;
    if (modified_ == MSTrue)
        _modifiedFlag |= MSNotebookTabAttribute::TabBackground;
}
...
void MSNotebook::NotebookTab::tabForeground(unsigned long fg_, MSBoolean modified_)
{
    _tabForeground = fg_;
    if (modified_ == MSTrue)
        _modifiedFlag |= MSNotebookTabAttribute::TabForeground;
}
...
void MSNotebook::NotebookTab::tabFont(Font fid_, MSBoolean modified_)
{
    font(fid_);
    if (modified_ == MSTrue)
        _modifiedFlag |= MSNotebookTabAttribute::TabFont;
}
...
{
    if (sensitive() == MSTrue) {
        if (arrow()->selected() == MSFalse)
            arrow()->select(MSTrue);
        MSNotebook* notebook = (MSNotebook*)owner();
        NotebookEntry* first;
        if (arrow()->arrowType() == MSArrow::Up || arrow()->arrowType() == MSArrow::Left)
            first = notebook->previousEntry();
        else
            first = notebook->nextEntry();
...
void MSNotebook::NotebookArrow::browse(void)
{
    ...
    NotebookEntry* entry;
    if (arrow()->arrowType() == MSArrow::Up || arrow()->arrowType() == MSArrow::Left)
        entry = notebook->previousBrowseEntry();
    else
        entry = notebook->nextBrowseEntry();
...
void MSNotebook::NotebookArrow::button1Press(const XEvent* event_)
{
    if (event_->xbutton.state & ControlMask) {
        ...
        server()->bell();
    if (mode() == Browse)
        repeatThreshold(MSNotebookArrowBrowseRepeatThreshold);
    else
        repeatThreshold(MSNotebookArrowSearchRepeatThreshold);
    if (arrowTimer() == 0)
        _arrowTimer = new MSArrowTimer(repeatThreshold(), this);
    else
        arrowTimer()->expirationInterval(repeatThreshold());
    if (mode() == Browse)
        repeatInterval(MSNotebookArrowBrowseRepeatInterval);
    else
        repeatInterval(MSNotebookArrowSearchRepeatInterval);
    arrowTimer()->reset();
}
