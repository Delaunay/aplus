...
static const unsigned long MSOptionMenuEventMask = (ExposureMask | ButtonPressMask | ButtonReleaseMask | Button1MotionMask);
...
{
    if (optionMenu() == 0)
        _optionMenu = new MSOptionPopupMenu(this);
...
{
    if (optionMenu() != 0 && optionMenu()->mapped() == MSTrue)
        optionMenu()->grab();
}
...
void MSOptionMenu::buttonPress(const XEvent* pEvent_)
{
    if (isProtected() == MSFalse) {
        if (pEvent_->xbutton.x >= fieldValue()->x() && pEvent_->xbutton.x <= fieldValue()->x() + fieldValue()->width() && pEvent_->xbutton.y >= fieldValue()->y() && pEvent_->xbutton.y <= fieldValue()->y() + fieldValue()->height()) {
...
void MSOptionMenu::placement(void)
{
    if (fieldValue() != 0 && fieldLabel() != 0) {
        ...
        int vh = fieldValue()->textHeight() + 2 * (marginHeight() + fieldValue()->shadowThickness() + fieldValue()->highlightThickness());
        int lh = fieldLabel()->textHeight() + 2 * (fieldLabel()->shadowThickness() + fieldLabel()->highlightThickness());
    ...
    if (frozen() == MSFalse && owner()->mapped() == MSTrue && mapped() == MSTrue) {
        ...
        if (pString != 0 && buffer.length() > 0) {
            ...
            if (len > 0) {
                ...
                int vw = fieldValue()->width() - 2 * fieldValue()->offset() - 2 * MSOptionMenuDefaultMargin - MSOptionMenuSymbolWidth;
...
void MSOptionMenu::updateBackground(unsigned long oldbg_)
{
    ...
    if (optionMenu() != 0)
        optionMenu()->background(background());
}
...
void MSOptionMenu::updateForeground(unsigned long oldfg_)
{
    ...
    if (optionMenu() != 0)
        optionMenu()->foreground(foreground());
}
...
void MSOptionMenu::updateSensitivity(void)
{
    ...
    if (optionMenu() != 0)
        optionMenu()->sensitive(sensitive());
}
