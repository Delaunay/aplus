...
static const unsigned long MSPageEventMask = (ExposureMask | ButtonPressMask | ButtonReleaseMask | Button1MotionMask);
...
MSPage::MSPage(MSWidget* owner_)
    : MSCompositeText(owner_)
{
    init();
}

MSPage::MSPage(MSWidget* owner_, MSCharMatrix& model_)
    : MSCompositeText(owner_)
{
    init();
    couple(&model_);
...
void MSPage::button1Press(const XEvent* event_)
{
    ...
    if (isProtected() == MSFalse && acceptFocus() == MSTrue && traverseFocus(this) == MSTrue) {
        ...
        if (moved == MSTrue) {
            ...
            rubberBandRect().configure(0, 0, 0, 0);
...
void MSPage::button1Release(const XEvent* event_)
{
    if (isProtected() == MSFalse) {
        if (event_->xbutton.button == Button2)
            activateCallback("button2up");
        else if (event_->xbutton.button == Button3)
            activateCallback("button3up");
    }
}
