...
void MSPane::updateBackground(unsigned long oldbg_)
{
    ...
    for (unsigned i = 0; i < sashList().length(); i++)
        _sashList(i)->background(background());
}
...
{
    if (sashAction() == MSFalse)
        MSLayout::childInsert(widget_);
}
void MSPane::childDestroy(MSWidget* widget_)
{
    if (sashAction() == MSFalse)
        MSLayout::childRemove(widget_);
}
void MSPane::childMap(MSWidget* widget_)
{
    if (sashAction() == MSFalse)
        MSLayout::childMap(widget_);
}
void MSPane::childUnmap(MSWidget* widget_)
{
    if (sashAction() == MSFalse)
        MSLayout::childUnmap(widget_);
}
void MSPane::childConfigure(MSWidget* widget_)
{
    if (sashAction() == MSFalse)
        MSLayout::childConfigure(widget_);
}
...
void MSPane::placement(void)
{
    MSLayout::placement();
    recomputeVectors();
    if ((rows() > 0 || columns() > 0) && mapped() == MSTrue)
        placeSashes();
}
...
void MSPane::removeExtraSashes(void)
{
    ...
    for (unsigned i = 0; i < sashList().length(); i++) {
        pSash = (MSSash*)_sashList(i);
        if ((orientation() == MSLayoutManager::Vertical && pSash->row() >= rows() - 1) || (orientation() == MSLayoutManager::Horizontal && pSash->column() >= columns() - 1)) {
            delete pSash;
        } else
            newSashList.append(pSash);
    }
