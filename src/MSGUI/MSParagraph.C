...
MSParagraph::MSParagraph(const MSParagraph& paragraph_)
    : MSPrintItem(paragraph_)
...
{
    _firstLineIndentPixel = int(MSPointsPerInch * x_);
    return *this;
}
...
{
    _xPixel = int(x_ * MSPointsPerInch);
    return *this;
}
MSParagraph& MSParagraph::yOrigin(double x_)
{
    _yPixel = int(x_ * MSPointsPerInch);
    return *this;
}
...
int MSParagraph::computePrintSize(MSReport* report_, int, int y_, int w_, int, int topMargins_, int margins_)
{
    ...
    if (style() & MSP::Box)
        ...
        border = (style() & MSP::BoxT ? lineWidth() : 0) + (style() & MSP::BoxB ? lineWidth() : 0);
    ...
    if (remainingHeight <= 0 || (((pageAlignment() & MSTop) || orphanRowHeight > remainingHeight) && (y_ != report_->bodyTop(report_->pageCount())))) {
        _pageCount++;
...
void MSParagraph::computeParagraphSize(MSReport* report_)
{
    ...
    if (fdata != 0) {
                    ...
                    if ((width += w) > tlw - spaceWidth) {
                        if (w > tlw) {
                            ...
                            if (spaceCharWidth < minSpaceCharWidth) {
                                ...
                                outputText().lastElement() << (aString << " ");
                        }
                    } else {
                        if (j == 0)
                            outputText() << (aString << " ");
                    ...
                    if ((width += w) > tlw) {
                        ...
                        if (width - w + ww < tlw) {
                            if (j == 0)
                                ...
                                outputText().lastElement() << bString;
                            outputText() << "";
                            width = w = 0;
                        } else if (ww > tlw) {
        ...
        MSMessageLog::errorMessage("Error: font data not available - unable to print paragraph\n");
}
...
int MSParagraph::print(MSReport* report_, int x_, int y_, int w_, int h_, int bottomIndent_, int leftMargin_)
{
    ...
    if (pageBreakRow() < 0 && (remainingHeight < 0 || ((orphanRowHeight > remainingHeight || pageAlignment() & MSTop) && y_ != report_->bodyTop(report_->pageCount())))) {
        pageBreakRow(0);
        _currentPage++;
...
void MSParagraph::printParagraphDecorations(MSReport* report_, int x_, int y_, int w_, int h_, int pageEnd_)
{
    ...
    if (y_ - pageEnd_ > MinSize) {
        ...
        if (bgGrayScale() < 1.)
            report_->fillRectangle(x_, y_, w_ - 1, h - 1);
        if (Box & style())
            report_->strokeRectangle(x_, y_, w_ - 1, h - 1);
        else
            report_->printBox(style(), x_, y_, w_, h);
    }
}
    ...
    while (count < word_.length() && fontSize() > 0) {
        ...
        if (count == 0 && j == 1 && maxWidth == lineWidth_) {
            ...
            if (maxWidth < lineWidth_) {
                ...
                outputText() << word_.subString(count, j);
        }
        count += j;
    }
    return w;
