...
{
    if (activateCallback(MSWidgetCallback::dismiss) == MSFalse)
        hide();
}
...
void MSPopup::propertyNotify(const XEvent* event_)
{
    ...
    if (event_->xproperty.atom == prop) {
        ...
        if (status == Success && actualTarget == XA_INTEGER && actualFormat == propertyFormat(prop) && itemCount > 0) {
            data = (int*)actData;
...
MSAttrValueList& MSPopup::get(MSAttrValueList& avList_)
{
    ...
    for (int i = 0; index.length() < 2 && i < avList_.length(); i++) {
        if (avList_[i].valueType() == MSAttrValue::Callback && (avList_[i].attribute() == "quit" || avList_[i].attribute() == "saveyourself")) {
            index << i;
        }
    }
