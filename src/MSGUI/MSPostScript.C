...
void MSPostScript::defaultFontName(const char* str_)
{
    ...
    if ((tempStr = formatFontString(str_)) != 0 && (fontStr = (char*)fontHashTable()->lookup(tempStr)) != 0) {
...
MSBoolean MSPostScript::findFont(const char* fontString_)
{
    ...
    if (str.length() != 0) {
        ...
        if ((dp = opendir(fontPath())) != 0) {
            while (MS_READDIR(dp, dirEntry, fp) != 0)
                if (str == fp->d_name) {
                    status = MSTrue;
                    break;
                }
        ...
        if (status != MSTrue && (dp = opendir(altDefaultFontPath())) != 0) {
            ...
            closedir(dp);
            if (status != MSTrue && afmfilesHashTable() != 0 && (dp = opendir(altDefaultFontPath())) != 0) {
                MSString afmfile((char*)afmfilesHashTable()->lookup(fontString_));
                MSString tmp = afmfile;
                tmp << ".afm";
                while (MS_READDIR(dp, dirEntry, fp) != 0)
...
void MSPostScript::addFont(const char* fontString_)
{
    ...
    if (fontString_ != 0) {
        if ((MSPrintFontData*)printFontIDHashTable().lookup(fontString_) == (MSPrintFontData*)printFontIDHashTable().notFound()) {
...
{
    MSPrintFontData* data = 0;
    if ((data = (MSPrintFontData*)fontDataHashTable().lookup(fid_)) == (MSPrintFontData*)fontDataHashTable().notFound()) {
        if ((data = (MSPrintFontData*)fontDataHashTable().lookup(1)) == (MSPrintFontData*)fontDataHashTable().notFound()) {
...
MSBoolean MSPostScript::printOpen()
{
    ...
    if (outputMode() == EPS)
        file.change(".ps", ".eps", 0, 1);
    if (outputMode() == PPM)
        file.change(".ps", ".ppm", 0, 1);
    pout.open(file);
...
void MSPostScript::printProlog(void)
{
    ...
    if ((n = documentFonts().length()) > 0) {
        ...
        for (unsigned i = 1; i < n; i++)
            pout << "%%+ font " << documentFonts()(i) << endl;
    } else
        pout << endl;
...
MSBoolean MSPostScript::setForeground(void)
{
    ...
    case Colorfg:
        if (gcValues().foreground == whitePixel())
...
void MSPostScript::setBackground(int mode_)
{
    ...
    case Colorfg:
        if (mode_ == -1)
            ...
            if (gcValues().background == whitePixel())
