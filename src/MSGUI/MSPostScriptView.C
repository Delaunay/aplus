...
static const unsigned long MSPostScriptViewEventMask = (ExposureMask | PropertyChangeMask | ButtonPressMask /*|ButtonReleaseMask*/);
...
MSPostScriptViewOutChannel::MSPostScriptViewOutChannel(MSPostScriptView* owner_, const char* name_, int fd_)
    : _owner(owner_)
    , MSChannel(name_, fd_, Default, Read, 0)
{
    enable();
}
...
MSPostScriptViewInChannel::MSPostScriptViewInChannel(MSPostScriptView* owner_, const char* name_, int fd_)
    : _owner(owner_)
    , MSChannel(name_, fd_, Default, Write, 0)
{
    _inputBuffer = 0;
    _inputBufferPtr = 0;
    _inputQueue = 0;
...
void MSPostScriptViewInChannel::process(void)
{
    ...
    do {
        ...
        if (_bufferBytesLeft == 0) {
            ...
            if (_inputQueue && _inputQueue->seek_needed) {
            ...
            if (_bytesLeft > BUFSIZ) {
                _bufferBytesLeft = fread(_inputBuffer,
                    sizeof(char), BUFSIZ,
                    _inputQueue->fp);
            ...
            if (_bytesLeft > 0 && _bufferBytesLeft == 0) {
...
void MSPostScriptView::firstPage(void)
{
    if (isBusy() == MSTrue)
        return;
    if (_doc != 0 && pageCount() > 0 && currentPage() != 0)
        currentPage(0);
}
...
void MSPostScriptView::lastPage(void)
{
    if (isBusy() == MSTrue)
        return;
    if (_doc != 0 && pageCount() > 0 && currentPage() != (pageCount() - 1))
...
{
    if (isFileLoaded() == MSFalse)
        return;
    int i;

    if (pageCount() > 0) {
        if (pageNumber >= _doc->numPages())
            pageNumber = _doc->numPages() - 1;
...
void MSPostScriptView::zoomFactor(double zoomFactor_)
{
    if (_zoomFactor != zoomFactor_) {
        if (zoomFactor_ <= 0.0 || zoomFactor_ > MSPostScriptViewMaxZoom || zoomFactor_ * _defaultXdpi < 1.0 || zoomFactor_ * _defaultYdpi < 1.0)
            return;
...
{
    if (_replyWin == 0) {
        if (stopInterpreter() == 5)
            ghostScriptExecError();
        else
            ghostScriptFileError("");
    } else {
        stopInterpreter();
...
MSPostScriptView::ErrorCode MSPostScriptView::loadFile(const MSString& file_, int startPage_)
{
    ...
    if (_doc != 0) {
        ...
        if (orient == MSPostScriptViewDocument::NONE)
            orient = _doc->pageOrientation();
        switch (orient) {
...
void MSPostScriptView::printPages(FILE* fp, const MSIndexVector& pageIndex_)
{
    ...
    for (i = 0; i < _doc->_doc->numpages; i++) {
        if (_doc->_doc->pageorder == MSPostScriptViewDocument::DESCEND)
            j = (_doc->_doc->numpages - 1) - i;
        else
            j = i;
...
MSBoolean MSPostScriptViewDocument::scanFile(FILE* file)
{
    ...
    while (preread || readline(line, sizeof line, file, &position, &line_len)) {
        if (!preread)
            section_len += line_len;
        preread = 0;
        if (line[0] != '%' || iscomment(line + 1, "%EndComments") || line[1] == ' ' || line[1] == '\t' || line[1] == '\n' || !isprint(line[1])) {
            break;
        } else if (line[1] != '%') {
            /* Do nothing */
            ...
            preread = 1;
            while (readline(line, sizeof line, file, &position, &line_len) && DSCcomment(line) && iscomment(line + 2, "+")) {
                section_len += line_len;
                doc->media = (struct documentmedia*)
                    realloc(doc->media,
                        (doc->nummedia + 1) * sizeof(struct documentmedia));
                if (doc->media == NULL) {
                    MSTKTHROWEXCEPTION(MSOutOfMemory("MSPostScriptView Out Of Memory.\n"));
                }
        ...
        } else if (doc->nummedia == NONE && iscomment(line + 2, "DocumentPaperSizes:")) {
    ...
    if (doc->epsf && DSCcomment(line) && iscomment(line + 2, "BeginPreview")) {
        ...
        beginsection = 0;
        while (readline(line, sizeof line, file, &position, &line_len) && !(DSCcomment(line) && iscomment(line + 2, "EndPreview"))) {
            section_len += line_len;
        }
        section_len += line_len;
        readline(line, sizeof line, file, &position, &line_len);
    ...
    if (DSCcomment(line) && iscomment(line + 2, "BeginDefaults")) {
        ...
        beginsection = 0;
        while (readline(line, sizeof line, file, &position, &line_len) && !(DSCcomment(line) && iscomment(line + 2, "EndDefaults"))) {
            section_len += line_len;
            if (!DSCcomment(line)) {
                /* Do nothing */
            } else if (doc->default_page_orientation == NONE && iscomment(line + 2, "PageOrientation:")) {
    ...
    if (!(DSCcomment(line) && (iscomment(line + 2, "BeginSetup") || iscomment(line + 2, "Page:") || iscomment(line + 2, "Trailer") || iscomment(line + 2, "EOF")))) {
        doc->beginprolog = beginsection;
        beginsection = 0;
        preread = 1;

        while ((preread || readline(line, sizeof line, file, &position, &line_len)) && !(DSCcomment(line) && (iscomment(line + 2, "EndProlog") || iscomment(line + 2, "BeginSetup") || iscomment(line + 2, "Page:") || iscomment(line + 2, "Trailer") || iscomment(line + 2, "EOF")))) {
            if (!preread)
                section_len += line_len;
            preread = 0;
        }
    ...
    if (!(DSCcomment(line) && (iscomment(line + 2, "Page:") || iscomment(line + 2, "Trailer") || iscomment(line + 2, "EOF")))) {
        doc->beginsetup = beginsection;
        beginsection = 0;
        preread = 1;
        while ((preread || readline(line, sizeof line, file, &position, &line_len)) && !(DSCcomment(line) && (iscomment(line + 2, "EndSetup") || iscomment(line + 2, "Page:") || iscomment(line + 2, "Trailer") || iscomment(line + 2, "EOF")))) {
            if (!preread)
                section_len += line_len;
            preread = 0;
            if (!DSCcomment(line)) {
            ...
            } else if (page_media_set == NONE && iscomment(line + 2, "PaperSize:")) {
...
newpage:
    ...
    continuepage:
        while (readline(line, sizeof line, file, &position, &line_len) && !(DSCcomment(line) && (iscomment(line + 2, "Page:") || iscomment(line + 2, "Trailer") || iscomment(line + 2, "PageTrailer") || iscomment(line + 2, "EOF")))) {
            section_len += line_len;
            if (!DSCcomment(line)) {
                /* Do nothing */
            } else if (doc->pages[doc->numpages].orientation == NONE && iscomment(line + 2, "PageOrientation:")) {
                sscanf(line + lengthOfString("%%PageOrientation:"), "%s", text);
                if (strcmp(text, "Portrait") == 0) {
            ...
            } else if (doc->pages[doc->numpages].media == NULL && iscomment(line + 2, "PageMedia:")) {
                cp = gettext(line + lengthOfString("%%PageMedia:"), NULL);
                for (dmp = doc->media, i = 0; i < doc->nummedia; i++, dmp++) {
                    if (strcmp(cp, dmp->name) == 0) {
                ...
                free(cp);
            } else if (doc->pages[doc->numpages].media == NULL && iscomment(line + 2, "PaperSize:")) {
                cp = gettext(line + lengthOfString("%%PaperSize:"), NULL);
                for (dmp = doc->media, i = 0; i < doc->nummedia; i++, dmp++) {
                    /* Note: Paper size comment uses down cased paper size
		     * name.  Case insensitive compares are only used for
		     ...
		     */
                ...
                free(cp);
            } else if ((page_bb_set == NONE || page_bb_set == ATEND) && iscomment(line + 2, "PageBoundingBox:")) {
    ...
    preread = 1;
    while ((preread || readline(line, sizeof line, file, &position, &line_len)) && !(DSCcomment(line) && iscomment(line + 2, "EOF"))) {
        if (!preread)
            section_len += line_len;
        preread = 0;
        if (!DSCcomment(line)) {
                    ...
                    *cp++ = ')';
                    ...
                    if (*(line + 2) >= '0' && *(line + 2) <= '9') {
                        if (*(line + 3) >= '0' && *(line + 3) <= '9') {
                            *cp++ = ((*(line + 1) - '0') * 8 + *(line + 2) - '0') * 8 + *(line + 3) - '0';
                            line += 4;
                        } else {
    ...
    long* position, unsigned int* line_len)
{
    ...
    *line_len = strlen(line);
    if (!(DSCcomment(line) && iscomment(line + 2, "Begin"))) {
        ...
        strcpy(save, line + 7);
        while (readline(line, size, fp, NULL, &nbytes) && !(DSCcomment(line) && iscomment(line + 2, "EndDocument"))) {
            *line_len += nbytes;
        }
        *line_len += nbytes;
        strcpy(line, save);
        ...
        strcpy(save, line + 7);
        while (readline(line, size, fp, NULL, &nbytes) && !(DSCcomment(line) && iscomment(line + 2, "EndFeature"))) {
            *line_len += nbytes;
        }
        *line_len += nbytes;
        strcpy(line, save);
        ...
        strcpy(save, line + 7);
        while (readline(line, size, fp, NULL, &nbytes) && !(DSCcomment(line) && iscomment(line + 2, "EndFile"))) {
            *line_len += nbytes;
        }
        *line_len += nbytes;
        strcpy(line, save);
        ...
        strcpy(save, line + 7);
        while (readline(line, size, fp, NULL, &nbytes) && !(DSCcomment(line) && iscomment(line + 2, "EndFont"))) {
            *line_len += nbytes;
        }
        *line_len += nbytes;
        strcpy(line, save);
        ...
        strcpy(save, line + 7);
        while (readline(line, size, fp, NULL, &nbytes) && !(DSCcomment(line) && iscomment(line + 2, "EndProcSet"))) {
            *line_len += nbytes;
        }
        *line_len += nbytes;
        strcpy(line, save);
        ...
        strcpy(save, line + 7);
        while (readline(line, size, fp, NULL, &nbytes) && !(DSCcomment(line) && iscomment(line + 2, "EndResource"))) {
            *line_len += nbytes;
        }
        *line_len += nbytes;
        strcpy(line, save);
        ...
        while (readline(line, size, fp, NULL, &nbytes) && !(DSCcomment(line) && iscomment(line + 2, "EndData"))) {
            *line_len += nbytes;
        }
        *line_len += nbytes;
        strcpy(line, save);
        ...
        while (readline(line, size, fp, NULL, &nbytes) && !(DSCcomment(line) && iscomment(line + 2, "EndBinary"))) {
            *line_len += nbytes;
        }
        *line_len += nbytes;
        strcpy(line, save);
