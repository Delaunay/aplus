...
{
    if (_textFontStruct->min_byte1 != 0 || _textFontStruct->max_byte1 != 0)
        return MSTrue;
    else if (_textFontStruct->max_char_or_byte2 > 255)
        return MSTrue;
    else
        return MSFalse;
}
...
{
    if (doubleByte()) {
        return (pString_ != 0) ? (int)XTextWidth16((XFontStruct*)_textFontStruct, (XChar2b*)pString_, strlen(pString_) / 2) : 0;
    } else {
        return (pString_ != 0) ? (int)XTextWidth((XFontStruct*)_textFontStruct, pString_, strlen(pString_)) : 0;
    }
}
...
{
    if (doubleByte()) {
        return (pString_ != 0) ? (int)XTextWidth16((XFontStruct*)_textFontStruct, (XChar2b*)pString_, len_ / 2) : 0;
    } else {
...
{
    return (_textFontStruct->per_char != 0 && aChar_ >= _textFontStruct->min_char_or_byte2 && aChar_ <= _textFontStruct->max_char_or_byte2) ? charStruct(aChar_)->width : charWidth();
}

INLINELINKAGE int MSPrimitiveText::charLbearing(char aChar_) const
{
    return (_textFontStruct->per_char != 0 && aChar_ >= _textFontStruct->min_char_or_byte2 && aChar_ <= _textFontStruct->max_char_or_byte2) ? charStruct(aChar_)->lbearing : charLbearing();
}

INLINELINKAGE int MSPrimitiveText::charRbearing(char aChar_) const
{
    return (_textFontStruct->per_char != 0 && aChar_ >= _textFontStruct->min_char_or_byte2 && aChar_ <= _textFontStruct->max_char_or_byte2) ? charStruct(aChar_)->rbearing : charRbearing();
}
