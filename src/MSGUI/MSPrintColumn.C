...
MSPrintColumn::MSPrintColumn(MSPrintManager* parent_, MSFloatVector& columnWidths_, unsigned numColumns_, const MSSymbol& tag_)
    : _tag(tag_)
    , MSPrintManager(parent_)
{
    init();
...
MSPrintColumn::MSPrintColumn(unsigned numColumns_, const MSSymbol& tag_)
    : _tag(tag_)
{
    init();
    _numColumns = numColumns_;
...
MSPrintColumn::MSPrintColumn(MSFloatVector& columnWidths_, unsigned numColumns_, const MSSymbol& tag_)
    : _tag(tag_)
{
    init();
    _numColumns = numColumns_;
...
void MSPrintColumn::computeColumnWidths(void)
{
    ...
    if ((len = columnWidths().length()) != 0) {
        ...
        for (unsigned i = 0; i < len; i++)
            columnPixelWidth() << unsigned(_columnWidths[i] * MSPointsPerInch * scale);

        if (len < columnCount() && cw < w) {
            unsigned residual = (int)(w - cw) / (columnCount() - len);
            for (unsigned j = len; j < columnCount(); j++)
                columnPixelWidth() << residual;
        }
    } else
        columnPixelWidth() << (w / columnCount());
}
...
void MSPrintColumn::sortPrintItemsByRowColumn(void)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        minCol = item(i)->printColumn();
        if (minCol >= 0 && columns.indexOf(minCol) == columns.length())
            columns << minCol;
    }
    columns.sortUp();
...
void MSPrintColumn::computeConfiguration(void)
{
    ...
    if (columnCount() > 0) {
        ...
        if ((count % numColumns()) == 0)
            fixedRowCount(count / numColumns());
        else
            fixedRowCount(count / numColumns() + 1);
    }
}
    ...
    if (headingsOn() == MSTrue) {
        for (col = 0, i = 0; i < printItemList().count(); i += rowCount(col++)) {
            ...
            if (pItem->pageCount() == 0)
                heights[0] = heights(0) > h ? heights(0) : h;
        }
...
int MSPrintColumn::print(MSReport* report_, int x_, int y_, int, int, int bottomIndent_, int leftIndent_)
{
     ...
     */
        ...
        if (minHeight > remainingHeight || (currentPage() == 0 && (owner() == 0 && pageAlignment() & MSTop && y_ != report_->bodyTop()))) {
            _currentPage++;
            //       pageBreakRow(0);
        ...
        for (i = j = row = row_start, col = 0; i < printItemList().count(); i++, j++, row++) {
            ...
            if (row >= rowCount(col)) {
                ...
                row = j = row_start;
                y = y_start;
                if (i >= printItemList().count())
                    continue;
            }
            if (headingsOn() == MSTrue && row == 0)
                continue;
            if (y - bottomIndent_ >= report_->pageEnd()) {
                int x = x_ + columnPixel(col) + left;
                int w = columnPixelWidth(col);
                if (rowPageCounts()(row) == 0)
                    rowHeight = rowHeights()(row);
                else if (rowPageCounts()(row) == item(i)->currentPage())
                    rowHeight = rowHeights()(row);
                else
                    rowHeight = y - report_->bodyBottom(report_->pageCount()) - bottomIndent_;

                if (report_->printOnPage(*item(i), report_->pageCount(), report_->pageCountTotal()) == MSTrue && (pageBreakRow() < 0 || item(i)->pageBreakRow() >= 0) || row > pageBreakRow()) {
