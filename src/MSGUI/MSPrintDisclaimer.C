...
void MSPrintDisclaimer::computeSize(void)
{
    ...
    if (dout.fail() != ios::failbit) {
        ...
        MSMessageLog::warningMessage("Warning: unable to open %s\n", DefaultDisclaimer);
    char buf[buflen];
    if (fontName().length() == 0)
        font(DefaultDisclaimerFont);
    ...
    if (style() != NoDisclaimer) {
        ...
        if (orient == Portrait)
            width(MSPageSizeXTable[owner()->pageSize() - Letter] - leftPixel() - rightPixel());
        else
            width(MSPageSizeYTable[owner()->pageSize() - Letter] - leftPixel() - rightPixel());
        ...
        if (fdata != 0) {
            ...
            while (pin.eof() != ios::eofbit && ct < buflen) {
                ...
                if ((mark2 - mark1 - len) > 1) {
                    if (w == 0)
                        _residualSpace[_rowCount - 1] += _spaceWidth[_rowCount - 1];
                    else
                        w += (int)(spaceChar * (mark2 - mark1 - len - 1L));
                }
            ...
            if (style() == Box || style() == Rule)
                if (rowCount() > 2)
                    ct += 2;
                else if (style() == Toprule)
                    ct++;
        } else {
