...
MSBoolean MSPrintFont::initAfmfilesHashTable(const char* path_)
{
    if (_afmfilesHashTable == 0) {
        ...
        if (defaultFontPath() == 0) {
            static char buf[256];
            if (MSXGetFontDir(buf, 256))
                _defaultFontPath = buf;
            else
                _defaultFontPath = "C:\\MSTK\\Fonts";
            //check for empty string, and make sure we go to default
            //path in that case.
            if (path_ != 0 && path_[0] == '\0')
                path_ = 0;
        }
#endif
        MSString path(path_ != 0 ? path_ : defaultFontPath());
        if (path.last() != MS_DIR_SEPARATOR)
            path << MS_DIR_SEPARATOR;
        path << "afmfiles.upr";
        pin.open(path);
        ...
        while (line != "FontAFM" && pin.eof() != ios::eofbit)
            line = MSString::lineFrom(pin);
