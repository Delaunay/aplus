...
void MSPrintFontData::loadFont(void)
{
    ...
    if (strcmp(_pbuf, "StartFontMetrics") != 0) {
        showFontError();
        return;
    }
    while (strcmp(_pbuf, "FontName") != 0 && pin.eof() != ios::eofbit)
        pin >> _pbuf;
    ...
    pin.seekg(mark);
    while (strcmp(_pbuf, "IsFixedPitch") != 0 && pin.eof() != ios::eofbit)
        pin >> _pbuf;
    if (pin.eof() == ios::eofbit) {
        showFontError();
        return;
    ...
    pin.seekg(mark);
    while (strcmp(_pbuf, "UnderlinePosition") != 0 && pin.eof() != ios::eofbit)
        pin >> _pbuf;
    if (pin.eof() == ios::eofbit) {
        showFontError();
        return;
    ...
    pin.seekg(mark);
    while (strcmp(_pbuf, "UnderlineThickness") != 0 && pin.eof() != ios::eofbit)
        pin >> _pbuf;
    if (pin.eof() == ios::eofbit) {
        showFontError();
        return;
    ...
    pin.seekg(mark);
    while (strcmp(_pbuf, "FontBBox") != 0 && pin.eof() != ios::eofbit)
        pin >> _pbuf;
    if (pin.eof() == ios::eofbit) {
        showFontError();
        return;
    ...
    pin.seekg(mark);
    while (strcmp(_pbuf, "CapHeight") != 0 && pin.eof() != ios::eofbit)
        pin >> _pbuf;
    if (pin.eof() == ios::eofbit) {
        showFontError();
        return;
    ...
    pin >> _capHeight;
    pin.seekg(mark);
    while (strcmp(_pbuf, "XHeight") != 0 && pin.eof() != ios::eofbit)
        pin >> _pbuf;
    if (pin.eof() == ios::eofbit) {
        showFontError();
        return;
    ...
    pin >> _xHeight;
    pin.seekg(mark);
    while (strcmp(_pbuf, "Descender") != 0 && pin.eof() != ios::eofbit)
        pin >> _pbuf;
    if (pin.eof() == ios::eofbit) {
        showFontError();
        return;
    ...
    pin >> _descender;
    pin.seekg(mark);
    while (strcmp(_pbuf, "Ascender") != 0 && pin.eof() != ios::eofbit)
        pin >> _pbuf;
    if (pin.eof() == ios::eofbit) {
        showFontError();
        return;
    ...
    pin >> _ascender;
    pin.seekg(mark);
    while (strcmp(_pbuf, "StartCharMetrics") != 0 && pin.eof() != ios::eofbit)
        pin >> _pbuf;
    if (pin.eof() == ios::eofbit) {
        showFontError();
        return;
