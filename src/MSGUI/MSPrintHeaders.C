...
void MSPrintHeaders::removeAllHeaders(void)
{
    ...
    for (unsigned i = n - 1; i < n; i--) {
        ...
        if (header(i)->isDestroyable() == MSTrue)
            delete header(i);
    }
...
void MSPrintHeaders::removeAllFooters(void)
{
    ...
    for (unsigned i = n - 1; i < n; i--) {
        ...
        if (footer(i)->isDestroyable() == MSTrue)
            delete footer(i);
    }
...
MSBoolean MSPrintHeaders::removeHeader(MSPrintItem* item_)
{
    ...
    if ((ret = headerList().remove(item_)) == MSTrue && item_->isDestroyable() == MSTrue)
        delete item_;
    return ret;
}
...
{
    MSBoolean ret = MSFalse;
    if ((ret = footerList().remove(item_)) == MSTrue && item_->isDestroyable() == MSTrue)
        delete item_;
    return ret;
}
...
{
    for (unsigned i = 0; i < textList().count(); i++)
        if (tag_ == text(i)->tag())
            return *text(i);
