...
MSPrintManager* MSPrintManager::topLevel(void)
{
    ...
    while (pPrintBase->parent() != 0)
        pPrintBase = pPrintBase->parent();
    return pPrintBase;
}
...
void MSPrintManager::removeAll(void)
{
    ...
    for (unsigned i = n - 1; i < n; i--) {
        if (item(i)->isDestroyable() == MSTrue)
            delete item(i);
        else
            item(i)->printManager(0);
    }
...
MSBoolean MSPrintManager::removePrintItem(const MSSymbol& tag_)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (tag_ == printItem->printTag()) {
            if (printItem->isDestroyable() == MSTrue)
                delete printItem;
...
MSBoolean MSPrintManager::removePrintItem(MSPrintItem* item_)
{
    ...
    if (status == MSTrue && item_->isDestroyable() == MSTrue)
        delete item_;
    return status;
}
...
{
    if (i_ < paragraphList().count())
        return *((MSParagraph*)paragraphList().array(i_));
...
const MSPrintItem& MSPrintManager::printItem(const MSSymbol& tag_) const
{
    unsigned i, n = printItemList().count();
    for (i = 0; i < n; i++)
        if (tag_ == item(i)->printTag())
            return *item(i);
    n = childList().count();
    for (i = 0; i < n; i++) {
        MSPrintItem& aPrintItem = childList().array(i)->printItem(tag_);
        if (&aPrintItem != (MSPrintItem*)(&defaultParagraph()))
            return aPrintItem;
    }
    if (parent() == 0)
        MSMessageLog::warningMessage("Warning: paragraph \"%s\" not found\n", tag_.symbolName());
