...
{
    pout << "%%Trailer" << endl;
    if (cancelReportStatus() == MSTrue)
        pout << "%%Report generation terminated at " << pageCount() << endl;
    if (outputMode() == EPS)
        pout << "end" << endl
             << "%%EOF" << endl;
}
...
void MSReport::computePageSize()
{
    ...
    if (pageNumbering() == MSTrue && pageNumIncludeStatus() != MSTrue) {
        ...
        if (pageNumber().justification() & MSTop)
            pageNumberTop = h;
        else
            pageNumberBottom = h;
    }
...
void MSReport::printBox(unsigned long style_, double x_, double y_, double w_, double h_)
{
    if (style_ & Box) {
        ...
        setAttributes();
        if (BoxL & style_)
            printLine(x_, y_, x_, y_ - h_);
        if (BoxR & style_)
            printLine(x_ + w_, y_, x_ + w_, y_ - h_);
        if (BoxT & style_)
            printLine(x_, y_, x_ + w_, y_);
        if (BoxB & style_)
            printLine(x_, y_ - h_, x_ + w_, y_ - h_);
    }
}
...
{
    if ((style_ & MSLeft) && (style_ & MSRight))
        style_ -= MSRight;
    if ((style_ & MSTop) && (style_ & MSBottom))
        style_ -= MSBottom;
    if ((style_ & Underline) && (style_ & DUnderline))
        style_ -= Underline;
    return style_;
}
...
MSBoolean MSReport::printOnPage(MSPrintItem& item_, int pageCount_, int pageCountTotal_)
{
    ...
    if (item_.occurrence() & EveryPage)
        ret = MSTrue;
    else if (item_.occurrence() & OddPage && (pageCount_) % 2 == 0)
        ret = MSFalse;
    else if (item_.occurrence() & EvenPage && (pageCount_) % 2 == 1)
        ret = MSFalse;
    else if ((item_.occurrence() & IfNextPage && pageCount_ < pageCountTotal_) || (item_.occurrence() & FirstPage && pageCount_ == 1) || (item_.occurrence() & EvenPage && (pageCount_) % 2 == 0) || (item_.occurrence() & OddPage && (pageCount_) % 2 == 1) || (item_.occurrence() & AnyButFirstAndLast && (pageCount_ > 1 && pageCount_ < pageCountTotal_)) || (item_.occurrence() & LastPage && pageCountTotal_ > 1 && pageCount_ == pageCountTotal_))
        ret = MSTrue;
    return ret;
}
    ...
    for (unsigned i = 0, n = headerList().count(); i < n; i++) {
        if (printOnPage(*header(i), pageCount_, pageCountTotal()) == MSTrue) {
            if (header(i)->printFont().length() == 0)
                header(i)->printFont(defaultFont());
    ...
    for (unsigned i = 0, n = footerList().count(); i < n; i++) {
        if (printOnPage(*footer(i), pageCount_, pageCountTotal()) == MSTrue) {
            ...
            if (footer(i)->printFont().length() == 0)
                footer(i)->printFont(defaultFont());
...
void MSReport::printPageHeaders(void)
{
    ...
    for (unsigned i = 0; i < headerList().count(); i++) {
        if (printOnPage(*header(i), pageCount(), pageCountTotal()) == MSTrue) {
            if (pageNumIncludeStatus() == MSFalse)
                printHeaderStatus(MSTrue);
...
MSBoolean MSReport::insertPageNumString(MSStringVector& aStringVector_)
{
    ...
    for (unsigned i = 0; i < aStringVector_.length(); i++) {
        if (aStringVector_(i).indexOf("%p") < aStringVector_(i).length() || aStringVector_(i).indexOf("%t") < aStringVector_(i).length()) {
            int total = pageCountTotal();
            aStringVector_[i].change("%p", pageNumberString());
            if (pageNumbers().length() == 1)
                total += pageNumbers()(0) - 1;
            aStringVector_[i].change("%t", MSString(total));
            status = MSTrue;
        }
    }
    if (status == MSTrue)
        pageNumIncludeStatus(MSTrue);
    return status;
}
...
void MSReport::computePageNumber(void)
{
    ...
    if (pageNumbers().length() > 0) {
        if (pg < pageNumbers().length())
            pg = pageNumbers()(actual);
        else
            pg += pageNumbers().lastElement();
    } else {
        actual++;
        pg++;
    }
...
void MSReport::printPageNumber(void)
{
    if (pageNumbering() == MSTrue && pageNumIncludeStatus() != MSTrue) {
        ...
        if (pageNumber().text().length() == 0) {
            ...
            if (pageNumIncludeStatus() != MSTrue)
                pageNumber().outputText().lastElement() << pageNumberString();
        }
...
void MSReport::computeOutputSize(void)
{
    ...
    for (unsigned p = 0, passes = 2; p < passes; p++) {
        ...
        for (unsigned i = 0; i < printItemList().count(); i++) {
            ...
            if (pItem->printFont().length() == 0)
                pItem->printFont(defaultFont());
        ...
        if (p == 0 && conditionalPageSize() == MSTrue) {
            headerHeights().removeAll();
            footerHeights().removeAll();
            if (lastPageCnt != 0 && lastResidual == 0)
                _pageCount--;
            ...
            for (unsigned i = 0; i < printItemList().count(); i++) {
                ...
                if (pItem->currentPage() - 1 < pItem->pageCount()) {
                    ...
                    if (pageCount() < pageCountTotal()) {
                        if (pItem->residual() != 0 || pItem->pageCount() - pItem->currentPage() > 0)
                            i--;
                        _pageCount++;
                        pageEnd(bodyBottom(pageCount()));
