...
MSReportTable::~MSReportTable(void)
{
    ...
    for (i = n - 1; i < n; i--)
        delete columnList()->array(i);
    delete _columnList;
    n = hiddenColumnList()->count();
    for (i = n - 1; i < n; i--)
        delete hiddenColumnList()->array(i);
    delete _hiddenColumnList;
    if (groupHeadingList() != 0) {
        for (unsigned i = 0; i < groupHeadingList()->count(); i++)
            delete groupHeading(i);
        delete _groupHeadingList;
    }
    if (breakTextList() != 0)
        delete _breakTextList;
    if (_grandTotalBaseText != 0)
        delete _grandTotalBaseText;
    if (printManager() != 0)
...
const char* MSReportTable::formatOutput(MSString& buffer_, unsigned row_, unsigned col_)
{
    ...
    if (field != 0)
        field->formatOutput(buffer_, row_);
...
void MSReportTable::headingForeground(unsigned long fg_)
{
    if (headingForeground() != fg_) {
        ...
        for (i = 0; i < columnList()->count(); i++) {
            if (reportColumn(i)->headingForeground() == headingForeground())
                reportColumn(i)->headingForeground(fg_);
        }
        for (i = 0; i < hiddenColumnList()->count(); i++) {
            MSTableColumn* tc = hiddenColumnList()->array(i);
            if (tc->headingForeground() == headingForeground())
                tc->headingForeground(fg_);
        }
...
MSTableColumn* MSReportTable::reportColumn(const MSSymbol& tag_)
{
    unsigned i, n = columnList()->count();
    for (i = 0; i < n; i++)
        if (tag_ == reportColumn(i)->tag())
            return reportColumn(i);
    n = hiddenColumnList()->count();
    for (i = 0; i < n; i++)
        if (tag_ == hiddenColumnList()->array(i)->tag()) {
...
void MSReportTable::permuteColumns(const MSIndexVector& aIndexVector_)
{
    ...
    for (i = 0; i < iv.length(); i++)
        columnList()->assign(0, iv(i));
    for (i = 0; i < n; i++)
        if ((pBaseColumn = reportColumn(i)) != 0)
            hiddenColumnList()->add(pBaseColumn);
...
void MSReportTable::permuteColumns(const MSSymbolVector& aSymbolVector_)
{
    ...
    for (i = 0; i < k; i++) {
        if ((pBaseColumn = hiddenColumnList()->array(i)) != 0)
            newHiddenColumnList->add(pBaseColumn);
    }
    for (i = 0; i < n; i++) {
        if ((pBaseColumn = columnList()->array(i)) != 0)
            newHiddenColumnList->add(pBaseColumn);
    }
...
void MSReportTable::computeBreaks(void)
{
    ...
    for (unsigned i = 0; i < numRows() + 1; i++) {
        for (unsigned j = n; j <= n; j--) {
            MSTableColumn* field = reportColumn(j);
            if (field != 0 && field->breakOn() == MSTrue && ((i < field->numRows() && field->breakCriteria(i) == MSTrue) || i == field->numRows())) {
                breakIndex() << i;
                breakColumn() << j;
                if (field->breakTextList().count() >= field->breakIndex().length())
                    breakTextColumn() << j;
                if (field->pageBreakOn() == MSTrue)
                    pageBreakIndex() << i;
                field->breakIndex() << i;
    ...
    if (reportTotalOn() == MSTrue || reportGrandTotalOn() == MSTrue) {
        ...
        if (reportGrandTotalOn() == MSTrue) {
            ...
            MSString buffer;
            MSFloat::MSFloatFormat format = report()->asciiFloatFormat() != 0 ? report()->asciiFloatFormat() : MSFloat::Decimal0;
            aFloat.format(buffer, MSFormat(format));
            if (sv.length() > 0)
                sv[sv.length() - 1] << buffer;
            else
                sv << buffer;
            grandTotalBaseText() = sv;
            if (grandTotalBaseText().column() > numColumns())
                grandTotalBaseText().column(numColumns() - 1);
        }
    }
    breakIndex().sortUp();
        ...
        if (column != 0) {
            ...
            for (i = 0; i < count; i++) {
                ...
                if (col != 0 && col->model() != 0) {
                    ...
                    if (col->weights().length() > 0)
                        col->weights().permute(indexVector);
                }
            }
            for (i = 0; i < hcount; i++) {
...
{
    if (_breakTextList == 0)
        _breakTextList = new MSPointerArray<MSParagraph>;
...
{
    if (groupHeadingList() == 0)
        _groupHeadingList = new MSPointerArray<MSParagraph>;
    MSParagraph* printText = new MSParagraph(printText_);
    if (printText->fontName().length() == 0)
        printText->font(reportFont());
...
void MSReportTable::removeGroupHeading(const MSSymbol& tag_)
{
    ...
    for (unsigned i = 0; i < groupHeadingList()->count(); i++) {
        if (tag_ == groupHeading(i)->tag())
            printText = groupHeading(i);
    }
    if (printText != 0) {
        delete printText;
...
void MSReportTable::clearBreaks(void)
{
    ...
    for (unsigned int i = 0; i < columnList()->count(); i++) {
        if (reportColumn(i)->breakOn() != MSTrue)
            reportColumn(i)->breakString().removeAll();
...
int MSReportTable::print(MSReport* report_, int x_, int y_, int, int, int, int)
{
    ...
    if (remainingHeight <= 0 || (minHeight >= remainingHeight && pageBreakRow() < 0) || (pageBreakRow() < 0 && pageAlignment() & MSTop && y_ != report_->bodyTop(report_->pageCount()))) {
        _currentPage++;
        pageBreakRow(0);
        //Height(minHeight);
        _report = oldReport;
...
int MSReportTable::computePrintSize(MSReport* report_, int, int y_, int w_, int, int topMargins_, int margins_)
{
    ...
    if (numColumns() > 0) {
        ...
        if (remainingHeight <= 0 || minHeight >= remainingHeight || (pageAlignment() & MSTop && y_ != report_->bodyTop(report_->pageCount()))) {
            _residual = 1; //hack
            _pageCount++;
        ...
        if (pageCountTotal() > 1) {
            ...
            for (int rh, i = 0, j, k = 0; i < columns; i++) {
                for (rh = 0, j = 0; j < maxRows && k < rows; j++, k++)
                    rh += rowHeights()(k) + rowLeadings()(k);
            ...
            if (height > remainingHeight)
                height -= lastRowLeading;
            if (height + bottomPixel() < remainingHeight)
                height += bottomPixel();
        ...
        if ((field = reportColumn(j)) != 0) {
            ...
            for (unsigned b, p = 0, d = 0, k = 0, i = 0; i < rows; i++, d++) {
                ...
                while (k < breakIndexLength && d == breakIndex()(k) - 1) {
                    ...
                    if (breakTextField != 0 && b < breakTextField->breakTextList().count() && breakTextField->breakText(b)->style() & PosAbove) {
                        breakTextIndex() << i + 1;
                        breakText(breakTextField->breakText(b));
                        rowHeights()[i] = rowLeadings()[++i] = 0;
                    } else if (breakTextIndex().indexOf(i + 1) < breakTextIndex().length())
                        i++;
                    int h = 0, leading = 0;
...
void MSReportTable::computeRowColumnPartitions()
{
    ...
    if (columnPageCount > 0) {
        ...
        int estimatedPageSize = (int)((report()->bodyTopBase() - report()->bodyBottomBase()) / (report()->uniformScaling() == MSTrue ? x_printScale() : 1));
        int tableHeader = tableHeaderHeight();
        int tableFooter = tableFooterHeight();
        estimatedPageSize -= headingHeight() + tableTop() + tableBottom();
        estimatedPageSize -= (int)((tableHeader + tableFooter) / ((scaleHeaders() == MSFalse && report()->uniformScaling() == MSTrue) ? x_printScale() : 1));
        ...
        if (newPageTotal > report()->pageCountTotal())
            report()->pageCountTotal(newPageTotal);
        ...
        if (report()->uniformScaling() == MSTrue) {
            if (x_printScale() < y_printScale())
                y_printScale(x_printScale());
            else
                x_printScale(y_printScale());
        }
...
void MSReportTable::computeColumnPartitions(void)
{
    ...
    if (fixedFieldWidth() < partitionWidth) {
        ...
        for (actual = j = numFixedFields(); j < numColumns(); j++, actual++) {
            if (report()->cancelReportStatus() == MSTrue)
                return;
        ...
        MSMessageLog::errorMessage("Error: Fixed Field Width exceeds page width--print aborted!\n");
...
void MSReportTable::computeRowPartitions(double columnHeight_)
{
    ...
    for (actual = 0, k = 0, i = 0; i < rowCount(); i++, actual++) {
        ...
        if (height - rowLeadings()(i) > pageSize || (set > 0 && actual == set) || (column < newspaperColumns() - 1 && columnHeights(column) > columnHeight_ + heightOffset)) {
            if (set > 0) {
                if (set > actual) {
                    set = set < rowCount() ? set : rowCount();
                    for (i++, actual++; actual < set; i++, actual++)
                        height += rowHeights()(i) + rowLeadings()(i);
                    if (i < rowCount())
                        rowPageBreak() << i--;
...
int MSReportTable::tablePageSize(int column_)
{
    ...
    int ystart = column_ >= newspaperColumns() || report()->yPixel() <= tableBottomPosition(pc) ? tableTopPosition(pc) : report()->yPixel();
...
int MSReportTable::setTableX(int x_, int i_)
{
    ...
    if (tabStop != 0 && tabStop > x_ && (tabStop + cw) < tableRight()) {
        if (report()->tabStop(i_ + 2) != 0 && (tabStop + cw) > report()->tabStop(i_ + 2))
            tabStop = x;
    } else
        tabStop = x;
    return tabStop;
...
int MSReportTable::columnsPerPage(int page_)
{
    ...
    if ((span = columnPageSpan()) > 0) {
        ...
        if (page_ == span - 1)
            cols += (numColumns() - numFixedFields()) % span;
    } else if (columnControl().length() > 0) {
        if (page_ < columnControl().length())
            cols = columnControl()(page_);
        else
            cols = columnControl().lastElement();
    }
    return cols;
}
...
int MSReportTable::rowsPerPage(int page_)
{
    ...
    if ((span = rowPageSpan()) > 0) {
        ...
        if (page_ < rowControl().length())
            rows = rowControl()(page_);
        else
            rows = rowControl().lastElement();
...
void MSReportTable::computeTableHeaderSize(int pageCount_)
{
    ...
    for (unsigned i = 0, n = headerList().count(); i < n; i++) {
        if (report()->printOnPage(*header(i), pageCount_, pageCountTotal()) == MSTrue) {
            if (header(i)->printFont().length() == 0)
                header(i)->printFont(report()->defaultFont());
...
void MSReportTable::computeTableFooterSize(int pageCount_)
{
    ...
    for (unsigned i = 0, n = footerList().count(); i < n; i++) {
        if (report()->printOnPage(*footer(i), pageCount_, pageCountTotal()) == MSTrue) {
            ...
            if (footer(i)->printFont().length() == 0)
                footer(i)->printFont(report()->defaultFont());
...
void MSReportTable::computeBreakTextSize(MSParagraph& breakText_)
{
    ...
    for (unsigned i = 0; i < breakText_.column(); i++)
        x = setTableX(x, i);
    int frameThickness = frameLineWidth() + frameOffset();
...
int MSReportTable::printTableSegment(int x_, int y_)
{
    ...
    if (segmentTotal > 0) {
        ...
        if (x_printScale() >= 1 && tableWidth < pageWidth) {
            ...
            if (margin < 0)
                margin = 0;
            x = justification() & MSRight ? x + margin - frameRight : justification() & MSCenter ? x + margin / 2 : x;
        }
        ...
        for (unsigned i = 0; i < newspaperColumns() && segment() < segmentTotal; i++) {
            ...
            if (segment() % newspaperColumns() > 0)
                report()->translate(w + frameLeft, 0);
            if (numFixedFields() > 0) {
...
void MSReportTable::printTableHeadings(int cs_, int ce_)
{
    ...
    for (unsigned j = cs_; j < ce_; j++) {
        ...
        if (field != 0) {
            ...
            if (report()->gcValues().font == 0)
                report()->fontSize(report()->defaultFontSize());
...
void MSReportTable::printTableRows(int rs_, int cs_, int re_, int ce_)
{
    ...
    for (unsigned j = cs_; j < ce_; j++) {
        ...
        if (field != 0 && j < numColumns()) {
            ...
            for (t = breakTextCount(), i = rs_, k = breakCount(), d = rs_ - k - t; i < re_; i++, d++) {
                MSParagraph* text;
                while ((k < breakIndexLength && d == (breakIndex()(k)) || (t < breakTextIndexLength && i == breakTextIndex()(t))) && i < re_) {
    ...
    if (cs_ >= numFixedFields())
        breakCount(re_ != rowCount() ? k : 0);
    if (cs_ >= numFixedFields())
        breakTextCount(re_ != rowCount() ? t : 0);
}
...
void MSReportTable::printTableFrame(int x_, int y_, int w_, int h_)
{
    if (frameStyle() & (Box | BoxL | BoxR | BoxT | BoxB)) {
        ...
        if (frameStyle() & MSCenter && w1 < w && w1 > 0)
            w = w1;
                ...
                for (unsigned k = 0; k < headingRows; k++) {
                    ...
                    if (outputStyle() & Aligned) {
                        ...
                        if (styles & MSRight)
                            ...
                            headings[k] << cp << pad;
                        if (j < lastColumn)
                            headings[k] << delimiter();
                    } else
                        headings[k] << quotes << cp << quotes << delimiter();
                }
            }
        }
        ...
        for (j = 0; j < numColumns(); j++) {
            ...
            if (field != 0 && i < field->numRows()) {
                ...
                if (outputStyle() & Aligned)
                    pad = MSString::copy(" ", cw(j) - len);
                if (styles & MSRight)
                    report()->pout << pad << buffer;
                else
                    report()->pout << buffer << pad;
                if (j < lastColumn)
                    report()->pout << delimiter();
            }
        }
        report()->pout << endl;
    }
...
void MSReportTable::printCellBox(unsigned long mask_, int x_, int y_, int h_, int j_)
{
    ...
    w = w < cw ? cw : w;
    if (x_printScale() == 1 && (x_ + w) > tableRight())
        w = tableRight() - x_;
    if (report()->bgGrayScale() < 1.)
        report()->fillRectangle(x_, y_, w, h_);
...
void MSReportTable::sortGroupHeadingsByColumn(void)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        minCol = groupHeading(i)->column();
        if ((columns.length() == 0 || columns.indexOf(minCol) == columns.length()) && minCol >= 0)
            columns << minCol;
    }
    columns.sortUp();
    // sort by columnSpan
    ...
    for (i = 0; i < columns.length(); i++) {
        ...
        for (row = 0, j = 0; j < count; j++)
            if (groupHeading(j)->column() == column)
        ...
        for (j = 0; j < count; j++) {
            if ((row = groupHeading(j)->row()) == i) {
                if (groupHeading(j)->column() <= span) {
                    ...
                    span = groupHeading(j)->column() + groupHeading(j)->columnSpan() - 1;
            }
        }
    }
...
void MSReportTable::sortTextListByRow(MSPointerArray<MSParagraph>& list_)
{
    for (int i = 0; i < list_.count(); i++) {
        ...
        for (int j = i + 1; j < list_.count(); j++)
            if (list_.array(j)->row() < list_.array(min)->row())
                min = j;
        ...
        for (unsigned k = 0; k < groupHeadingList()->count(); k++) {
            ...
            if ((heading = groupHeading(k)) != 0 /*&&heading->outputText().length()>0*/) {
                ...
                for (int c = cs_; c < ce_; c++)
                    if (c >= column && c < span)
                        columnsPerRow[heading->row()] << c;

                if (column < numColumns() && ((column >= cs_ && column <= ce_) || span > cs_)) {
                    int index = (span < ce_ ? span : ce_) - 1;
                    int ystart = tableTopPosition(report()->pageCount()) - tableHeaderHeight() - groupHeadingHeight();
                    for (unsigned r = 0; r < heading->row() + 1; r++)
                        ystart += groupHeadingHeights()(r);
                    if (column > cs_) {
                    ...
                    for (int w = 0, n = 0, i = 0; i < heading->outputText().length(); i++) {
                        ...
                        for (; w < cw && n < heading->outputText()(i).length(); n++)
                            w += (int)fdata->textWidth(report()->fontSize(), cp + n, 1);
                        if (w > cw)
                            w -= (int)fdata->textWidth(report()->fontSize(), cp + (--n), 1);
...
void MSReportTable::printGroupHeadingBox(unsigned long mask_, int x_, int y_, int w_, int lh_, int rh_)
{
    ...
    if (report()->bgGrayScale() < 1.)
        report()->fillRectangle(x_, y_, w_, h);
    if (Box & mask_ && lh_ == rh_)
        report()->strokeRectangle(x_, y_, w_, h);
    else {
        report()->setAttributes();
        if (BoxL & mask_ || Box & mask_)
            report()->printLine(x_, y_, x_, y_ - lh_);
        if (BoxR & mask_ || Box & mask_)
            report()->printLine(x_ + w_, y_, x_ + w_, y_ - rh_);
        if (BoxT & mask_ || Box & mask_)
            report()->printLine(x_, y_, x_ + w_, y_);
        if (BoxB & mask_)
            report()->printLine(x_, y_ - h, x_ + w_, y_ - h);
    }
}
