...
MSRowColumnView::Editor::Editor(MSWidget* owner_)
    : MSTextField(owner_)
{
...
MSRowColumnView::Hsb::Hsb(MSWidget* owner_)
    : MSHScrollBar(owner_)
{
    inc(1);
    pageInc(1);
...
MSRowColumnView::Vsb::Vsb(MSWidget* owner_)
    : MSVScrollBar(owner_)
{
    inc(1);
    _highlightThickness = 0;
...
MSRowColumnView::Panner::Panner(MSWidget* owner_)
    : MSPrimitive(owner_)
{
    _shadowThickness = 2;
    _highlightThickness = 0;
...
MSRowColumnView::MSRowColumnView(MSWidget* owner_, const char* title_)
...
MSRowColumnView::MSRowColumnView(MSWidget* owner_, const MSStringVector& title_)
    : MSCompositeText(owner_)
{
    _label = 0;
    _label = new MSLabel(this, title_);
...
MSRowColumnView::~MSRowColumnView(void)
{
    ...
    removeAllCycles();
    if (dragRowWindow() != 0)
        XDestroyWindow(display(), dragRowWindow());
    if (dragRowCursor() != 0)
        delete _dragRowCursor;
    if (vsb() != 0)
        safeDestroy(_vsb);
    if (hsb() != 0)
        safeDestroy(_hsb);
...
void MSRowColumnView::removeCycle(MSColorCycle* cycle_)
{
    ...
    if (cycleList().length() == 0 && cycleTimer() != 0)
        cycleTimer()->stop();
}
...
void MSRowColumnView::updateSelectionMode(MSSelectionMode selectionMode_)
{
    ...
    if (selectionMode() != selectionMode_ && (selectionMode_ == MSMultiple || selectionMode_ == MSSingle)) {
    ...
    if (isProtected() == MSFalse) {
        if (editor()->mapped() == MSTrue)
            ...
            if (keyTranslate(key) == MSFalse) {
                ...
                keyPressNotify(editor(), e_, k_, s_, b_);
                if (editor()->length() > 0)
                    moveEditorToSelection(editor()->text());
            }
        }
    }
}
...
{
    if (sensitive() == MSTrue)
        MSCompositeText::buttonRelease(pEvent_);
}

void MSRowColumnView::defaultDoubleClickBehavior(const XEvent* pEvent_)
{
    if (hasCallback(MSWidgetCallback::doubleclick) == MSTrue)
        doubleClick();
    else
...
void MSRowColumnView::fillSelection(int from_, int to_)
{
    if (to_ >= from_) {
        ...
        int lr = lastRow();
        for (int i = fr; i <= lr; i++) {
            if (i >= from_ && i <= to_ && selected(i) == MSFalse)
                drawSelected(i);
        }
    }
}
...
void MSRowColumnView::unfillSelection(int from_, int to_)
{
    if (to_ >= from_) {
        ...
        int lr = lastRow();
        for (int i = fr; i <= lr; i++) {
            if (i >= from_ && i <= to_ && selected(i) == MSTrue)
                undrawSelected(i);
        }
    }
}
...
void MSRowColumnView::set(MSAttrValueList& avList_)
{
    ...
    for (unsigned i = 0; i < avList_.length(); i++) {
        if (avList_[i].attribute() == "rows") {
            ...
            if (avList_[i].value() == "MSMultiple")
                selectionMode(MSMultiple);
            else if (avList_[i].value() == "MSToggle")
                selectionMode(MSToggle);
            else
                selectionMode(MSSingle);
            ...
            if (avList_[i].value() == "MSBackground")
                cycleColorMode(MSBackground);
            else if (avList_[i].value() == "MSReversevideo")
                cycleColorMode(MSReverseVideo);
            else
                cycleColorMode(MSForeground);
            index << i;
...
MSAttrValueList& MSRowColumnView::get(MSAttrValueList& avList_)
{
    ...
    avList_ << MSAttrValue("selectionMode",
        (selectionMode() == MSMultiple) ? "MSMultiple" : (selectionMode() == MSToggle) ? "MSToggle" : "MSSingle",
...
int MSRowColumnView::rowFromEvent(const XEvent* pEvent_)
{
    ...
    if (ey > headingsHeight())
        return yToRow(ey - headingsHeight()) + firstRow();
    return -1;
}
...
void MSRowColumnView::dragRow(const XEvent* pEvent_)
{
    ...
    if (row != -1) {
        ...
        int newRow;
        if (oldY < pannerY + headingsHeight())
            newRow = firstRow();
