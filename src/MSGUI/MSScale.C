...
// DelayTimer
// #########################################################
MSScale::DelayTimer::DelayTimer(MSScale* s_, unsigned long ms_)
    : MSRegularTimer(ms_, 0)
{
    _scale = s_;
}
...
// RepeatTimer
// #########################################################
MSScale::RepeatTimer::RepeatTimer(MSScale* s_, unsigned long interval_)
...
// Editor
// #########################################################
MSScale::Editor::Editor(MSWidget* owner_)
    : MSTextField(owner_)
{
...
// ValueWin
// #########################################################
MSScale::ValueWin::ValueWin(MSWidget* owner_)
...
// Slider
// #########################################################
MSScale::Slider::Slider(MSWidget* owner_)
    : MSWidgetCommon(owner_)
{
...
MSScale::MSScale(MSWidget* owner_, const char* title_)
    : MSPrimitive(owner_, title_)
{
    init();
...
MSScale::MSScale(MSWidget* owner_, const MSStringVector& title_)
    : MSPrimitive(owner_, title_)
{
    init();
    _modelType = MSFloat::symbol();
...
MSScale::MSScale(MSWidget* owner_, MSFloat& model_, const char* title_)
    : MSPrimitive(owner_, title_)
{
    init();
    _modelType = MSFloat::symbol();
...
MSScale::MSScale(MSWidget* owner_, MSFloat& model_, const MSStringVector& title_)
    : MSPrimitive(owner_, title_)
{
    init();
    _modelType = MSFloat::symbol();
...
MSScale::MSScale(MSWidget* owner_, MSInt& model_, const char* title_)
    : MSPrimitive(owner_, title_)
{
    init();
...
MSScale::MSScale(MSWidget* owner_, MSInt& model_, const MSStringVector& title_)
    : MSPrimitive(owner_, title_)
{
    init();
    _modelType = MSInt::symbol();
...
void MSScale::editorActivate(void)
{
    if (editor()->mapped() == MSTrue) {
        if (validate((char*)editor()->string()) == MSTrue)
            editorEscape();
    }
}
...
MSBoolean MSScale::assignValue(double x_)
{
    if (MSView::model() != 0) {
        ...
        if (modelType() == MSFloat::symbol())
            *((MSFloat*)MSView::model()) = value;
        else if (modelType() == MSInt::symbol())
            *((MSInt*)MSView::model()) = (int)(value);
        return MSTrue;
    }
    return MSFalse;
}
...
double MSScale::currentValue(void)
{
    ...
    if (MSView::model() != 0) {
        if (modelType() == MSFloat::symbol())
            value = (double)*((MSFloat*)MSView::model());
        else if (modelType() == MSInt::symbol())
            value = (double)*((MSInt*)MSView::model());
    }
...
{
    if (event_->xbutton.button != Button1 && event_->xbutton.button != Button2)
        return;
    if (_delayTimer != 0) {
        _delayTimer->stop();
        _delayTimer = 0;
...
void MSScale::up(void)
{
    valueChange(valueInc());
    startTimedUpdate();
}
void MSScale::down(void)
{
    valueChange(-valueInc());
    startTimedUpdate();
}
void MSScale::left(void)
{
    valueChange(-valueInc());
    startTimedUpdate();
}
void MSScale::right(void)
{
    valueChange(valueInc());
    startTimedUpdate();
}
void MSScale::pageUp(void)
{
    valueChange(valuePageInc());
    startTimedUpdate();
}
void MSScale::pageDown(void)
{
    valueChange(-valuePageInc());
    startTimedUpdate();
}
...
void MSScale::drawTitles(void)
{
    ...
    if (title().maxLength() > 0) {
        ...
        for (int i = 0; i < title().length(); i++) {
            ...
            x = MSLeft & titleAlignment() ? x_org() : (MSRight & titleAlignment() ? x_end() - XTextWidth(fontInfo, pString, len) : (width() - XTextWidth(fontInfo, pString, len)) / 2);
    ...
    if (subtitle().maxLength() > 0) {
        ...
        for (int i = 0; i < subtitle().length(); i++) {
            ...
            x = MSLeft & subtitleAlignment() ? x_org() : (MSRight & subtitleAlignment() ? x_end() - XTextWidth(fontInfo, pString, len) : (width() - XTextWidth(fontInfo, pString, len)) / 2);
            XDrawString(display(), window(), gc, fontInfo, x, y, pString, len);
            y += fontInfo->ascent + fontInfo->descent;
        }
    }
...
{
    if (slider()->background() == oldbg_)
        slider()->background(background());
    if (valueWin()->background() == oldbg_)
        valueWin()->background(background());
...
{
    if (valueWin()->foreground() == oldfg_)
        valueWin()->foreground(foreground());
    if (labelForeground() == oldfg_)
        _labelForeground = foreground();
    if (subtitleForeground() == oldfg_)
        _subtitleForeground = foreground();
    if (mintitleForeground() == oldfg_)
        _mintitleForeground = foreground();
    if (maxtitleForeground() == oldfg_)
        _maxtitleForeground = foreground();
...
void MSScale::updateFont(Font oldfid_)
{
    ...
    if (fid != oldfid_) {
        if (oldfid_ == subtitleFont())
            _subtitleFont = fid;
        if (oldfid_ == mintitleFont())
            _mintitleFont = fid;
        if (oldfid_ == maxtitleFont())
            _maxtitleFont = fid;
        if (oldfid_ == labelFont())
            _labelFont = fid;
        if (oldfid_ == valueFont())
            valueWin()->font(fid);
        computeSize();
        redraw();
    }
...
void MSScale::set(MSAttrValueList& avList_)
{
    ...
    for (unsigned i = 0; i < avList_.length(); i++) {
        if (avList_[i].attribute() == "labelFont")
            labelFont(avList_[i].value()), index << i;
        else if (avList_[i].attribute() == "labelForeground")
            labelForeground(avList_[i].value()), index << i;
        else if (avList_[i].attribute() == "labelInc")
            labelInc(avList_[i].value().asDouble()), index << i;
        else if (avList_[i].attribute() == "labelAlignment")
            labelAlignment(MSAttrValue::stringToAlignment(avList_[i].value())), index << i;
        else if (avList_[i].attribute() == "valueMin")
            valueMin(avList_[i].value().asDouble()), index << i;
        else if (avList_[i].attribute() == "valueMax")
            valueMax(avList_[i].value().asDouble()), index << i;
        else if (avList_[i].attribute() == "valueInc")
            valueInc(avList_[i].value().asDouble()), index << i;
            ...
            valueAlignment(MSAttrValue::stringToAlignment(avList_[i].value())), index << i;
        else if (avList_[i].attribute() == "subtitle")
            subtitle(MSAttrValue::stringToStringVector(avList_[i].value())), index << i;
        else if (avList_[i].attribute() == "subtitleFont")
            subtitleFont(avList_[i].value()), index << i;
        else if (avList_[i].attribute() == "subtitleForeground")
            subtitleForeground(avList_[i].value()), index << i;
        else if (avList_[i].attribute() == "subtitleAlignment")
            subtitleAlignment(MSAttrValue::stringToAlignment(avList_[i].value())), index << i;
        else if (avList_[i].attribute() == "mintitle")
            mintitle(MSAttrValue::stringToStringVector(avList_[i].value())), index << i;
        else if (avList_[i].attribute() == "mintitleFont")
            mintitleFont(avList_[i].value()), index << i;
        else if (avList_[i].attribute() == "mintitleForeground")
            mintitleForeground(avList_[i].value()), index << i;
        else if (avList_[i].attribute() == "mintitleAlignment")
            mintitleAlignment(MSAttrValue::stringToAlignment(avList_[i].value())), index << i;
        else if (avList_[i].attribute() == "maxtitle")
            maxtitle(MSAttrValue::stringToStringVector(avList_[i].value())), index << i;
        else if (avList_[i].attribute() == "maxtitleFont")
            maxtitleFont(avList_[i].value()), index << i;
        else if (avList_[i].attribute() == "maxtitleForeground")
            maxtitleForeground(avList_[i].value()), index << i;
        else if (avList_[i].attribute() == "maxtitleAlignment")
            maxtitleAlignment(MSAttrValue::stringToAlignment(avList_[i].value())), index << i;
        else if (avList_[i].attribute() == "sliderBackground")
            sliderBackground(avList_[i].value()), index << i;
        else if (avList_[i].attribute() == "majorTickSize")
            majorTickSize(avList_[i].value().asInt()), index << i;
        else if (avList_[i].attribute() == "minorTickSize")
            minorTickSize(avList_[i].value().asInt()), index << i;
        else if (avList_[i].attribute() == "minorTickCount")
            minorTickCount(avList_[i].value().asInt()), index << i;
    }
...
{
    if (x_ > 0. && x_ <= (_valueMax - _valueMin))
        _valueInc = x_;
}
