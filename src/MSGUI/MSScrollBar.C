...
MSScrollBar::Elevator::Elevator(MSScrollBar* owner_)
...
MSScrollBar::PopupMenu::PopupMenu(MSScrollBar* scrollbar_)
    : MSPopupMenu(scrollbar_->server())
{
...
void MSScrollBar::PopupMenu::activate(void)
{
    ...
    if (_scrollbar != 0) {
        switch (choice) {
        case Openlook:
            _scrollbar->style(MSScrollBar::Openlook);
            break;
        case Motif:
            _scrollbar->style(MSScrollBar::Motif);
            break;
        case Windows:
            _scrollbar->style(MSScrollBar::Windows);
            break;
        case GotoTop:
            _scrollbar->value(_scrollbar->min());
            break;
        case GotoBottom:
...
{
    if (inc_ > 0 && inc_ <= (max() - min()))
        _inc = inc_;
}
void MSScrollBar::pageInc(int pInc_)
{
    if (pInc_ > 0 && pInc_ <= (max() - min()))
        _pageInc = pInc_;
}
...
{
    if (value_ == value() && value() <= max() - viewSize())
        return;

    if (value_ < min())
        _value = min();
    else if (value_ > max() - viewSize())
        _value = max() - viewSize();
    else
        _value = value_;
    redrawElevator();
...
void MSScrollBar::expose(const XEvent* pEvent_)
{
    if (mapped() == MSTrue && owner()->mapped() == MSTrue && frozen() == MSFalse) {
        ...
        if (pEvent_->xexpose.width == width() || pEvent_->xexpose.height == height()) {
            redraw();
        } else if ((pEvent_->xexpose.x >= sliderAreaRect().x()) && (pEvent_->xexpose.y >= sliderAreaRect().y()) && (pEvent_->xexpose.width <= sliderAreaRect().width()) && (pEvent_->xexpose.height <= sliderAreaRect().height())) {
...
void MSScrollBar::updateSliderValue(void)
{
    ...
    case Dec:
        ...
        break;
    case PageDown:
        changeAmount = pageInc();
        break;
    case PageUp:
        changeAmount = -pageInc();
        break;
    case Home:
        changeAmount = min() - value();
        break;
    case End:
        changeAmount = (max() - viewSize()) - value();
        break;
    case NoChange:
    case Drag:
    case ValueChange:
    ...
    if (changeType() == Inc || changeType() == PageDown) {
        if (tempValue > max() - viewSize())
            tempValue = max() - viewSize();
        if (tempValue <= value())
            return;
    ...
    if (tempValue != _savedValue) {
        ...
        if (changeType() == Inc || changeType() == Dec)
            adjustPointer();
...
void MSScrollBar::release(void)
{
    if (_elevator == 0)
        return;
    if (_elevator->_arrow1->selected() == MSTrue)
        _elevator->_arrow1->select(MSFalse);
    if (_elevator->_arrow2->selected() == MSTrue)
        _elevator->_arrow2->select(MSFalse);
    if (_delayTimer != 0) {
        _delayTimer->stop();
        _delayTimer = 0;
    }
...
{
    activateCallback(MSWidgetCallback::pagedecrement);
    change();
}
void MSScrollBar::pageUp(void)
{
    activateCallback(MSWidgetCallback::pageincrement);
    change();
}
void MSScrollBar::drag(void)
{
    activateCallback(MSWidgetCallback::drag);
    change();
}
void MSScrollBar::home(void)
{
    activateCallback(MSWidgetCallback::home);
    change();
}
void MSScrollBar::end(void)
{
    activateCallback(MSWidgetCallback::end);
    change();
}
...
void MSScrollBar::buttonPress(const XEvent* pXEvent_)
{
    if (isProtected() == MSFalse)
        buttonPressNotify(this, pXEvent_);
}

void MSScrollBar::buttonRelease(const XEvent* pXEvent_)
{
    if (isProtected() == MSFalse) {
        if (pXEvent_->xbutton.button == Button1 || pXEvent_->xbutton.button == Button2)
            release();
    }
}
...
void MSScrollBar::button1Press(const XEvent* pXEvent_)
{
    switch (style()) {
    case Openlook:
        openlookButton1Press(pXEvent_);
        break;
    case Windows:
    case Motif:
        motifButton1Press(pXEvent_);
        break;
    }
}
...
void MSScrollBar::button2Press(const XEvent* pXEvent_)
{
    switch (style()) {
    case Openlook:
        openlookButton2Press(pXEvent_);
        break;
    case Windows:
    case Motif:
        motifButton2Press(pXEvent_);
        break;
    }
}
