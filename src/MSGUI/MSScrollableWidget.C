...
MSScrollableWidget::Vsb::Vsb(MSWidget* owner_)
    : MSVScrollBar(owner_)
{
    inc(2);
...
MSScrollableWidget::Hsb::Hsb(MSWidget* owner_)
    : MSHScrollBar(owner_)
{
    inc(2);
    highlightThickness(0);
...
MSScrollableWidget::ViewPort::ViewPort(MSWidget* owner_)
...
MSScrollableWidget::DrawAreaWidget::DrawAreaWidget(MSWidget* owner_)
    : MSWidgetCommon(owner_)
{
    _shadowThickness = 0;
    _highlightThickness = 0;
...
MSScrollableWidget::MSScrollableWidget(MSWidget* owner_)
    : MSWidgetCommon(owner_)
{
    init();
}
...
void MSScrollableWidget::naturalSize(void)
{
    if (firstMap() == MSTrue) {
        ...
        if (dw <= 0 || dw > naturalWidth())
            w = naturalWidth() + margin;
        else
            w = dw + margin;
        if (dh <= 0 || dh > naturalHeight())
            h = naturalHeight() + margin;
        else
            h = dh + margin;
        resize(w, h);
    }
...
void MSScrollableWidget::configure(void)
{
    if (firstMap() == MSTrue) {
        ...
        if (vsb()->mapped())
            viewableWidth -= vsb()->width();
        int viewableHeight = height() - margin;
        if (hsb()->mapped())
            viewableHeight -= hsb()->height();
