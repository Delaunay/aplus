...
MSScrolledWindow::MSScrolledWindow(MSWidget* owner_)
    : MSManager(owner_)
{
    init();
}
...
MSScrolledWindow::Hsb::Hsb(MSWidget* owner_)
    : MSHScrollBar(owner_)
{
    inc(2);
...
MSScrolledWindow::Vsb::Vsb(MSWidget* owner_)
    : MSVScrollBar(owner_)
{
    inc(2);
    highlightThickness(0);
...
void MSScrolledWindow::visibilityObscured(void)
{
    visible(MSFalse);
    if (scrollWindow() != 0)
        visibilityObscuredNotify(scrollWindow());
}
...
void MSScrolledWindow::visibilityUnobscured(void)
{
    visible(MSTrue);
    if (scrollWindow() != 0)
        visibilityUnobscuredNotify(scrollWindow());
}
...
void MSScrolledWindow::updateBackground(unsigned long oldbg_)
{
    ...
    if (vsb()->background() == oldbg_)
        vsbBackground(background());
    if (hsb()->background() == oldbg_)
...
void MSScrolledWindow::set(MSAttrValueList& avList_)
{
    ...
    for (unsigned i = 0; i < avList_.length(); i++) {
        if (avList_[i].attribute() == "viewHeight")
            viewHeight(avList_[i].value().asInt()), index << i;
        else if (avList_[i].attribute() == "viewWidth")
            viewWidth(avList_[i].value().asInt()), index << i;
