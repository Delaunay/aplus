...
MSShadow::MSShadow(MSWidget* pWidget_)
    : _server(pWidget_->server())
    , _shadowColors(0)
{
    if (pWidget_ != 0)
        color(pWidget_->background());
}
...
{
    if (_server != 0)
        color(server_->defaultBackground());
}
...
void MSShadow::setMonochromeColors(void)
{
    if (_shadowColors->_bgColor.pixel == BlackPixelOfScreen(_server->screen())) {
        ...
        _shadowColors->_fgColor.red = _shadowColors->_fgColor.green = _shadowColors->_fgColor.blue = (unsigned short)MaxIntensity;

        _shadowColors->_bsColor.pixel = WhitePixelOfScreen(_server->screen());
        _shadowColors->_bsColor.red = _shadowColors->_bsColor.green = _shadowColors->_bsColor.blue = (unsigned short)MaxIntensity;

        _shadowColors->_selectColor.pixel = WhitePixelOfScreen(_server->screen());
        _shadowColors->_selectColor.red = _shadowColors->_selectColor.green = _shadowColors->_selectColor.blue = (unsigned short)MaxIntensity;

        _shadowColors->_tsColor.pixel = BlackPixelOfScreen(_server->screen());
        _shadowColors->_tsColor.red = _shadowColors->_tsColor.green = _shadowColors->_tsColor.blue = 0;
        ...
        _shadowColors->_fgColor.red = _shadowColors->_fgColor.green = _shadowColors->_fgColor.blue = 0;

        _shadowColors->_bsColor.pixel = BlackPixelOfScreen(_server->screen());
        _shadowColors->_bsColor.red = _shadowColors->_bsColor.green = _shadowColors->_bsColor.blue = 0;

        _shadowColors->_selectColor.pixel = BlackPixelOfScreen(_server->screen());
        _shadowColors->_selectColor.red = _shadowColors->_selectColor.green = _shadowColors->_selectColor.blue = 0;

        _shadowColors->_tsColor.pixel = WhitePixelOfScreen(_server->screen());
        _shadowColors->_tsColor.red = _shadowColors->_tsColor.green = _shadowColors->_tsColor.blue = (unsigned short)MaxIntensity;
    }
}
...
unsigned long MSShadow::brightness(void)
{
    ...
    unsigned long luminosity = (unsigned long)((RedLuminosity * (double)red) + (GreenLuminosity * (double)green) + (BlueLuminosity * (double)blue));
    ...
    return (unsigned long)(((intensity * IntensityFactor) + (light * LightFactor) + (luminosity * LuminosityFactor)) / 100);
}
...
{
    unsigned long b = brightness();
    if (b < ColorDarkThreshold)
        calculateColorsForDarkBackground();
    else if (b > ColorLiteThreshold)
