...
static const unsigned long MSShellEventMask = (FocusChangeMask | StructureNotifyMask | KeyPressMask | PropertyChangeMask);
#else
static const unsigned long MSShellEventMask = (LeaveWindowMask | EnterWindowMask | StructureNotifyMask | KeyPressMask | PropertyChangeMask);
#endif
...
{
    if (_created == MSFalse && pEvent_->xmap.window == _window)
        setOffsets();
}

void MSShell::enterNotify(const XEvent* pEvent_)
{
    if (pEvent_->xcrossing.mode == NotifyNormal)
        processFocusIn();
}
...
void MSShell::processFocusIn(void)
{
    if (_focusWidget != 0 && this != MSWidget::_focusWindow) {
        if (_focusWidget->sensitive() == MSTrue && _focusWidget->acceptFocus() == MSTrue) {
            ...
            focusWidget(0);
    } else if (_focusWidget != 0 && this == MSWidget::_focusWindow)
        focusIn();
...
void MSShell::configureNotify(const XEvent* pEvent_)
{
    if (pEvent_->xconfigure.window == _window && pEvent_->xconfigure.event == _window) {
        if (pEvent_->xconfigure.send_event == True) {
            if (_created == MSTrue) {
                if (pEvent_->xconfigure.x + pEvent_->xconfigure.width >= 0 && pEvent_->xconfigure.y + pEvent_->xconfigure.height >= 0 && pEvent_->xconfigure.x <= _server->width() && pEvent_->xconfigure.y <= _server->height()) {
    ...
    unsigned int state_, const char* pString_)
{
    ...
    if (keyTranslate(keyPress) == MSFalse) {
        if (keysym_ != XK_Tab && state_ == Mod1Mask && isprint(pString_[0])) {
            if (processAltKey(pEvent_, keysym_, state_, pString_) == MSTrue)
                return;
        }
        if (_focusWidget != 0) {
            if (keysym_ == XK_Tab) {
                ...
                if (state == ControlMask)
                    traverseToNextShell();
                else if (state == (ShiftMask + ControlMask))
                    traverseToPreviousShell();
...
MSBoolean MSShell::processFunctionKey(const XEvent*, KeySym keysym_, unsigned int, const char*)
{
    ...
    if (num >= 0 && num < 12) {
        ...
        while (pWidget != 0) {
            if (activateCallback(pWidget, syms[num]) == MSTrue)
                return MSTrue;
            else
                pWidget = pWidget->owner();
        }
    }
    return MSFalse;
}
...
void MSShell::propertyNotify(const XEvent* pEvent_)
{
    ...
    if (pEvent_->xproperty.atom == wmStateAtom)
        updateWMState();
    else {
...
{
    for (unsigned i = 0; i < _groupList.numberOfFollowers(); i++) {
        if (_groupList(i)->_remap == MSTrue)
            _groupList(i)->_pShell->map();
    }
}
...
MSBoolean MSShell::doesLeaderFollowThis(MSShell* leader_)
{
    ...
    if (leader_ != 0) {
        for (unsigned i = 0; i < _groupList.numberOfFollowers() && r == MSFalse; i++) {
            if (_groupList(i)->_pShell == leader_)
                r = MSTrue;
            else
                r = _groupList(i)->_pShell->doesLeaderFollowThis(leader_);
        }
    }
    return r;
}
...
void MSShell::defaultLeader(MSShell* defaultLeader_)
{
    ...
    if (oldDefault != 0 && oldDefault != defaultLeader_) {
        ...
        for (unsigned i = 0; i < oldList.numberOfFollowers(); i++) {
            if (_defaultLeader != 0)
                _defaultLeader->addFollower(oldList(i)->_pShell);
...
MSBoolean MSShell::traverseFocus(MSWidget* newFocusWidget_)
{
    ...
    if (newFocusWidget_ != this) {
        if (_nestedTraversal == MSFalse) {
            ...
            if (_focusWidget != 0 && _focusWidget != newFocusWidget_)
                lf = loseFocusNotify(_focusWidget);
            if (lf == MSTrue) {
...
void MSShell::removeFromFocusList(MSWidget* pWidget_)
{
    ...
    if (index < _traversalList.length())
        _traversalList.removeAt(index);
}
...
MSWidget* MSShell::getNextFocus(void)
{
    if (_traversalList.length() > 0) {
        ...
        for (i = index + 1; i < _traversalList.length(); i++) {
            ...
            if (isTraversable(pWidget) == MSTrue)
                return pWidget;
        }
        for (i = 0; i < index; i++) {
            pWidget = _traversalList(i);
...
void MSShell::setDefaultFocus(void)
{
    ...
    for (unsigned i = 0; i < _traversalList.length(); i++) {
        pWidget = _traversalList(i);
        if (pWidget != 0 && pWidget->sensitive() == MSTrue && pWidget->acceptFocus() == MSTrue)
            break;
        else
            pWidget = 0;
    }
...
void MSShell::traverseToNextShell(void)
{
    if (_shellList.length() > 0) {
        ...
        for (i = index + 1; i < _shellList.length(); i++) {
            ...
            if (pWidget != 0 && pWidget != this && pWidget->mapped() == MSTrue && server()->name() == pWidget->server()->name()) {
...
void MSShell::setBusyState(MSBoolean state_)
{
    if (state_ == MSTrue) {
        if (busyCount() == 0)
            XDefineCursor(display(), _window, _server->watchCursor());
        _busyCount++;
    } else {
        if (busyCount() == 1)
            XUndefineCursor(display(), _window);
        if (busyCount() > 0)
            _busyCount--;
    }
}
...
void MSShell::set(MSAttrValueList& avList_)
{
    ...
    for (unsigned i = 0; i < avList_.length(); i++) {
        if (avList_[i].attribute() == "x")
            moveTo(avList_[i].value().asInt(), y_origin()), index << i;
        else if (avList_[i].attribute() == "y")
            moveTo(x_origin(), avList_[i].value().asInt()), index << i;
        else if (avList_[i].attribute() == "width")
            width(avList_[i].value().asInt()), index << i;
        else if (avList_[i].attribute() == "height")
            height(avList_[i].value().asInt()), index << i;
    }
...
void MSShell::loadStateFrom(const MSWidgetState& widgetState_)
{
    ...
    for (cursor.setToFirst(); cursor.isValid() == MSTrue; cursor.setToNext()) {
        ...
        if (child->instanceName().length() > 0) {
            ...
            if ((list = (MSAttrValueList*)widgetState_.lookup(child->instanceFullname())) != (MSAttrValueList*)widgetState_.notFound()) {
                child->set(*list);
            }
        }
    }
