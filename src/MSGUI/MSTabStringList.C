...
MSTabStringList::MSTabStringList(MSWidget* owner_, const char* title_)
    : MSStringList(owner_, title_)
    , _tabSize(MSTabStringDefaultTabSize)
{
}
...
MSTabStringList::MSTabStringList(MSWidget* owner_, MSStringVector& model_, const char* title_)
    : MSStringList(owner_, model_, title_)
    , _tabSize(MSTabStringDefaultTabSize)
{
}

MSTabStringList::MSTabStringList(MSWidget* owner_, MSStringVector& model_, const MSStringVector& title_)
    : MSStringList(owner_, model_, title_)
    , _tabSize(MSTabStringDefaultTabSize)
{
}
        ...
        if (tabStops().length() == 0) {
            ...
            while (string.length() > 0) {
                ...
                if (nextTab) {
                    ...
                    if (tabWidth != 0)
                        position += tabWidth - (position % tabWidth);
                    string.remove(0, 1);
                }
            }
        } else {
    ...
    if (tabStops().length() == 0) {
        ...
        while (string.length() > 0) {
            ...
            if (nextTab) {
                ...
                if (tabWidth != 0)
                    x_ += tabWidth - (pixLength % tabWidth);
                pixLength = 0;
                string.remove(0, 1);
            }
        }
