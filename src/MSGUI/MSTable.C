...
public:
    MSTableColumnIterator(MSWidgetVector& vector_, MSTable::ColumnGroupList& tableGroupList_)
        : _vector(vector_)
        , _tableGroupList(tableGroupList_)
    {
    }
...
MSTable::MSTable(MSWidget* owner_, const char* title_)
    : MSReportTable(title_)
    , MSArrayView(owner_, title_)
{
    init();
}
...
MSTable::~MSTable(void)
{
    ...
    if (_choicesMenu != 0)
        safeDestroy(_choicesMenu);

    XDestroyWindow(display(), dragWindow());
    unsigned i, n = columnList()->count();
    for (i = n - 1; i < n; i--)
        safeDestroy(tableColumn(i));
    n = hiddenColumnList()->count();
    for (i = n - 1; i < n; i--)
        safeDestroy((MSTableColumn*)hiddenColumnList()->array(i));
}
...
int MSTable::selectedDataRow(void) const
{
    ...
    if (dataRow != -1) {
        ...
        if (isBreakRow == MSTrue)
            ...
            if (pTableColumn != 0) {
                if (dataRow >= pTableColumn->numRows())
                    dataRow = -1;
            } else
                dataRow = -1;
        }
...
{
    if (selectedDataRow_ == -1)
        selectedRow(-1);
    else {
        if (selectedColumn() == -1 && numColumns() > 0)
            _selectedColumn = 0;
        if (selectedColumn() != -1) {
            ...
            if (pTableColumn != 0) {
                if (pTableColumn->numRows() > selectedDataRow_) {
                    if (showBreaks() == MSTrue)
                        selectedRow(adjustPositionForBreaks(selectedDataRow_));
...
void MSTable::selectionDataVector(const MSIndexVector& selectionDataVector_)
{
    if (selectionMode() == MSMultiple) {
        if (showBreaks() == MSFalse && viewVector().length() == 0)
            selectionVector(selectionDataVector_);
        else {
            MSIndexVector viewSelectionVector;
            for (unsigned i = 0; i < selectionDataVector_.length(); i++) {
                if (showBreaks() == MSTrue)
                    viewSelectionVector << adjustPositionForBreaks(selectionDataVector_(i));
                else if (viewVector().length() > 0) {
                    int viewRow;
                    ...
                    viewSelectionVector << selectionDataVector_(i);
            }
...
const MSString& MSTable::virtualHelpString(int x_, int y_)
{
    ...
    if (x_ >= 0 && y_ >= 0) {
        y_ -= panner()->y_origin();
        x_ -= panner()->x_origin();
        col = (x_ < fixedColumnPixelWidth()) ? xToColumn(x_ - labelWidth()) : xToColumn(x_ - labelWidth()) + firstColumn();
...
void MSTable::insertChild(MSWidget* widget_)
{
    ...
    if (visible() != pTableColumn->visible()) {
        if (visible() == MSTrue)
            visibilityUnobscuredNotify(pTableColumn);
        else
            visibilityObscuredNotify(pTableColumn);
    }
}
...
void MSTable::removeChild(MSWidget* widget_)
{
    ...
    if (columnList()->remove(pTableColumn) == MSTrue) {
        ...
        hiddenColumnList()->remove(pTableColumn);
}
...
void MSTable::headingFont(Font fid_)
{
    if (headingFont() != fid_) {
        ...
        for (i = 0; i < n; i++)
            if (tableColumn(i)->headingFont() == headingFont())
                tableColumn(i)->headingFont(fid_);
        n = hiddenColumnList()->count();
        for (i = 0; i < n; i++) {
            MSTableColumn* tc = (MSTableColumn*)hiddenColumnList()->array(i);
            if (tc->headingFont() == headingFont())
                tc->headingFont(fid_);
        }
        _headingFont = fid_;
        unfreeze();
...
void MSTable::headingAlignment(unsigned long alignment_)
{
    if (headingAlignment() != alignment_) {
        ...
        unsigned i, n = numColumns();
        for (i = 0; i < n; i++)
            if (tableColumn(i)->headingAlignment() == headingAlignment())
                tableColumn(i)->headingAlignment(alignment_);
        n = hiddenColumnList()->count();
        for (i = 0; i < n; i++) {
            MSTableColumn* tc = (MSTableColumn*)hiddenColumnList()->array(i);
            if (tc->headingAlignment() == headingAlignment())
                tc->headingAlignment(alignment_);
        }
...
void MSTable::updateInternalState(void)
{
    ...
    if (n > 0)
        for (unsigned i = 0; i < n; i++)
            maxRowsSet(tableColumn(i)->numRows());
    unsigned after = numRows();
    if (selectionMode() == MSMultiple) {
        if (after >= before && after < vsb()->max())
            before = vsb()->max();
        if (after < before) {
            unsigned i, index;
...
void MSTable::columnChoiceActivate(const MSString& aString_)
{
    if (viewValidate(aString_.string(), selectedRow(), selectedColumn()) == MSTrue) {
        ...
        if (showBreaks() == MSTrue)
            drawRowColumn(selectedRow(), selectedColumn());
    }
}
...
int MSTable::columnPixelWidth(int column_)
{
    if (column_ < numColumns()) {
        ...
        if (pTableColumn != 0)
            return pTableColumn->columnPixelWidth();
        else
            return MSTableDefaultColumnWidth * charWidth('M') + (columnSpacing() << 1);
    } else
        return MSTableDefaultColumnWidth * charWidth('M') + (columnSpacing() << 1);
}
...
void MSTable::clearColumn(int column_)
{
    if (inColRange(column_) == MSTrue)
        clearRows(firstRow(), lastRow(), column_, column_);
}

void MSTable::drawFixedColumnHeadings(Window window_)
{
    if (fixedColumns() > 0)
        drawHeadings(window_, 0, fixedColumns() - 1);
}
...
void MSTable::updateColumnHeadings(void)
{
    if (mapped() == MSTrue && frozen() == MSFalse) {
        if (fixedColumns() > 0)
            drawColumnHeadings(panner()->window(), 0, fixedColumns() - 1);
    ...
    MSBoolean rowSelected_, MSBoolean cellSelected_, MSBoolean cellValid_)
{
    ...
    MSBoolean drawOptionMenu = MSBoolean(hasOptions && (style == ChoiceOptionMenuAlwaysDrawn || cellSelected_ == MSTrue));
...
void MSTable::columnUpdate(int column_)
{
    ...
    if (pTableColumn != 0) {
        ...
        if (editor()->mapped() == MSTrue && selectedColumn() == column_ && selectedRow() >= pTableColumn->numRows())
            unmapEditor();
        ...
        if (pTableColumn != 0) {
            ...
            if (row_ != -1) {
                if (showBreaks() == MSTrue) {
                    ...
                    if ((viewRow = viewVector().indexOf(row_)) == viewVector().length())
                        return;
                }
            }
            ...
            if (cycleList().length() != 0) {
                if (cycleTimer() == 0)
                    _cycleTimer = new MSArrayView::CycleTimer(this, cycleInterval());
                else
                    cycleTimer()->reset();
            }
...
void MSTable::updateTitle(void)
{
    ...
    if (fid != label()->font()) {
        ...
        if (hh != headingsHeight()) {
            ...
            drawHeadings(panner()->window());
    } else if (fg != label()->foreground())
        drawHeadings(panner()->window());
}
...
void MSTable::visibilityObscured(void)
{
    ...
    unsigned n = numColumns();
    for (unsigned i = 0; i < n; i++)
        visibilityObscuredNotify(tableColumn(i));
}
...
void MSTable::visibilityUnobscured(void)
{
    ...
    unsigned n = numColumns();
    for (unsigned i = 0; i < n; i++)
        visibilityUnobscuredNotify(tableColumn(i));
}
...
void MSTable::dataAreaSelection(const XEvent* pEvent_)
{
    ...
    // nor Control key is held down
    else if ((!(pEvent_->xbutton.state & ShiftMask) && !(pEvent_->xbutton.state & ControlMask)) && columnResize() == MSTrue && insideSeparator(pEvent_->xbutton.x, pEvent_->xbutton.y, col) == MSTrue && (tcol = tableColumn(col)) != 0 && tcol->resizable() == MSTrue) {
        ...
        if (pEvent_->xbutton.button == Button1) {
            ...
            if (isBreakRow == MSFalse) {
                ...
                if (inRange == MSTrue && hasOptions == MSTrue && col == oldSelectedColumn && row == oldSelectedRow && isViewValid(row, col) == MSTrue) {
                    showOptions(row, col);
                    return;
                }
            }
...
const MSStringVector& MSTable::cellOptions(int row_, int column_)
{
    ...
    MSTableColumn* pTableColumn = tableColumn(column_);
    if (pTableColumn != 0)
        return pTableColumn->cellChoices(row_);
    else
        return nullStringVector;
}
...
void MSTable::defaultButton3Behavior(const XEvent* pEvent_)
{
    const MSTableColumn* pTableColumn = tableColumn(selectedColumn());
    if (pTableColumn != 0) {
        if (pTableColumn->choices().length() > 0 && columnChoiceStyle(selectedColumn()) == ChoicePopupMenu) {
            if (_choicesMenu == 0)
...
unsigned MSTable::numRows(void) const
{
    ...
    if (showBreaks() == MSTrue)
        return MSReportTable::numRows() + breakIndex().length();
    else if (viewVector().length() > 0)
        return viewVector().length();
    else
        return MSReportTable::numRows();
}
...
MSWidgetVector MSTable::children(void)
{
    ...
    unsigned i;
    for (i = 0; i < n; i++)
        vector.append(columnList()->array(i));
    n = hiddenColumnList()->count();
    for (i = 0; i < n; i++)
        vector.append(hiddenColumnList()->array(i));
    return vector;
}
...
void MSTable::dragColumn(const XEvent* pEvent_, int column_)
{
    ...
    while ((keys & Button1Mask) || (keys & Button2Mask) || (keys & Button3Mask)) {
        if (sameScreen == True) {
            ...
            if (newx < left && firstColumn() != fixedColumns()) {
                if (newx + width > left || fixedColumns() == 0)
                    scrollLeft(1);
            } else if (newx + width > right && lastColumn() != numColumns() - 1)
                scrollRight(1);
        }
    ...
    int col = (finalX < (fixedColumnPixelWidth() + labelWidth())) ? xToColumn(finalX - labelWidth()) : xToColumn(finalX - labelWidth()) + firstColumn();
    ...
    if (col != column_ && (col != column_ - 1 || finalX < labelWidth())) {
        ...
        if (lastx > startx) //Insert right
        {
            if (col == numColumns() - 1 || lastx - offset > right)
                aIndexVector.append(column_);
            else
                aIndexVector.insertAt(col, column_);
        } else // Insert left
        {
            if (col == 0 && lastx - offset < rootx + labelWidth())
                aIndexVector.insertAt(0, column_);
            else
                aIndexVector.insertAt(col + 1, column_);
        }
        ...
        } else // Insert left
        {
            if (fixedColumns() > 0) {
                if (col == fixedColumns() - 1)
                    firstColumn(fixedColumns());
            } else if (col == 0 && lastx - offset < rootx)
                firstColumn(0);
        }
        if (showBreaks() == MSTrue)
            computeBreaks();
...
void MSTable::motionNotify(const XEvent* pEvent_)
{
    if (pEvent_->xbutton.subwindow == panner()->window()) {
        ...
        if (insideSeparator(ep->xbutton.x, ep->xbutton.y, col) == MSTrue) {
            ...
            if (tcol != 0 && tcol->resizable() == MSTrue)
                XDefineCursor(display(), panner()->window(), dragCursor()->cursor());
            else
                XUndefineCursor(display(), panner()->window());
        } else
            XUndefineCursor(display(), panner()->window());
...
void MSTable::resizeColumn(int column_, int startPos_)
{
    ...
    server()->ungrabPointer(window(), CurrentTime);
    int dummy;
    if (insideSeparator(lastX, iy, dummy) == MSTrue)
        XDefineCursor(display(), panner()->window(), dragCursor()->cursor());
    else
        XUndefineCursor(display(), panner()->window());
}
...
void MSTable::set(MSAttrValueList& avList_)
{
    ...
    for (unsigned i = 0; i < avList_.length(); i++) {
        if (avList_[i].attribute() == "headingFont") {
            ...
            if (avList_[i].value() == "ChoicePopupMenu")
                choiceStyle(ChoicePopupMenu);
            else if (avList_[i].value() == "ChoiceOptionMenu")
                choiceStyle(ChoiceOptionMenu);
            else
                choiceStyle(ChoiceOptionMenuAlwaysDrawn);
            index << i;
        }
    }
...
MSAttrValueList& MSTable::get(MSAttrValueList& avList_)
{
    ...
    if (choiceStyle() == ChoicePopupMenu)
        value = "ChoicePopupMenu";
    else if (choiceStyle() == ChoiceOptionMenu)
        value = "ChoiceOptionMenu";
    else
        value = "ChoiceOptionMenuAlwaysDrawn";
...
unsigned MSTable::getDataRow(unsigned viewRow_, MSBoolean& isBreakRow_) const
{
    if (showBreaks() == MSTrue) {
        ...
        if (index < breakIndex().length() && modelRow == breakIndex()(index))
            isBreakRow_ = MSTrue;
        else
            isBreakRow_ = MSFalse;
        return modelRow;
...
{
    if (viewVector().length() == 0)
        return viewRow_;
    else if (viewRow_ < viewVector().length())
        return viewVector()(viewRow_);
    else
        return viewVector().length();
}
        ...
        if (dynamicRecompute() == MSTrue) {
            ...
            if (row_ == -1) {
                ...
                for (i = 0; i < n && breakIndex()(i) <= row_; i++)
                    ;
                //	   cout<<row_<<"\t"<<i<<endl;
                if ((i + 1 == n) || (i + 1 < n && breakIndex()(i + 1) > row_))
                    tc->breakInvalid() << i;
                for (unsigned lastMatch = 0, match = 0, j = i + 1; j < n; j++) {
                    if (breakIndex()(j) == breakIndex()(j - 1))
                        match++;
                    else
                        match = 0;
                    if (match > lastMatch) {
                        if (tc->breakInvalid().indexOf(j) == tc->breakInvalid().length())
                            tc->breakInvalid() << j;
                        lastMatch = match;
                    }
                }
...
const char* MSTable::viewFormatOutput(MSString& buffer_, unsigned row_, unsigned column_)
{
    ...
    if (tc != 0) {
        ...
        if (isBreakRow == MSTrue) {
            ...
            if ((viewVector().length() != 0 && showBreaks() == MSFalse) || (tc->suppressDuplicate() == MSFalse || tc->isDuplicate(modelRow) == MSFalse))
...
MSBoolean MSTable::viewValidate(const char* string_, unsigned viewRow_, unsigned column_)
{
    ...
    if (pTableColumn != 0) {
        ...
        if (isBreakRow == MSTrue || modelRow >= pTableColumn->numRows())
            ...
            return validate(string_, modelRow, column_);
    } else
        return MSFalse;
}
...
MSBoolean MSTable::isViewProtected(unsigned viewRow_, unsigned column_)
{
    ...
    if (pTableColumn != 0) {
        ...
        if (isBreakRow == MSTrue || modelRow >= pTableColumn->numRows())
            ...
            return isCellProtected(modelRow, column_);
    } else
        return MSTrue;
}
    ...
    if (pTableColumn != 0) {
        ...
        if (isBreakRow == MSTrue) {
            ...
            if (dynamicRecompute() == MSTrue)
                ...
                if (len > 0 && pTableColumn->breakInvalid().indexOf(index) < len)
                    return MSFalse;
                else
                    return MSTrue;
            }
        } else if (modelRow < pTableColumn->numRows())
            return isValid(modelRow, column_);
        else
            return MSTrue;
    } else
        return MSTrue;
...
unsigned long MSTable::viewCellForeground(unsigned viewRow_, unsigned column_)
{
    ...
    if (pTableColumn != 0) {
        ...
        if (isBreakRow == MSTrue) {
            ...
            return cellForeground(modelRow, column_);
        else
            return foreground();
    }
    return foreground();
...
unsigned long MSTable::viewCellBackground(unsigned viewRow_, unsigned column_)
{
    ...
    if (pTableColumn != 0) {
        ...
        if (isBreakRow == MSTrue) {
            ...
            return cellBackground(modelRow, column_);
        else
            return background();
    } else
        return background();
...
Font MSTable::viewCellFont(unsigned viewRow_, unsigned column_)
{
    ...
    if (pTableColumn != 0) {
        ...
        if (isBreakRow == MSTrue || modelRow >= pTableColumn->numRows())
            ...
            return cellFont(modelRow, column_);
    } else
        return font();
}
...
MSAlignment MSTable::viewCellAlignment(unsigned viewRow_, unsigned column_)
{
    ...
    if (pTableColumn != 0) {
        ...
        if (isBreakRow == MSTrue || modelRow >= pTableColumn->numRows()) {
            ...
            return cellAlignment(modelRow, column_);
    } else
        return MSLeft;
}
...
const char* MSTable::formatOutput(MSString& buffer_, unsigned row_, unsigned column_)
{
    MSTableColumn* pTableColumn = tableColumn(column_);
    if (pTableColumn != 0)
        pTableColumn->formatOutput(buffer_, row_);
    return buffer_.string();
}
...
MSBoolean MSTable::validate(const char* pString_, unsigned row_, unsigned column_)
{
    ...
    if (pTableColumn != 0) {
        ...
        if (pTableColumn->validationCallback() != 0) {
            if (pTableColumn->validationCallback()->validate(aString) == MSFalse)
                return MSFalse;
        }
...
unsigned long MSTable::cellBackground(unsigned row_, unsigned column_)
{
    ...
    if (pTableColumn == 0) {
        ...
        return pTableColumn->cellBackground(row_);
}
    ...
    MSIntVector& heightVector_)
{
    ...
    for (unsigned i = 0; i < len; i++) {
        ...
        if (heightVector_.length() == i)
            heightVector_ << groupHeight;
        else
            heightVector_[i] = MSUtil::max(heightVector_(i), groupHeight);
    }
}
...
void MSTable::drawGroupHeading(Window window_, MSTableColumnGroup* group_, int cs_, int ce_, int level_)
{
    ...
    for (int i = 1; i <= level_; i++)
        y += groupHeadingsHeightVector()(i - 1);
    ...
    if (level_ == groupHeadingsHeightVector().length() - 1)
        h += (columnHeadingsHeight() - rowSpacing());
            ...
            if (groupHeadingsHeightVector()(i) > 0) {
                ...
                for (int col = cs_; col <= ce_; col++) {
                    ...
                    if (columnGroupList.length() > i) {
                        ...
                        int sepH = h;
                        if (i == lastRow)
                            sepH += (columnHeadingsHeight() - rowSpacing());
...
MSTableColumnGroup MSTable::columnGroups(void) const
{
    ...
    for (unsigned i = 0; i < nCols; i++) {
        ...
        unsigned j = 0;
        for (j = 0; j < len; j++) {
            if (g->isEmpty() == MSFalse && g->nodeList().lastElement().type() == MSTableColumnGroup::Node::Group) {
    ...
    if (group.nodeList().length() == 1 && group.nodeList()[0].type() == MSTableColumnGroup::Node::Group) {
    ...
    if (pTableColumn != 0) {
        if (row_ < numRows()) {
            ...
            if (options.length() > 0) {
                ...
                if ((selectedItem = options.indexOf(buffer)) == options.length())
                    selectedItem = 0;
                int rootx, rooty;
