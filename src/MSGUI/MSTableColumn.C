...
MSTableColumn::MSTableColumn(MSReportTable* owner_, const char* heading_, const MSSymbol& tag_)
    ...
    , _heading(heading_)
    , _tag(tag_)
    , MSWidgetView(owner_->displayServer())
{
    init();
}
...
void MSTableColumn::init(void)
{
    ...
    if (table() != 0) {
        ...
        if (font() != headingFont())
            headingFontStruct((XFontStruct*)server()->fontStruct(headingFont()));
        else
            headingFontStruct((XFontStruct*)fontStruct());
        createGCs();
        childCreateNotify();
    } else
        reportTable()->addColumn(this);
}
...
{
    if (table() != 0) {
        if (table()->foregroundColors().length() == 0)
            return foreground();
        else
            return table()->foregroundColors()(row_ % table()->foregroundColors().length());
    } else
        return foreground();
}
...
{
    if (table() != 0) {
        if (table()->backgroundColors().length() == 0)
            return background();
        else
            return table()->backgroundColors()(row_ % table()->backgroundColors().length());
    } else
        return background();
}
...
void MSTableColumn::updateData(void)
{
    if (table() != 0) {
        ...
        reportTable()->maxRowsSet(numRows());
}

void MSTableColumn::redraw(void)
{
    if (table() != 0 && frozen() == MSFalse)
        table()->drawColumn(column());
}
...
int MSTableColumn::headingWidth(void) const
{
    ...
    for (unsigned i = 0; i < n; i++) {
        ...
        tw = MSUtil::max(tw, XTextWidth((XFontStruct*)headingFontStruct(), (const char*)aString, aString.length()));
    }
    return tw;
}
...
int MSTableColumn::textWidth(const char* pString_) const
{
    if (_fontStruct->max_char_or_byte2 > 255) {
        return (pString_ != 0) ? XTextWidth16((XFontStruct*)_fontStruct, (XChar2b*)pString_, strlen(pString_) / 2) : 0;
    } else {
        return (pString_ != 0) ? XTextWidth((XFontStruct*)_fontStruct, (char*)pString_, strlen(pString_)) : 0;
    }
}
...
{
    if (_fontStruct->max_char_or_byte2 > 255) {
        return (pString_ != 0) ? XTextWidth16((XFontStruct*)_fontStruct, (XChar2b*)pString_, len_ / 2) : 0;
    } else {
...
{
    int r = charWidth();
    if (_fontStruct->per_char != 0 && aChar_ >= _fontStruct->min_char_or_byte2 && aChar_ <= _fontStruct->max_char_or_byte2) {
...
int MSTableColumn::headingCharWidth(char aChar_) const
{
    ...
    if (headingFontStruct()->per_char != 0 && aChar_ >= headingFontStruct()->min_char_or_byte2 && aChar_ <= headingFontStruct()->max_char_or_byte2) {
	...
	index<<i;
      ...
      */
            ...
            if (avList_[i].value().length() != 0)
                breakFg(avList_[i].value());
            index << i;
        } else if (avList_[i].attribute() == "breakBg") {
            if (avList_[i].value().length() != 0)
                breakBg(avList_[i].value());
            index << i;
            ...
            if (avList_[i].value() == "MSClipStars")
                clipMode(MSClipStars);
            else
                clipMode(MSNoClipping);
            index << i;
        } else if (avList_[i].attribute() == "columnAlignment") {
            if (avList_[i].value() == "MSRight")
                columnAlignment(MSRight);
            else if (avList_[i].value() == "MSLeft")
                columnAlignment(MSLeft);
            else
                columnAlignment(MSCenter);
            index << i;
...
void MSTableColumn::removeBreakText(const MSSymbol& tag_)
{
    ...
    for (unsigned i = 0; i < breakTextList().count(); i++)
        if (tag_ == breakText(i)->tag())
            printText = breakText(i);
    if (printText != 0) {
        delete printText;
