...
{
    if (node_.type() == MSTableColumnGroup::Node::Column)
        os_ << "Column Name: " << node_.column()->heading() << endl;
    else if (node_.type() == MSTableColumnGroup::Node::Group)
        os_ << *(node_.group()) << endl;
    else
        os_ << "Undefined Node Type" << endl;
    return os_;
}
...
{
    if (node_._group != 0)
        _group = new MSTableColumnGroup(*(node_._group));
}
...
{
    if (isOkToAdd(tableColumn_) == MSTrue)
        _nodeList << Node(tableColumn_);
    return *this;
}

MSTableColumnGroup& MSTableColumnGroup::operator<<(MSTableColumnGroup& tableColumnGroup_)
{
    if (isOkToAdd(tableColumnGroup_) == MSTrue)
        _nodeList << Node(tableColumnGroup_);
    return *this;
}
...
MSBoolean MSTableColumnGroup::isOkToAdd(MSTableColumnGroup& group_)
{
    ...
    for (unsigned i = 0; i < len; i++) {
        ...
        if (node.type() == Node::Column) {
            if (isOkToAdd(node.column()) == MSFalse)
                return MSFalse;
        } else if (node.type() == Node::Group) {
            if (isOkToAdd(*node.group()) == MSFalse)
                return MSFalse;
        }
    }
    return MSTrue;
        ...
        if (column_->table() == table()) {
            ...
            for (unsigned i = 0; i < len; i++) {
                ...
                if (node.type() == Node::Column) {
                    ...
                    if (node.group()->isOkToAdd(column_) == MSFalse)
                        return MSFalse;
                }
            }
            return MSTrue;
    ...
    ColumnGroupList& groupList_)
{
    ...
    for (unsigned i = 0; i < len; i++) {
        Node& node = nodeList()[i];
        if (node.type() == Node::Column) {
            if (iterator_.applyTo(node.column(), groupList_) == MSFalse)
                return MSFalse;
        } else if (node.type() == Node::Group) {
            if (node.group()->depthFirstNodeIteration(iterator_, groupList_) == MSFalse)
                return MSFalse;
        }
    }
...
{
    if (&group_ != this) {
        if ((_table == group_._table) && (_tag == group_._tag) && (_heading == group_._heading) && (_font == group_._font) && (_foreground == group_._foreground)) {
            return MSTrue;
        } else
            return MSFalse;
    }
...
{
    if (&group_ != this) {
        if ((_table == group_._table) && (_tag == group_._tag) && (_heading == group_._heading) && (_font == group_._font) && (_foreground == group_._foreground) && (_nodeList == group_._nodeList)) {
            return MSTrue;
        } else
            return MSFalse;
    }
