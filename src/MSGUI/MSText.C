...
static const unsigned long MSTextEventMask = (ExposureMask | ButtonPressMask | ButtonReleaseMask | Button1MotionMask);
...
MSText::Vsb::Vsb(MSWidget* owner_)
    : MSVScrollBar(owner_)
{
    inc(1);
...
MSText::Panner::Panner(MSWidget* owner_)
    : MSPrimitive(owner_)
{
    _shadowThickness = 2;
    _highlightThickness = 0;
...
MSText::MSText(MSWidget* owner_, const char* title_)
    : MSCompositeText(owner_)
{
    _label = 0;
...
MSText::MSText(MSWidget* owner_, const MSStringVector& title_)
    : MSCompositeText(owner_)
{
    _label = 0;
    _label = new MSLabel(this, title_);
...
unsigned MSText::lastLine(void)
{
    if (numLines() > 0) {
        ...
        while (result > 0 && pline->start() >= length())
            pline = line(--result);
        return result;
    }
    return 0;
    ...
    if (line_ < numLines()) {
        ...
        for (i = line_; i < numLines(); i++) {
            ...
            if (i == line_) {
                ...
                line(i)->dirty(MSTrue);
            } else if (lineChange == MSTrue)
                line(i)->dirty(MSTrue);
            else if (done == MSFalse && lineChange == MSFalse) {
                if (ep == line(i)->start() || sp == line(i)->end())
                    lineChange = MSTrue;
                else {
                    lineDelta = (line(i)->end() - line(i)->start()) - ((ep - sp));
                    if (lineDelta != 0 && tw < w)
        ...
        if (lineChange == MSTrue)
            resetVsb();
        lineStatus();
    } else
        MSMessageLog::warningMessage("MSText::resetFromInsert: line out of bounds");
}
...
void MSText::resetLinesFrom(unsigned start_)
{
    if (start_ < numLines()) {
        ...
        MSMessageLog::warningMessage("MSText::resetLinesFrom: line out of bounds");
}
...
void MSText::moveCursor(int row_, int col_)
{
    ...
    if (firstLine() + cursorY() >= maxNumLines())
        _cursorY = maxNumLines() - firstLine() - 1;
    _cursorX = col_;
    _cursorY = (cursorY() < numLines()) ? cursorY() : numLines() - 1;

    if (line(cursorY())->start() > length())
        _cursorY = lastLine();
    if (cursorX() >= lineLength(cursorY()))
        _cursorX = lineLength(cursorY()) - 1;
        ...
        if (pEvent_->xmotion.subwindow == panner()->window()) {
            ...
            if (row < numLines()) {
                if (line(row)->start() < length()) {
                    ...
                    if (end < startDrag())
                        selectRange(end, startDrag() - end);
                    else
                        selectRange(startDrag(), end - startDrag());
                }
            }
        } else {
            if (scrollTimer() == 0)
                _scrollTimer = new ScrollTimer(this);
                    ...
                    if (primaryIsOurs() == MSTrue) {
                        if (pos > selectionStart() && pos <= selectionEnd()) {
                            ...
                            if (pos <= selectionStart())
                                selectRange(selectionStart() + selected.length(), selectionLength());
                        }
                    } else {
                        moveCursor(row, col);
                        convertSelection();
...
void MSText::updateFont(Font oldfid_)
{
    ...
    if (cursorPixmap() != 0 && cursorPixmap()->height() != textHeight())
        makeIBeamCursor();
    if (frozen() == MSFalse) {
        panner()->clear();
...
void MSText::makeIBeamCursor(void)
{
    if (frozen() == MSFalse) {
        ...
        if (cursorHeight > height() - offset2)
            cursorHeight = height() - offset2;
        if (cursorWidth > width() - offset2)
            cursorWidth = width() - offset2;
...
void MSText::createGCs(void)
{
    if (imageMSGC() == 0) {
        ...
        imageMSGC()->color(foreground() ^ background(), background());
}
...
void MSText::kill(void)
{
    if (isProtected() == MSFalse && text().length() > 0) {
        ...
        if (len >= 0) {
            ...
            if (len == 0 && src[offset] == NewLineChar)
                len = 1;
            else if (len > 1) {
                unsigned index = len + offset - 1;
                if (index < text().length()) {
                    if (src[index] == NewLineChar)
                        len--;
                }
            }
...
{
    if (isProtected() == MSFalse && yankBuffer().length() > 0)
        insertString(yankBuffer().string());
}
...
void MSText::startOfLine(void)
{
    if (isProtected() == MSFalse)
        moveCursorX(0);
}
void MSText::endOfLine(void)
{
    if (isProtected() == MSFalse)
        moveCursorX(lineLength(cursorY()) - 1);
}
void MSText::up(void)
{
    if (isProtected() == MSFalse)
        moveCursorY(cursorY() - 1);
}
void MSText::down(void)
{
    if (isProtected() == MSFalse)
        moveCursorY(cursorY() + 1);
}
void MSText::deleteChar(void)
{
    if (isProtected() == MSFalse)
        deleteString(cursorPosition(), 1);
}
void MSText::backspace(void)
{
    if (isProtected() == MSFalse)
        deleteString(cursorPosition() - 1, 1);
}
void MSText::forwardChar(void)
{
    if (isProtected() == MSFalse)
        moveCursorX(cursorX() + 1);
}
void MSText::backwardChar(void)
{
    if (isProtected() == MSFalse)
        moveCursorX(cursorX() - 1);
}
void MSText::save(void)
{
    if (isProtected() == MSFalse)
        activateCallback(MSWidgetCallback::save);
}

void MSText::shiftTab(void)
{
    if (sensitive() == MSFalse)
        top()->traverseToPrevious();
}
void MSText::tab(void)
{
    if (sensitive() == MSTrue && isProtected() == MSFalse)
        insertString(MSTextTabString);
    else
        top()->traverseToNext();
}
...
void MSText::scrollUp(unsigned n_)
{
    if (n_ > 0 && n_ < numLines()) {
        ...
        firstLine(firstLine() + n_);
    else
        MSMessageLog::warningMessage("MSText::ScrollUp error: zero increment specified");
}
...
void MSText::scrollDown(unsigned n_)
{
    if (n_ > 0 && n_ < numLines()) {
        ...
        MSMessageLog::warningMessage("MSText::ScrollDown error: zero increment specified");
}
...
void MSText::resetLinePosition(unsigned line_, unsigned pos_)
{
    if (line_ < numLines()) {
        ...
        MSMessageLog::warningMessage("MSText::resetLinesPosition: line out of bounds");
}
...
void MSText::moveCursorY(int y_)
{
    ...
    if (y_ < 0 && firstLine() > 0) {
        ...
        if (y_ >= numLines()) {
            ...
            if (line(y_)->start() < length())
                moveCursor(y_, cursorX());
            else
                server()->bell();
        } else
            server()->bell();
        ...
        for (; r < el_;) {
            if (line(r)->dirty() == MSTrue) {
                ...
                if (dl > 0) {
                    ...
                    if (selectionLength() == 0 || (pos > selectionEnd()) || (pos + dl < selectionStart())) {
                        XDrawImageString(display(), panner()->window(), textGC(), textFontStruct(),
                            x, y + textAscent(), (char*)cp + pos, dl);
                    } else {
                        if (pos >= selectionStart()) {
...
{
    if (vsb()->value() < firstLine())
        scrollDown(firstLine() - vsb()->value());
    else if (vsb()->value() > firstLine())
        scrollUp(vsb()->value() - firstLine());
}
...
void MSText::set(MSAttrValueList& avList_)
{
    ...
    for (unsigned i = 0; i < avList_.length(); i++) {
        if (avList_[i].attribute() == "rows")
            rows(avList_[i].value().asInt()), index << i;
...
void MSText::selectRange(unsigned selectionStart_, unsigned selectionLength_)
{
    if (selectionStart() != selectionStart_ || selectionLength() != selectionLength_) {
        if ((length() == 0 && selectionStart_ == 0, selectionLength_ == 0) || (selectionStart_ < length() && selectionStart_ + selectionLength_ <= length())) {
