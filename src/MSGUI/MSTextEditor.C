...
static void Handler_CSI(ccf_Context cc, int Code)
{
    ...
    case 0x39: /* 9 */
        ...
        cc->i.csi.parameter[cc->i.csi.current] = cc->i.csi.parameter[cc->i.csi.current] * 10 + Code - 0x30;
        return; /* Continue CSI */
...
static void Handler_LoadGx(ccf_Context cc, int Code)
{
     ...
     */
        ...
        cc->bytes[save_Gn] = Code > MAX_MULTIPLE_BYTES ? MAX_MULTIPLE_BYTES : Code;
    ExitControl(cc);
}
...
static void Handler_LoadMGx(ccf_Context cc, int Code)
{
    ...
    if (Code == 0x29 || Code == 0x2A || Code == 0x2B || Code == 0x2D || Code == 0x2E || Code == 0x2F) {
        cc->saveGn = (ccf_Gs)(Code & 3);
        cc->saveS = (Code & 4) != 0;
    } else if (Code > 0x3F) {
        Code = (*cc->call_DG)(cc->client_data, save_Gn, save_S, MSTrue, Code);
        if (Code)
            cc->bytes[cc->saveGn] = Code > MAX_MULTIPLE_BYTES ? MAX_MULTIPLE_BYTES : Code;
        ExitControl(cc);
    }
}
...
static void Handler_GC_Switch(ccf_Context cc, int Code)
{
  ...
  */
     ...
     */
        ...
        cc->multiple_byte = cc->GC_multiple_byte = cc->bytes[cc->current->Gn];
    }
    Handler_GC(cc, Code);
}
...
//
#define FIELD_MASK(a, w) (unsigned(~0 << (a) ^ ~0 << ((a) + (w))))
#define FIELD_VALUE(a, v) (unsigned((v) << a))
//
// SnipContent
...
#define fontSelectInfo(m) \
    ((m.bits & Weight_MASK) | (m.bits & Italicized_MASK) | (m.bits & Font_MASK) | m.size_modification)

#define IsSameMode(m1, m2) \
    (m1.bits == m2.bits && m1.tag == m2.tag && m1.size_modification == m2.size_modification)
...
{
    if (mode._callback)
        delete mode._callback; // delete my copy
}
    ...
    int x, int y, int h)
{
    ...
    if (p->h != 0) {
        ...
        case MARK_Framed:
            ...
            p->y -= ascent;
            p->h = ascent + descent;
            if (my_r && XRectInRegion(my_r, p->x, p->y, width, p->h) == RectangleOut)
                break;
            /*
...
void MSTextEditorTypes::ExposeContext::changeForeground(MSTextEditorTypes::Snip* s, Drawable)
{
    ...
    if (s && (s->mode.bits & Foreground_MASK) && e->color(Foreground_COLOR(s->mode.bits) - 1) != FRAME_NO_PIXEL)
...
void MSTextEditorTypes::ExposeContext::changeBackground(MSTextEditorTypes::Snip* s, Drawable)
{
    ...
    if (s && (s->mode.bits & Background_MASK) && e->color(Background_COLOR(s->mode.bits) - 1) != FRAME_NO_PIXEL)
    ...
    unsigned int state_, const char* b_)
{
    ...
    if (!keysym_)
        return;
    if (keyTranslate(keyPress) == MSFalse && !(state_ & (ControlMask | Mod1Mask)) && strlen(b_) == 1 && b_[0]) {
...
#define fontCacheInfo(m, size) \
    (((m.bits & Weight_MASK) != 0) | (((m.bits & Italicized_MASK) != 0) << 1) | (size << 2))
...
void MSTextEditor::computeWidth(LayoutContext* cx, Snip* t)
{
    ...
    if (t->widget)
        insetSnipExtents(cx, t);
    else
        cx->editor->snipExtents(t);
    ...
    Snip* last, Snip* end)
{
    ...
    case JUSTIFIED:
        ...
        //
        if (text->spaces && wExtra > 0 && (end->quad || (IsLayoutContent(end->mode.bits) && end->next != NULL))) {
            Snip* t;
    ...
    LayoutContext* cx, SequenceState* item, Snip* last)
{
    ...
    if (cx->editor->_itemization->identifier_alignment == START)
        ...
        offset = cx->editor->_itemization->identifier_end_offset - item->wNow;
        //
        ...
        for (; start < end && (s = start->s) != last; start++) {
            ...
            if (IsEditableContent(s->mode.bits)) {
                if (!s->valid || start->x != s->x || start->y != s->y) {
                    changes++;
                    if (s->layout)
                        updateExposeArea(expose, start->x,
...
void MSTextEditor::snipExtents(MSTextEditorTypes::Snip* snip_)
{
    ...
    if (h->font == NULL) // chris, was 0 &&
    {
        ...
        hit = ((fontkey ^ (unsigned long)h->character_set) ^ (unsigned long)fId) % NumberOf(font_cache);
        for (i = hit;;) {
            if (font_cache[i].font == 0) {
                cache_miss++;
                break;
            } else if (font_cache[i].fontkey == fontkey && font_cache[i].fontlist == fId && font_cache[i].charset == h->character_set) {
                cache_hit++;
                break;
            }
            i += 1;
            if (i == NumberOf(font_cache))
                i = 0;
            if (i == hit) {
                //
    ...
    int font_nr, int, int, int)
{
    ...
    if (f != 0) {
        if (vsb() != 0)
            vsb()->inc(f->max_bounds.ascent + f->max_bounds.descent);
        if (hsb() != 0)
            hsb()->inc(f->max_bounds.width);
    }
...
void MSTextEditor::layout(int set_width, Snip* from, Region expose_region)
{
    ...
    while (*h) {
        ...
        if (cx.beginLine == NULL) {
            ...
            itemization = MSFalse;
            expect = expect_point;
            if (firstSeq && _itemization && _itemization->identifier_alignment != NOALIGNMENT)
        ...
        cx.lineLength = set_width - _indentation - cx.lineOffset - cx.rightWidgetIndent;
        do {
            //
    ...
    for (s = _first; s; s = s->next) {
        ...
        if (m) {
            ...
            if (m & (Background_MASK | ImageInversion_MASK))
                cx.changeBackground(s, d);
            if (m & (Foreground_MASK | ImageInversion_MASK))
                cx.changeForeground(s, d);
        ...
        if (!s->space && s->xWidth > 0 && s->data != NULL && (cx.my_r == NULL || XRectInRegion(cx.my_r, x, y - s->ascent, s->xWidth, s->ascent + s->descent) != RectangleOut)) {
        ...
        if (!s->floating)
            x += s->xWidth;
        if (s->hasEndLine() && (cx.p.bits & (Underline_MASK | CrossedOut_MASK | Framing_MASK)))
...
void MSTextEditor::drawCursor()
{
    _cursor->draw(gc_XOR(), panner()->window(), &_origin, _line_spacing,
        ((_cursor->location.snip && _cursor->location.snip->data) ? snipWidth(_cursor->location.snip, _cursor->location.snip->data, 1) : 9));
}
...
MSString MSTextEditor::extractSelectedContent()
{
    ...
    if (range[0] < range[1])
        for (Snip* s = dot[0].snip; s; s = s->next) {
            if (s->content.head && s->length > 0 && s->content.head->bytes * s->length > 0) {
...
#define IgnoreFormattedContent(cx) \
    (((cx)->_mode.bits & Content_MASK) == Content_FORMATTED && !((cx)->_lock.bits & Content_MASK))
...
static void Do_C0(MSTextEditorTypes::InsertContext* cx, int c)
{
    ...
    case CR:
        ...
        //
        t = cx->_last;
        if (t == NULL || t->endseq || t->mode.tag != cx->_mode.tag || ((t->mode.bits ^ cx->_mode.bits) & Content_MASK))
            t = cx->appendSnip();
        else
            t->valid = MSFalse;
        t->endseq = MSTextEditor::End;
    ...
    case LF:
        ...
        //
        t = cx->_last;
        if (t == NULL || t->hasEndLine() || t->space || t->widget || t->mode.tag != cx->_mode.tag || ((t->mode.bits ^ cx->_mode.bits) & Content_MASK))
            t = cx->appendSnip();
        else
            t->valid = MSFalse;
...
static void selectGraphicRendition(MSTextEditorTypes::InsertContext* cx, int r)
{
    ...
    case 0:
        ...
        //
        mask = Weight_MASK | Italicized_MASK | Underline_MASK | Blinking_MASK | ImageInversion_MASK | CrossedOut_MASK | Font_MASK | Foreground_MASK | Background_MASK | Framing_MASK | Overlined_MASK;
    ...
    Snip* after, int position, int mode)
{
    ...
    if (after == NULL)
        return beginContent(); /* Bad call, easy way out.. */
    cx = new InsertContext;
    if (!cx)
        return NULL;
    if (position < 0)
        position = 0;
    if (!after->hasEditableContents())
        return NULL;
    cx->_editor = this;
    cx->_list = &_first;
    cx->_last = after;
    ...
    if (n > 0 || (n == 0 && after->hasEndPosition())) {
        ...
        //
        if (position > 0 && s && (t = cx->beginAppend(position, after->content.head->character_set, after->content.head->bytes)) != NULL) {
    ...
    int n_chars_, char* cset_, int bytes_per_char_)
{
    ...
    if (_used + n > _size || (_head && _head->character_set != cset_) || fontinfo != _fontinfo) {
        flushAppend();
        if (_size < n) {
        ...
        _head->refs += 1;
    } else if ((t = _last) == NULL || t->content.head != _head || t->space || t->endseq || t->brk || !IsSameMode(t->mode, _mode)) {
        t = appendSnip();
        t->content.head = _head;
        t->data = &_head->s[_used];
        _head->refs += 1;
...
void MSTextEditorTypes::InsertContext::flushAppend()
{
    ...
    if (s && h && _used + HEAD_MIN_FUZZ < _size && (head = (SnipData*)realloc((char*)h, size)) != h) {
        //
...
void MSTextEditor::coordinates(MSTextEditorTypes::Snip* s, int offset, XPoint* xy)
{
    ...
    if (offset == 0 && (s->space || (s->endseq && s->length == 0))) {
        ...
        xy->x += snipWidth(s, s->data, offset);
    else if (s->xWidth == 0 || offset > s->length) {
        //
...
int MSTextEditor::snipWidth(MSTextEditorTypes::Snip* snip_, char* str_, int len_)
{
    ...
    if (h == NULL)
        return 0;
    if (h->font == NULL)
        snipExtents(snip_); // Force loading of Font
...
typedef struct Rendition {
    //  Rendition(unsigned int m, unsigned int value, int set):
    //    mask(m), value(value), set(set)
    //  {}

    Rendition(int m, int value, int set)
        : mask(m)
        , value(value)
        , set(set)
    {
...
MSBoolean MSTextEditor::saveInString(MSString& string_, MSBoolean textonly_)
{
    ...
    SnipMode* prev_mode = &init_mode;
    for (Snip* s = _first; s; s = s->next) {
        if (textonly_ == MSFalse)
            string_ << change_ISO6429(prev_mode, &s->mode);
        if (s->content.head && s->content.head->bytes * s->length > 0) {
...
void MSTextEditor::modifySelection(const XEvent* event, int mode)
{
    ...
    case SelectEnd:
        ...
        if (_regions[0].range[0] == position) {
            if (position != _cursor->position || _cursor->location.snip != dot.snip) {
...
MSTextEditorTypes::Vsb::Vsb(MSWidget* owner_)
    : MSVScrollBar(owner_)
{
    inc(20);
...
MSTextEditorTypes::Hsb::Hsb(MSWidget* owner_)
    : MSHScrollBar(owner_)
{
    inc(5);
    _highlightThickness = 0;
...
MSTextEditor::Panner::Panner(MSWidget* owner_)
    : MSPrimitive(owner_)
{
    _shadowThickness = 2;
...
void MSTextEditor::updateScrollbars(void)
{
    if (vsb()->width() > 1) {
        ...
        if (last_line_y < pHeight())
            last_line_y = pHeight() - 1;
...
void MSTextEditor::scrollbarsUpdated(void)
{
    ...
    if (_cursor->y - rh < _origin.y || _cursor->y - _origin.y > vsb()->pageInc()) {
        ...
        XPoint p;
        p.x = _cursor->x;
        if (_cursor->y - rh < _origin.y)
            p.y = _origin.y + rh + 5;
        else
            p.y = _origin.y + vsb()->pageInc() - rh;
...
void MSTextEditor::insert(const char* str_, long len_)
{
    ...
    if (!startEditing(CURRENT, 0))
        return;
    if (len_ > 0 && str_)
        _inserting->feedContent(str_, len_);
    endEditing(0);
}
...
void MSTextEditor::moveInsertionPoint(unsigned long location_)
{
    ...
    moveForwardChar();
    moveBackwardChar(); // hack for getting attributes right
...
const char* MSTextEditor::getInsertionPointCurrentForeground() const
{
    ...
    int fn = 0;
    if (s)
        fn = Foreground_COLOR(s->mode.bits);
    if (fn < 0 || fn > 9)
        fn = 0;
...
const char* MSTextEditor::getInsertionPointCurrentBackground() const
{
    ...
    int fn = 0;
    if (s)
        fn = Background_COLOR(s->mode.bits);
    if (fn < 0 || fn > 9)
        fn = 0;
...
static const char* GRSetFontTable[] = {
    MSTextEditor::GRSetFontDefault,
    MSTextEditor::GRSetFont1,
    MSTextEditor::GRSetFont2,
    MSTextEditor::GRSetFont3,
    MSTextEditor::GRSetFont4,
    MSTextEditor::GRSetFont5,
    MSTextEditor::GRSetFont6,
    MSTextEditor::GRSetFont7,
    MSTextEditor::GRSetFont8,
    MSTextEditor::GRSetFont9,
};
    ...
    DeleteOrKill mode)
{
    ...
    if (move == LINE_END && end == _cursor->position)
        end++;
...
MSTextEditorTypes::Snip* MSTextEditorTypes::InsertContext::endContent(Region expose)
{
    Snip* last = _last;
    if (expose && _expose)
        XUnionRegion(_expose, expose, expose);
    flushAppend();
    ...
    TextLocation* dot) // New Location after the scan
{
    ...
    case POSITION:
        ...
        //
        while (amount > 0 && s) {
            if (s->hasEditableContents() || s->hasEndLine()) // chris.
            {
                dot->snip = s;
        ...
        if (amount >= 0) {
            ...
            do {
                ...
                while (s) {
                    ...
                    if (s->space || (s->endseq && s->length < dot->offset))
                        break;
                    position -= dot->offset;
                    dot->offset = 0;
                }
    ...
    while (r && !end_line) {
        ...
        if (y >= goal->y) {
            if (x >= goal->x)
                ...
                while (++n <= r->length)
                    if (x + snipWidth(r, r->data, n) > goal->x)
                        break;
                //
...
MSTextEditorTypes::Snip* MSTextEditor::deleteContent(InsertContext* cx, long amount)
{
    ...
    if (amount > 0) {
        ...
        *h = t; // Link chain (t) to h //
        ...
        //
        if ((t = cx->_last) == NULL)
            return NULL; // No delete backwards from begin
        if (t->next)
            t->next->valid = MSFalse; // Hint for layout process
        for (;; t = t->previous()) {
            if (t->hasEditableContents()) {
                amount += (vlength = t->virtualLength());
                if (amount >= 0)
