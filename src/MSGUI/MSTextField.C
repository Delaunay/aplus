...
MSTextField::MSTextField(MSWidget* owner_)
...
void MSTextField::drawInsertCursor(void)
{
    if (cursorPixmap() != 0 && imageMSGC() != 0) {
        ...
        if (xx < offset) {
            ...
            xx = offset;
        } else if (xx > width() - offset)
            src_w = cursorPixmap()->width() - xx + width() - offset;
...
{
    if (masking() == MSTrue)
        position_ = _inputMask.indexOf(inputMaskCharacter(), position_);
    moveCursor(position_);
}
        ...
        if (newPosition_ >= length()) {
            ...
            if (newPosition_ > cursorPosition()) {
                ...
                if (scrollIndex() > 0 && cursorPosition() == scrollIndex()) {
                    ...
                    si = (si > 0) ? si : 0;
                } else if (scrollIndex() > 0 && newPosition_ < scrollIndex())
                    si = newPosition_;
            }
        }
...
{
    createGCs();
    if (width() == MSDefaultWidth && height() == MSDefaultHeight)
        computeSize();
    makeIBeamCursor();
}
...
void MSTextField::insertString(const char* pString_)
{
    if (pString_ != 0) {
        ...
        if (selectionStart() != selectionEnd()) {
            ...
            if (length() + slen <= maxLength())
                (void)_string.insert(pString_, cursorPosition());
            else {
                verifyBell();
                startBlinking();
                return;
            ...
            (void)_string.overlayWith(pString_, cursorPosition());
...
void MSTextField::deleteString(unsigned startIndex_, unsigned numChars_)
{
    if (startIndex_ < text().length()) {
        ...
        if (masking() == MSTrue) {
            ...
            _string.remove(startIndex_, numChars_);
...
void MSTextField::keyPress(const XEvent* pEvent_, KeySym keysym_, unsigned int state_, const char* pString_)
{
    ...
    if (sensitive() == MSTrue) {
        ...
        if (isprint(pString_[0]))
            insertString(pString_);
        else
            key(keysym_, state_, pString_);
    }
}
...
{
    if (masking() == MSFalse && yankBuffer().length() > 0)
        insertString(yankBuffer());
}
...
void MSTextField::backspace(void)
{
    ...
    } else if (cursorPosition() > firstCursorPosition())
...
void MSTextField::backwardChar(void)
{
    ...
    if (cursorPosition() > firstCursorPosition())
        cursorPositionBack(cursorPosition() - 1);
}
...
void MSTextField::color(unsigned long fg_, unsigned long bg_)
{
    if (fg_ != foreground() || bg_ != background()) {
        ...
        if (firstMap() == MSTrue && imageMSGC() != 0)
            imageMSGC()->color(fg_ ^ bg_, bg_);
        foreground(fg_);
        background(bg_);
    }
}
...
MSBoolean MSTextField::matchStringToMask(const MSString& aString_) const
{
    ...
    if (_inputMask.length() == aString_.length()) {
        ...
        for (i = 0; i < _inputMask.length(); i++) {
            if (_inputMask(i) != inputMaskCharacter() && _inputMask(i) != aString_(i))
                break;
        }
        if (i == _inputMask.length())
            match = MSTrue;
    }
    return match;
}
        ...
        while (keys & Button1Mask) {
            if (sameScreen == True) {
                if (winx <= (int)textX()) {
                    ...
                    pos = computeCursorPosition(winx);
                if (pos != lastPos) {
                    if (startPos < pos) {
