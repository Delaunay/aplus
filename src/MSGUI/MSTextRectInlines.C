...
INLINELINKAGE int MSTextRect::textWidth(const char* pString_) const
{
    if (doubleByte() == MSTrue)
        return (pString_ != 0) ? (int)XTextWidth16(_textFontStruct, (XChar2b*)pString_, strlen(pString_) / 2) : 0;
    else
        return (pString_ != 0) ? (int)XTextWidth(_textFontStruct, pString_, strlen(pString_)) : 0;
}
...
INLINELINKAGE int MSTextRect::textWidth(const char* pString_, int len_) const
{
    if (doubleByte() == MSTrue)
        ...
        return (pString_ != 0) ? (int)XTextWidth(_textFontStruct, pString_, len_) : 0;
}
