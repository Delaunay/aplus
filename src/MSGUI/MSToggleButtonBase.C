...
int MSToggleButtonBase::computePixmapXCoord(const MSPixmap* pmap_)
{
    ...
    int r;
    if (alignment() & MSLeft)
        r = offset + textHeight() + spacing();
    else if (alignment() & MSRight)
        r = width() - offset - pmap_->width();
    else {
...
int MSToggleButtonBase::computePixmapYCoord(const MSPixmap* pmap_)
{
    ...
    if (alignment() & MSTop)
        ...
        r = height() - offset - pmap_->height();
    else
        r = (height() - pmap_->height()) / 2;
    return r;
}
...
int MSToggleButtonBase::computeXCoord(int /*row_*/, int col_, const char* pString_, int len_)
{
    ...
    if (pString_ != 0) {
        if (alignment() & MSLeft)
            r = leftOffset + textWidth(pString_, col_);
        else if (alignment() & MSRight) {
    ...
    for (unsigned i = 0; i < avList_.length(); i++) {
        if (avList_[i].attribute() == "selectColor") {
            if (avList_[i].value().length() == 0)
                selectColor(selectShadowColor());
            else
                selectColor(avList_[i].value());
            index << i;
