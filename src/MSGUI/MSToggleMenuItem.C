...
const MSPixmap* MSToggleMenuItem::currentPixmap(void) const
{
    if (sensitive() == MSFalse) {
        return insensitivePixmap() != 0 ? insensitivePixmap() : 0;
    }

    if (showState() == ShowPixmap)
        return pixmap() != 0 ? pixmap() : 0;

    // pixmap is displayed on the left hand side, its value reflects menuItems state
    else {
        if (armed() == MSTrue)
            return (armedPixmap() != 0) ? armedPixmap() : 0;
        else if (showDisarmState() == MSTrue)
            return pixmap() != 0 ? pixmap() : 0;
        return 0;
    }
}
...
void MSToggleMenuItem::set(MSAttrValueList& avList_)
{
    ...
    for (unsigned i = 0; i < avList_.length(); i++) {
        if (avList_[i].attribute() == "selectColor")
            selectColor(avList_[i].value()), index << i;
