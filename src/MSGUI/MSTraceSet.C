...
MSUnsignedVector MSTraceSet::lineWidth(void) const
{
    ...
    for (int i = 0; i < traceList().count(); i++)
        vector << (unsigned)trace(i)->lineWidth();
    return vector;
}
MSUnsignedVector MSTraceSet::symbolSize(void) const
{
    MSUnsignedVector vector;
    for (int i = 0; i < traceList().count(); i++)
        vector << (unsigned)trace(i)->symbolSize();
    return vector;
}
...
MSUnsignedVector MSTraceSet::lineWeight(void) const
{
    MSUnsignedVector vector;
    for (int i = 0; i < traceList().count(); i++)
        vector << (unsigned)trace(i)->lineWeight();
    return vector;
}
...
void MSTraceSet::deleteTraces(void)
{
    ...
    for (int i = traceList().count(); i > 0; i--) {
        if ((pTrace = trace(i - 1)) != 0) {
            ...
            if (graph()->selectTrace() != 0 && pTrace == graph()->selectTrace())
                graph()->selectTrace(0);
...
void MSTraceSet::computeExtents(const MSIndexVector& iv_)
{
    ...
    if (lastDataCount() == 0 || dataCount() <= lastDataCount())
        append = MSFalse;
    if (append == MSTrue) {
        for (unsigned i = 0; i < length; i++) {
            ...
            if (row + 1 <= lastDataCount()) {
                ...
                double xx = x(row);
                if (row > 0 && overlap() != MSTrue && xx < x(row - 1))
                    overlap(MSTrue);
                ...
                if (xx > xMax())
                    xMax(xx);
                if (col == 0)
                    needNormalize = MSTrue;
            }
        }
        if (needNormalize == MSTrue)
...
unsigned long MSTraceSet::lineColor(unsigned i_) const
{
    ...
    if (len > 0)
        return lineColors()(i_ < len ? i_ : len - 1);
    else
        return len;
}
...
unsigned long MSTraceSet::fillColor(unsigned i_) const
{
    ...
    if (len > 0)
        return fillColors()(i_ < len ? i_ : len - 1);
    else
        return len;
}
...
unsigned long MSTraceSet::traceLineColor(int, int c_) const
{
    unsigned len = lineColors().length();
    if (len > 0)
        return lineColors()(c_ < len ? c_ : len - 1);
    else
        return len;
}
...
unsigned long MSTraceSet::traceFillColor(int r_, int c_) const
{
    ...
    if (len > 0)
        return fillColors()(index < len ? index : len - 1);
    else
        return len;
}
...
{
    for (unsigned i = 0; i < x_.length(); i++)
        if (x_(i) < 0.0 || x_(i) > 100.0) {
            MSMessageLog::warningMessage("Warning:  MSTraceSet::pieOffsets value out of range");
        }
    if (pieOffsets() == 0)
        _pieOffsets = new MSFloatVector(x_);
    else
        *pieOffsets() = x_;
    graph()->redrawImmediately();
}
...
double MSTraceSet::pieOffset(unsigned i_) const
{
    ...
    if (pieOffsets() != 0 && pieOffsets()->length() > 0)
        offset = (*pieOffsets())(i_ % pieOffsets()->length());
...
void MSTraceSet::pieProfiles(const MSFloatVector& x_)
{
    ...
    if (pieProfiles() == 0)
        _pieProfiles = new MSFloatVector(x_);
    else
        *pieProfiles() = x_;
    graph()->redrawImmediately();
}
...
{
    double profile = 1.0;
    if (pieProfiles() != 0 && pieProfiles()->length() > 0)
        profile = (*pieProfiles())(i_ % pieProfiles()->length());
...
void MSTraceSet::pieAngle(int x_)
{
    if (x_ != pieAngle()) {
        _pieAngle = x_ % 360;
        if (primarySliceAlignment() == MSNone)
            graph()->redrawImmediately();
    }
}
...
{
    for (int i = 0; i < traceList().count(); i++)
        trace(i)->textSymbol(MSStringVector(x_[i % x_.length()]));
    graph()->updateLegendStatus(MSTrue);
    graph()->redrawImmediately();
}
...
{
    for (int i = 0; i < traceList().count(); i++)
        trace(i)->stipple((Pixmap)x_[i % x_.length()]);
    graph()->updateLegendStatus(MSTrue);
    graph()->redrawImmediately();
}
...
{
    for (int i = 0; i < traceList().count(); i++)
        trace(i)->lineWidth(x_[i % x_.length()]);
    graph()->updateLegendStatus(MSTrue);
    graph()->redrawImmediately();
}
...
{
    for (int i = 0; i < traceList().count(); i++)
        trace(i)->lineWeight(x_[i % x_.length()]);
    graph()->updateLegendStatus(MSTrue);
    graph()->redrawImmediately();
}
...
{
    for (int i = 0; i < traceList().count(); i++)
        trace(i)->lineStyle(x_[i % x_.length()]);
    graph()->updateLegendStatus(MSTrue);
    graph()->redrawImmediately();
}
...
{
    for (int i = 0; i < traceList().count(); i++)
        trace(i)->xAxis(MSTop & x_[i % x_.length()] ? 1 : 0);
    graph()->redrawImmediately();
}
...
{
    for (int i = 0; i < traceList().count(); i++)
        trace(i)->yAxis(MSRight & x_[i % x_.length()] ? 1 : 0);
    graph()->redrawImmediately();
}
...
void MSTraceSet::style(const MSUnsignedLongVector& x_)
{
    int ct = traceList().count();
    if (ct > 0) {
        if (trace(0)->style() >= Close)
            style(x_(0));
        if (x_(0) >= Close)
            style(x_(0));
        else
            for (int i = 0; i < ct; i++)
                if (x_[i % x_.length()] < Close)
                    trace(i)->style(x_[i % x_.length()]);
    }
    graph()->legend()->redraw();
    graph()->redrawImmediately();
}
...
{
    for (int i = 0; i < traceList().count(); i++)
        trace(i)->symbolSize(x_[i % x_.length()]);
    graph()->updateLegendStatus(MSTrue);
    graph()->redrawImmediately();
}
...
void MSTraceSet::lineColor(const MSStringVector& x_)
{
    ...
    for (unsigned i = 0; i < x_.length(); i++)
        colors << owner()->server()->pixel(x_(i));
    lineColors() = colors;
    graph()->updateLegendStatus(MSTrue);
    graph()->redrawImmediately();
}
            ...
            for (int i = traceList().count() - 1; i > -1; i--) {
                if ((pTrace = trace(i)) != 0) {
                    if (pTrace->virtualCol() != designateCol) {
                        ...
                        if (style_ == Close)
                            pTrace->virtualCol(cols >= 5 ? 3 : cols == 4 ? 2 : 1);
                    }
                }
            }
            computeExtents();
...
void MSTraceSet::update(const MSIndexVector& iv_)
{
    ...
    if (iv_.length() == 0) {
        if (trace(0) == 0 || style >= Close)
            updateTraceStyle(style);
        else
            configureTraces();
    } else {
        computeExtents(iv_);
...
{
    if (format() != aFormat_) {
        if (format().formatType() == MSFormat::NoFormat || aFormat_.formatType() == format().formatType()) {
            _format = aFormat_;
            graph()->redrawImmediately();
        }
    }
...
static const char* traceStyleNames = "MSNone\nMSG::Area\nMSG::Bar\nMSG::Candle\nMSG::Close\nMSG::ColorProfile\nMSG::Fill\nMSG::HL\n\
...
static const char* symbolNames = "MSNone\nMSG::Circle\nMSG::Circle|MSG::Fill\nMSG::Cross\nMSG::Diamond\nMSG::Diamond|MSG::Fill\nMSG::Square\n\
MSG::Square|MSG::Fill\nMSG::Triangle\nMSG::Triangle|MSG::Fill\nMSG::X\nMSG::Cross|MSG::X";

static int symbolValues[] = { MSNone, MSG::Circle, MSG::Circle | MSG::Fill, MSG::Cross, MSG::Diamond, MSG::Diamond | MSG::Fill, MSG::Square,
    ...
    for (unsigned i = 0; i < avList_.length(); i++) {
        if (avList_[i].attribute() == "legend") {
            ...
            if (ct > 0) {
                ...
                if (index != aStyleVector.length())
                    style(traceStyleValues[index]);
            }
            index << i;
            ...
            if (ct > 0) {
                ...
                if (index != aSymbolVector.length())
                    symbol(symbolValues[index]);
            }
            index << i;
            ...
            if (avList_[i].value() == "MSG::HoldX")
                constraint(HoldX);
            else if (avList_[i].value() == "MSG::HoldY")
                constraint(HoldY);
            else
                constraint(0);
            index << i;
            ...
            if (avList_[i].value().length() == 0)
                tag(MSSymbol::nullSymbol());
            else
                tag(MSSymbol(avList_[i].value()));
            index << i;
        }
    }
...
MSAttrValueList& MSTraceSet::get(MSAttrValueList& avList_)
{
    ...
    aSize = 2;
    if (ct > 0)
        aSize = ((const MSTraceSet*)this)->lineWeight(0);
    avList_ << MSAttrValue("lineWeight", MSString(aSize));

    aSize = 1;
    if (ct > 0)
        aSize = ((const MSTraceSet*)this)->lineWidth(0);
    avList_ << MSAttrValue("lineWidth", MSString(aSize));

    aSize = MSSolid;
    if (ct > 0)
        aSize = ((const MSTraceSet*)this)->lineStyle(0);
    ...
    if (ct > 0)
        traceStyle = ((const MSTraceSet*)this)->style(0);
    MSStringVector aStyleVector(traceStyleNames);
    ...
    if (ct > 0)
        aSymbol = ((const MSTraceSet*)this)->symbol(0);
    MSStringVector aSymbolVector(symbolNames);
    aString = "";
    ...
    if (ct > 0)
        aSize = ((const MSTraceSet*)this)->symbolSize(0);
