...
// Editor
template <class Element>
MSTreeListView<Element>::Editor::Editor(MSWidget* owner_)
    : MSTextField(owner_)
{
    _highlightThickness = 0;
...
{
    if (_collapsedButtonPixmap != 0)
        delete _collapsedButtonPixmap;
    if (_expandedButtonPixmap != 0)
        delete _expandedButtonPixmap;
    if (vsb() != 0)
        safeDestroy(vsb());
    if (hsb() != 0)
        safeDestroy(hsb());
    if (clipWindow() != 0)
        safeDestroy(clipWindow());
    if (redrawPixmap() != 0)
        delete redrawPixmap();
    if (stipplePixmap() != 0)
        delete stipplePixmap();
    if (dotPixmap() != 0)
        delete dotPixmap();
    if (dashPixmap() != 0)
        delete dashPixmap();
...
void MSTreeListView<Element>::collapseSubTree(ScreenVectorCursor& cursor_, unsigned long pos_)
{
    if (frozen() == MSFalse) {
        ResourceTreeCursor& resourceCursor = screenVector().elementAt(cursor_);
        ManagedInfo& node = resourceTree().elementAt(resourceCursor);
        if (_selectedNode != 0 && node->modelCursor() != *_selectedNode && locateSelectedNode(resourceCursor) == MSTrue)
            resetSelectedNode();
...
void MSTreeListView<Element>::updateScreenVector(ScreenVectorCursor& cursor_, ResourceTreeCursor resourceCursor_)
{
    for (resourceCursor_.setToFirstExistingChild(); resourceCursor_.isValid(); resourceCursor_.setToNextExistingChild()) {
        if (screenVector().isEmpty()) {
            ...
            screenVector().addAsNext(resourceCursor_, cursor_);
        ManagedInfo& node = resourceTree().elementAt(resourceCursor_);
        if (isExpanded(node) == MSTrue)
            updateScreenVector(cursor_, resourceCursor_);
    }
}
...
void MSTreeListView<Element>::buildScreenVector(void)
{
    ...
    if (resourceTree().isEmpty() == False) {
        ...
        if (isExpanded(info) == MSTrue)
            updateScreenVector(screenCursor, resourceCursor);
    }
}
        ...
        for (modelCursor.setToFirstExistingChild(); modelCursor.isValid(); modelCursor.setToNextExistingChild()) {
            ...
            if (parentExpanded_) {
                if (screenVector().isEmpty()) {
                    ...
                    screenVector().addAsNext(newResourceCursor, screenCursor_);
            }
            MSBoolean expanded = MSFalse;
            if (parentExpanded_ == MSTrue && isExpanded(newNodeInfo) == MSTrue)
                expanded = MSTrue;
            updateResourceTreeAndScreenVector(newResourceCursor, screenCursor_, expanded, iterator_);
        }
    } else
        reportError("Warning: Invalid Cursor in MSTreeListView::updateResourceTreeAndScreenVector()");
}
...
void MSTreeListView<Element>::setNodeAttributes(MSTreeListView<Element>::Iterator& iterator_)
{
    ...
    if (resourceTree().isEmpty() == True)
        buildResourceTreeAndScreenVector(&iterator_);
    else {
...
MSTreeListView<Element>::TreeModelCursor MSTreeListView<Element>::firstNode(void) const
{
    if (MSView::model() != 0 && numRows() > 0 && firstRow() < numRows()) {
        ...
        return TreeModelCursor(TreeModel());
}
...
MSTreeListView<Element>::TreeModelCursor MSTreeListView<Element>::selectedNode(void) const
{
    if (MSView::model() != 0) {
        if (_selectedNode == 0)
            return TreeModelCursor(modelTree());
        else
        ...
        if (info->expandedState() != expandedState_) {
            ...
            if (info->expandable() == MSTrue) {
                if (firstMap() == MSTrue) {
                    ...
                    ScreenVectorCursor screenCursor = findScreenCursor(resourceCursor, pos);
                    if (screenCursor.isValid() && isRowOnScreen(pos - 1)) {
                        if (info->expandedState() == MSTrue)
                            expandSubTree(screenCursor, pos);
                        else
                            collapseSubTree(screenCursor, pos);
                    }
                }
            }
        }
    ...
    if (resourceCursor.isValid()) {
        ...
        if (info->expandable() != expandable_) {
            ...
            if (firstMap() == MSTrue) {
                ...
                ScreenVectorCursor screenCursor = findScreenCursor(resourceCursor, pos);
                if (screenCursor.isValid() && isRowOnScreen(pos - 1)) {
                    if (info->expandable() == MSTrue && info->expandedState() == MSTrue)
                        expandSubTree(screenCursor, pos);
                    else
                        updateRow(info, pos - 1);
                }
...
const MSPixmap* MSTreeListView<Element>::pixmap(const MSString& key_) const
{
    ...
    if (cursor.isValid())
        return &_pixmapRegistry.elementAt(cursor);
    else
        return 0;
}
...
void MSTreeListView<Element>::drawRow(Window window_, const ManagedInfo& node_, int x_, int y_)
{
    ...
    if (drawLine) {
        ...
        if (node_->isLastChild() == MSTrue)
            XDrawLine(display(), window_, linegc().gc(), x, y_, x, midY);
        else
            XDrawLine(display(), window_, linegc().gc(), x, y_, x, fullY);
        NodeInfo* parent = node_->parent();
        for (int i = node_->level() - 2; i >= 0; i--) {
            x -= loffset;
            if (parent->isLastChild() == MSFalse)
                XDrawLine(display(), window_, linegc().gc(), x, y_, x, fullY);
    ...
    if (showButtons() == MSTrue) {
        ...
        } else if (showRootNode() == MSFalse || level > 0)
            x += bWidth + rowHorizontalSpacing();
    }
    x += rowHorizontalSpacing();
    y = y_;
    if (isSelected(node_->modelCursor()) == MSTrue)
        drawPixmaps(window_, selectedPixmap(node_), x, y);
    else if (isSensitive(node_) == MSTrue)
        drawPixmaps(window_, pixmap(node_), x, y);
    else
        drawPixmaps(window_, insensitivePixmap(node_), x, y);
    if (tabSpacing() > 0)
        drawTabbedString(window_, node_, x, y);
    else
        drawString(window_, node_, x, y);
}
        ...
        if (strings.length() > 0) {
            ...
            if (isSelected(node_->modelCursor()) == MSTrue) {
                if (strings.length() > 1) {
                    ...
                    for (i = 1; i < len; i++)
                        tWidth += maxWidths()(i) + tps;
                    tWidth += fontObject().textWidth(strings(len));
                } else
                    tWidth = fontObject().textWidth(strings(0));
                gc().foreground(selectedRowBackground());
                int y = y_ + (maxRowHeight() - tHeight - offset2) / 2;
                if (hasFocus() == MSTrue) {
                    gc().lineWidth(selectedHighlightThickness());
...
{
    if (nodeInfo_->expandable() == MSFalse)
        return MSFalse;
    else
        return nodeInfo_->expandedState();
}
...
{
    if (cursor_.isValid())
        msTreeFormatType(elementForOps(modelTree().elementAt(cursor_)), buffer_);
...
{
    if (msTreeSetType(elementForOps(modelTree().elementAt(cursor_)), pString_) == MSError::MSSuccess)
        return MSTrue;
    else
        return MSFalse;
}
...
void MSTreeListView<Element>::adjustRowColumn(void)
{
    ...
    int maxRH = (maxRowHeight() > 0) ? maxRowHeight() : fontObject().textHeight() + selectedHighlightThickness() * 2 + highlightGap() * 2;
...
{
    if (firstRow() < 0 || vsb()->mapped() == MSFalse)
        _firstRow = 0;
    else {
...
void MSTreeListView<Element>::adjustFirstColumn(void)
{
    if (hsb()->mapped() == MSTrue) {
        ...
        if (hsb()->value() + viewSize > maxRowWidth()) {
            if (viewSize <= maxRowWidth())
                hsb()->valueChange(0);
            else
                hsb()->valueChange(maxRowWidth() - viewSize);
        }
    } else
        hsb()->valueChange(0);
}
...
void MSTreeListView<Element>::adjustSize(void)
{
    ...
    computeMaxRowSize();
    if (oldMaxRW != maxRowWidth() || oldMaxRH != maxRowHeight())
        computeSize();
    else
        redrawImmediately();
}
...
MSBoolean MSTreeListView<Element>::locateSelectedNode(ResourceTreeCursor resourceCursor_)
{
    if (_selectedNode != 0 && resourceCursor_.isValid()) {
        ...
        if (info->modelCursor() == *_selectedNode)
            ...
            for (resourceCursor_.setToFirstExistingChild(); resourceCursor_.isValid(); resourceCursor_.setToNextExistingChild()) {
                if (locateSelectedNode(resourceCursor_) == MSTrue)
                    return MSTrue;
            }
            return MSFalse;
        }
    ...
    int& maxHeight_)
{
    ...
    if (buffer.length() > 0) {
        ...
        if (tabSpacing() > 0) {
            ...
            if (maxWidths_.length() > 0)
                maxWidths_[0] = (firstSegmentWidth > maxWidths_(0)) ? firstSegmentWidth : maxWidths_(0);
            else
                maxWidths_ << firstSegmentWidth;
            for (unsigned i = 1; i < strings.length(); i++) {
                int textWidth = fontObject().textWidth(strings(i));
                if (maxWidths_.length() > i)
                    maxWidths_[i] = (textWidth > maxWidths_(i)) ? textWidth : maxWidths_(i);
                else
                    maxWidths_ << textWidth;
            }
        } else {
...
MSTreeListView<Element>::ResourceTreeCursor MSTreeListView<Element>::findResourceCursor(const TreeModelCursor& modelCursor_) const
{
    ...
    for (resourceTree().setToFirst(resourceCursor, MSPreorder);
         ...
         resourceTree().setToNext(resourceCursor, MSPreorder)) {
        const ManagedInfo& info = resourceTree().elementAt(resourceCursor);
        if (info->modelCursor() == modelCursor_)
            break;
    }
...
MSTreeListView<Element>::ScreenVectorCursor MSTreeListView<Element>::findScreenCursor(const ResourceTreeCursor& resourceCursor_, unsigned long& pos_) const
{
    ...
    for (screenCursor.setToFirst(); screenCursor.isValid(); screenCursor.setToNext()) {
        ...
        if (((ResourceTreeCursor&)rCursor) == resourceCursor_)
            break;
    }
    return screenCursor;
}
        ...
        if (editor()->mapped() == MSTrue)
            ...
            case XK_KP_Add:
                if (_selectedNode != 0) {
                    ...
                    if (state == MSFalse)
                        expandedState(*_selectedNode, MSTrue);
                }
                break;
            case XK_KP_Subtract:
            case XK_F24:
            case XK_minus:
                if (_selectedNode != 0) {
                    MSBoolean state = expandedState(*_selectedNode);
                    if (state == MSTrue)
                        expandedState(*_selectedNode, MSFalse);
                }
                break;
    ...
    if (screenVector().isEmpty() == False) {
        if (_selectedNode == 0) {
            ...
            for (screenVector().setToFirst(screenCursor); screenCursor.isValid(); screenVector().setToNext(screenCursor)) {
                ...
                ManagedInfo& info = resourceTree().elementAt(resourceCursor);
                if (info->modelCursor() == *_selectedNode) {
                    if (screenVector().isFirst(screenCursor) == False)
                        screenCursor.setToPrevious();
                    else
                        screenCursor.invalidate();
                    break;
                } else
                    pos_++;
            }
    ...
    if (screenVector().isEmpty() == False) {
        if (_selectedNode == 0) {
            ...
            for (screenVector().setToFirst(screenCursor); screenCursor.isValid(); screenVector().setToNext(screenCursor)) {
                ...
                ManagedInfo& info = resourceTree().elementAt(resourceCursor);
                if (info->modelCursor() == *_selectedNode) {
                    if (screenVector().isLast(screenCursor) == False)
                        screenCursor.setToNext();
                    else
                        screenCursor.invalidate();
                    break;
                } else
        ...
        if (isSensitive(info) == MSTrue) {
            if (labelSelected(info, event_->xbutton.x, event_->xbutton.y) == MSTrue) {
                if (isSelected(info->modelCursor()) == MSTrue) {
                    if (isDoubleClick(event_) == MSTrue)
                        doubleClickNotify();
                } else {
...
MSBoolean MSTreeListView<Element>::isSelected(TreeModelCursor& _modelCursor)
{
    if (_selectedNode == 0)
        ...
        return (*_selectedNode == _modelCursor) ? MSTrue : MSFalse;
}
...
void MSTreeListView<Element>::updateForeground(unsigned long oldFg_)
{
    ...
    if (selectedRowBackground() == oldFg_ && selectedRowForeground() == background())
        _selectedRowBackground = foreground();
...
void MSTreeListView<Element>::updateBackground(unsigned long oldBg_)
{
    ...
    if (selectedRowForeground() == oldBg_ && selectedRowBackground() == foreground())
        _selectedRowForeground = background();
...
{
    if (showButtons() == MSTrue)
        return collapsedButtonPixmap().width() + buttonShadowThickness() * 2;
    else
        return 0;
}
...
{
    if (showButtons() == MSTrue)
        return collapsedButtonPixmap().height() + buttonShadowThickness() * 2;
    else
        return 0;
}
    ...
    if (resourceTree().isEmpty() == False) {
        ...
        for (resourceTree().setToFirst(resourceCursor, MSPreorder);
            ...
            if (info->modelCursor() == modelCursor) {
                ...
                if (isExpanded(info) == MSTrue)
                    rebuild = MSTrue;
                else if (isExpandable(info) != nodeExpandable)
                    needRedraw = MSTrue;
                break;
            }
        }
    ...
    if (resourceTree().isEmpty() == False) {
        ...
        for (resourceTree().setToFirst(resourceCursor, MSPreorder);
            ...
            if (nodeInfo->modelCursor() == treeEvent_.cursor()) {
                ...
                if (isExpandable(nodeInfo) != nodeExpandable)
                    redrawNecessary = MSTrue;
                break;
            }
        }
        ...
        if (reconfigNecessary == MSTrue)
            configure();
        if (redrawNecessary == MSTrue)
            redrawImmediately();
    }
}
...
MSBoolean MSTreeListView<Element>::isVisible(const TreeModelCursor& modelCursor_) const
{
    ...
    if (resourceCursor.isValid()) {
        ...
        while (resourceTree().setToParent(resourceCursor)) {
            const ManagedInfo& info = resourceTree().elementAt(resourceCursor);
            if (info->expandedState() == MSFalse || info->expandable() == MSFalse) {
                visible = MSFalse;
                break;
            }
        }
