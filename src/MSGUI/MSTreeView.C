...
// Editor
template <class Element>
MSTreeView<Element>::Editor::Editor(MSWidget* owner_)
    : MSTextField(owner_)
{
...
void MSTreeView<Element>::buildCollapsedButtonPixmap(void)
{
    static char MSTreeView_CollapsedPixmap_bits[] = {
        char(0x00), char(0x00), char(0x78), char(0x00), char(0xfc), char(0x00), char(0xfe), char(0x01), char(0xfe), char(0x01), char(0xfe), char(0x01),
        char(0xfe), char(0x01), char(0xfc), char(0x00), char(0x78), char(0x00), char(0x00), char(0x00)
    };
...
void MSTreeView<Element>::buildExpandedButtonPixmap(void)
{
    static char MSTreeView_ExpandedPixmap_bits[] = {
        char(0x00), char(0x00), char(0x78), char(0x00), char(0xfc), char(0x00), char(0xce), char(0x01), char(0x86), char(0x01), char(0x86), char(0x01),
        char(0xce), char(0x01), char(0xfc), char(0x00), char(0x78), char(0x00), char(0x00), char(0x00)
    };
...
void MSTreeView<Element>::resizeRedrawPixmap(int w_, int h_)
{
    ...
    if (redrawPixmap() != 0 && redrawPixmap()->width() >= w_ && redrawPixmap()->height() >= h_)
        return;
...
int MSTreeView<Element>::computeTreeDepth(void)
{
    ...
    if (_startCursor.isValid())
        treeCursor = _startCursor;
    else
        treeCursor.setToRoot();
...
void MSTreeView<Element>::drawNode(ElementTreeCursor& treeCursor_, Window window_, MSBoolean clear_)
{
    ...
    if (aNode.width() > 0 && aNode.height() > 0) {
        ...
        if (displayButton == MSTrue) {
            if (orientation() == Horizontal) {
                if (aNode.expandedState() == MSTrue)
                    buttonPixmap = _expandedButtonPixmap;
                else
                    buttonPixmap = _collapsedButtonPixmap;
        ...
        MSBoolean selected;
        if (selectedCursor().isValid() == True && selectedCursor() == treeCursor_)
            selected = MSTrue;
        else
            selected = MSFalse;
        ...
        if (showPixmaps() == MSTrue) {
            if (selected == MSTrue)
                drawPixmaps(window_, selectedPixmap(&aNode), x, y, hLimit);
            else if (sensitive == MSTrue)
                drawPixmaps(window_, pixmap(&aNode), x, y, hLimit);
            else
                drawPixmaps(window_, insensitivePixmap(&aNode), x, y, hLimit);
        }
...
void MSTreeView<Element>::setPositions(ElementTreeCursor treeCursor_, int depth_)
{
    ...
    if (orientation() == Vertical) {
        for (j = 0; j < depth_; j++)
            sum += _verticalPositions(j);
        aNode.y(depth_ * verticalSpacing() + sum + offset);
    } else {
        for (j = 0; j < depth_; j++)
            sum += _horizontalPositions(j);
...
int MSTreeView<Element>::computeHorizontalPositions(ElementTreeCursor treeCursor_, int depth_)
{
    ...
    if (aNode.expandable() == MSTrue && aNode.expandedState() == MSTrue) {
        if (elementTree().numberOfSubtreeElements(treeCursor_) > 1)
            hasChildren = MSTrue;
    }

    if (hasChildren == MSFalse)
        aNode.x(hpos);
    else {
        ...
        if (aNode.x() < hpos) {
            ...
            for (int j = depth_ + 1; j <= depth; j++)
                horizontalPosition(j, horizontalPosition(j) + delta);
            aNode.x(hpos);
        }
    }
...
{
    if (showRootNode() == MSFalse && elementTree().isRoot(treeCursor_) == True)
        return MSFalse;
        ...
        if (traverseFocus(this) == MSTrue) {
            ...
            if (editor()->mapped() == MSFalse) {
                ...
                if (treeCursor.isValid()) {
                    TreeNode& aNode = nodeAt(treeCursor);
                    if (aNode.x() <= x && aNode.y() <= y && aNode.x() + aNode.width() >= x && aNode.y() + aNode.height() >= y) {
                        MSBoolean labelSelected;
                        MSBoolean buttonSelected;
                        if (showButtons() == MSTrue && aNode.expandable() == MSTrue) {
                            if (orientation() == Horizontal) {
                                if (x < (aNode.x() + aNode.width() - collapsedButtonPixmap().width() - buttonShadowThickness() * 2 - buttonNodeMargin())) {
                                ...
                                if (y < (aNode.y() + aNode.height() - collapsedButtonPixmap().height() - buttonShadowThickness() * 2 - buttonNodeMargin())) {
                                    labelSelected = MSTrue;
                                    buttonSelected = MSFalse;
                                } else if (x < (aNode.x() + aNode.width() / 2 - ((collapsedButtonPixmap().width() + buttonShadowThickness() * 2) / 2)) || x > (aNode.x() + aNode.width() / 2 + ((collapsedButtonPixmap().width() + buttonShadowThickness() * 2) / 2))) {
                                    labelSelected = MSFalse;
                                    buttonSelected = MSFalse;
                                } else {
                                    labelSelected = MSFalse;
                        ...
                        if (labelSelected == MSTrue && (showLabels() == MSTrue || showPixmaps() == MSTrue)) {
                            if (isSensitive(&aNode) == MSTrue) {
                                MSBoolean alreadySelected = (selectedCursor().isValid() == True && selectedCursor() == treeCursor) ? MSTrue : MSFalse;
                            ...
                            backgroundAreaSelection(pEvent_);
                    }
                }
    ...
    if (selectedCursor().isValid() == True && selectedCursor() != startCursor()) {
        ...
        for (treeCursor.setToPreviousExistingChild();
            ...
            if (isSensitive(&node) == MSTrue) {
                if (selectNode(treeCursor) == MSTrue)
                    nodeSelectionNotify();
                return;
            }
        }
...
void MSTreeView<Element>::processReshapeEvent(TreeEvent& treeEvent_)
{
    if (elementTree().isEmpty() == False) {
        ...
        if (elementCursor.isValid()) {
            ...
            if (node.expandable() == MSTrue && node.expandedState() == MSTrue)
                rebuild = MSTrue;
        }
...
{
    if (selectedCursor().isValid())
        return elementTree().elementAt(selectedCursor()).cursor();
    else
...
void MSTreeView<Element>::rebuildScreen(Iterator* iterator_)
{
    if (hasModel() == MSTrue && elementTree().isEmpty() == True) {
        ...
        if (modelCursor.isValid()) {
            NodeAttribute attr;
            if (iterator_ == 0)
                nodeAttribute(modelCursor, attr);
            else
                iterator_->nodeAttribute(modelCursor, attr);
...
void MSTreeView<Element>::updateElementTree(TreeModelCursor modelCursor_, ElementTreeCursor& elementCursor_, Iterator* iterator_)
{
    if (elementCursor_.isValid()) {
        for (modelCursor_.setToFirstExistingChild(); modelCursor_.isValid(); modelCursor_.setToNextExistingChild()) {
            NodeAttribute attr;
            if (iterator_ == 0)
                nodeAttribute(modelCursor_, attr);
            else
                iterator_->nodeAttribute(modelCursor_, attr);
...
MSTreeView<Element>::positionToCursor(int x_, int y_)
{
    ElementTreeCursor elementCursor(elementTree());
    if (_startCursor.isValid())
        elementCursor = _startCursor;
    else
        elementCursor.setToRoot();
    if (elementCursor.isValid() == True) {
        TreeNode& aNode = nodeAt(elementCursor);
        if (showRootNode() == MSTrue) {
            if (aNode.x() <= x_ && aNode.y() <= y_ && aNode.x() + aNode.width() >= x_ && aNode.y() + aNode.height() >= y_) {
...
{
    if (locateCursor_ == cursor_)
        return MSTrue;
    else {
        TreeNode& aNode = nodeAt(cursor_);
        if (aNode.expandable() == MSTrue && aNode.expandedState() == MSTrue) {
            for (cursor_.setToFirstExistingChild(); cursor_.isValid(); cursor_.setToNextExistingChild()) {
                if (locateTreeCursor(locateCursor_, cursor_) == MSTrue)
                    return MSTrue;
            }
        } else
            return MSFalse;
...
{
    if (treeNode_->expandable() == MSFalse)
        return MSFalse;
    else
        return treeNode_->expandedState();
}
...
{
    if (selectedCursor().isValid() && treeNode_ == &nodeAt(selectedCursor()))
        return selectedNodeForeground();
    else
...
{
    if (startCursor().isValid())
        return elementTree().elementAt(startCursor()).cursor();
    else
        return TreeModelCursor(TreeModel());
}
...
typename MSTreeView<Element>::ElementTreeCursor MSTreeView<Element>::findElementCursor(const TreeModelCursor& modelCursor_) const
{
    ...
    for (elementTree().setToFirst(elementCursor, MSPreorder);
         elementCursor.isValid();
         elementTree().setToNext(elementCursor, MSPreorder)) {
        if (elementCursor.element().cursor() == modelCursor_)
            break;
    }
    return elementCursor;
}
