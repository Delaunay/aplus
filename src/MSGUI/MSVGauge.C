...
void MSVGauge::drawGauge(void)
{
    if (mapped() == MSTrue) {
        ...
        if (_startValue.isSet() == MSTrue) {
            ...
            startValue = y - SliderAreaShadowThickness;
...
void MSVGauge::updateGauge(int h_)
{
    if (mapped() == MSTrue) {
        ...
        if (h == gaugeHeight() && direction == _direction)
            return;
            ...
            if (h > thickness && thickness > 0)
                updateGaugeShadow(x, starty, h, thickness, direction);
        } else {
    ...
    for (unsigned i = 0; i < avList_.length(); i++) {
        if (avList_[i].attribute() == "startValue") {
            ...
            if (avList_[i].value().length() == 0)
                ...
                if (value.set(avList_[i].value()) == MSError::MSSuccess)
                    startValue(value);
            }
        }
    }
...
MSAttrValueList& MSVGauge::get(MSAttrValueList& avList_)
{
    if (_startValue.isSet() == MSTrue) {
        ...
        avList_ << MSAttrValue("startValue", "");
