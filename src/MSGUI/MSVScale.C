...
MSVScale::VSlider::VSlider(MSWidget* owner_)
...
{
    int y = y_ < y_org() + slider()->offset() ? y_org() + slider()->offset() : y_ > y_end() - slider()->offset() - slider()->height() ? y_end() - slider()->offset() - slider()->height() : y_;
        ...
        if (tickPositionLength != 0) {
            ...
            for (unsigned i = 0; i < tickPositionLength; i++) {
                ...
                if (buffer.length() > 0) {
                    ...
                    y += MSBottom & labelAlignment() ? -fontInfo->descent / 2 : MSTop & labelAlignment() ? fontInfo->ascent : fontInfo->ascent / 2;
