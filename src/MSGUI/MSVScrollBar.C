...
MSVScrollBar::VElevator::VElevator(MSVScrollBar* owner_)
    : MSScrollBar::Elevator(owner_)
{
...
void MSVScrollBar::configureForMotif(void)
{
    if (_elevator != 0) {
        ...
        w = h = width() - 2 * offset;
        if (height() < 2 * (h + offset) + MinScrollBarLength + 2)
            h = (height() - (MinScrollBarLength + 2 + 2 * offset)) / 2;
...
void MSVScrollBar::motifButton1Press(const XEvent* pXEvent_)
{
    ...
    if ((xx >= _elevator->x()) && (xx <= _elevator->x() + _elevator->width()) && (yy >= _elevator->y()) && (yy <= _elevator->y() + _elevator->height())) {
    ...
    // ... in arrow 1
    else if ((xx >= _elevator->_arrow1->x()) && (yy >= _elevator->_arrow1->y()) && (xx <= _elevator->_arrow1->x() + _elevator->_arrow1->width()) && (yy <= _elevator->_arrow1->y() + _elevator->_arrow1->height())) {
        _changeType = Dec;
        _elevator->_arrow1->select(MSTrue);
    }
    // ... in arrow 2
    else if ((xx >= _elevator->_arrow2->x()) && (yy >= _elevator->_arrow2->y()) && (xx <= _elevator->_arrow2->x() + _elevator->_arrow2->width()) && (yy <= _elevator->_arrow2->y() + _elevator->_arrow2->height())) {
        _changeType = Inc;
        _elevator->_arrow2->select(MSTrue);
    }
    // ... in the trough (i.e. slider area)...
    else if ((xx >= sliderAreaRect().x()) && (yy >= sliderAreaRect().y()) && (xx <= sliderAreaRect().x() + sliderAreaRect().width()) && (yy <= sliderAreaRect().y() + sliderAreaRect().height())) {
        // Page the slider up or down
        if (yy < _elevator->y())
            _changeType = PageUp;
        else
            _changeType = PageDown;
    } else
        return; //... in the highlight area.
    startDelayTimer();
    updateSliderValue();
}
...
void MSVScrollBar::openlookButton1Press(const XEvent* pXEvent_)
{
    ...
    //  Calculate whether the selection point is in the slider
    if ((_elevator->height() == DefaultElevatorHeight) && (xx >= _elevator->x()) && (xx <= _elevator->x() + _elevator->width()) && (yy >= _elevator->y() + DefaultElevatorBoxHeight) && (yy <= _elevator->y() + _elevator->height() - DefaultElevatorBoxHeight)) {
        _separation_y = yy - _elevator->y();
        _initial_y = _elevator->y();
        _slidingOn = MSTrue;
        _changeType = Drag;
    ...
    // ... in arrow 1
    else if ((xx >= _elevator->_arrow1->x()) && (yy >= _elevator->y()) && (xx <= _elevator->_arrow1->x() + _elevator->_arrow1->width()) && (yy <= _elevator->y() + DefaultElevatorBoxHeight)) {
        _changeType = Dec;
        _elevator->_arrow1->select(MSTrue);
    }
    // ... in arrow 2
    else if ((xx >= _elevator->_arrow2->x()) && (yy >= _elevator->y() + _elevator->height() - DefaultElevatorBoxHeight) && (xx <= _elevator->_arrow2->x() + _elevator->_arrow2->width()) && (yy <= _elevator->y() + _elevator->height())) {
        _changeType = Inc;
        _elevator->_arrow2->select(MSTrue);
    }
    // ... in top marker
    else if ((xx >= sliderAreaRect().x()) && (yy >= ht) && (xx <= sliderAreaRect().x() + sliderAreaRect().width()) && (yy <= ht + MSVScrollBarDefaultMarkerHeight)) {
        _changeType = Home;
    }
    // ... in bottom marker
    else if ((xx >= sliderAreaRect().x()) && (yy >= height() - ht - MSVScrollBarDefaultMarkerHeight) && (xx <= sliderAreaRect().x() + sliderAreaRect().width()) && (yy <= height() - ht)) {
        _changeType = End;
    }
    // ... in the trough (i.e. slider area)...
    else if ((xx >= sliderAreaRect().x()) && (yy >= sliderAreaRect().y()) && (xx <= sliderAreaRect().x() + sliderAreaRect().width()) && (yy <= sliderAreaRect().y() + sliderAreaRect().height())) {
        // Page the slider up or down
        if (yy < _elevator->y() + DefaultElevatorBoxHeight)
            _changeType = PageUp;
        else
            _changeType = PageDown;
    } else
...
void MSVScrollBar::motionLoop(void)
{
    ...
    while (keys & mask) {
        ...
        if (button_x < sliderAreaRect().x())
            button_x = sliderAreaRect().x();

        if (button_x > sliderAreaRect().x() + sliderAreaRect().width())
            button_x = sliderAreaRect().x() + sliderAreaRect().width();

        if (button_y < sliderAreaRect().y())
            button_y = sliderAreaRect().y();
        ...
        if (newY < sliderAreaRect().y())
            newY = sliderAreaRect().y();
...
int MSVScrollBar::calcSliderValue(int, int y_)
{
    if (y_ <= sliderAreaRect().y()) {
        ...
        if (intTemp < min())
            intTemp = min();
        else if (intTemp > max() - viewSize())
            intTemp = max() - viewSize();
        return (intTemp);
    }
}
