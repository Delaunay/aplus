...
void MSWidget::destroy(void)
{
    ...
    if (applicationAllowWidgetDestruction() == MSTrue)
        delete this;
    else {
...
void MSWidget::prepareForDestruction(void)
{
    ...
    if (_window != 0)
        _server->widgetHashTable()->remove(_window); // no more events
    if (pTopWidget != 0)
        pTopWidget->removeFromFocusList(this);
    if (pTopWidget != 0 && pTopWidget->inputFocus() == this)
        (void)pTopWidget->traverseFocus(0);
    if (focusWindow() == this)
        _focusWindow = 0;
    childDestroyNotify();
...
void MSWidget::decoupleAllWidgets(void)
{
    ...
    for (aCursor.setToFirst(); aCursor.isValid(); aCursor.setToNext()) {
        ...
        if (aWindow != 0)
            _server->widgetHashTable()->remove(aWindow);
    }
}
...
void MSWidget::event(const XEvent* pEvent_)
{
    if (!_eventOverride || !(*_eventOverride)(pEvent_->type))
        ...
        case NoExpose:
            ...
            break;
        case GraphicsExpose:
            graphicsExpose(pEvent_);
            break;
        case VisibilityNotify:
            visibilityNotify(pEvent_);
            break;
        case EnterNotify:
            if (_server->eventGrabbed(pEvent_, this) == MSTrue)
                enterNotify(pEvent_);
            break;
        case LeaveNotify:
            leaveNotify(pEvent_);
        ...
        case ConfigureNotify:
            configureNotify(pEvent_);
            break;
        case CreateNotify:
            createNotify(pEvent_);
            break;
        case DestroyNotify:
            destroyNotify(pEvent_);
            break;
        case ReparentNotify:
            reparentNotify(pEvent_);
            break;
        case MapNotify:
            mapNotify(pEvent_);
            break;
        case UnmapNotify:
            ...
            break;
        case PropertyNotify:
            propertyNotify(pEvent_);
            break;
        case SelectionNotify:
            selectionNotify(pEvent_);
            break;
        case SelectionClear:
            selectionClear(pEvent_);
            break;
        case SelectionRequest:
            selectionRequest(pEvent_);
            break;
        case ClientMessage:
            clientMessage(pEvent_);
            break;
        case MotionNotify:
            if (_server->eventGrabbed(pEvent_, this) == MSTrue)
                motionNotify(pEvent_);
            break;
        case ButtonRelease:
            if (_server->eventGrabbed(pEvent_, this) == MSTrue)
                buttonRelease(pEvent_);
            break;
        case ButtonPress:
            if (_server->eventGrabbed(pEvent_, this) == MSTrue)
                buttonPress(pEvent_);
        ...
        case KeyPress:
            if (_server->eventGrabbed(pEvent_, this) == MSTrue)
                keyPressEvent(pEvent_);
            else
                _server->bell();
            break;
        case FocusIn:
            focusInEventNotify(pEvent_);
            break;
        case FocusOut:
            focusOutEventNotify(pEvent_);
            break;
        default:
            break;
        }
    ...
    if (_eventHandler != 0)
        (*_eventHandler)(this, pEvent_, _eventHandlerData);
}
...
void MSWidget::pointerXY(int& x_, int& y_) const
{
    ...
    Window root, child;
    if (_window != 0)
        XQueryPointer(display(), _window, &root, &child, &rx, &ry, &ix, &iy, &keys);
    else
        XQueryPointer(display(), top()->_window, &root, &child, &rx, &ry, &ix, &iy, &keys);
    x_ = rx;
    y_ = ry;
}
...
MSWidget* MSWidget::top(void)
{
    ...
    while (pTopWidget->_owner != 0)
        pTopWidget = pTopWidget->_owner;
    return pTopWidget;
}
...
{
    if (_window != 0)
        XWarpPointer(display(), _window, _window, 0, 0, 0, 0, x_, y_);
}
...
void MSWidget::moveTo(int x_, int y_)
{
    if (x_origin() != x_ || y_origin() != y_) {
        ...
        MSRect::y(y_);
        if (_window != 0)
            XMoveWindow(display(), _window, x_, y_);
        childMoveNotify();
    }
}
...
{
    if (_owner != 0)
        _owner->childPosition(this, at_);
}
At MSWidget::at(void)
{
    if (_owner != 0)
        return _owner->childPosition(this);
    else
        return At(0, 0, 1, 1, resizeConstraints());
}
...
void MSWidget::background(unsigned long pixel_)
{
    if (pixel_ != background()) {
        ...
        _bg = pixel_;
        if (_window != 0)
            XSetWindowBackground(display(), _window, pixel_);
...
void MSWidget::reparent(MSWidget* owner_)
{
    if (_owner != owner_) {
        ...
        if (_window != 0)
            XReparentWindow(display(), _window,
                owner_->_window, x_origin(), y_origin());
        _owner = owner_;
        _server = oldOwner->_server;
        if (oldOwner != 0)
            oldOwner->childRemove(this);
        if (_owner != 0) {
        ...
        if (oldOwner->_owner == 0 && _owner->_owner == 0) {
            ...
            if (pFocusWidget != 0)
                _owner->traverseFocus(pFocusWidget);
        }
    }
}
...
{
    return (MSWidget::focusWindow() != 0 && MSWidget::focusWindow() != this) ? MSWidget::focusWindow()->inputFocus() : 0;
}
...
void MSWidget::addToFocusList(void)
{
    MSWidget* pTopWidget = top();
    if (pTopWidget != 0)
        pTopWidget->addToFocusList(this);
}
    ...
    if (selection_ == XA_PRIMARY) {
        if (server()->primarySelectionOwner() != this) {
            ...
            if (server()->primarySelectionOwner() != 0) {
                if ((unsigned long)(server()->widgetHashTable()->lookup(server()->primarySelectionOwner()->window())) != server()->widgetHashTable()->notFound()) {
...
{
#ifndef MS_WINDOWS
    if (pEvent_->xselectionrequest.selection == XA_PRIMARY && pEvent_->xselectionrequest.owner == _window) {
        ...
        if (target == XA_STRING) {
            ...
            if (cp != 0) {
                Atom property;
                if (pEvent_->xselectionrequest.property == None)
                    property = XA_PRIMARY;
                else
                    property = pEvent_->xselectionrequest.property;
...
{
#ifndef MS_WINDOWS
    if (pEvent_->xselection.selection == XA_PRIMARY && pEvent_->xselection.requestor == _window && pEvent_->xselection.property != (Atom)None && pEvent_->xselection.target != (Atom)None) {
        if (propertyData(pEvent_->xselection.property, pEvent_->xselection.target) == MSSuccess && server()->pasteBuffer().length() > 0) {
...
{
    Atom r = XA_STRING;
    if (target_ == XA_INTEGER || target_ == XA_WINDOW || target_ == XA_ATOM)
        r = target_;
    else if (target_ == _server->atom(MSAtomTable::Targets))
        r = XA_ATOM;
    else if (target_ == _server->atom(MSAtomTable::ClientWindow))
        r = XA_WINDOW;
    else if (target_ == _server->atom(MSAtomTable::TimeStamp))
        r = XA_INTEGER;
    else if (target_ == _server->atom(MSAtomTable::Length))
        r = XA_INTEGER;
...
{
    if (target_ == XA_ATOM)
        atomsFromData(data_, itemCount_);
...
{
    if (pWidget_ != 0)
        pWidget_->keyPress(pEvent_, keysym_, state_, pString_);
}
...
{
    if (pWidget_ != 0)
        pWidget_->keyRelease(pEvent_, keysym_, state_, pString_);
}
    ...
    KeySym keysym_, unsigned int state_, const char* pString_)
{
    ...
    if (pWidget_ != 0) {
        if (pWidget_->keyTranslate(keyPress) == MSTrue)
            ...
            keyPressNotify(pWidget_, pEvent_, keysym_, state_, pString_);
    }
}
    ...
    void* clientData_, MSWidgetIterationType type_)
{
    ...
    if (type_ == MSBreadthFirst) {
        ...
        return (depthFirstIteration(defaultIterator));
}
...
MSBoolean MSWidget::allWidgetsDo(MSWidgetIterator& iterator_, MSWidgetIterationType type_)
{
    if (type_ == MSBreadthFirst) {
        ...
        return (depthFirstIteration(iterator_));
}
...
MSBoolean MSWidget::depthFirstIteration(MSWidgetIterator& iterator_)
{
    ...
    for (unsigned i = 0; i < len; i++) {
        if (vector(i)->depthFirstIteration(iterator_) == MSFalse)
            return MSFalse;
    }
    ...
    MSWidgetIterator& iterator_)
{
    ...
    for (i = 0; i < len; i++)
        if (iterator_.applyTo(aWidgetVector_(i)) == MSFalse)
            return MSFalse;
    ...
    if (breadthVector.length())
        return (breadthFirstIteration(breadthVector, iterator_));
    else
        return MSTrue;
}
