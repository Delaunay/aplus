...
void MSWidgetCursor::buildBreadthFirst(MSWidgetVector& aWidgetVector_)
{
    ...
    for (i = 0; i < len; i++)
        breadthVector.append(aWidgetVector_(i)->children());
    if (breadthVector.length() > 0)
        buildBreadthFirst(breadthVector);
}
