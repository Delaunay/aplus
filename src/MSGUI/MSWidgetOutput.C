...
{
    if (mode_ == Print && displayPrintMode() == MSP::PPM)
        _outputMode = DrawPPM;
    else
        _outputMode = mode_;
}
...
{
    if (_displayPrint != 0)
        delete _displayPrint;
}

MSDisplayPrint* MSWidgetOutput::displayPrint(void)
{
    if (_displayPrint._displayPrint == 0)
        _displayPrint._displayPrint = new MSDisplayPrint;
...
{
    if (_outputMode == Draw)
        ::XDrawArc(dpy_, id_, gc_, x_, y_, w_, h_, angle1_, angle2_);
        ...
        _displayPrint._displayPrint->printArc(gc_, x_, y_, w_, h_, angle1_, angle2_);
}

void MSWidgetOutput::XDrawArcs(Display* dpy_, Window id_, GC gc_, XArc* arcs_, int n_)
{
    if (_outputMode == Draw)
        ::XDrawArcs(dpy_, id_, gc_, arcs_, n_);
    else if (displayPrintMode() == MSP::PPM) {
        ...
        _displayPrint._displayPrint->printArcs(gc_, arcs_, n_);
}
...
{
    if (_outputMode == Draw)
        ::XDrawLine(dpy_, id_, gc_, x1_, y1_, x2_, y2_);
    else if (displayPrintMode() == MSP::PPM) {
        ...
        _displayPrint._displayPrint->printLine(gc_, x1_, y1_, x2_, y2_);
}
...
{
    if (_outputMode == Draw)
        ::XDrawLines(dpy_, id_, gc_, points_, n_, mode_);
    else if (displayPrintMode() == MSP::PPM) {
        ...
        _displayPrint._displayPrint->printLines(gc_, points_, n_, mode_);
}
...
{
    if (_outputMode == Draw)
        ::XDrawSegments(dpy_, id_, gc_, segments_, n_);
    else if (displayPrintMode() == MSP::PPM) {
        ...
        _displayPrint._displayPrint->printSegments(gc_, segments_, n_);
}

void MSWidgetOutput::XDrawPoint(Display* dpy_, Window id_, GC gc_, int x_, int y_)
{
    if (_outputMode == Draw)
        ::XDrawPoint(dpy_, id_, gc_, x_, y_);
    else if (displayPrintMode() == MSP::PPM) {
        int x = x_ + _displayPrint._displayPrint->x_org();
        int y = y_ + _displayPrint._displayPrint->y_org();
        ::XDrawPoint(dpy_, displayPrintPixmap(), gc_, x, y);
    } else
        _displayPrint._displayPrint->printPoint(gc_, x_, y_);
}
...
{
    if (_outputMode == Draw)
        ::XDrawPoints(dpy_, id_, gc_, points_, n_, mode_);
    else if (displayPrintMode() == MSP::PPM) {
        XPoint* points = new XPoint[n_];
        for (unsigned i = 0; i < n_; i++) {
            points[i].x = points_[i].x + _displayPrint._displayPrint->x_org();
        ...
        _displayPrint._displayPrint->printPoints(gc_, points_, n_, mode_);
}
...
void MSWidgetOutput::XDrawRectangles(Display* dpy_, Window id_, GC gc_, XRectangle* rects_, int n_)
{
    if (_outputMode == Draw)
        ...
        _displayPrint._displayPrint->printRectangles(gc_, rects_, n_);
}
    ...
    const XFontStruct* fs_, int x_, int y_, const char* string_, int n_)
{
    if (_outputMode == Draw) {
        if (doubleByte(fs_))
            ::XDrawString16(dpy_, id_, gc_, x_, y_, (XChar2b*)string_, n_ / 2);
        else
            ::XDrawString(dpy_, id_, gc_, x_, y_, string_, n_);
    } else if (displayPrintMode() == MSP::PPM) {
        int x = x_ + _displayPrint._displayPrint->x_org();
        int y = y_ + _displayPrint._displayPrint->y_org();
        if (doubleByte(fs_))
            ::XDrawString16(dpy_, displayPrintPixmap(), gc_, x, y, (XChar2b*)string_, n_ / 2);
        else
            ::XDrawString(dpy_, displayPrintPixmap(), gc_, x, y, string_, n_);
    } else
        _displayPrint._displayPrint->printString(gc_, x_, y_, string_, n_);
}
    ...
    const char* string_, int n_)
{
    if (_outputMode == Draw) {
        if (doubleByte(fs_))
            ::XDrawImageString16(dpy_, id_, gc_, x_, y_, (XChar2b*)string_, n_ / 2);
        else
            ::XDrawImageString(dpy_, id_, gc_, x_, y_, string_, n_);
    } else if (displayPrintMode() == MSP::PPM) {
        int x = x_ + _displayPrint._displayPrint->x_org();
        int y = y_ + _displayPrint._displayPrint->y_org();
        if (doubleByte(fs_))
            ::XDrawImageString16(dpy_, displayPrintPixmap(), gc_, x, y, (XChar2b*)string_, n_ / 2);
        else
            ::XDrawImageString(dpy_, displayPrintPixmap(), gc_, x, y, string_, n_);
    } else
        _displayPrint._displayPrint->printImageString(gc_, x_, y_, string_, n_);
}
...
{
    if (_outputMode == Draw)
        ::XFillArc(dpy_, id_, gc_, x_, y_, w_, h_, angle1_, angle2_);
    else if (displayPrintMode() == MSP::PPM) {
        int x = x_ + _displayPrint._displayPrint->x_org();
        int y = y_ + _displayPrint._displayPrint->y_org();
        ::XFillArc(dpy_, displayPrintPixmap(), gc_, x, y, w_, h_, angle1_, angle2_);
    } else
        _displayPrint._displayPrint->printFillArc(gc_, x_, y_, w_, h_, angle1_, angle2_);
}

void MSWidgetOutput::XFillArcs(Display* dpy_, Window id_, GC gc_, XArc* arcs_, int n_)
{
    if (_outputMode == Draw)
        ::XFillArcs(dpy_, id_, gc_, arcs_, n_);
    else if (displayPrintMode() == MSP::PPM) {
        XArc* arcs = new XArc[n_];
        for (unsigned i = 0; i < n_; i++) {
            arcs[i] = arcs_[i];
        ...
        _displayPrint._displayPrint->printFillArcs(gc_, arcs_, n_);
}
...
{
    if (_outputMode == Draw)
        ::XFillPolygon(dpy_, id_, gc_, points_, n_, shape_, mode_);
    else if (displayPrintMode() == MSP::PPM) {
        XPoint* points = new XPoint[n_];
        for (unsigned i = 0; i < n_; i++) {
            points[i].x = points_[i].x + _displayPrint._displayPrint->x_org();
        ...
        _displayPrint._displayPrint->printFillPolygon(gc_, points_, n_, shape_, mode_);
}
...
{
    if (_outputMode == Draw)
        ::XFillRectangle(dpy_, id_, gc_, x_, y_, w_, h_);
    else if (displayPrintMode() == MSP::PPM) {
        int x = x_ + _displayPrint._displayPrint->x_org();
        int y = y_ + _displayPrint._displayPrint->y_org();
        ::XFillRectangle(dpy_, displayPrintPixmap(), gc_, x, y, w_, h_);
    } else
        _displayPrint._displayPrint->printFillRectangle(gc_, x_, y_, w_, h_);
}
...
{
    if (_outputMode == Draw)
        ::XFillRectangles(dpy_, id_, gc_, rects_, n_);
    else if (displayPrintMode() == MSP::PPM) {
        XRectangle* rects = new XRectangle[n_];
        for (unsigned i = 0; i < n_; i++) {
            rects[i] = rects_[i];
        ...
        _displayPrint._displayPrint->printFillRectangles(gc_, rects_, n_);
}
    ...
    XPoint* points_, int n_, int shape_, int mode_)
{
    if (_outputMode == Draw)
        ...
        _displayPrint._displayPrint->printFillPolygon(gc_, points_, n_, shape_, mode_, MSTrue);
}
    ...
    int x_, int y_, int w_, int h_)
{
    if (_outputMode == Draw)
        ...
        _displayPrint._displayPrint->printFillRectangle(gc_, x_, y_, w_, h_, MSTrue);
}
    ...
    XRectangle* rects_, int n_)
{
    if (_outputMode == Draw)
        ...
        _displayPrint._displayPrint->printFillRectangles(gc_, rects_, n_, MSTrue);
}
...
{
    if (_outputMode == Draw)
        ::XCopyArea(dpy_, src_, dest_, gc_, x_src_, y_src_, w_, h_, x_dest_, y_dest_);
    else if (displayPrintMode() == MSP::PPM) {
...
{
    if (_outputMode == Draw)
        ::XSetClipRectangles(dpy_, gc_, x_, y_, rects_, n_, order_);
    else if (displayPrintMode() == MSP::PPM) {
        XRectangle* rects = new XRectangle[n_];
        for (unsigned i = 0; i < n_; i++) {
            rects[i] = rects_[i];
        ...
        _displayPrint._displayPrint->printSetClipRectangles(gc_, x_, y_, rects_, n_, order_);
}

void MSWidgetOutput::XSetDashes(Display* dpy_, GC gc_, int dash_offset_, const char* dash_list_, int n_)
{
    if (_outputMode == Print)
        _displayPrint._displayPrint->printSetDashes(gc_, dash_list_, n_);
    else
        ::XSetDashes(dpy_, gc_, dash_offset_, dash_list_, n_);
}

void MSWidgetOutput::XFreeGC(Display* dpy_, GC gc_)
{
    if (_outputMode == Print)
        _displayPrint._displayPrint->printFreeGC(gc_);
    ::XFreeGC(dpy_, gc_);
}
    ...
    int thickness_, GC topGC_, GC bottomGC_)
{
    if (mapped() == MSTrue && thickness_ > 0) {
        ...
        if (thickness_ == 1) {
            GC gc = bottomGC_;
            if (style_ == MSEtchedIn)
                gc = topGC_;
            drawFlatShadow(window_, aRect_, thickness_, gc);
            return;
        }

        if (thickness_ > (width >> 1))
            thickness_ = (width >> 1);
        if (thickness_ > (height >> 1))
            thickness_ = (height >> 1);
        if (thickness_ <= 0)
            return;
...
{
    if (canDraw() == MSTrue)
        drawHighlightBorder(highlightThickness());
}
void MSWidgetOutput::undrawHighlight(void)
{
    if (canDraw() == MSTrue)
        undrawHighlightBorder(highlightThickness());
}
    ...
    MSShadowStyle style_, int st_)
{
    if (st_ > 0) {
        ...
        switch (style_) {
        case MSRaised:
            drawBevelShadow(window_, aRect_, st_, topGC, bottomGC);
            break;
        case MSSunken:
            drawBevelShadow(window_, aRect_, st_, bottomGC, topGC);
            break;
        case MSEtchedIn:
            drawEtchedShadow(window_, aRect_, MSEtchedIn, st_, bottomGC, topGC);
            break;
        case MSEtchedOut:
            drawEtchedShadow(window_, aRect_, MSEtchedOut, st_, topGC, bottomGC);
            break;
        case MSFlat:
            drawFlatShadow(window_, aRect_, st_, bottomGC);
            break;
        }
    }
}
