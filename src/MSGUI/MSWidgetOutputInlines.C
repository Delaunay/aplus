...
{
    if (fs_->min_byte1 != 0 || fs_->max_byte1 != 0)
        return MSTrue;
    else if (fs_->max_char_or_byte2 > 255)
        return MSTrue;
    else
        return MSFalse;
}
    ...
    int x_, int y_, int w_, int h_, int exposures_)
{
    if (_outputMode < Print)
        ::XClearArea(dpy_, id_, x_, y_, w_, h_, exposures_);
}

INLINELINKAGE int MSWidgetOutput::XTextWidth(const XFontStruct* fs_, const char* str_, int len_) const
{
    return (doubleByte(fs_) == MSTrue) ? ::XTextWidth16((XFontStruct*)fs_, (XChar2b*)str_, len_ / 2) : ::XTextWidth((XFontStruct*)fs_, (char*)str_, len_);
}
...
{
    return (doubleByte(fs_) == MSTrue) ? ::XTextExtents16((XFontStruct*)fs_, (XChar2b*)str_, len_ / 2, dir_, ascent_, descent_, overall_) : ::XTextExtents((XFontStruct*)fs_, (char*)str_, len_, dir_, ascent_, descent_, overall_);
}

INLINELINKAGE int MSWidgetOutput::XTextLength(const XFontStruct* fs_, const char* str_) const
{
    if (fs_ != 0 && str_ != 0)
        return (doubleByte(fs_) == MSTrue) ? (strlen(str_) / 2) : strlen(str_);
    return 0;
}
