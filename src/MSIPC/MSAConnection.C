...
{
    if (isSet(MSProtocolConnection<MSA>::Reset) == MSTrue)
        return MSFalse;
    if (readChannel() == 0)
        return 0;
    ...
    if (b == NULL)
        return MSFalse;
    sendTheBuffer(b);
    if (isSet(MSProtocolConnection<MSA>::WritePause) == MSFalse)
        writeChannel()->enable();
...
void MSAConnection::doRead(void)
{
    ...
    if ((n = readTheBuffer(db, _bytesToNextMessage)) < 0)
        return;
...
MSA MSAConnection::getAobjectFromBuffer(MSBuffer* buf_)
{
    ...
    if (isSet(MSProtocolConnection<MSA>::Reset) == MSTrue)
        return MSA();
    if ((s = hb->put() - hb->get()) < 4) {
        if ((n = MSBuffer::buffToBuff(buf_, hb, 4 - s)) < 0)
            return MSA();
        if ((s = hb->put() - hb->get()) < 4)
            return MSA();
        _bytesToNextMessage = MSA::longAt(hb->get());
    ...
    if ((n = MSBuffer::buffToBuff(buf_, db, _bytesToNextMessage)) < 0)
        return MSA();
    if ((_bytesToNextMessage -= n) == 0) {
        MSA d = MSA::importAObject((char*)db->get(), db->put() - db->get(), (char*)0);
        hb->reset();
...
int MSAConnection::doSyncRead(MSA& result_)
{
    ...
    if ((_bytesToNextMessage -= n) == 0) {
        ...
        db->clear();
        unset(MSProtocolConnection<MSA>::Read);
        if ((result_.aStructPtr()) == (MSAStruct*)0) {
            resetWithError(MSProtocolConnection<MSA>::Read);
            return 0;
        }
    }
