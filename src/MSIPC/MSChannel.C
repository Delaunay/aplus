...
MSChannel::~MSChannel(void)
{
    ...
    if (_pCtlObject->readChannel() == 0 && _pCtlObject->writeChannel() == 0 && _pCtlObject->exceptionChannel() == 0) {
...
void MSChannel::init(void)
{
    if (_pFds == 0)
        _pFds = new MSFds;
    if (_pChannelList == 0)
        _pChannelList = new MSNodeItem;
#ifdef MS_NEED_CTL
    if (_pCtlChannelList == 0)
        _pCtlChannelList = new MSNodeItem;
#endif
}
...
int MSChannel::select(int fd_, MSChannel::Type type_, struct timeval* pTimeout_)
{
    ...
    switch (type_) {
    case Read:
        rc = select(size, &fdSet, 0, 0, pTimeout_);
        break;
    case Write:
        rc = select(size, 0, &fdSet, 0, pTimeout_);
        break;
    case Exception:
        rc = select(size, 0, 0, &fdSet, pTimeout_);
        break;
    }
...
MSBoolean MSChannel::processChannels(void)
{
    if (MSChannel::_pChannelList != 0) {
        ...
        if (np == hp)
            return MSFalse; // nothing found to do
...
void MSChannel::ctlHandler(int, int reason_, void* client_)
{
    ...
    if (reason_ == IPC_READ_MASK)
        pChannel = obj->readChannel();
    else if (reason_ == IPC_WRITE_MASK)
        pChannel = obj->writeChannel();
    else if (reason_ == IPC_EXCEPT_MASK)
        pChannel = obj->exceptionChannel();
    if (pChannel != 0)
        pChannel->process();
    else {
