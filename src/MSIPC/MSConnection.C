...
void MSConnection::createEstablishChannel(void)
{
    _establishChannel = new MSChannel(name().string(), fd(),
        priority(), MSChannel::Write, new MSMethodCallback<MSConnection>(this, &MSConnection::processEstablish));
...
#endif
    {
        MSMessageLog::infoMessage("MSConnection::setBlockingMode(%s):Error seting %d fd to non blocking mode",
            name().string(), fd_);
    }
}
