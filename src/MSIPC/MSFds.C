...
void MSFds::fdscopy(fd_set* p1, fd_set* p2)
{
    if (p2 != (fd_set*)(0)) {
        if (p1 == (fd_set*)(0))
            memset((char*)p2, 0, sizeOf());
        else
            memcpy((char*)p2, (char*)p1, sizeOf());
    }
}
...
void MSFds::fdsand(fd_set* p1, fd_set* p2, fd_set* r)
{
    if ((p1 != (fd_set*)(0)) && (p2 != (fd_set*)(0))) {
        ...
        for (int i = 0; i < hmany; i++)
            MS_FDSBITS(r)
            [i] = MS_FDSBITS(p1)[i] & MS_FDSBITS(p2)[i];
    } else
        fdszero(r);
}
...
void MSFds::fdsand(fd_set* p1, fd_set* p2, fd_set* r)
{
    if ((p1 != (fd_set*)(0)) && (p2 != (fd_set*)(0))) {
        ...
        int i;
        for (i = 0; i < p1->fd_count; i++)
            r->fd_array[i] = p1->fd_array[i];
        r->fd_count = p1->fd_count;
        for (i = 0; i < p2->fd_count; i++)
            fdsset(r, p2->fd_array[i]);
    } else
        fdszero(r);
}
...
void MSFds::fdsor(fd_set* p1, fd_set* p2, fd_set* r)
{
    if (p1 != (fd_set*)(0)) {
        if (p2 != (fd_set*)(0)) {
            ...
            r->fd_count = p1->fd_count;
            for (i = 0; i < p2->fd_count; i++) {
                if (fdsisset(r, p2->fd_array[i]))
                    fdsclr(r, p2->fd_array[i]);
                else
                    fdsset(r, p2->fd_array[i]);
            }
        } else
