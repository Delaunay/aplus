...
MSIPService::MSIPService(const MSString& serviceName_)
    : _hostport("", -1)
...
MSIPService::~MSIPService(void)
{
    if (_remprogEntries != 0)
        delete[] _remprogEntries;
    if (_remprogBuffer != 0)
        free(_remprogBuffer);
}
...
{
    if (serviceName_ == NULL || *serviceName_ == '\0')
        return MSIPService::CN_TYPE_ERR;

    if (strchr(serviceName_, '@') != NULL)
        return MSIPService::CN_TYPE_HOSTPORT;
   ...
   */
    ...
    if (serviceName_[0] >= '0' && serviceName_[0] <= '9')
        return MSIPService::CN_TYPE_ERR;
    ...
    if ((cp = serviceName().indexOf("@")) != serviceName().length()) {
        ...
        if ((cp = serviceName().indexOf(":")) != serviceName().length()) {
            if ((sp = getservbyname((char*)serviceName().subString(cp + 1).string(), "tcp")) == NULL) {
                if ((v = (serviceName().subString(cp + 1)).asInt()) == 0)
                    return MSFalse;
...
{
    if (_countService > 0)
        _countService = _currentService = 0;
...
int MSIPService::getRemprogByName(void)
{
    ...
    if (_remprogBuffer != (char*)0)
        free(_remprogBuffer);
    _remprogBuffer = (char*)(0);
    if (_remprogEntries != (Remprog*)0)
        delete[] _remprogEntries;
    _remprogEntries = (Remprog*)0;
...
void MSIPService::compress(register char* fromptr_)
{
    ...
    while ((*toptr = *fromptr_++) != '\0') { /* convert newlines and tabs to */
