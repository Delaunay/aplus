...
void MSMainLoop::selectAndProcess(void)
{
    ...
    if (_zeroTimeOut == MSTrue)
        next = 0;
    else {
        ctl_timeout = ctl_get_timeout();
        if ((tvpnext = MSTimer::nextTimeVal()) == (struct timeval*)(0))
            next = ctl_timeout;
        else {
            ...
            if (tvp.tv_sec < 0)
                ...
                next = tvp.tv_sec * 1000000 + tvp.tv_usec;
