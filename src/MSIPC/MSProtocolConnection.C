...
MSBoolean MSProtocolConnection<Type>::establish(void)
{
    ...
    _timer = new MSRegularTimer(0, 0,
        new MSMethodCallback<MSProtocolConnection<Type>>(this, &MSProtocolConnection<Type>::doConnectCall));
    return MSTrue;
}
...
int MSProtocolConnection<Type>::readTheBuffer(MSBuffer* b_, int n_)
{
    ...
    if (isSet(MSProtocolConnection<Type>::Reset) == MSTrue)
        return 0;
    if ((n = b_->read(fd(), n_)) < 0)
        resetWithError(MSProtocolConnection<Type>::Read);
    else if (n > 0) {
        set(MSProtocolConnection<Type>::Read);
    }
    return n;
}
...
{
    if (readCallback() != 0)
        (*readCallback())(message_, readCallbackData());
}
...
{
    if (sentCallback() != 0)
        (*sentCallback())(sent_, sentCallbackData());
}
...
{
    if (resetCallback() != 0)
        (*resetCallback())(state_, resetCallbackData());
}
...
{
    _timer = 0;
    if (isSet(MSProtocolConnection<Type>::ReadPause) == MSFalse)
        readChannel()->enable();
    if (isSet(MSProtocolConnection<Type>::Reset) == MSTrue)
        unset(MSProtocolConnection<Type>::Reset);
    connectNotify();
}
...
void MSProtocolConnection<Type>::cleanup(void)
{
    ...
    if (0 != _syncErrorBuffer) {
        delete _syncErrorBuffer;
        _syncErrorBuffer = 0;
    }
    if (service().isValid() == MSTrue) {
        service().isReady(MSFalse);
    }
...
int MSProtocolConnection<Type>::doWrite(MSBoolean sw_)
{
    ...
    while (cont == MSTrue && ((np = hp->next()) != hp) && isSet(MSProtocolConnection<Type>::WritePause) == MSFalse) {
        ...
        if (n < 0) {
            _timer = new MSRegularTimer(0, 0,
                new MSMethodCallback<MSProtocolConnection<Type>>(this, &MSProtocolConnection<Type>::doWriteResetCall));
            set(MSProtocolConnection<Type>::Reset);
            if (sw_ == MSTrue && msgs != 0) {
                sentNotify(msgs);
            }
            return msgs;
        }
