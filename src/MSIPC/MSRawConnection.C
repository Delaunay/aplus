...
void MSRawConnection::doRead(void)
{
    ...
    if (isSet(MSProtocolConnection<MSString>::Reset) == MSTrue)
        return;
    if ((n = readTheBuffer(db, MaxReadSize)) < 0)
        return;
...
int MSRawConnection::send(const MSString& aString_)
{
    ...
    if (isSet(MSProtocolConnection<MSString>::WritePause) == MSFalse)
        writeChannel()->enable();
...
int MSRawConnection::syncSend(const MSString& aString_, int seconds, int microseconds, MSBoolean isAbsolute)
{
    ...
    if (isSet(MSProtocolConnection<MSString>::Reset))
        return 0;
