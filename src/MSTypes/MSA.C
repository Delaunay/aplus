...
MSA::MSA(const MSSymbol& msg_, MSBoolean encloseSymbols_)
{
    ...
    if (encloseSymbols_ == MSTrue) {
        aStructPtr(ga(ETYPE, 0, 1, d));
        if (_aStructPtr)
            _aStructPtr->p[0] = si((char*)msg_.symbolName());
    } else
        _aStructPtr = (MSAStruct*)si((char*)msg_.symbolName());
}
...
void MSA::iterativeSpecify(const MSIndexVector& location_, const MSA& aObjectB_)
{
    ...
    for (unsigned i = 0; i < length; i++) {
        if ((*index) == 0 || (*index)->t != ETYPE || (*index)->n == 0 || ((*index)->n == 0 && (*index)->r > 0))
            return;
        if ((*index)->c != 1) {
        ...
        if ((*index)->r == 0)
            index = (MSAStruct**)&((*index)->p[0]);
        if ((*index)->r >= 1)
            index = (MSAStruct**)&((*index)->p[location_(i)]);
    }
    dc(*index);
...
{
    if (_aStructPtr == 0 || _aStructPtr->t != ETYPE || _aStructPtr->n == 0 || (_aStructPtr->n == 0 && _aStructPtr->r > 0))
        return MSA();
    long r = rank();
    if (r == 0 && ((MSAStruct*)_aStructPtr->p[0])->t != STRINGTYPE)
        return MSA((MSAStruct*)_aStructPtr->p[0]);
    else if (r >= 1)
        return MSA((MSAStruct*)_aStructPtr->p[pickIndex_]);
    return MSA();
}
...
{
    if (_aStructPtr)
        return MSA(gpick(pickIndex_._aStructPtr, _aStructPtr), MSTrue);
    else
        return MSA();
}
...
MSA MSA::iterativePick(const MSIndexVector& pickIndex_) const
{
    ...
    for (unsigned i = 0; i < length; i++) {
        ...
        if ((*index)->r == 0)
            ...
            if ((*index)->r >= 1)
                index = (MSAStruct**)&((*index)->p[pickIndex_(i)]);
        }
    }
    return MSA(*index);
}
...
MSIntVector MSA::shape(void) const
{
    ...
    for (long i = 0; i < _aStructPtr->r; i++)
        aShape.replaceAt(i, _aStructPtr->d[i]);
    return aShape;
}
...
static long dp(MSAStruct* aobj_)
{
    ...
    if (t == MSA::STRINGTYPE)
        return (-1);
    if ((t == MSA::INTEGERTYPE) || (t == MSA::FLOATTYPE) || (t == MSA::CHARACTERTYPE))
        return (0);
    else {
        long i = 0;
        long xrho = (aobj_->n);
        for (; i < xrho; ++i) {
            if ((k = 1 + (dp((MSAStruct*)aobj_->p[i]))) > depth)
                depth = k;
        }
    }
    return depth;
...
long MSA::depth(void) const
{
    ...
    if (_aStructPtr != 0) {
        ...
        if (t == STRINGTYPE)
            depth = -1;
        else if (t == ETYPE && _aStructPtr->n != 0)
            depth = dp(_aStructPtr);
    }
    return depth;
}
...
long MSA::asLong(void) const
{
    long d = 0;
    if (_aStructPtr) {
        if (_aStructPtr->t == INTEGERTYPE)
            return ((long)*((long*)_aStructPtr->p));
        if (_aStructPtr->t == CHARACTERTYPE)
            return ((long)*((char*)_aStructPtr->p));
    }
    return d;
}
...
unsigned long MSA::asUnsignedLong(void) const
{
    ...
    if (_aStructPtr) {
        if (_aStructPtr->t == INTEGERTYPE)
            return ((unsigned long)*((long*)_aStructPtr->p));
        if (_aStructPtr->t == CHARACTERTYPE)
            return ((unsigned long)*((char*)_aStructPtr->p));
    }
    return d;
}
...
double MSA::asDouble(void) const
{
    double d = 0;
    if (_aStructPtr) {
        if (_aStructPtr->t == FLOATTYPE)
            return ((double)*((double*)_aStructPtr->p));
        if (_aStructPtr->t == INTEGERTYPE)
            return ((double)*((long*)_aStructPtr->p));
        if (_aStructPtr->t == CHARACTERTYPE)
            return ((double)*((char*)_aStructPtr->p));
    }
    return d;
}
...
MSInt MSA::asMSInt(void) const
{
    if (_aStructPtr) {
        if (_aStructPtr->t == INTEGERTYPE)
            return MSInt((int)*(long*)_aStructPtr->p);
        if (_aStructPtr->t == CHARACTERTYPE)
            return MSInt((int)*(char*)_aStructPtr->p);
    }
    return MSInt();
}
...
MSFloat MSA::asMSFloat(void) const
{
    if (_aStructPtr) {
        if (_aStructPtr->t == FLOATTYPE)
            return MSFloat((double)*((double*)_aStructPtr->p));
        if (_aStructPtr->t == INTEGERTYPE)
            return MSFloat((double)*((long*)_aStructPtr->p));
    }
    return MSFloat();
}
...
MSSymbol MSA::asMSSymbol(void) const
{
    if (_aStructPtr) {
        ...
        if (_aStructPtr->t == ETYPE && _aStructPtr->n != 0 && ((MSAStruct*)*_aStructPtr->p)->t == ETYPE && ((MSAStruct*)*_aStructPtr->p)->n != 0 && ((MSAStruct*)*(((MSAStruct*)_aStructPtr->p))->p)->t == STRINGTYPE) {
            return MSSymbol((const char*)((MSAStruct*)*(((MSAStruct*)_aStructPtr->p))->p)->p);
        }
    }
    return MSSymbol();
}
...
MSString MSA::asMSString(void) const
{
    if (_aStructPtr) {
        ...
        if (_aStructPtr->t == ETYPE && _aStructPtr->n != 0 && ((MSAStruct*)*_aStructPtr->p)->t == ETYPE && ((MSAStruct*)*_aStructPtr->p)->n != 0 && ((MSAStruct*)*(((MSAStruct*)_aStructPtr->p))->p)->t == STRINGTYPE) {
            return MSString((const char*)((MSAStruct*)*(((MSAStruct*)_aStructPtr->p))->p)->p, ((MSAStruct*)*(((MSAStruct*)_aStructPtr->p))->p)->n);
        }
    }
    return MSString();
}
...
MSFloatMatrix MSA::asMSFloatMatrix(void) const
{
    ...
    if (_aStructPtr && _aStructPtr->r >= 2) {
        if (_aStructPtr->t == FLOATTYPE)
            d = msaConvertData(_aStructPtr, (double*)0, (double*)0);
        else if (_aStructPtr->t == INTEGERTYPE)
            d = msaConvertData(_aStructPtr, (long*)0, (double*)0);
    }
    if (d != 0)
        return MSFloatMatrix(d, allButLastAxis(), lastAxis());
    else
        return MSFloatMatrix();
}
...
MSIntVector MSA::asMSIntVector(void) const
{
    ...
    if (_aStructPtr) {
        n = _aStructPtr->n;
        if (_aStructPtr->t == INTEGERTYPE)
            d = msaConvertData(_aStructPtr, (long*)0, (int*)0);
        else if (_aStructPtr->t == CHARACTERTYPE)
            d = msaConvertData(_aStructPtr, (char*)0, (int*)0);
    }
    if (d != 0)
        return MSIntVector(d, n);
    else
        return MSIntVector();
}
...
MSFloatVector MSA::asMSFloatVector(void) const
{
    ...
    if (d != 0)
        return MSFloatVector(d, n);
    else
        return MSFloatVector();
}
            ...
            if (_aStructPtr->r == 0 || _aStructPtr->r == 1) {
                ...
                if (_aStructPtr->r > 1) {
                    ...
                    for (unsigned i = 0; i < rows; i++)
                        d << MSString(((char*)_aStructPtr->p) + i * cols, cols);
                    return d;
                }
            }
        } else {
            ...
            if (_aStructPtr->t == ETYPE) {
                for (unsigned i = 0; i < n; i++) {
                    if (((MSAStruct*)(_aStructPtr->p[i]))->t == STRINGTYPE) {
                        ...
                        return MSSymbolVector(); // whole vector is illegal,not all CHARACTERTYPE
                }
                return d;
            }
        }
...
MSA::MSAplusType MSA::aPlusType(void) const
{
    if (_aStructPtr != 0) {
        ...
        case ETYPE: {
            if (_aStructPtr->n == 0 && _aStructPtr->r > 0)
                return MSANull;
            if (((MSAStruct*)_aStructPtr->p[0]) && ((MSAStruct*)_aStructPtr->p[0])->t == STRINGTYPE) {
                // this is a symbol with rank==0 or a symbol vector
                return MSASymbol;
            }
            if (((MSAStruct*)_aStructPtr->p[0]) && (((MSAStruct*)_aStructPtr->p[0])->t == INTEGERTYPE || ((MSAStruct*)_aStructPtr->p[0])->t == FLOATTYPE || ((MSAStruct*)_aStructPtr->p[0])->t == CHARACTERTYPE || ((MSAStruct*)_aStructPtr->p[0])->t == ETYPE))
                return MSABox;
            break;
        }
        }
...
MSAStruct* MSA::ic(MSAStruct* aobj_)
{
    ...
    if (aobj_ && QA(aobj_)) {
        if (aobj_->c == 0)
            r = gc(aobj_->t, aobj_->r, aobj_->n, aobj_->d, aobj_->p);
        else {
            ++r->c;
        }
    }
...
MSAStruct* MSA::extractpass(char** hpp, char** dpp, char* endp, long* erp, char* trp, int swap)
{
    ...
    *hpp = hp;
    ...
    if ((rt == 'I') && (rl == 4)) {
        ...
        for (i = 0; i < xrho; i++) {
            if (swap)
                ndn32copy(dp, (char*)&itmp, 1);
            else
                memcpy((char*)&itmp, dp, 4);
            ap[i] = itmp;
            dp += 4;
        }
        *dpp = dp;
    ...
    if ((rt == 'I') && (rl == 2)) {
        ...
        for (i = 0; i < xrho; i++) {
            if (swap)
                ndn16copy(dp, (char*)&stmp, 1);
            else
                memcpy((char*)&stmp, dp, 2);
            ap[i] = stmp;
            dp += 2;
        }
        *dpp = dp;
    ...
    if ((rt == 'E') && (rl == 4)) {
        ...
        for (i = 0; i < xrho; i++) {
            if (swap)
                ndn32copy(dp, (char*)&ftmp, 1);
            else
                memcpy((char*)&ftmp, dp, 4);
            ap[i] = ftmp;
            dp += 4;
        }
        *dpp = dp;
...
void MSA::fillExportBuffer(MSAStruct* aobj, char* hp, long hsz, char* trp, long for_a, int long_bytes)
{
    char* dp = hp + hsz;
...
MSA MSA::importAObject(char* cvp, long cvlen, char* trp)
{
    ...
    endp = cvp + cvlen;
    flag = *hp;
    if ((flag & 0xfC) != CDRFLAGSMAGIC)
        return MSA(); /* 0xfC == 11111100 */
    ...
    if ((flag & CDRFLAGSLITTLE) == 0)
        swap = 1;
#else
    if (flag & CDRFLAGSLITTLE)
        swap = 1;
#endif
...
{
    long i;
    double f, *fvec = (double*)aobj_->p;
    for (i = 0; i < aobj_->n; ++i) {
        if (fvec[i] != (f = rint(fvec[i])))
            return 1;
        ivec_[i] = (int)f;
    }
    return 0;
...
long MSA::gpi_num(MSAStruct* apick_, MSAStruct* aobj_)
{
    ...
    if (2 <= apick_->r || apick_->n != aobj_->r) {
        return (0);
    };
    if (FLOATTYPE == apick_->t) {
        if (gpu_fillivec(ivec, apick_)) {
            return (0);
        };
        ip = ivec;
    } else
    ...
    if (aobj_->d[0] <= (unsigned)(idx = ip[0])) {
        return (0);
    };
    for (i = 1; i < apick_->n; ++i) {
        if (aobj_->d[i] <= (unsigned)(ip[i])) {
            return (0);
        };
        idx *= aobj_->d[i];
        idx += ip[i];
...
long MSA::gpi_sym(long key_, MSAStruct* aobj_)
{
    long i, tt;
    MSAStruct *t0, *t1;
    if (ETYPE != aobj_->t || 2 != aobj_->n) {
        return (-1);
    }
    t0 = (MSAStruct*)aobj_->p[0];
    t1 = (MSAStruct*)aobj_->p[1];
    if ((!(0 == ((long)(t0)&7))) || (!(0 == ((long)(t1)&7))) || ETYPE != t1->t || t0->n != t1->n) {
        return (-1);
    }
    for (i = 0; i < t0->n; ++i) {
        tt = t0->p[i];
        if (!(STRINGTYPE == ((MSAStruct*)tt)->t)) {
            return (-1);
        }
        if (!strcmp((const char*)((MSAStruct*)key_)->p, (const char*)((MSAStruct*)tt)->p))
            return i;
    }
    return (-1);
}
...
MSAStruct* MSA::gp_scalar(long idx_, MSAStruct* aobj_)
{
    ...
    if ((unsigned)idx_ >= aobj_->n) {
        return (0);
    };
    if (STRINGTYPE == aobj_->t) {
        return (0);
    };
    switch (aobj_->t) {
    case INTEGERTYPE:
        return MSA::gi(aobj_->p[idx_]);
    case FLOATTYPE:
        return MSA::gf(((double*)aobj_->p)[idx_]);
    case CHARACTERTYPE:
        z = MSA::gs(CHARACTERTYPE);
        cvec = (char*)z->p;
        cvec[0] = ((char*)aobj_->p)[idx_];
        cvec[1] = '\0';
        return z;
    case ETYPE:
        return (MSAStruct*)pcki(idx_, aobj_);
    default:
        z = (MSAStruct*)MSA::gs(ETYPE);
        zz = MSA::gs(ETYPE);
        z->p[0] = (long)zz;
        zz->p[0] = aobj_->p[idx_];
        return z;
...
MSAStruct* MSA::gp_nested(MSAStruct* apick_, MSAStruct* aobj_)
{
    ...
    if (0 == apick_->n)
        return (MSAStruct*)MSA::ic(ares);
    for (i = 0; i < apick_->n; ++i) {
        atarg = (MSAStruct*)apick_->p[i];
        if ((!(0 == ((long)(ares)&7))) || ETYPE != ares->t) {
            return (0);
        };
        if (STRINGTYPE == atarg->t) {
            ...
            if (INTEGERTYPE == atarg->t || FLOATTYPE == atarg->t) {
                ...
                if (0 == atarg->n && 1 == ares->n) {
                    ares = (MSAStruct*)*ares->p;
                    continue;
                }
                if ((STRINGTYPE == atarg->t) || 1 != atarg->n) {
                    return (0);
                };
...
MSAStruct* MSA::gpick(MSAStruct* apick_, MSAStruct* aobj_)
{
    ...
    case FLOATTYPE:
        if (1 == apick_->n) {
            ...
            if (INTEGERTYPE == apick_->t)
                z = (MSAStruct*)pcki(*apick_->p, aobj_);
            else if (gpu_fillivec(&idx, apick_)) {
                return (0);
            } else
                z = (MSAStruct*)pcki(idx, aobj_);
        } else
            z = gp_num(apick_, aobj_);
        break;
    case ETYPE:
        z = gp_nested(apick_, aobj_);
        break;
    default: {
        return (0);
    }
        ...
        if (aStructPtr) {
            ...
            case STRINGTYPE:
                // we do't want any "naked" symbols so we need to enclose at top level
                {
                    ...
                    aStructPtr = ga(ETYPE, 0, 1, d);
                    if (aStructPtr != 0)
                        *aStructPtr->p = (long)tmpaStructPtr;
                }
                break;
            case ETYPE:
