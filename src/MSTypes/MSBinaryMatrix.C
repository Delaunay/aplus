                ...
                for (i = 0; i < _count; i++) {
                    if (startpos < slen) {
                        if (MSMSF_US != decode(startpos)) // can be next char in not MxN case
                        {
                            ...
                            set(i, (unsigned char)0); // make sure fills in on not MxN case
                    } else {
                        code = MSError::BadMSFString;
                        break;
                    }
...
MSError::ErrorStatus MSBinaryMatrix::set(unsigned index_, const char* pString_)
{
    ...
    if (cp == pString_)
        ...
        return set(index_, (unsigned char)(lnum == 0 ? 0 : 1));
}
...
long MSBinaryMatrix::compare(const MSBinaryMatrix& aBinaryMatrix_) const
{
    ...
    if (length() == aBinaryMatrix_.length())
        return 0;
    if (length() < aBinaryMatrix_.length())
        return -1;
    else
...
MSBinaryMatrix& MSBinaryMatrix::random(void)
{
    ...
    if (n > 0) {
        ...
        MSRandom rand;
        while (n--)
            *dp++ = (unsigned char)rand(2); // limit is always 2; nothing else makes sense
        changed();
    }
    return *this;
}
