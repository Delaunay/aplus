...
MSBoyerMoore::MSBoyerMoore(const MSString& sourceString_)
...
{
    if (_deltaTable == 0)
        _deltaTable = new unsigned[deltaTableSize()];
    unsigned i;
    if (searchPattern_.length() > 0) {
        if (searchPattern_ != searchPattern()) {
            _searchPattern = searchPattern_;
            for (i = 0; i < deltaTableSize(); i++)
                _deltaTable[i] = searchPattern().length();
            for (i = 1; i < searchPattern().length(); i++)
                _deltaTable[searchPattern_(i - 1)] = searchPattern().length() - i;
        ...
        if (searchPattern().length() > 0) {
            ...
            while (i <= slen) {
                ...
                while (pat[j - 1] == str[i - 1]) {
                    if (j > 1) {
                        --j, --i;
                    } // move left for next comparison
                    else
                        return i - 1; // we've reached the beginning of pat
                }
