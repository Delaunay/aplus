        ...
        while (!fin.eof()) {
            ...
            if (line.length() > 0) {
                ...
                if (position < line.length())
                    _parseAndAddHolidaySet(holidaySet(), line, line.subString(0, position), c);
            }
        }
        return MSTrue;
    } else {
    ...
    const MSResourceHolidaySet& aSet2_)
{
    ...
    while (tradeDate >= firstDayOfMonth) {
        if (_isValidTradeDate(tradeDate, aSet1_) == MSTrue && _isValidTradeDate(tradeDate, aSet2_) == MSTrue)
            return tradeDate;
        tradeDate--;
    }
    return tradeDate;
...
{
    if (startingDate_.isSet() == MSFalse)
        return MSDate::nullDate();
    ...
    if (_locateOrInstallHolidaySet(holidaySet(), resourceCode_, c) == MSTrue) {
        ...
        if (_isValidTradeDate(forwardDate, aSet) == MSTrue)
            return forwardDate;
        else {
            do {
                forwardDate++;
    ...
    if (holidayString_.length() > 0) {
        ...
        if (position < holidayString_.length()) {
            ...
            if (resourceCode.length() > 0 && resourceCode == rCode_) {
                ...
                if (set_.addOrReplaceElementWithKey(aResourceHolidaySet, c_) == MSTrue)
                    return MSTrue;
            }
        }
    }
...
{
    if (date_.isSet() == MSFalse)
        return MSFalse;
    MSDay day = date_.weekDay();
    if (day == MSDate::Saturday || day == MSDate::Sunday)
        return MSFalse;
    else
        return (_isHoliday(date_, set_) == MSTrue) ? MSFalse : MSTrue;
}
...
MSBoolean _isValidTradeDate(const MSDate& date_, MSHolidaySet& set_, const MSResourceCode& rCode_)
{
    ...
    if (day == MSDate::Saturday || day == MSDate::Sunday)
        ...
        return (_isHoliday(date_, set_, rCode_) == MSTrue) ? MSFalse : MSTrue;
}
...
MSBoolean _isValidTradeDate(const MSDate& date_, MSHolidaySet& set_, const MSResourceCodeSet& rCodeSet_)
{
    ...
    if (day == MSDate::Saturday || day == MSDate::Sunday)
        ...
        return (_isHoliday(date_, set_, rCodeSet_) == MSTrue) ? MSFalse : MSTrue;
}
