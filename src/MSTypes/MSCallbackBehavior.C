...
void MSCallbackBehavior::callback(const MSSymbol& name_, MSCallback* pCallback_)
{
    ...
    if ((node = callbackNode(name_)) != 0)
        node->callback(pCallback_);
    else {
        if (_pCallbackVector == 0)
            _pCallbackVector = new CallbackVector;
...
MSBoolean MSCallbackBehavior::activateCallback(const MSSymbol& name_)
{
    if (_pCallbackVector != 0) {
        ...
        {
            if (afterState == CallbackVector::Dirty)
                _pCallbackVector->removeMarkedElements();
...
{
    _callbackVector.append((unsigned long)node_);
    return *this;
}
