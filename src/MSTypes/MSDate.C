...
MSDate::MSDate(const MSString& aString_, const char* strptimeFormat_)
{
    ...
    if (pString == 0)
        ...
        _date = asJulianNumber(aTimeStruct.tm_mon + 1, aTimeStruct.tm_mday, aTimeStruct.tm_year + 1900);
#endif
}
...
{
    MSString aString;
    return MSString(format(aString, MSDate::defaultFormat()));
}
...
MSString MSDate::asMSF(void) const
{
    MSString buf;
    if (isSet() == MSTrue)
        (void)format(&buf, MSDate::Database);
    return buf;
}
    ...
    if (pString_ == 0)
        ...
        if (*pString_ == '\0' || strcmp(pString_, NullString) == 0 || strcmp(pString_, "/  /") == 0) {
            ...
            if (MSUtil::isNumeric(buf) == MSFalse) {
                if (MSUtil::isSpace(buf) == MSTrue)
                    _date = nullDate(); // May be full of spaces
                else
                    rc = MSError::BadDate;
            } else {
                // format of yyyymmdd
...
{
    if ((MSUtil::isNumeric(m_) == MSFalse) || (MSUtil::isNumeric(d_) == MSFalse) || (MSUtil::isNumeric(y_) == MSFalse)) {
        _date = nullDate();
...
{
    if (day_ <= 0 || assertIndexOfMonth(month_) == MSFalse)
        return MSFalse;
    unsigned d = _daysInMonth[month_];
    if (month_ == 2 && leapYear(year_) == MSTrue)
        d++;
...
{
    if (assertIndexOfMonth(month_) == MSFalse)
        return 0;
...
MSJulian MSDate::asJulianNumber(MSMonth month_, MSDay day_, MSYear year_)
{
    unsigned long c, ya;
    if (year_ <= 99)
        year_ += year_ <= MS_PIVOT_YEAR ? 2000 : 1900;
...
int MSDate::findMatch(const char* str_, const char** candidates_, int icand_)
{
    ...
    while (N--) {
        *up++ = islower(*str_) ? toupper(*str_) : *str_;
        str_++;
    }
    *up = '\0';
    while (icand_--) {
        if (strncmp(p, candidates_[icand_], len) == 0)
            break;
    }
    delete[] p;
#else
    while (icand_--) {
        if (strnicmp(str_, candidates_[icand_], len) == 0)
            break;
    }
#endif
    return icand_;
...
{
    if (assertIndexOfMonth(month) == MSFalse)
        return 0;
    unsigned firstDay = _firstDayOfEachMonth[month];
    if (month > 2 && leap() == MSTrue)
        firstDay++;
    return firstDay;
}
...
MSDay MSDate::lastDayOfMonth(MSMonth month_) const
{
    MSDay answer;
    if (month_ == 12)
        return leap() == MSTrue ? MSDay(366) : MSDay(365);
    else
        answer = firstDayOfMonth(month_ + 1);
...
MSDate MSDate::previous(MSDay desiredDayOfWeek_) const
{
    ...
    if (desiredDayOfWeek_ > thisDayOfWeek)
        thisDayOfWeek += 7 - desiredDayOfWeek_;
    else
        thisDayOfWeek -= desiredDayOfWeek_;
...
void MSDate::normalizeAndSet(int m_, int d_, int y_, int lastOfMonth_)
{
    ...
    if (lastOfMonth_)
        d_ = (m_ == 2) && (leapYear(y_) == MSTrue) ? 29 : _daysInMonth[m_];
    else if ((d_ > 28) && (m_ == 2))
        d_ = leapYear(y_) == MSTrue ? 29 : 28;
    _date = asJulianNumber(m_, d_, y_);
    changed();
}
...
{
    MSString s;
    return aStream_ << aDate_.format(s) << flush;
}
...
MSDate::MSDateLocale MSDate::initLocale(void)
{
    ...
    if (cp) {
        if (strcmp(cp, "japanese") == 0)
            return MSDate::Japanese;
        else if (strcmp(cp, "european") == 0)
            return MSDate::European;
        else
