...
MSString MSFloat::asMSF(void) const
{
    MSString aString;
    if (isSet() == MSTrue)
        (void)formatReal(aString, MSFloat::MaximumPrecision, MSFormat::NoModifier, _real);
    return aString;
}
...
MSError::ErrorStatus MSFloat::internalSet(const char* pString_)
{
    ...
    if (*pString_ == '\0') {
        ...
        return (MSError::BadReal);
    } // Disallow commas after the decimal point
    else if (*pString_ == ',') {
        return (MSError::BadReal);
    ...
    if (strchr(buf, ',') != NULL) {
        ...
        if ((cp2 = strchr(buf, '.')) == NULL) {
            cp2 = buf;
            cp2 += strlen(buf);
        } // no decimal point, go for the end of the string
    ...
    if (sscanf(buf, "%d %d/%d", &units, &numerator, &denominator) == 3) {
        ...
        if (denominator > 0)
            _real = double(numerator) / double(denominator);
        else
            return (MSError::BadReal);
    } else
        _real = atof(buf);
...
{
    return (aFormat_.formatType() == MSFormat::Float) ? formatReal(aString_, aFormat_.floatFormat(), aFormat_.formatModifier(), _real) : format(aString_);
}
...
static void addCommas(register char* inBuffer_, register char* outBuffer_)
{
    ...
    if ((cp == NULL) || ((places = cp - inBuffer_) <= 3))
        strcpy(outBuffer_, inBuffer_);
    else {
