...
{
    double* dp = data();
    for (unsigned i = target_; moveCount_ > 0; i++, moveCount_--)
        dp[i] = dp[i + 1];
}
...
{
    double* dp = data();
    for (unsigned i = target_ + moveCount_ - 1; moveCount_ > 0; i--, moveCount_--)
        dp[i] = dp[i - 1];
}
...
MSError::ErrorStatus MSTypeMatrix<double>::setFromMSF(const char* pString_)
{
    ...
    if ((pString_ != 0 && (*pString_ == MSMSF_US) && (strlen(pString_) > sizeof(MSMSF_US)))) {
        ...
        for (i = 0; i < 2; i++) {
            value[i] = 0;
            if (isdigit(*pcurrent))
                value[i] = strtoul(pcurrent, &pstring, 10);
            if (*pstring != '\0') {
                ...
                for (i = 0; i < _count; i++) {
                    if (startpos < slen) {
                        if (MSMSF_US != decode(startpos)) // can be next char in not MxN case
                        {
                            ...
                            set(i, double(0)); // make sure fills in on not MxN case
                    } else {
                        code = MSError::BadMSFString;
                        break;
                    }
...
{
    unsigned n = length();
    for (unsigned i = startPos_; i < n; i++)
        if (elementAt(i) == aValue_)
            return i;
    return n;
}
...
{
    if (aColumn_ + 1 > columns() || bColumn_ + 1 > columns() || aColumn_ == bColumn_)
        return *this;
...
{
    if (aRow_ + 1 > rows() || bRow_ + 1 > rows() || aRow_ == bRow_)
        return *this;
...
MSTypeMatrix<double>& MSTypeMatrix<double>::assignColumn(unsigned col_, double scalar_)
{
    ...
    if (receiverList() != 0) {
        ...
        for (i = 0, j = col_; i < rows(); i++, j += columns())
            mp[j] = scalar_, iv.set(i, j);
        changed(iv);
    } else
        for (i = 0, j = col_; i < rows(); i++, j += columns())
            mp[j] = scalar_;
...
MSTypeMatrix<double>& MSTypeMatrix<double>::assignRow(unsigned row_, double scalar_)
{
    ...
    double* mp = data();
    unsigned i, j;
    for (i = row_ * columns(), j = 0; j < columns(); i++, j++)
        mp[i] = scalar_;
    if (receiverList() != 0) {
        MSIndexVector iv;
...
MSTypeMatrix<double>& MSTypeMatrix<double>::assignColumn(unsigned col_, const MSTypeVector<double>& aTypeVector_)
{
    ...
    double* mp;
    unsigned i;
    for (i = 0, mp = data() + col_; i < rows(); i++, mp += columns())
        *mp = aTypeVector_(i);
    changed();
    return *this;
}
...
MSTypeMatrix<double>& MSTypeMatrix<double>::assignRow(unsigned row_, const MSTypeVector<double>& aTypeVector_)
{
    ...
    double* mp;
    unsigned i;
    for (i = 0, mp = data() + row_ * columns(); i < columns(); i++, mp++)
        *mp = aTypeVector_(i);
    if (receiverList() != 0) {
        MSIndexVector iv;
        changed(iv.series(columns(), row_ * columns()));
...
MSTypeMatrix<double>& MSTypeMatrix<double>::rotateColumns(int position_)
{
    ...
    if (columnPosition > 0 && columnPosition != columns()) {
        ...
        double* dp = d->elements();
        for (j = 0; j < rows(); j++) {
            for (i = columnPosition; i < columns(); i++)
                *dp++ = *mp++;
            mp -= columns();
    ...
    if (numberOfColumns > 0 && numberOfColumns != columns()) {
        ...
        if (numberOfColumns > columns()) {
            if (numberOfColumns_ > 0) {
                ...
                for (i = 0; i < rows(); i++) {
                    for (j = 0; j < numberOfColumns; j++)
                        *dp++ = j < numberOfColumns - columns() ? 0 : *mp++;
                }
            }
        } else {
            if (numberOfColumns_ > 0) {
...
{
    if (row_ + 1 > rows() || columns() == 0)
        return MSTypeVector<double>();
...
MSTypeVector<double> MSTypeMatrix<double>::columnAt(unsigned column_) const
{
    ...
    double* dp = d->elements();
    unsigned i;
    for (i = 0, mp = data() + column_; i < rows(); i++, mp += columns())
        *dp++ = *mp;
