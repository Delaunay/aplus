...
MSFormat::MSFormat(void)
    : _formatType(MSFormat::NoFormat)
    , _formatModifier(MSFormat::NoModifier)
{
    _format._any = 0;
}
...
MSBoolean MSFormat::operator!=(const MSFormat& aFormat_) const
{
    if (_formatType != aFormat_._formatType)
        return MSTrue;
    else if (_formatModifier != aFormat_._formatModifier)
        return MSTrue;
    else if (_format._any != aFormat_._format._any)
        return MSTrue;
    return MSFalse;
}
