...
template <class Type>
MSMutex MSGenericVector<Type>::_operationsMutex;
template <class Type>
MSMutex MSGenericVectorOps<Type>::_nullDataMutex;
#endif
...
template <class Type>
MSGenericVector<Type>::MSGenericVector(MSVectorImpl* pImpl_)
    : _pImpl(pImpl_)
{
}
...
unsigned MSGenericVectorOps<Type>::mergeSortDown(unsigned n_, Type* sp_, unsigned* p_, unsigned low_, unsigned high_)
{
    ...
    if (indexCompareDown(sp_, high_, low_)) {
        m = low_;
        low_ = high_;
        high_ = m;
    }
    ...
    L:
        ...
        if (indexCompareDown(sp_, m, high_)) {
            low_ = m;
            goto L;
        }
    }
...
template <class Type>
MSGenericData<Type>::MSGenericData(unsigned int size_)
    : MSData(size_)
{
}
