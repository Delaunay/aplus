...
MSHashEntry* MSHashTable::addElement(unsigned long key_, unsigned whichBucket_)
{
    ...
    if (bucket(whichBucket_) != 0)
        bucket(whichBucket_)->prev(entry);
...
MSBoolean MSHashTable::remove(unsigned long key_)
{
    ...
    MSHashEntry* entry = searchBucketFor(bucket(whichBucket), key_);
    if (entry != 0) {
        if (bucket(whichBucket) == entry)
            bucket(whichBucket, entry->next());
        delete entry;
        return MSTrue;
    }
    return MSFalse;
...
{
    while (entry_ != 0) {
        if (strcmp(entry_->stringKey(), key_) == 0)
            return entry_;
        entry_ = entry_->next();
    }
    return entry_;
...
void MSHashTable::addEntry(MSHashEntry* entry_)
{
    ...
    if (bucket(whichBucket) != 0)
        bucket(whichBucket)->prev(entry_);
...
//#######################################################################
// MSStringHashTable
MSStringHashTable::MSStringHashTable(unsigned size_)
    : MSHashTable(size_)
{
}
