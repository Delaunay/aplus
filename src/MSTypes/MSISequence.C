...
{
    MSICollectionCheck((this != &sequence), "identical collection") for (Node* node = sequence.ivFirst; node != 0; node = node->ivNext)
    {
    ...
    allElementsDo(MSBoolean (*function)(Element&, void*), void* env)
{
    for (Node* node = ivFirst; node != 0; node = node->ivNext) {
        if (!ivOps.functionIteration((void*)function, env, node))
            return MSFalse;
    }
    return MSTrue;
}
...
{
    for (Node* node = ivFirst; node != 0; node = node->ivNext) {
        if (!ivOps.constantFunctionIteration((void*)function, env, node))
            return MSFalse;
    }
    return MSTrue;
}
...
inline MSBoolean MSISequence<Element>::isConsistent() const
{
    ...
    if (ivFirst != 0 && ivFirst->ivPrevious != 0)
        return MSFalse;
...
void MSISequence<Element>::removeAt(MSISequence<Element>::Node* node)
{
    if (node == ivFirst)
        ivFirst = node->ivNext;
    if (node == ivLast)
        ivLast = node->ivPrevious;

    if (node->ivNext != 0)
        node->ivNext->ivPrevious = node->ivPrevious;
    if (node->ivPrevious != 0)
        node->ivPrevious->ivNext = node->ivNext;
...
{
    if (node == 0)
        newNode->ivNext = ivFirst;
    else
        newNode->ivNext = node->ivNext;

    newNode->ivPrevious = node;

    if (newNode->ivNext != 0)
        newNode->ivNext->ivPrevious = newNode;

    if (newNode->ivPrevious != 0)
        newNode->ivPrevious->ivNext = newNode;
    ...
    for (last = ivLast; last != ivFirst; last = last->ivPrevious) {
        for (current = ivFirst; current != last;) {
            ...
            if (0 < ivOps.functionComparison((void*)comparisonFunction,
                ...
                if (current->ivNext != 0)
                    current->ivNext->ivPrevious = current;
                if (next->ivPrevious != 0)
                    next->ivPrevious->ivNext = next;
