...
MSIndexVector::MSIndexVector(const MSIndexVector& vect_)
    : MSVector()
    , _pImpl(new MSBuiltinVectorImpl(*vect_._pImpl))
{
    // ASSERTION:  pOps has been initialized already at latest when vect_ was created
}

MSIndexVector::MSIndexVector(unsigned int length_)
    : MSVector()
{
    _pImpl = new MSBuiltinVectorImpl(&ops(), &ops(), length_);
}

MSIndexVector::MSIndexVector(unsigned int length_, const unsigned int filler_)
    : MSVector()
{
    _pImpl = new MSBuiltinVectorImpl(&ops(), &ops(), length_, (void*)&filler_);
}

MSIndexVector::MSIndexVector(const char* pString_)
    : MSVector()
{
    _pImpl = new MSBuiltinVectorImpl(&ops(), &ops());
...
MSIndexVector::MSIndexVector(MSIndexVector::Data* pData_, unsigned int len_)
    : MSVector()
{
    _pImpl = new MSBuiltinVectorImpl(&ops(), &ops(), (void*)pData_, len_);
}

MSIndexVector::MSIndexVector(const unsigned int* pElements_, unsigned int len_)
    : MSVector()
{
...
MSIndexVector::MSIndexVector(MSBuiltinVectorImpl* pImpl_)
    : MSVector()
    , _pImpl(pImpl_)
{
}
...
MSIndexVector::Data::Data(unsigned int size_)
    : MSData(size_)
{
}
