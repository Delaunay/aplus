...
MSError::ErrorStatus MSInt::set(const MSString& aString_)
{
    ...
    if (aString.indexOf('.') < aString.length())
        ...
        if (slen == 0)
            ...
            if (slen > MSInt::MaximumLength + 1)
                code = MSError::IntTooBig;
        } else if (slen > MSInt::MaximumLength)
            code = MSError::IntTooBig;
        if (code == MSError::MSSuccess) {
            _int = strtol(aString.string(), &pReturnVal, 10);
            if (*pReturnVal != 0)
                _int = 0, code = MSError::BadInt;
        }
    }
    return changed(), code;
}
...
{
    return (aFormat_.formatType() == MSFormat::Int) ? formatInt(aString_, aFormat_.intFormat(), aFormat_.formatModifier(), _int) : format(aString_, MSInt::WithoutCommas);
}
