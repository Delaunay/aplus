    ...
    if (aMMap.beamIn(fileName_) == MSTrue) {
        if (aMMap.type() == type_) {
            if (aMMap.rank() == 1) {
                ...
                MSMessageLog::errorMessage("Unable to map data: %s - rank != 1.\n", fileName_);
        } else
            MSMessageLog::errorMessage("Unable to map data: %s - incorrect type.\n", fileName_);
    }
    if (d != 0 && count > 0) {
    ...
    if (aMMap.beamIn(fileName_) == MSTrue) {
        if (aMMap.type() == type_) {
            if (aMMap.rank() == 2) {
                ...
                MSMessageLog::errorMessage("Unable to map data: %s - rank != 2.\n", fileName_);
        } else
            MSMessageLog::errorMessage("Unable to map data: %s - incorrect type.\n", fileName_);
    }
    if (d != 0 && count > 0) {
    ...
    unsigned int rows_, unsigned int columns_, int rank_)
{
    ...
    if (fileName_ != 0) {
        ...
        if (fd != -1) {
            ...
            for (; wc != -1 && n > 0; d += wc, n -= wc)
                wc = write(fd, d, n);

            fsync(fd);
            close(fd);
        } else
            MSMessageLog::errorMessage("Unable to map data: %s - cannot open file\n", fileName_);
    }
}
