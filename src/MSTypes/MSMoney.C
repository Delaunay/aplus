        ...
        if (buff != 0) {
            ...
            if (found == 1) {
                ...
                if (code == MSError::MSSuccess)
                    _currency = isoConvert(&pString_[i + sizeof(MSMSF_US)]);
            } else
...
static char* lookupFraction(FractionTable* fractionTable_, double fractionValue_)
{
    ...
    while (low <= high) {
        mid = (low + high) / 2;
        if (fractionValue_ < fractionTable_->table[mid].value)
            high = mid - 1;
        else if (fractionValue_ > fractionTable_->table[mid].value)
            low = mid + 1;
        else
            break; // found it
    }
...
{
    if (aFormat_.formatType() == MSFormat::Money)
        return format(aString_, aFormat_.moneyFormat(), justify_);
    else
    ...
    MSBoolean justify_) const
{
    ...
    case CommaDecimal7:
    case CommaDecimal8:
        MSFloat::format(aString_, (MSFloatFormat)moneyFormat_);
        break;
    case Eights:
        if (justify_)
            fieldWidth = 3;
        ...
        if (mantisa != 0)
            sprintf(buf, "%d %-*s", mantisa, fieldWidth, fract_str);
        else {
            if (fraction < 0.0)
                sprintf(buf, "-%-*s", fieldWidth, fract_str);
            else
                sprintf(buf, "%-*s", fieldWidth, fract_str);
        }
        aString_ = buf;
    ...
    case TwoFiftySixths: {
        ...
        if (rounded == fraction) {
            ...
            if (mantisa != 0)
                sprintf(buf, "%d %-*s", mantisa, fieldWidth, (char*)fract_str);
            else {
                if (fraction < 0.0)
                    sprintf(buf, "-%-*s", fieldWidth, fract_str);
                else
            ...
            // use decimal format
            if (justify_)
                fieldWidth = 5; // align decimal point with space
...
MSMoney::MSMoney(const MSMoney& a_, const MSMoney& b_, MSFloat::FloatOperator operator_)
    : MSFloat(a_, b_, operator_)
    , _currency(a_._currency) // assumes a_._currency==b_._currency
{
}

MSMoney::MSMoney(const MSMoney& a_, const MSFloat& b_, MSFloat::FloatOperator operator_)
    : MSFloat(a_, b_, operator_)
    , _currency(a_._currency)
{
}
