...
void MSObservableTree<Element>::removeSubtree(const MSTabularTreeCursor<Element>& cursor_)
{
    ...
    if (cursor2.isValid())
        changed(cursor2, pos, MSObservableTreeDelete);
    else
        changed();
}
    ...
    const MSTabularTreeCursor<Element>& cursor_)
{
    ...
    if (cursor2.isValid())
        tree_.changed(cursor2, p, MSObservableTreeDelete);
    else
        tree_.changed(); // whole tree is deleted
}
    ...
    MSTabularTreeCursor<Element> const& cursorFrom_)
{
    ...
    if (cursor2.isValid())
        tree_.changed(cursor2, pos2, MSObservableTreeDelete);
    else
        tree_.changed(); // whole tree is deleted
}
