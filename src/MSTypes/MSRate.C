...
{
    MSString s;
    return MSString(format(s, MSRate::Percent4));
}
...
MSError::ErrorStatus MSRate::set(const char* pString_)
{
    ...
    if ((len > 1) && (((pString_[len - 2] == 'b') && (pString_[len - 1] == 'p')) || ((pString_[len - 2] == 'B') && (pString_[len - 1] == 'P')))) {
        MSString copy(pString_);
        copy.truncate(2);
        MSError::ErrorStatus status = MSFloat::internalSet(copy.string());
        if (status != MSError::MSSuccess)
    ...
    if (MSFloat::internalSet(pString_) != MSError::MSSuccess)
        return MSError::BadRate;
    return changed(), MSError::MSSuccess;
}
...
const char* MSRate::format(MSString& aString_, MSRateFormat format_) const
{
    ...
    case CommaDecimal7:
    case CommaDecimal8:
        return formatReal(aString_, MSFloatFormat(format_), MSFormat::NoModifier, _real);
    case Percent0:
        sprintf(buf, "%.0f%%", _real * 100);
        break;
    case Percent1:
        sprintf(buf, "%.1f%%", _real * 100);
        break;
    case Percent2:
        sprintf(buf, "%.2f%%", _real * 100);
        break;
    case Percent3:
        sprintf(buf, "%.3f%%", _real * 100);
        break;
    case Percent4:
        sprintf(buf, "%.4f%%", _real * 100);
        break;
    case Percent5:
        sprintf(buf, "%.5f%%", _real * 100);
        break;
    case BasisPoint:
        sprintf(buf, "%.0fbp", _real * 10000);
        break;
    default:
        MSError::error(MSError::MSFailure, "MSRate::MSRateFormat", "Invalid Value");
...
{
    MSString aString;
    return aStream_ << aRate_.format(aString, MSRate::Percent4);
}
