...
istream& operator>>(istream& aStream_, MSString& aString_)
{
    ...
    while (!aStream_.fail()) {
        ...
        if (isspace(aChar))
            ...
            if (bytes == result.length())
                result += MSString(0, bytes);
...
*   class MSString.                                                            *
*******************************************************************************/
MSString::MSString(const void* pBuffer, unsigned len, char padCharacter)
    : _pBuffer(MSString::defaultBuffer())
{
...
MSString::MSString(int n)
    : _pBuffer(MSString::defaultBuffer())
{
    initBuffer((long)n);
}

MSString::MSString(long n)
    : _pBuffer(MSString::defaultBuffer())
{
    initBuffer(n);
}

MSString::MSString(short n)
    : _pBuffer(MSString::defaultBuffer())
{
    initBuffer((long)n);
}

MSString::MSString(unsigned n)
    : _pBuffer(MSString::defaultBuffer())
{
    initBuffer((unsigned long)n);
}

MSString::MSString(unsigned long n)
    : _pBuffer(MSString::defaultBuffer())
{
    initBuffer(n);
}

MSString::MSString(unsigned short n)
    : _pBuffer(MSString::defaultBuffer())
{
    initBuffer((unsigned long)n);
}

MSString::MSString(double aDouble)
    : _pBuffer(MSString::defaultBuffer())
{
    initBuffer(aDouble);
}

MSString::MSString(char aChar)
    : _pBuffer(MSString::defaultBuffer())
{
    initBuffer((const void*)&aChar, 1, 0, 0, 0, 0, 0);
}

MSString::MSString(unsigned char aChar)
    : _pBuffer(MSString::defaultBuffer())
{
    initBuffer((const void*)&aChar, 1, 0, 0, 0, 0, 0);
}

MSString::MSString(signed char aChar)
    : _pBuffer(MSString::defaultBuffer())
{
    initBuffer((const void*)&aChar, 1, 0, 0, 0, 0, 0);
}

MSString::MSString(const char* p)
    : _pBuffer(MSString::defaultBuffer())
{
    initBuffer((const void*)p, lengthOf(p), 0, 0, 0, 0, 0);
}

MSString::MSString(const unsigned char* p)
    : _pBuffer(MSString::defaultBuffer())
{
    initBuffer((const void*)p, lengthOf((const char*)p), 0, 0, 0, 0, 0);
}

MSString::MSString(const signed char* p)
    : _pBuffer(MSString::defaultBuffer())
{
    initBuffer((const void*)p, lengthOf((const char*)p), 0, 0, 0, 0, 0);
}
...
MSBoolean MSString::isLike(const char* pPattern, unsigned patternLen, char zeroOrMore, char anyChar) const
{
    ...
    if (_pBuffer->compare(pPattern, patternLen) == MSStringBuffer::equal)
        return MSTrue;
    // Index into strings:
...
MSBoolean MSString::isAbbrevFor(const char* pFullString, unsigned fullStringLen, unsigned minLen) const
{
    if (minLen == 0)
        minLen = length();
    if (length() <= fullStringLen && length() >= minLen && memcmp(data(), pFullString, length()) == 0)
        return MSTrue;
    else
        return MSFalse;
}
...
MSString::CharPick MSString::operator[](unsigned index)
{
    ...
    if (index >= _pBuffer->length()) { // It does,
        MSStringBuffer* oldBuffer = _pBuffer;
        initBuffer(oldBuffer->contents(), oldBuffer->length(), 0, index - oldBuffer->length() + 1, 0, 0, ' ');
        oldBuffer->removeRef();
    } else { // It doesn't,but make sure we've got our own copy.
        prepareToChange();
    }
...
{
    unsigned count = 0;
    while ((startPos = _pBuffer->indexOf(aTest, startPos)) < _pBuffer->length())
        count++, startPos++;
    return count;
}
...
Words::Words(const MSString& aString)
    : p(new WordOccurrence(0))
    , count(0)
{
...
unsigned MSString::findPhrase(const MSString& aPhrase, unsigned startWord, IndexType charOrWord) const
{
    ...
    while (stringIndex + phraseWords.count <= stringWords.count) {
        ...
        while (j < phraseWords.count) {
            if (stringWords[stringIndex + j].len != phraseWords[j].len)
                break;
        ...
        if (j >= phraseWords.count)
            return ((charOrWord == CharIndex) ? stringWords[stringIndex].pos : stringIndex);
        else
            stringIndex++; // Loop exit was due to mismatch, advance index.
    }
    // No match was found.
...
unsigned MSString::indexOfWord(unsigned wordNumber, unsigned startPos, unsigned numWords) const
{
    ...
    while (startPos < length() && result == length()) {
        ...
        if (startPos < length()) {
            if (numWords == wordNumber)
                result = startPos; // This is the word we are looking for:
            else
                startPos = indexOfAnyOf(whiteSpace, startPos); // Skip to end of this word:
            numWords++;
        }
    }
    return result;
...
MSString MSString::word(unsigned wordNumber) const
{
    ...
    unsigned end = 0;
    if (start < length())
        end = indexOfAnyOf(whiteSpace, start);
    if (end > start)
        return MSString(data() + start, end - start);
    else
        return MSString::null;
}
...
MSString MSString::words(unsigned firstWord, unsigned numWords) const
{
    ...
    if (start < length()) {
        if (numWords != 0) {
            // Check for overflow...
            if (numWords - 1 > UINT_MAX - firstWord)
                numWords = UINT_MAX - firstWord + 1; // Use maximum possible.
            unsigned end = indexOfWord(firstWord + numWords - 1); // Get index of last word:
            // If found, get index of end of last word:
            if (end == length())
                end = lastIndexOfAnyBut(whiteSpace) + 1; // Word not found,use last word.
...
void MSString::decimalMath(unsigned char newDigit)
{
    ...
    if (n == length())
        n = oldBuffer->length(); // All digits are zeros.
