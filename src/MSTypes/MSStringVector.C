...
static int __dummy01__()
{
    if (0)
        operator<<(cout, MSBaseVector<MSString, MSVectorModelAllocator<MSString>>());
    return 0;
}
...
MSTypeVector<MSString>::MSTypeVector(unsigned int length_)
    : ObjectVectorString(length_)
{
}
...
MSTypeVector<MSString>::MSTypeVector(const MSTypeVector<MSString>& vect_)
    : ObjectVectorString(vect_)
{
}

MSTypeVector<MSString>::MSTypeVector(const BaseVectorString& vect_)
    : ObjectVectorString(vect_)
{
}

MSTypeVector<MSString>::MSTypeVector(const char* pString_, const char delimiter_)
    : ObjectVectorString(pString_, delimiter_)
{
}
