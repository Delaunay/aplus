    ...
    MSTabularTreeNode<Element>* node)
{
    ...
    if (position > numChildren) {
        ...
        if (pNodes == 0)
            MSTKTHROWEXCEPTION(MSTabularTreeOutOfMemory(MSTabularTreeOutOfCollectionMemoryText));
        unsigned long i;
        for (i = 0; i < numChildren; i++)
            pNodes[i] = node->_pChildren[i];
        for (i = numChildren; i < newNumChildren; i++)
            pNodes[i] = 0;
        if (node->_pChildren != 0)
            delete[] node->_pChildren;
...
unsigned long MSTabularTree<Element>::removeSubtree(MSTabularTreeNode<Element>* node)
{
    ...
    for (unsigned long n = 0; n < node->_numberOfChildren; n++) {
        if (node->_pChildren[n] != 0)
            result += removeSubtree(node->_pChildren[n]);
    }
    delete node;
    return result + 1;
}
...
{
    return ((MSTabularTreeConstantIterator<Element>*)iterator)->applyTo(((MSTabularTreeNode<Element> const*)node)->_element);
}

template <class Element>
MSBoolean MSTabularTree<Element>::iteratorIteration(void* iterator, void* node)
{
    return ((MSTabularTreeIterator<Element>*)iterator)->applyTo(((MSTabularTreeNode<Element>*)node)->_element);
}
    ...
    MSTabularTreeIterationOrder order) const
{
    ...
    if (order == MSPostorder) {
        ...
        while (setToFirstExistingChild(firstChild) == MSTrue)
            node = firstChild;
    }
    return MSTrue;
}
    ...
    MSTabularTreeIterationOrder order) const
{
    if (order == MSPreorder) {
        ...
        if (setToNextExistingChild(nextChild) == MSTrue) {
            node = nextChild;
            while (setToFirstExistingChild(nextChild) == MSTrue)
                node = nextChild;
            return MSTrue;
        } else
            return setToParent(node);
    }
}
    ...
    MSTabularTreeIterationOrder order) const
{
    ...
    if (order == MSPreorder) {
        ...
        while (setToLastExistingChild(lastChild) == MSTrue)
            node = lastChild;
    }
    return MSTrue;
}
    ...
    MSTabularTreeIterationOrder order) const
{
    if (order == MSPostorder) {
        ...
        if (setToPreviousExistingChild(previousChild) == MSTrue) {
            node = previousChild;
            while (setToLastExistingChild(previousChild) == MSTrue)
                node = previousChild;
            return MSTrue;
        } else
            return setToParent(node);
...
unsigned long MSTabularTree<Element>::numberOfSubtreeElements(MSTabularTreeNode<Element> const* node) const
{
    ...
    for (unsigned long n = 0; n < node->_numberOfChildren; n++) {
        if (node->_pChildren[n] != 0)
            result += numberOfSubtreeElements(node->_pChildren[n]);
    }
    return result + 1;
}
...
unsigned long MSTabularTree<Element>::numberOfSubtreeLeaves(MSTabularTreeNode<Element> const* node) const
{
    ...
    for (unsigned long n = 0; n < node->_numberOfChildren; n++) {
        if (node->_pChildren[n] != 0)
            result += numberOfSubtreeLeaves(node->_pChildren[n]);
    }
    // node is a leaf
    if (result == 0)
        result = 1;
...
void MSTabularTree<Element>::attachSubtreeAsRoot(MSTabularTree<Element>& tree,
    MSTabularTreeNode<Element>* node)
{
    if (node->_pParentNode == 0)
        tree._pRootNode = 0;
    else
        node->_pParentNode->_pChildren[tree.position(node) - 1] = 0;
    _pRootNode = node;
    if (_pRootNode != 0)
...
void MSTabularTree<Element>::attachSubtreeAsChild(MSTabularTreeNode<Element>* parent,
    ...
    MSTabularTreeNode<Element>* from)
{
    if (from->_pParentNode == 0)
        tree._pRootNode = 0;
    else
        from->_pParentNode->_pChildren[tree.position(from) - 1] = 0;
    parent->_pChildren[pos - 1] = from;
    if (from != 0)
        from->_pParentNode = parent;
}
...
{
    if (order == MSPreorder) {
        if (!(*apply)(iterator, subtreeRoot))
            return MSFalse;
        for (int i = 0; i < subtreeRoot->_numberOfChildren; i++) {
            MSTabularTreeNode<Element>* child = subtreeRoot->_pChildren[i];
            if (child != 0) {
                if (!allElementsDo(iterator, order, apply, child))
                    return MSFalse;
            }
        }
    } else {
...
{
    if (_pRootNode != 0)
        return checkNode(node, _pRootNode);
    return MSFalse;
}
    ...
    MSTabularTreeNode<Element> const* current) const
{
    ...
    for (int i = 0; i < current->_numberOfChildren; i++) {
        if (current->_pChildren[i] != 0 && checkNode(node, current->_pChildren[i]))
            return MSTrue;
    }
    return MSFalse;
}
...
MSBoolean MSTabularTree<Element>::isConsistent(MSTabularTreeNode<Element> const* node) const
{
    for (int i = 0; i < node->_numberOfChildren; i++) {
        if (node->_pChildren[i] != 0 && (node->_pChildren[i]->_pParentNode != node || isConsistent(node->_pChildren[i]) == MSFalse))
            return MSFalse;
    }
    return MSTrue;
}
...
void MSTabularTree<Element>::checkCursor(MSTabularTreeCursor<Element> const& cursor) const
{
    ...
    if (cursor.isValid() == MSFalse)
        MSTKTHROWEXCEPTION(MSTabularTreeCursorInvalid(MSTabularTreeInvalidCursorText));
...
{
    if (_pRootNode != 0)
        MSTKTHROWEXCEPTION(MSTabularTreeRootAlreadyExists(MSTabularTreeRootAlreadyExistsText));
}
...
{
    if (n != 0)
        MSTKTHROWEXCEPTION(MSTabularTreeChildAlreadyExists(MSTabularTreeChildAlreadyExistsText));
}
...
unsigned long MSTabularTree<Element>::removeSubtree(MSTabularTreeCursor<Element> const& cursor)
{
    ...
    if (node->_pParentNode == 0)
        ...
        node->_pParentNode->_pChildren[position(cursor) - 1] = 0;
    ...
    const MSIndexVector& index_)
{
    ...
    for (i = 0; i < n; i++) {
        if (parent->_pChildren[i] != 0)
            removeSubtree(parent->_pChildren[i]);
    }
