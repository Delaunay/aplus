...
{
    MSString aString;
    return aStream_ << aTerm_.format(aString);
}
...
{
    MSString aString;
    return MSString(format(aString));
}
...
void MSTerm::getToken(const char* pString_, int* pos_, Token* token_, int* value_)
{
    ...
    while (isspace(pString_[*pos_]) || pString_[*pos_] == ',')
        (*pos_)++;

    // Are we at the end?
    if (*pos_ >= strlen(pString_)) {
        *token_ = END;
        return;
    }
    ...
    if (!isdigit(pString_[*pos_])) {
        *token_ = BAD;
        return;
    }

    while (isdigit(pString_[*pos_]))
        (*pos_)++;

    *value_ = atoi(&pString_[start]);

    if (pString_[*pos_] == 'y' || pString_[*pos_] == 'Y') {
        (*pos_)++;
        *token_ = YEARS;
        return;
    } else if (pString_[*pos_] == 'm' || pString_[*pos_] == 'M') {
        (*pos_)++;
        *token_ = MONTHS;
        return;
    } else if (pString_[*pos_] == 'w' || pString_[*pos_] == 'W') {
        (*pos_)++;
        *token_ = WEEKS;
        return;
    } else if (pString_[*pos_] == 'd' || pString_[*pos_] == 'D') {
        (*pos_)++;
        *token_ = DAYS;
        return;
    } else {
...
MSError::ErrorStatus MSTerm::set(const char* pString_)
{
    ...
    for (getToken(pString_, &pos, &token, &value); token != END; getToken(pString_, &pos, &token, &value)) {
        if (token == BAD) {
            return changed(), MSError::BadTerm;
        } else if (token == YEARS) {
            if (yearsDone)
                return changed(), MSError::BadTerm;
            else {
...
{
    return (aFormat_.formatType() == MSFormat::Term) ? format(aString_, aFormat_.termFormat()) : format(aString_);
}
