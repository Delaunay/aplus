...
MSTime::MSTime(const MSString& aString_, const char* strptimeFormat_, MSTimeZone zone_)
{
    ...
    // ourselves
    //
    if (isYearSet && tms.tm_year <= MS_PIVOT_YEAR)
        tms.tm_year += 100;
...
MSError::ErrorStatus MSTime::internalSet(const char* cp_, MSTimeZone zone_)
{
    ...
    if (cp_ == 0)
        ...
        case 3:
            ...
            //
            _time += SECS_IN_HOUR * (tmStruct.tm_hour - now->tm_hour) + SECS_IN_MIN * (tmStruct.tm_min - now->tm_min) + tmStruct.tm_sec - now->tm_sec;
            break;
        case 6:
...
unsigned long MSTime::differenceInDays(const MSTime& aTime_, MSTimeZone zone_) const
{
    ...
    if (t2 >= t1)
        return (t2 - t1) / SECS_IN_DAY;
    else
        return (t1 - t2) / SECS_IN_DAY;
}
...
long MSTime::greenwichMeanTime(struct tm* time_, MSTimeZone zone_)
{
    ...
    if (time_->tm_mon > 1 && year % 4 == 0 && (year % 100 != 0 || year % 400 == 0))
        julian++;
    // convert to seconds
    julian *= SECS_IN_DAY;
...
MSHashTable* MSTime::initZoneHashTable(void)
{
    ...
    ht->notFound(0x0);
    for (int i = 0; TimeZones[i].zname != 0; i++)
        ht->add(TimeZones[i].zname, (void*)&TimeZones[i]);
    return ht;
}
...
{
    MSString s;
    return os_ << aTime_.format(s) << flush;
}
