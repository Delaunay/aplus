...
MSError::ErrorStatus MSUnsigned::set(const MSString& aString_)
{
    ...
    if (aString.indexOf('.') < aString.length())
        code = MSError::BadInt;
    else {
        ...
        if (slen == 0)
            ...
            code = MSError::BadInt; // only positive values
        else if (slen > MSUnsigned::MaximumLength)
            code = MSError::IntTooBig;
        if (code == MSError::MSSuccess) {
            if (sscanf(aString.string(), "%u", &_unsigned) != 1)
                _unsigned = 0, code = MSError::BadInt;
            else if (_unsigned == 0) // could be the result of overflow in scanf
            {
...
{
    return (aFormat_.formatType() == MSFormat::Int) ? formatUnsigned(aString_, aFormat_.intFormat(), aFormat_.formatModifier(), _unsigned) : format(aString_, MSInt::WithoutCommas);
}
