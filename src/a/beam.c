...
{
    return (endian == ENDIAN_BIG) ? "big" : (endian == ENDIAN_LITTLE) ? "little" : (endian == ENDIAN_UNDEF) ? "undef" : "unknown";
}

static I hostEndian(void)
{
    return (0x61626364 == *EndianTag.u) ? ENDIAN_BIG : (0x64636261 == *EndianTag.u) ? ENDIAN_LITTLE : ENDIAN_UNDEF;
}
...
{
    I targetndn;
    if (Et != andn->t || 1 != andn->n || !QS(*andn->p))
        ERROUT(ERR_TYPE);
    targetndn = (*andn->p == MS(si("big"))) ? ENDIAN_BIG : (*andn->p == MS(si("little"))) ? ENDIAN_LITTLE : ENDIAN_UNDEF;
    if (ENDIAN_UNDEF == targetndn)
        ERROUT(ERR_DOMAIN);
...
A ep_SwapToEndian(A aobj, A andn) /* must use argument type 12, not 0!!! */
{
    ...
    if (targetndn != sourcendn)
        doSwapEndianInPlace(aobj, targetndn);
    return aplus_nl;
}
...
Z int loopen(C* path, int flags, mode_t mode)
{
    ...
    if (i)
        errno = log_EWouldBlock(i, fd, errno, path, fcn);
    R fd;
}
...
void ttmv32(int t, I* d, int* s, int n)
{
    C* x = tmv32(t, d, s, n);
...
A gc32(int t, int r, int n, int d[], int* p)
{
    I f;
    A z;
...
A cvt64(A32* aobj)
{ /* mapped objects only - floats, chars and longs */
    A newa;
    I i = aobj->i;
...
Z I mapDotMFile(int fd, int mode, C* fname, C* t)
{
    ...
    if (rc == 1 && mode == BEAM_RW && autoBeamConvert == 1) /* Convert .m file */
    {
        ...
        /* reset the items to match orignal*/
        {
            ...
            if (dbg_tb)
                printf("\343 Setting Items to %ld\n", itemCount);
            dc(mFile);
        }
...
A dbg_mfrsf(void)
{
    ...
    if (MappedFileArray) {
        DO(maxMFAindex, if (MappedFileArray[i].a)++ n);
    }
    modes = gv(It, n);
    args = gv(Et, n);
    fnames = gv(Et, n);
...
Z ssize_t writeAobjToFile(int fd, C* s, size_t n, I c)
{
    ...
    if ((!c) && t != -1 && a->i != (newItems = a->r ? a->d[0] : a->n)) {
        ...
        if (lt != -1)
            t = write(fd, (C*)&newItems, sizeof(I));
    }
    fsync(fd);
    R t;
} /* IBM write fix */
...
Z I items(I n, A z)
{
    ...
    {
        int ret;
        I totsize, rank, itemCount, items;
        struct stat statbuf;
