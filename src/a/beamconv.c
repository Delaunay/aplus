    ...
    int iHostWidth, int iHostEndian)
{
  ...
  */
    ...
    if (isAObject(a64LittleHeader)) {
        ...
        /* Check file length */
        if (dimsOK && noSizeChk || (sizeOK = checkFileSize64(a64LittleHeader) <= iLen)) {
            *width = 64;
            *endian = ENDIAN_LITTLE;
            return 0;
        }
    ...
    if (isAObject(a64BigHeader)) {
        ...
        /* Check file length */
        if (dimsOK && noSizeChk || (sizeOK = checkFileSize64(a64BigHeader) <= iLen)) {
            *width = 64;
            *endian = ENDIAN_BIG;
            return 0;
        }
    ...
    if (isAObject(a32LittleHeader)) {
        ...
        /* Check file length */
        if (dimsOK && noSizeChk || (sizeOK = checkFileSize32(a32LittleHeader) <= iLen)) {
            *width = 32;
            *endian = ENDIAN_LITTLE;
            return 0;
        }
    ...
    if (isAObject(a32BigHeader)) {
        ...
        /* Check file length */
        if (dimsOK && noSizeChk || (sizeOK = checkFileSize32(a32BigHeader) <= iLen)) {
            *width = 32;
            *endian = ENDIAN_BIG;
            return 0;
        }
	...
	a64BigHeader.i) + (a64BigHeader.t == 2));
      */
        /* Check file length */
        if (dimsOK && noSizeChk || (sizeOK = checkFileSize64(a32BigHeader) <= iLen)) {
            *width = 64;
            *endian = ENDIAN_BIG;
            return 0;
        }
...
int cvtIfNeeded(void* src, A* dest, I ilen, I cvtInPlace)
{
    ...
    if (srcEndian == hostEndian && srcWidth == 32 && hostWidth == 64) {
        A32* from = (A32*)src;
        long items = from->n;
    ...
    if (srcEndian != hostEndian && srcWidth == 32 && hostWidth == 32) {
    ...
    if (srcEndian != hostEndian && srcWidth == 64 && hostWidth == 64) {
