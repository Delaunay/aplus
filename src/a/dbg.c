...
{
    I idx = dbg_depth - 1, i;
    idx = MIN(99, 4 + MAX(0, idx) * dbg_ti);
    memset(trcb, ' ', idx);
    trcb[idx] = '\0';
    if (dbg_ch != ' ')
        for (i = 0; i < dbg_depth - 1; ++i)
            trcb[4 + (i * dbg_ti)] = dbg_ch;
    R trcb;
}
...
I nanbeamchk(C* f, A af)
{
    ...
    if (infs) {
        DH("\343%sWarning: found %ld Inf%s in %s.\n",
            dbg_pfx(), infs, (1 == infs) ? "" : "s", f);
        FFL;
    }
    if (nans) {
        DH("\343%sWarning: found %ld Nan%s in %s.\n",
            dbg_pfx(), nans, (1 == nans) ? "" : "s", f);
        FFL;
    }
...
#else
#define FPCLASS(x) (NANMASK == ((x)&NANMASK) ? ((x)&0x000fffff ? ((x)&0x00080000 ? 3 : 4) : 2) : (x)&0x00080000 ? 1 : 0)
...
I bitwisechk(A a, A w, I i)
{
    ...
    if (w)
        DO(w->n, if ((unsigned long)(w->p[i]) & (unsigned long)(~0x01))++ wnonbool;);
...
{
    CX cx = (CX)f->p[f->n + 2];
    if (dbg_hold || nix(cx->s) || (dbg_levels && dbg_levels < dbg_depth))
        R 0;
...
{
    S cxsym;
    if (dbg_hold || (!(('_' == *f) ? dbg_ts : dbg_tx)) || (dbg_levels && dbg_levels < dbg_depth))
        R 0;
    if ('_' != *f && cxl[0]) {
        symsplit(si(f), &cxsym);
        if (nix(cxsym))
...
I beamtrc(C* f, I x, I i)
{ /* Note that i is used only if 1==x. */
    if (dbg_hold)
        R 0;
    f = f ? f : "<?>";
    if (1 == x) {
        DH("\343%sBeaming in (%ld) %s\n", dbg_pfx(), i, f);
    } else {
        DH("\343%s%s %s\n", dbg_pfx(),
...
Z A dbghelp_vlist()
{
    ...
    for (count = 0, s = dbg_helpmsg0; *s; s++)
        ++count;
    for (s = dbg_helpflagsA; *s; s++)
        ++count;
    for (s = dbg_helpflagsB; *s; s++)
        ++count;
    for (s = dbg_helpmsg1; *s; s++)
        ++count;
    result = gv(Et, count);
    for (count = 0, s = dbg_helpmsg0; *s; ++s)
        result->p[count++] = (I)gsv(0, *s);
    for (s = dbg_helpflagsA; *s; ++s)
        result->p[count++] = (I)gsv(0, *s);
    for (s = dbg_helpflagsB; *s; ++s)
        result->p[count++] = (I)gsv(0, *s);
...
{
    I val = ('1' == c) ? 1 : 0;
    dbg_tf = dbg_tx = dbg_ts = dbg_tl = dbg_tb = dbg_tdep = dbg_tinv = dbg_tscb = dbg_tpcb = dbg_tdyld = dbg_tdef = dbg_tprcb = dbg_trcb = val;
}
...
Z A dbg_acxt(A aarg)
{
    ...
    if (qz(aarg)) {
        ...
        I* sl;
        if (0 == cxl[0])
            R gvi(Et, 2, ge(MS(si("all"))), 0);
        for (sl = (I*)cxl; *++sl; ++count)
            ;
        z = gv(Et, count);
        DO(count, z->p[i] = MS(cxl[i + 1]));
...
Z A dbg_cb(A aarg)
{
    I f, c, z;
    if (qz(aarg))
        R gvi(Et, 2, ic(dbg_cbfunc), ic(dbg_cbdata));
    if (aarg->t != Et || aarg->n != 2)
        ERROUT(ERR_DOMAIN);
    f = aarg->p[0];
    c = aarg->p[1];
    z = qz((A)f);
    if (!(QF(f) && !QS(f)) && !z)
        ERROUT(ERR_NONFUNCTION);
    dc(dbg_cbfunc);
    dc(dbg_cbdata);
    if (z) {
        dbg_cbfunc = (A)0;
...
#define ADVANCEARG(s, x) \
    if (usearg)          \
    ADVANCETWO(s, x)     \
    else ADVANCEONE(s, x)
#define ADVANCEONE(s, x)           \
    {                              \
        s = x;                     \
        x = cl(w = bl(s)), *w = 0; \
    }
#define ADVANCETWO(s, x)                                      \
    {                                                         \
        s = cl(u = bl(x)), *u = 0, x = cl(w = bl(s)), *w = 0; \
    }
...
#define DCF(x, v, f) CSR( \
    x, if (isn) R gi(f);  \
    else if (v) {f=iarg;R 0; } else ERROUT(ERR_DOMAIN));

#define DCFC(x, f) CSR( \
    x, if (isn) {A z=gs(Ct);(*(C*)(z->p))=dbg_ch;R z; } else if (isc) {dbg_ch=carg;R 0; } else ERROUT(ERR_DOMAIN));
...
Z A ep_dbg(A asym, A aarg)
{
    ...
    if (cmd == (C*)0)
        ERROUT(ERR_TYPE);
    isn = qz(aarg);
    isi = (It == aarg->t && 1 == aarg->n);
    if (isi)
        iarg = *aarg->p;
    isb = (isi && (0 == iarg || 1 == iarg));
    isc = (Ct == aarg->t && 1 == aarg->n);
    if (isc)
        carg = (*(C*)(aarg->p));
    switch (lu(cmd, dbgList)) {
        ...
        CSR(
            24, if (isn) R gvi(It, 2, dbg_levels, dbg_depth); else if (isi) {dbg_levels=iarg;R 0; } else ERROUT(ERR_DOMAIN)); /* levels */
