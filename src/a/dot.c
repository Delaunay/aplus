...
H2(dot)
{
    ...
    for (i = ia = iw = 0; i < z->n; ++i) {
        if (incra) {
            ...
            if (i == 0 || a->p[ia] != a->p[ia - 1]) {
                ...
                if (buflen < lena + 32) {
                    buflen = lena + 64;
                    if (buf)
                        buf = (char*)brealloc(buf, buflen = lena + 64);
                    else
                        buf = (char*)balloc(buflen);
                } else if (!buf)
                    buf = (char*)balloc(buflen);
        ...
        if (NULL == strchr(strw, '.')) {
#endif
            if (buflen < (2 + lena + lenw))
                buf = (char*)brealloc(buf, buflen = 2 + lena + lenw);
...
H1(undot)
{
    ...
    ND1 if (8 < a->r) ERROUT(ERR_MAXRANK);
    if (Et != a->t)
        ERROUT(ERR_TYPE);

    z = ga(Et, 1 + a->r, 2 * a->n, a->d);
    z->d[a->r] = 2;
    sp = z->p;
    for (i = 0; i < a->n; ++i) {
        if (!QS(a->p[i])) {
