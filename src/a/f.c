...
Z H1(rdc)
{
    ...
    ND1 r = a->r - 1;
    Q(r < 1, 7);
    R mv(d, a->d + 1, r), *d *= *a->d, rsh(a, r, d);
}
Z H1(monadicIota)
{
    A z;
    I1;
    {
        I r = a->n, *d = a->p, n = tr1(r, d);
        Q(n < 0, 9)
        Q(a->r > 1, 7) Q(r > MAXR, 13) W(ga(It, r, n, d)) d = z->p;
        DO(n, d[i] = i)
        R(I) z;
    }
}
...
{
    A z;
    ND1 W(gv(It, a->r)) mv(z->p, a->d, a->r);
    R(I)
    z;
}
...
GV2(C, m2)
{
    r = u != 1 ? tmv(t, (I*)r, (I*)a, v) : trp(t, (I*)r, (I*)a, v);
    u != 2 ? tmv(t, (I*)r, (I*)w, n - v) : trp(t, (I*)r, (I*)w, n - v);
}
#ifdef __lucid
...
GV0(C, k1)
{
    ...
    !u ? tmv(t, (I*)r, (I*)(w + Tt(t, v)), n) : u < 0 ? tmv(t, (I*)zer(tzer, (I*)r, -u), (I*)w, n + u) : zer(tzer, (I*)tmv(t, (I*)r, (I*)(w + Tt(t, v)), n - u), u);
}
Z I td(A a, A w, I i)
{
    A z;
...
{
    I s = 0, f = 0;
    DO(n, if (~1 & *p) if (f = 1, *p < 0) R - 1; s += *p++)
        R f ? -s : s;
}
extern I aw;
GV0(I, c0)
...
GV0(C, x3)
{
    I tzer = (Et == t && u && QS(((I*)w)[0])) ? Et + 1 : t;
    C* p = r + Tt(t, n);
    for (; r < p;)
        if (*a++)
            r = tmv(t, (I*)r, (I*)w, v), w += (aw != 2) ? Tt(t, v) : 0;
        else
            r = zer(tzer, (I*)r, v);
}
...
{
    A z;
    Z C* t[] = { "int", "float", "char", "null", "box", "sym", "func",
        "unknown" };
    W(gs(Et))* z->p = MS(si(t[QA(a) ? (Et < a->t ? 6 : Et > a->t ? a->t : !a->n ? 3 : Et == a->t ? (QA(a = (A)*a->p) && a->t < Xt ? 4 : QS(a) ? 5 : 6) : 6) : QP(a) ? 6 : QX(a) ? 6 : 7]));
    R(I)
    z;
}
Z H1(enc)
...
Z H2(e2)
{
    ...
    if (It == a->t || qz(a)) {
        ...
        R QS(z) ? (Cx = cxi(XS(z)), z = e1(w), Cx = cx, z) : (q = 6, 0);
}
...
Z H2(drf)
{
    ...
    if (!c || !v)
        R 0;
    R ic((A)gt(sv(cxi(c), v)));
}
Z I rake_1p(A a, I cd, I* pd)
{
    I z = 0;
    if (QF(a) || Et != a->t || fsy(a) && a->n)
        R 1;
    if (!a->n)
        R 0;
    if (cd >= *pd)
        *pd = cd + 1;
    DO(a->n, z += rake_1p((A)a->p[i], cd + 1, pd));
    R z;
}
Z void rake_2p(A a, A z)
{
    if (QF(a) || Et != a->t || fsy(a) && a->n) {
        z->p[z->n++] = QA(a) ? ic(a) : (I)a;
        R;
    }
    if (!a->n)
        R;
    DO(a->n, rake_2p((A)a->p[i], z));
    R;
}
Z H1(rake)
{
...
{
    if (type_ == 1)
        R sizeof(P1) / sizeof(PFI); /* Monadic */
    else
        R sizeof(P2) / sizeof(PFI); /* Dyadic  */
}
