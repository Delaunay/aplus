...
#else
#define C1(f) R (*(I(*)(I*, I*, I))(g = (PFI)(f))) \
              (z->p, a->p, a->n), (I)z;
#define C2(f) R (*(I(*)(I*, I*, I*, I))(g = (PFI)(f))) \
              (z->p, a->p, w->p, z->n), (I)z;
#endif
...
#define F2                                                                  \
    {                                                                       \
        if (a->t != Ft && !(a = ep_cf(0)) || w->t != Ft && !(w = ep_cf(1))) \
            R 0;                                                            \
    }
#define I1                              \
    {                                   \
...
#define I2                                                            \
    {                                                                 \
        if (a->t != It && !(a = ci(0)) || w->t != It && !(w = ci(1))) \
            R 0;                                                      \
    }
#define I0                  \
    {                       \
...
#define CHECKXCP                                                          \
    {                                                                     \
        if (nan_bits || ((fpe_bits = ieee_get_fp_control()) & FPERRMASK)) \
            q = 9;                                                        \
    }
#define NAN_CHECK(v, xx)  \
    {                     \
        F vv;             \
        vv = xx;          \
        if (isnan(vv))    \
...
#define CHECKXCP                                                   \
    {                                                              \
        if (nan_bits || ((fpe_bits = fp_read_flag()) & FPERRMASK)) \
            q = 9;                                                 \
    }
#define NAN_CHECK(v, xx)  \
    {                     \
...
#define CHECKXCP                                              \
    {                                                         \
        if (nan_bits || (fpe_bits = fetestexcept(FPERRMASK))) \
            q = 9;                                            \
    }
#define NAN_CHECK(v, xx)  \
    {                     \
...
#define CHECKXCP                                                  \
    {                                                             \
        if (nan_bits || ((fpe_bits = fpgetsticky()) & FPERRMASK)) \
            q = 9;                                                \
    }
#define NAN_CHECK(v, xx)  \
    {                     \
