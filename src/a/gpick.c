...
Z I gpi_num(A apick, A aobj)
{
    ...
    if (Et != aobj->t)
        ERROUT(ERR_DOMAIN);
    if (2 <= apick->r || apick->n != aobj->r)
        ERROUT(ERR_RANK);
    if (Ft == apick->t) {
        if (gpu_fillivec(ivec, apick))
            ERROUT(ERR_TYPE);
        ip = ivec;
    } else
        ip = apick->p;

    if (aobj->d[0] <= (I)(idx = ip[0]))
        ERROUT(ERR_INDEX);
    for (i = 1; i < apick->n; ++i) {
        if (aobj->d[i] <= (I)(ip[i]))
            ERROUT(ERR_INDEX);
...
Z I gpi_sym(I key, A aobj)
{
    I i, tt;
    A t0, t1;
    if (Et != aobj->t || 2 != aobj->n) {
        q = ERR_DOMAIN;
        R - 1;
    }
    t0 = (A)aobj->p[0];
    t1 = (A)aobj->p[1];
    if ((!QA(t0)) || (!QA(t1)) || Et != t1->t || t0->n != t1->n) {
        q = ERR_DOMAIN;
        R - 1;
    }
    for (i = 0; i < t0->n; ++i) {
...
Z A gp_nested(A apick, A aobj)
{
    ...
    if (0 == apick->n)
        R(A)
        ic(ares); /* ic_or_copy(ares) */
    for (i = 0; (0 == q) && i < apick->n; ++i) {
        atarg = (A)apick->p[i];
        if ((!QA(ares)) || Et != ares->t)
            ERROUT(ERR_DOMAIN);
        if (QS(atarg)) {
            if (-1 == issymbol)
                ERROUT(ERR_DOMAIN);
    ...
    case Ft:
        if (1 == apick->n) {
            if (1 != aobj->r)
                ERROUT(ERR_RANK);
            if (It == apick->t)
                z = (A)pcki(*apick->p, aobj);
            else if (gpu_fillivec(&idx, apick))
                ERROUT(ERR_TYPE)
            else
                z = (A)pcki(idx, aobj);
        } else
...
A* ep_gpp(A apick, A* paobj)
{
    ...
    case Ft:
        if (1 == apick->n) {
            if (It == apick->t)
                R gpp_scalar(*apick->p, paobj);
            if (gpu_fillivec(&idx, apick))
                ERROUT(ERR_TYPE);
            R gpp_scalar(idx, paobj);
        } else
            R gpp_num(apick, paobj);
    case Et:
        R gpp_nested(apick, paobj);
    default:
        ERROUT(ERR_TYPE);
    }
}
...
Z I pcki(I i, A a)
{
    ...
    Q(!a->r, 7)
    Q((I)i >= a->d[0], 10);
    R t == Et && (z = a->p[i], !QF(z)) ? ic((A)z) : (I)gc(t, 0, 1, 0, (I*)((C*)(a->p) + Tt(t, i)));
}
I pck(I i, A a)
{
    I z;
...
I pka(A p, A* v) { Q(!QA(p) || !QA(*v) || Et < (p)->t || Et < (*v)->t, 18)
    R(I) ep_gpp(p, v); }
