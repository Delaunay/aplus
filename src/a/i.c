...
I sym(A a)
{
    if (a->t != Et || !a->n)
        R 0;
    DO(a->n, if (!QS(a->p[i])) R 0)
    R 1;
}
I fsy(A a) { DO(a->n, if (!QF(a->p[i])) R 0)
    R 1; }
I cm(I* d, I* s, I n)
{
    I* t = d + n;
...
{
    I i, j;
    R !QA(a) || !QA(w) || (i = a->t) > Et || (j = w->t) > Et ? a != w : cm(&a->r, &w->r, 2 + a->r) ? 1 : i != j ? cfi(a, w) : (ct(i))(a->p, w->p, a->n);
}
...
#define CF(f, t)                                   \
    Z I f(t* p, I i, I j, I n, I u)                \
    {                                              \
        t *d = p + i * n, *s = p + j * n;          \
        DO(n, if (d[i] != s[i]) R u ^ d[i] < s[i]) \
            R i < j;                               \
    }
CF(lf, F)
CF(li, I)
CF(lc, UC)
Z I ls(I* p, I i, I j, I n, I u)
{
    I *d = p + i * n, *s = p + j * n;
    DO(n, if (d[i] != s[i]) R u ^ (0 > strcmp(XS(d[i])->n, XS(s[i])->n)))
        R i < j;
}
Z I mrg(PFI f, I n, I z, I* p, I l, I h, I u)
{
    ...
    if (h == m)
        R p[l] = -1, l;
    if ((*(I(*)(I, I, I, I, I))f)(z, h = mrg(f, n, z, p, m, h, u), l = mrg(f, n, z, p, l, m, u), n, u))
        m = l, l = h, h = m;
    for (t = l;; l = p[l] = h, h = m) {
    L:
        if (m = p[l], m == -1)
            R p[l] = h, t;
        if ((*(I(*)(I, I, I, I, I))f)(z, m, h, n, u)) {
            l = m;
            goto L;
        }
    }
...
Z void u1(I* r, I w, I n)
{
    *r = mrg((PFI)lf, 1, w, tp, 0, n, v);
    DO(n - 1, r[i + 1] = tp[r[i]])
}
Z void fsu(I* r, F* w, I n)
{
    ...
    HH h = (HH)(p + n), j, k;
    DO(
        n, if (!finite(w[i])) {u1(r,(I)w,n);R; }) /* Inf and NaN check */
...
H1(upg)
{
    ...
    if (!n || a->r != 1 || t > Ft)
        R srt(a, 0);
    W(gv(It, n))* --Y = (I)z, tp = k_tm(n * 3), ++Y;
    C1((t == Ft ? (PFI)fsu : (PFI)isu))
}
H1(dng)
{
    A z;
...
{
    register I r = 0, x;
    DO(n, r <<= 4; x = *p++; r ^= (0 <= x) ? x : x ^ 0xAAAAAAAA)
    R r;
}
...
{
    A a = *x;
    R !QA(a) || a->t > Et ? (I)a >> 4 : !a->n ? 0 : a->t == Et ? eh((A*)a->p, 1) : a->t == It ? *a->p : a->t == Ct ? ch((C*)a->p, a->n) : *(F*)a->p * (1 + 2e-13);
}
...
GV3(g0)
{
    ...
    (I*, I*, I) = ct(t),
             j = u, k = v, i = 0;
    for (n = Tt(t, k); i < j && (*f)((I*)a, (I*)w, k); ++i)
        a += n;
    *r = i;
}
Z I p2(I n)
...
G3(g1)
{
   ...
   */
#ifdef HASHDUMP
    {
        I buck = 0, tl = 0, ml = 0, cl = 0, *pe = p + bm, *cp = p;
        V v = vi(si("fndh", Rx));
        A aobj;
        for (; cp < pe; ++cp)
            if (*cp) {
    ...
    DO(
       ...
       for(h=(HH)p[m];h&&cmf((F *)(a+l*h->i),(F *)w,k);h=h->__h);if(h&&h->i<*r)*r=h->i; } ++r; w += l);
    R - 1;
}
GV3(c0)
{
    UC *t = a + u, *p;
    DO(n, for (p = a; p != t && *p != *w; ++p); *r++ = p - a; w++)
}
Z I cT[256];
GV3(c1)
{
...
{
    I j, k;
    DO(n, for (k = *w++, j = 0; j < u && a[j] != k; ++j); *r++ = j)
}
#ifdef BSTUB
G2(I, i2)
{
    ...
    for (a += i = k; i--; p[k + *a] = i) {
        if (*--a < -k || bm <= k + *a)
            R g1(r, (UC*)(a - i), (UC*)w, n);
        if ((k + *a) == ~bm)
            H("check: end limit: *a:%ld\n", *a);
    }
    for (a = w + n; w < a; ++w)
        *r++ = (*w < -k || bm <= k + *w) ? k : p[k + *w];
    R - 1;
}
#else
G2(I, i2)
...
{
    A z;
    ND2 X2 if (!(z = (A)fnd(w, a))) R 0;
    g = 0;
    DO(z->n, z->p[i] = z->p[i] != u)
    R(I) z;
}
