...
#define K(t, u, v, x, y) J(u, t, *s++ = *(t*)p; p += (I)j) \
J(v, t, *s++ = *(t*)(p + *j++))                            \
...
#define CI(T, f)                                                         \
    G0(T, f) { DO(n, Q((unsigned long)(n = *a++) >= e, 10); *r++ = w[n]) \
               R - 1; }
...
H2(ind)
{
        ...
        R xr(w, a, 0);
    {
        ...
        if (wr == 1 && wt < Et) {
            W(gd(wt, a))
            C2((!wt ? (PFI)jj0 : wt == 1 ? (PFI)jj1 : (PFI)jj2))
        }
        {
...
{
    I j = rm(v * *a, n);
    tmv(t, (I*)tmv(t, (I*)r, (I*)(w + Tt(t, j)), n - j), (I*)w, j);
}
GV0(C, r3)
{
    I j, k = Tt(t, 1);
...
Z C* h(C* r, C* w, I j)
{
    I n = d[j], k = c[j];
    if (d[++j] < 0)
        r = tst(t, (I*)r, 1, (I*)w, k, n);
    else
        DO(n, r = h(r, w, j); w += Tt(t, k));
    R r;
}
...
#define GT(Ttype, f)                           \
    Z I f(I b, Ttype* d, Ttype* s, I n)        \
    {                                          \
        d += n * b;                            \
        DO(n, if (s[i] != d[i]) R s[i] > d[i]) \
        R 0;                                   \
    }
#define GB(Ttype, f)                                                                       \
    Z void f(I* r, Ttype* o, Ttype* p, I m, I n)                                           \
    {                                                                                      \
        I b, l, h;                                                                         \
        DO(n, l = 0; h = m;                                                                \
...
GB(I, b0)
GB(F, b1) GT(I, bi) GT(F, bf) GT(UC, bc)

    H2(bin)
{
    A z;
    ND2;
    if (!a->t && w->t == Ft && w->n == 1 && ci(1))
        w = (A)Y[1];
    else {
        q = 0;
        X2
    }
    {
        ...
        if (ar == 1 && at < Ct) {
            ...
            DO(n,
                I l = 0;
                I h = u;
                while (l < h) if ((*(I(*)(I, I*, C*, I))f)(b = (l + h) >> 1, a->p, p, v)) l = b + 1;
