...
I profileExecute(n, P1, P2, i) I n, i;
I (*P1[])
(), (*P2[])();
...
void dec(A aobj)
{
    ...
    if (aobj->t == Et)
        DO(aobj->n, dc((A)aobj->p[i]))
    else if (aobj->t == Xt)
        DO(aobj->r, dc((A)aobj->d[i]))
    else
        ef(*aobj->p), mf((I*)(aobj->p[aobj->n + 1]));
    mf((I*)aobj);
}
...
I ee(E expr)
{
    ...
    if (QA(fnc)) {
        ...
        *--Y = fnc;
        ...
        i = U(fnc);
        EQ(0, QX(fnc) ? (i ? PX(i, n) : xin((A)*Y, n - 1, (A)0)) : n == 2 ? (*P2[i])(*Y, Y[1], i) : (*P1[i])(*Y, i));
    }
    DO(n, dc((A)(*Y++)));
    R z;
}
...
#endif
        end = (!u) ? tmv(at, (I*)(((C*)a->p) + Tt(at, an)), w->p, wn) : trp(at, (I*)(((C*)a->p) + Tt(at, an)), w->p, wn);
        if (Ct == at)
            *end = '\0';
        a->n = n, *a->d = *d;
        R(I)
...
Z I in(A* z, I g, I f, A a, A w, I r)
{
    A aobj;
    I j = (*z)->t;
    if (j != w->t && (!(w = j == Ft ? ep_cf(2) : j == It ? ci(2) : (q = 6, (A)0))))
        R 0;
    if (g) {
        if (f) {
            a = (A)*a->p;
...
Z I upd(I x, I d, I i, A p, I r, I o)
{
    ...
    if (p) {
        z = (I*)pka(p, (A*)v);
        if (q)
            R aplus_err(q, (A)MP(36)), 0;
    } else
        z = (I*)v;
    if (QE(i))
        Q(!(i = *Y = (I)e0(XE(i), (A)*z)), 9);
    if (f) {
        Q(g && 0 == ((A)(*z))->r, 7);
    ...
    if (msyncMode && v && v->a && QA(v->a) && ((A)(v->a))->c == 0 && ((A)(v->a))->t < 4) {
        A a = (A)v->a;
        I an = a->n;
        I at = a->t;
        if (g) { /* append assigment */
    ...
    if (2 > v->z) {
        if (Sf && v->f) {
            if (dbg_tscb)
                cbtrc(v, 0);
            v->z = 2, dc(af4((A)v->f, v->c, d, i, (I)p, v));
        }
        if (Sf && v->rff) {
            if (dbg_trcb)
                cbtrc(v, 2);
            v->z = 2, dc(af4((A)v->rff, v->rfc, d, i, (I)p, v));
        }
        val(v);
    }
    if (o && v->o)
        xup(v, (A)d, (A)i, p, r, DependencyEvaluationSemaphore ? 0 : 1); /*  call xup() unless in the middle of re-evaluation */
    R q ? 0 : 1;
}
...
I xis(E e)
{
    ...
    for (*--Y = w;
         !(!QE(a) ? set(a, ic((A)w), 1) : (e = XE(a), e->f == MN(7)) ? lst(e->n, e->a, (A)w) : peak(e->f) ? pea(e, (A)w) : mrg((I)e));)
        aplus_err(q, (A)MN(0));
    R* Y++;
}
#else
...
Z I mr0(E e)
{
    ...
    if (f != MP(36) && f != MP(74)) {
        n = e->n - 1;
        if (QE(y = f == MX(0) ? *e->a : e->a[n]) && (r = XE(y)->f == MP(22)))
            y = *XE(y)->a;
    ...
    if (f == MP(36))
        if (y = e->a[1], Y[1] = ev(*e->a), QE(y = e->a[1]))
            e = XE(y), f = e->f;
    if (f == MP(74)) {
