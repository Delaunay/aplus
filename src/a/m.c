...
Z int loaccess(C* path, int mode)
{
    ...
    if (i)
        errno = log_EWouldBlock(i, rc, errno, path, fcn);
    R rc;
}
...
Z int lofstat(int fd, struct stat* buf)
{
    ...
    if (i)
        errno = log_EWouldBlock(i, rc, errno, "...", fcn);
    R rc;
}
...
void setSigv(I flag)
{
    sigvFlag = (0 == flag) ? 0 : (2 == flag) ? 2 : 1;
    switch (sigvFlag) {
        CS(0, aplus_signal(SIGSEGV, sigv))
        CS(1, aplus_signal(SIGSEGV, SIG_DFL))
            CS(2, sigFullDump(SIGSEGV))
    }
}
void setSigb(I flag)
{
    sigbFlag = (0 == flag) ? 0 : (2 == flag) ? 2 : 1;
    switch (sigbFlag) {
        CS(0, aplus_signal(SIGBUS, sigv))
        CS(1, aplus_signal(SIGBUS, SIG_DFL))
...
void setSigb(I flag)
{
    sigbFlag = (0 == flag) ? 0 : (2 == flag) ? 2 : 1;
    if (sigbFlag)
        aplus_signal(SIGBUS, SIG_DFL);
    else
        aplus_signal(SIGBUS, sigv);
}
#endif
...
void aplus_nan(void)
{
    ieee_handler("set", "invalid", sigf); /*ieee_handler("set","division",sigf);*/
}
#endif
...
void setAtmpMmapFlags(atmpMode) int atmpMode;
{
...
{
    R 1;
} /* pretend like it worked, let malloc do an sbrk */
...
I ep_all(void)
{
    ...
    if (!j)
        R tw;
    DO(j, lofstat(f[i], &b); s += 512 * b.st_blocks)
    R s;
}
/*!!*/
