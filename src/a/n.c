...
{
    B t;
    R t.u = x.u - y.u, t.s = x.s - y.s, t.e = x.e - y.e, t;
}
I t2(I n, I i)
{
    B t;
...
Z H1(t1)
{
    ...
    if (tj) {
        ...
        *td = ++n, z = (A)zr(ga(It, 2, n * 4 + tj, td));
        ...
        DO(tj, if (v = sv(Cx, XS(a->p[i])), (z = (A)(v->e ? v->e : v->a)) && QA(z) && Et < z->t) * (tp[i] = z->p + z->n) = i + 1);
    }
    R t0 = ti(), ic(a);
}
...
{
    I i;
    for (*--Y = (I)a; a->n != 1 || a->t && !(a = ci(0)); a = (A)*Y)
        aplus_err(9, (A)MN(*K));
    R i = *a->p, dc((A)*Y++), i;
}
N(xpr)
{
    I z = (I)aplus_nl, n = e->n;
    ++K;
    DO(n, *K = -1 - i; dc((A)z); z = ev(e->a[i]))
    R-- K, z;
}
N(xif)
{
    I z;
    *++K = 2;
    z = ni((A)ev(*e->a)) ? ev(e->a[1]) : e->n > 2 ? (*K = 1, ev(e->a[2])) : (I)aplus_nl;
    R-- K, z;
}
N(xwh)
{
    I z;
    *++K = 3;
    for (*--Y = z = (I)aplus_nl; ni((A)ev(*e->a)); *Y = ev(e->a[1]), dc((A)z), z = *Y)
        ;
    R-- K, ++Y, z;
}
Z void n_set(A* a, I i)
{
    if ((*a)->c != 1 || (*a)->t != It || (*a)->n)
        dc(*a), *a = gi(i);
    else
...
Z H2(in)
{
    I n = a->r ? *a->d : 1;
    I r = 0;
    for (*--Y = ic(w), *--Y = (I)a; a = (A)fnd((A)*Y, (A)Y[1]), q;)
        aplus_err(q, (A)MP(19));
    dc((A)(*Y++));
    dc((A)(*Y++));
    DO(
        a->n, if (n > a->p[i]) {r=1;break; })
    dc(a);
    R r;
}
N(xca)
...
N(xti)
{
    ...
    A a;
    if (!e->n)
        R ta ? (t2(0, 0), (I)gc(ta->t, ta->r, ta->n, ta->d, ta->p)) : n_gt(ti());
    a = (A)*e->a;
...
I (*PN[])
(E) = { xis, xpr, xif, xwh, xdo, xca, xti, xli, n_rk, n_ea };
#define XI 10000
...
Z C* cxtdotname(V v)
{
    C *res, *cxt = v->cx->s->n, *name = v->s->n;
    res = (C*)malloc(2 + strlen(cxt) + strlen(name));
    sprintf(res, "%s.%s", cxt, name);
    R res;
}

Z C* defaultdoc(const C* s, I t, I n, I* y)
{
    C* z = (C*)malloc(128 + strlen(s));
    strcpy(z, s);
    if (n) {
        strcat(z, "{");
        while (n--) {
        ...
        strcat(z, "{}");
    strcat(z, " returns ");
    strcat(z, (8 == t) ? "null" : argtypes[t]);
    R z;
}

Z C* installdoc(const C* s, C* d)
{
    C* z = (C*)malloc(2 + strlen(s) + strlen(d));
    strcpy(z, s);
    strcat(z, "\n");
    strcat(z, d);
    R z;
...
I xinstall(PFI f, const C* s, I t, I n, I* y, C* d)
{
    ...
    if (n < 0)
        n = -n;
    if (n > 8)
        R-- xi, H("too many arguments for %s\n", s), 0;
    for (i = n; i-- > 0;)
        p |= y[i] << 4 * i;
    xfs_argtypes[xi] = p;
    if (*s != '_')
        v = sv(Cx, si(s)), v->a = MX(xi), v->t = 5, nm = cxtdotname(v);
    if (d == (C*)0)
        d = defaultdoc(nm ? nm : s, t, n, y);
    else
        d = installdoc(nm ? nm : s, d);
...
void install(PFI f, const C* s, I t, I n, I t0, I t1, I t2, I t3, I t4, I t5, I t6, I t7)
{
    ...
    {
    case 8:
        y[7] = t7; /* intentional fall-thru switch statement */
    case 7:
        y[6] = t6;
    case 6:
        y[5] = t5;
...
#define XFIN                   \
    ...
    ++dbg_depth;               \
    if (dbg_tx || dbg_ts)      \
    xftrc(xfs_name[idx], 0)
#define XFOUT                                 \
    if (dbg_xfpe)                             \
        xfpechk(xfs_name[idx], nExternalFPE); \
    suppressFpeDomain = 0;                    \
    if (dbg_tx)                               \
        xftrc(xfs_name[idx], 1);              \
    --dbg_depth
#define RA(x, p)                                                                                                            \
    {                                                                                                                       \
        XFIN;                                                                                                               \
...
A ep_xfsinfo(void)
{
    ...
    z1 = gv(Et, 6);
    zt = gv(Et, xi);
    DO(xi, zt->p[i] = (I)gsv(0, xfs_name[i]));
    z1->p[0] = (I)zt;
    zt = gv(Et, xi);
    DO(xi, zt->p[i] = (I)gsv(0, xfs_desc[i]));
    z1->p[1] = (I)zt;
