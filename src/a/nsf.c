...
#define qv0(v) ((v->a || v->e || v->f || v->c || v->p || v->q || v->cd || v->rff || v->rfc || v->rpf || v->rpc || v->scd || v->atb) ? 1 : 0)
...
{
    S z;
    C *st1 = s1->n, *st2 = s2->n, *buf;
    I n1 = strlen((DEV_STRARG)st1), n2 = strlen((DEV_STRARG)st2);
    if (strchr((DEV_STRARG)st2, '.'))
        R s2;
    buf = (C*)balloc(2 + n1 + n2);
    bcopy(st1, buf, n1);
    buf[n1] = '.';
    bcopy(st2, buf + (n1 + 1), n2);
    buf[1 + n1 + n2] = '\0';
    z = si(buf);
    bfree(buf);
    R z;
...
A ep_dep(A a)
{
    ...
    for (l = v->l; l; l = (I*)*l) {
        vl = (V)l[1];
        result->p[n++] = MS(symjoin(vl->cx->s, vl->s));
    }
    R result;
}
...
{
    V v, xv;
    I *l, idx = 0, count = 0, j, err, notin, **lvec;
    A result;
    struct buff* pb;
    NDC1(a);
    v = avlu(a, &err);
    ...
    while (idx < count) {
        ...
        if (v == (xv = (V)l[1]))
            continue;
        for (l = xv->l; l; l = (I*)*l) {
            for (j = 0, notin = 1; notin && j < count; ++j)
                if (lvec[j][1] == l[1])
                    notin = 0;
            if (notin) {
                buffputlong(pb, (I)l);
...
C* getaname(A aname)
{
    ...
    if (Ct == aname->t)
        name = (C*)aname->p;
    else if (Et == aname->t && QS(aname->p[0]))
        name = XS(aname->p[0])->n;
    else
        name = (C*)0;
    R name;
...
#define InfItCHK(aval)                                                                                  \
    ...
    if (Ft == aval->t)                                                                                  \
        ...
        corelim = (RLIMIT_CORE < *aval->p) ? RLIMIT_CORE : *aval->p;                                    \
    else                                                                                                \
        ERROUT(ERR_TYPE);                                                                               \
    if (RLIMIT_CORE != corelim && 0 > corelim)                                                          \
        ERROUT(ERR_DOMAIN);
#else
#define InfItCHK(aval)                                                                                \
    if (1 != aval->n)                                                                                 \
    ...
    if (Ft == aval->t)                                                                                \
        ...
        ERROUT(ERR_TYPE);                                                                             \
    if (RLIM_INFINITY != corelim && 0 > corelim)                                                      \
        ERROUT(ERR_DOMAIN);
#endif
...
#define RtCHK(aval, range) \
    ItCHK(aval);           \
    if (range < ival)      \
        ERROUT(ERR_DOMAIN);

#define NmCHK(aval)                       \
    if ((cval = getaname(aval)) == (C*)0) \
        ERROUT(ERR_TYPE);
...
void dbg_flg(void)
{
    ...
    if (1 != APL)
        H("%s *** Input mode set not apl.  mode:[%s]\n\n", CC,
            ModeList[APL]);
    if (1 == sq)
        H("%s *** stop flag not set to trace.  stop:[%ld]\n\n", CC, sq);
    if (0 == sq)
        H("%s *** stop flag not set to trace.  stop:[%ld]\n\n", CC, sq);
    if (0 == stdinFlag)
        H("%s *** Standard input disabled.  stdin:[%ld]\n\n", CC,
            stdinFlag);
    if (0 == Df)
        H("%s *** Dependencies disabled.  Df:[%ld]\n\n", CC, Df);
    if (0 == Ef)
        H("%s *** Suspension on errors disabled.  Ef:[%ld]\n\n", CC, Ef);
    if (0 == Gf)
        H("%s *** Protected execute (monadic do) disabled.  Gf:[%ld]\n\n",
            CC, Gf);
    if (0 == Sf)
        H("%s *** Callbacks disabled.  Sf:[%ld]\n\n", CC, Sf);
}
...
A ep_nl(A acontext, A aname)
{
    ...
    if (MP(39) == (I)aname)
        R getCircleFuncSyms(); /* circle primitive */
    NDC2(acontext, aname);
    name = getaname(aname);
    cxt = ct(acontext, &err);
    if (name == (C*)0)
...
I excxt(CX cx)
{
    ...
    if (!cx || Rx == cx)
        R 1;
    for (i = 0; i < cx->ht->nb; ++i)
        for (v = (V)cx->ht->b[i]; v; v = v->v)
            if (qv0(v))
                R 1;
    cx->flag &= ~1;
    R 0;
...
A ep_hashstat(A a)
{
    ...
    NDC1(a);
    if (qz(a))
        R SymbolTableHashChainLengths();
    if (Et != a->t || 1 != a->n || !QS(*a->p))
        ERROUT(ERR_DOMAIN);
    ht = cxi(XS(*a->p))->ht;
    z = gv(It, ht->nb);
    bzero(z->p, ht->nb * sizeof(I));
    for (i = 0; i < ht->nb; ++i)
        for (v = (V)ht->b[i]; v; v = v->v)
            z->p[i]++;
    R z;
}
