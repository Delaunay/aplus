...
Z C* i2f(I t, F* d, I* s, I n) { DO(n, *d++ = *s++)
    R(C*) d; }
...
H1(pct)
{
    ...
    I i = 0, j = 0, k = 0, n, *p;
    ND1 n = a->n;
    I1 W(a->c == 1 ? (A)ic(a) : gv(It, n)) if (n)
    {
        Q(a->r > 1, 7)
        Q(!*a->p, 9) for (p = z->p; ++i < n;) if (a->p[i]) p[j++] = i - k,
                                                           k = i;
        p[j++] = n - k, z->n = *z->d = j;
    }
    R(I)
        ...
        ND2 else ND1;
    {
        ...
        if (h) {
            ...
            if (!g) {
                ...
                if (at = atOnExit, w)
                    wt = wtOnExit;
                if (at != a->t && !(a = at ? ep_cf(1) : ci(1)))
                    R 0;
                if (w && wt != w->t && !(w = wt ? ep_cf(2) : ci(2)))
                    R 0;
                OF(i, ir, n);
                W(ga(t, j, i, d));
                pp = (C*)z->p;
        ...
        if (!w) {
            for (ap = (C*)a->p; ir--; ap += Tt(at, an))
                if (h)
                    (*(I(*)(C*, C*, I))g)(pp, ap, an), pp += Tt(t, n);
                else
                    a = gc(at, ar, an, ad, (I*)ap), *p++ = e ? a : (A)fa(f, (I)a, 0);
        } else {
            for (i = 0; i < ia; ++i)
                for (j = 0; j < iw; ++j)
                    for (k = 0; k < ii; ++k) {
...
{
    A tempa, z;
    if (!QA(a) || It == a->t || Ft == a->t)
        R ds(a, w, 17);
    Q(Et != a->t, 6);
    tempa = (A)fnd(getCircleFuncSyms(), a);
    DO(
        tempa->n, if (CIRCFUNCCOUNT == tempa->p[i]) {q=9;R 0; }; tempa->p[i] -= 7;);
    dc((A)Y[0]);
    Y[0] = (I)tempa;
    z = (A)ds(tempa, w, 17);
    R(I)
