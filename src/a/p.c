...
Z HTN chtgi(HT ht, C* key)
{
    ...
    for (n = *htn; n; n = n->n)
        if (streq((DEV_STRARG)key, (DEV_STRARG)n->s))
            R n;
    R(HTN)
    0;
}
...
I lu(C* s, C* t[])
{
    I i = 0;
    for (; t[i];)
        if (!strcmp((DEV_STRARG)s, (DEV_STRARG)t[i++]))
            R i;
    R 0;
}
C* pp(I a)
{
    R QS(a) ? XS(a)->n : (QN(a) ? (APLpick(n0, ns, n2)) : QP(a) ? (APLpick(p0, ps, p2)) : xfs_name)[U(a)];
}
C* ppd(I a)
{
    R QS(a) ? XS(a)->n : (QN(a) ? (APLpick(n0, ns, n2)) : QP(a) ? (APLpick(p0, ps, p2)) : xfs_desc)[U(a)];
}
I aplus_pi(C* s)
{
    HTN z = chtgi(APLpick(p0ht, psht, p2ht), s);
    if (z)
        R z->a;
    if (*s == '_' && (ISlower(*(s + 1)) || '_' == *(s + 1)))
        R xslu(s);
    R 0;
}
Z C* fn(C* s, I n)
...
A sikAsAObj()
{
    ...
    *b = '\0';
    ...
    dc(z);
    if (dbg_tdoErrorStack)
        doErrorStacktrc(q, aWrk);
    R aWrk;
}

I sik_exp(I tt)
{ /* experimental version of sik for monadic do */
    I *p = K, s, i = 0;
    C b[999];
    A z = 0;
    *b = '\0';
...
Z I padata(A a, I f, I x)
{
    ...
    if (t > Et) {
        if (t == Xt)
            ...
            H("%s.%s", ((CX)(a->p[a->n + 2]))->s->n, XS(*a->d)->n);
        else
            H("%s", (C*)a->p[a->n + 1]);
        R 1;
    } else {
        ...
        if (!j) {
            ...
            p = a->p;
            if (r < 2 && sym(a)) {
                DO(an, H("%s`%s", ix, XS(*p++)->n))
                R 1;
            }
        }
        if (r > 1)
            for (mv(d, a->d, r), n = d[k = r - 1]; --k;)
                d[k] *= d[k + 1];
        else
            n = r ? an : 1;
        for (;;) {
            if (j)
                DO(n, H("< "); paf((A)(*p++), f + 1, x); if (i < n - 1 || an > n) in(f))
            else
                DO((n <= an) ? n : an, if (q == 1) {if(dca)dc(a);R 0; } PC(*s++));
            if (0 >= (an -= n)) {
                if (dca)
                    dc(a);
                R 1;
...
I paf(A a, I f, I x)
{
    ...
    case 6:
        CS(7, H("%s%s", ix, (f) ? pp((I)a) : ppd((I)a)));
        CS(1, pcxv(XV(a)));
...
Z void h(C* s)
{
    if ((1 == APL) && b[1] == '-')
        b[1] = '\242';
    strncpy(s, b, strlen((DEV_STRARG)b));
}
...
Z I dfmt(C* b, C* s, I m, I n, F x)
{
    ...
    if (4 == j)
        R sprintf(b, s, m, n, 0.0), strlen(b);
    k = strlen((DEV_STRARG)iin[j]);
    l = ' ' == *s;
    DO(m + l, b[i] = ' ')
    strncpy(' ' == *s ? b : b + m - k, iin[j], k);
    R 0;
}
...
H1(mth)
{
    ...
    ND1;
    {
        ...
        if (at == Et && !sym(a)) {
            ...
            Q((a = (A)*a->p, !QF(a)), 6);
            R(I)
            gsv(0, !QA(a) ? pp((I)a) : a->t == Xt ? "*derived*" : (C*)a->p[a->n + 1]);
        }
        p.i = a->p;
...
*/
            ...
            /* matrix (rank >= 2) */
            if (It == at) {
                DO(an, BRK k = SH(p.i[i]); if (k > m) m = k);
            } else if (Ft == at) {
                DO(an, BRK l = mfmt(b, s, p.f[i]); k = bd(); if (k > j) j = k; k = l - k; if (k > m) m = k;);
                m += j;
            } else {
                DO(an, BRK k = 2 + strlen(XS(p.i[i])->n); if (k > m) m = k);
            }
            W(ga(Ct, ar, an * m, ad));
            z->d[ar - 1] *= m;
            zr(z);
            d = (C*)z->p;
            if (It == at) {
                DO(an, h(d + m - SH(*p.i++)); d += m);
            } else if (Ft == at) {
                DO(an, mfmt(b, s, *p.f++); h(d + j - bd()); d += m);
            }
#ifdef OLDCODE
            else {
                DO(an, SH(XS(*p.i++)->n); h(d); d += m);
            }
#else
            /* Can't use SH(x), b[30] not large enough to hold long symbol names */
            else {
...
H1(mth)
{
    ...
    ND1;
    {
        ...
        if (ar)
            DO(
                an, BRK if (at) {l=mfmt(b,s,p.f[i]);k=bd();if(k>j)j=k;k=l-k; } else k = SH(p.i[i]); if (k > m) m = k)
        else
        ...
        if (ar)
            ...
            DO(an, k = at ? mfmt(b, s, *p.f++) : SH(*p.i++); h(d); d += k);
        R(I)
        z;
    }
}
...
H2(dth)
{
    ...
    ND2 if (sym(w)) F1 else F2
    {
        ...
        if (!wr)
            ...
            u = tr(wr - 1, wd), v = wd[wr - 1];
        Q(n != v && k, 8)
        Q(n > 99, 12)
        DO(n, x = p[i]; if (f[i] = x < 0) x = -x; j += g[i] = x; h[i] = .5 + 10 * (x - g[i]);)
            W(ga(Ct, wr, u * (j = k ? j : j * v), wd)) z->d[wr - 1]
            = j,
            s = (C*)z->p, cp = s;
        for (p = (F*)(r = w->p); u--;)
            DO(v,
...
void p0hti(void)
{
    ...
    p2ht = hti(P2HTSIZE);
    for (i = 0; n0[i]; ++i)
        chtsi(p0ht, n0[i], MN(i));
    for (i = 0; p0[i]; ++i)
        chtsi(p0ht, p0[i], MP(i));
    for (i = 0; ns[i]; ++i)
        chtsi(psht, ns[i], MN(i));
    for (i = 0; ps[i]; ++i)
        chtsi(psht, ps[i], MP(i));
    for (i = 0; n2[i]; ++i)
        chtsi(p2ht, n2[i], MN(i));
    for (i = 0; p2[i]; ++i)
        chtsi(p2ht, p2[i], MP(i));
}
