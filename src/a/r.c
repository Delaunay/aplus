...
{
    E e;
    R cvl(a0) || QE(a0) && (e = XE(a0), e->n == 2 && e->f == MP(36) && cvl(e->a[1]));
}
Z I rvl(I a0)
{
    E e;
    R pvl(a0) || QE(a0) && (e = XE(a0), e->n == 1 && e->f == MP(22) && pvl(e->a[0]));
}
...
Z I rt(void)
{
    ...
    case MN(3):
        CSR(
            MN(2), *++K = U(f); if (a = rt(), y) prr(3, 0); b = mr() ? re() : (I)aplus_nl;
            if (*t == ELSE) {++t;*K=1;c=re(); } --K; R me(c ? 3 : 2, f, a, b, c));
        CS('[', prr(6, 0))
        CS('(', --t; a = rl(MN(7))) CS('{', --t; a = rl(MN(1)));
    default:
        y = ty(a = f);
    }
    if (!y)
...
{
    E e;
    I rlb[RLBLEN], *b = rlb, n = 0, blen = RLBLEN;
    if (*t++ == '[')
        b[n++] = f, f = MX(0);
    *++K = 0;
    for (; *t != ']' && *t != '}' && *t != ')' && *t;) {
        if (n == blen) {
            blen *= 2;
            if (b == rlb) {
                b = (I*)balloc(blen * sizeof(I));
                bcopy(rlb, b, RLBLEN * sizeof(I));
            } else
                b = (I*)brealloc((char*)b, blen * sizeof(I));
        }
        if (--*K, b[n++] = *t == ';' ? (I)aplus_nl : (re()), *t == ';')
            ++t;
    }
    if (t[-1] == ';')
        b[n++] = (I)aplus_nl;
...
Z I r_rf(void)
{
    I a = rt(), f;
    for (; mr() && ty(f = *t) > 1; y = 1)
        if (++t, a = ty(f) == 2 ? me(1, f, a, 0, 0) : me(2, f, a, rt(), 0), y > 1)
            prr(1, 0);
    R* t == '{' && (!QN(a) || MN(6) == a) ? rl(a) : a;
}
Z I vs(E e) { DO(e->n, if (!vl(e->a[i])) R 0)
    R 1; }
I peak(I f)
{
    E e = XE(f);
    R QE(f) && e->f == MN(9) && *e->a == MP(74);
}
Z I as(I a)
{
    I z;
    E e;
    if (!(pvl(a) || QE(a) && (e = XE(a), peak(e->f) || (e->f == MN(7) ? vs(e) : e->f != MP(36) && e->f != MP(74) && rvl(e->a[e->f == MX(0) ? 0 : e->n - 1])))))
        prr(5, 0);
    ++t, z = me(2, MN(0), a, re(), 0);
    if (QV(a))
        XV(a)->t = y;
    R z;
}
...
Z I rz(I* b0)
{
    ...
    for (r = t = b0; *r; ++r)
        if (QS(*r) && !ispu(*r))
            *r = (i = lk(*r, (A)(Qs ? Qs : *X))) ? i : MV(vi(XS(*r), Cx));
    R re();
}

void f0(C* s)
{
    A aobj = (A)sv(Cx, si(s))->e;
    if (aobj)
        H("%s\n", (C*)aobj->p[aobj->n + 1]);
}
void f1(C* s)
...
Z void inv_pass1_dbg(V v, I i, I d)
{
    ...
    if (2 >= v->z) {
        ...
        for (v->z |= 8; l; l = (I*)*l)
            inv_pass1_dbg((V)l[1], l[2] ? i : 0, 1);
        if (z && !qz(z))
            i = mrg(z, (A)i), dc(z), v->i = (A)i;
        if (d)
            --dbg_depth;
    }
}
    ...
    if (Df && !v->z && v->e) {
        ...
        if (z) {
            ...
            if (Sf && v->rff) {
                if (dbg_trcb)
                    cbtrc(v, 2);
                dc(af4((A)v->rff, v->rfc, z, (I)iwv, 0, v));
            }
            dc((A)z);
        }
        if (dbg_tdep)
...
{
    if (QV(a))
        n ? s1(XV(a), (I)v, 0) : s0(XV(a), (I)v);
    else if (QE(a)) {
        E e = XE(a);
        (void)s2(v, e->f, n);
        if (n > 1 && e->f == MX(0) && QV(*e->a) && e->a[1] == ML(1)) {
            s1(XV(*e->a), (I)v, 1);
            R 0;
        }
        if (e->f == MN(0) && e->n == 2)
...
void sad(V v, A a)
{
    rmd(v), v->e = (I)a, s2(v, *a->p, a->r), inv(v, 0, 1);
    if (a->r == 2)
        v->i = aplus_nl;
}
...
I rd(I* b)
{
        ...
        ;
    {
        ...
        if (t - b < 4) { /* f a:{} or a f b:{}   */
            if (t - b == 3)
                r = *b, *b = b[1], b[1] = r; /* a f b:{} => fab */
            goto L;
        } /* operators */
        if (y = *b != '(') { /* l (lo mop) r:{}   OR  l (lo dop ro) r:{}  */
            if (b[1] != '(' || t[-2] != ')')
                prr(9, 0);
            if (ispu(b[0]) || ispu(b[2]) || ispu(b[3]) || ispu(t[-1]))
                prr(9, 0);
            if (7 == t - b && ispu(b[4]))
                prr(9, 0);
            b[1] = b[3];
