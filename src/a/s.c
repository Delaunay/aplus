...
int infi(void)
{
    FI[4] = -(FI[5] = aplusInfinity = DBL_INFINITY);
    R 0;
}
#else
int infi(void)
{
    FI[4] = -(FI[5] = aplusInfinity = HUGE_VAL);
    R 0;
}
#endif

#define DD(f, u, v, x)                \
    Z void f(u* r, v* a, v* w, I n)   \
    {                                 \
        u* t = r + n;                 \
        I i = aw != 1, j = aw != 2;   \
        RESETXCP;                     \
...
#define DDidx(f, u, v, x)                       \
    ...
    {                                           \
        ...
        I k;                                    \
        RESETXCP;                               \
        for (k = 0; k < n; k++, i += i, j += j) \
            r[k] = x;                           \
        CHECKXCP;                               \
    }

#ifdef HPC_PWR2
#define DDHpc(f, u, v, x)          \
    Z void f(r, a, w, n) u* r;     \
    v *a, *w;                      \
    I n;                           \
    {                              \
        RESETXCP;                  \
...
#define Bw(x, f)                                         \
    Z unsigned long f(unsigned long a, unsigned long w)  \
    {                                                    \
...
{
    F z;
    R !y ? x : (z = fl(x / y)) == -fl(-x / y) ? 0.0 : x - z * y;
}
I rm(I a, I b)
{
    I r;
    R a < b&& a >= 0 ? a : !b ? a : (r = a % b, b < 0 && r > 0 || r < 0 && b > 0 ? r + b : r);
}
...
#define X2_1                                           \
    {                                                  \
        ...
        } else if (w->t == Et && etNotOk(a, w, i))     \
            R q = 6, 0;                                \
    }
/* if(z->c==1&&t==wt)ic(z) can't reuse when possible errors */

#define FF !j ? df[i][wt] : (i < 9 || i > 14) ? (i & 1 ? (wt == Et ? (PFI)e0 : (PFI)c0) : wt == Et ? (PFI)e1 : (PFI)c1) : (diff_t) ? ddne[i - 11] : (wt == Et) ? dde[i - 9] : ddc[i - 9]
...
{
    A z;
    ND2 if (a->t == Et) R bwcv(a, w);
    Q(a->t != w->t || a->t != It, ERR_TYPE)
    if (aw = a->n == 1 && (w->n != 1 || w->r >= a->r))
        z = w;
    else if (w->n == 1)
        aw = 2, z = a;
    else {
        Q(a->r != w->r, ERR_RANK)
        Q(cm(a->d, w->d, a->r), ERR_LENGTH) z = a->c > 1 ? w : a;
    }
    W(gd(It, z))
    C2(ddbw[i - 76])
}
I ds(A a, A w, I i)
{
    A z;
    I t, wt, j = 0, diff_t = 0;
    ND2 if (i == 7 || i > 14) F2 else if (i < 2) I2 else X2_1
        wt
        = w->t;
    X0 if (aw = a->n == 1 && (w->n != 1 || w->r >= a->r)) z = w;
    else if (w->n == 1) aw = 2, z = a;
    else { Q(a->r != w->r, 7)
           Q(cm(a->d, w->d, a->r), 8) z = a->c > 1 ? w : a; }
    t = i < 9 ? wt : i < 15 ? It : Ft;
    W(gd(t, z))
    C2((unsigned long)(FF))
}
...
#define IP(f, x, y)                                                                              \
    ...
    {                                                                                            \
        I k = v;                                                                                 \
        F *ap = a, *wp = w, s, t;                                                                \
        DO(u, DO(k, a = ap; w = wp++; t = x; DO(aw, y; ++a; w += k)* r++ = t) ap += aw; wp -= k) \
    }
...
#endif
Z void m2(F* r, F* x, F* y, I l, I n)
{
    F *p = x + l, *q = y + 1, a, b, c, d;
    a = b = c = d = 0;
    DO(l, a += *x * *y; b += *x++ * *q; c += *p * *y; y += n; d += *p++ * *q; q += n)
    *r = a, r[1] = b, r[n] = c, r[n + 1] = d;
}
Z void m1(F* r, F* x, F* y, I l, I n)
{
    F *q = y + 1, a, b;
    a = b = 0;
    DO(l, a += *x * *y; y += n; b += *x++ * *q; q += n)* r = a, r[1] = b;
}
Z void m0(F* r, F* x, F* y, I l, I n)
{
    F *p = x + l, a, c;
    a = c = 0;
...
G2(F, mmu)
{
    ...
    RESETXCP;
    DO(m >> 1,
        DO(n >> 1, m2(r, a, w, l, n); w += 2; r += 2) if (n & 1) m0(r++, a, w++, l, n);
        w -= n; a += 2 * l; r += n);
    CHECKXCP;
    if (q)
        R 0;
...
I is(A a, A w, I i)
{
    ...
    {
        I ar = a->r, *ad = a->d;
        I wt = w->t, wr = w->r, *wd = w->d, r, n;
        aw = *wd;
        Q(!ar || !wr, 7)
        Q(ad[--ar] != aw, 8) if (i == 2 && !ar && wr == 1) { W(gs(Ft)) C2(dot) } u = tr(ar, ad),
                                                                                 v = tr(--wr, ++wd), r = ar + wr;
        Q(r > MAXR, 13)
        OF(n, u, v) W(ga(wt, r, n, ad)) mv(z->d + ar, wd, wr);
        C2((i == 2 ? (PFI)mmu : i == 4 ? (PFI)x4 : (PFI)x5))
    }
}
#define RG(q, f, x, y)        \
    Z void f(q* r, q* w, I n) \
    {                         \
        q s = x, *t = w + n;  \
        RESETXCP;             \
        for (; w < t; ++w)    \
        ...
        RGHpc(F, q2, FI[4], if (s < *w) s = *w, hpc_vrsf2)
            RG(I, r3, II[5], if (s > *w) s = *w)
                RGHpc(F, q3, FI[5], if (s > *w) s = *w, hpc_vrsf3)
                    ...
                    PFI fr[][2]
    = { (PFI)b0, 0, (PFI)b1, 0, (PFI)r0, (PFI)q0, (PFI)r1, (PFI)q1, (PFI)r2, (PFI)q2, (PFI)r3, (PFI)q3 };

GV1(I, s0)
{
    *r = *w++;
    (*(I(*)(I*, I*, I*, I))f)(r + 1, r, w, n - 1);
}
GV1(F, s1)
{
    *r = *w++;
    (*(I(*)(F*, F*, F*, I))f)(r + 1, r, w, n - 1);
}
#ifdef HPC_PWR2
...
I rs(A a, I i)
{
    A z;
    i -= 46;
    ND1 if (i < 2) I1 else X1
    {
        I at = a->t, ar = a->r, *ad = a->d;
        if (ar) {
            u = *ad++;
            if (!u && (i == 4 || i == 5))
                at = Ft;
        ...
        W(ga(at, ar, v = tr(ar, ad), ad))
        if (!u)
            R trp(at, z->p, at ? (I*)(FI + i) : II + i, v), (I)z;
        f = df[i][t = at], aw = 0;
        C1(rr)
    }
}
...
G2(I, p0)
{
    ...
    DO(u - 1, s = *(a += v);
        p = r;
        DO(n, fp = ((F)*p) * ((F)s) + ((F)*w++);
            Q(fp > (F)(LONG_MAX) || fp < (-1.0 * fabs((F)(LONG_MIN))), -2)* p++ = (I)fp))
    R - 1;
}
#else
G2(I, p0)
{
    ...
    DO(u - 1, s = *(a += v);
        p = r;
        DO(n, fp = ((F)*p) * ((F)s) + ((F)*w++);
            Q(fp > (F)(LONG_MAX) || fp < (F)(LONG_MIN), -2)* p++ = (I)fp))
    R - 1;
}
#endif
GV2(F, p1)
...
{
    I s;
    DO(v, s = *w++; r += n; a += u; DO(u, r -= v; *r = rm(s, *--a); s = *a ? (s - *r) / *a : 0) r++)
}
GV2(F, o1)
{
    F s;
    RESETXCP;
    DO(v, s = *w++; r += n; a += u; DO(u, r -= v; *r = frm(s, *--a); s = *a ? (s - *r) / *a : 0.0) r++)
    CHECKXCP;
}
H2(dcd)
{
    ...
    ND2 if (a->t || w->t) F2
    {
        ...
        u = an, v = wn;
        Q(ar > 1, 7)
        W(ga(wt, wr + ar, u * v, wd - ar)) if (ar)* z->d
            = u;
        if (!u)
            R(I)
            z;
...
{
    A z;
    ND1 X1 MT(0) if (a->t == It) MF(I, I, *w < 0 ? -1 : *w > 0, FALSE) else MF(I, F, *w < 0 ? -1 : *w > 0, FALSE)
}
H1(not)
{
    A z;
    ND1 I1 MT(1) MF(I, I, !*w, FALSE)
}
H1(bwnot)
{
    A z;
...
{
    A z;
    ND1 F1 MT(1) MF(F, F, (*w ? 1.0 / *w : aplusInfinity), FALSE)
}
#ifdef HPC_PWR2
H1(aln)
{
    A z;
    ND1 F1 MT(1) NANMFhPC(F, F, hpc_InitNatLog(); hpc_mAlnFF(r, w, a->n))
}
H1(aen)
{
    A z;
    ND1 F1 MT(1) NANMFhPC(F, F, hpc_InitExponential(); hpc_mAenFF(z->p, a->p, a->n))
}
#else
...
{
    A z;
    ND1 F1 MT(1) MF(F, F, 3.14159265358979323846 * *w, FALSE)
}
H2(sqr)
{
    A z;
    ND2 F1 if (!w->r && It == w->t && 2 == *w->p) { MT(1) MF(F, F, *w * *w, FALSE) } R ds(a, w, 15);
}
...
Z H1(fli)
{
    ...
    MT(0)
    {
        ...
        F x, *w = (F*)a->p;
        F imaxmin[2];
        imaxmin[0] = (((F)LONG_MAX) + 1) - CT;
        imaxmin[1] = (F)LONG_MIN;
        RESETXCP;
        for (; r < t; ++w) {
            x = *w < -CT ? (*w * CT1) - 1 : (*w * CT2);
            if (x > imaxmin[0] || x < imaxmin[1]) {
                q = 9;
                break;
            }
            *r++ = (I)x;
...
Z H1(fli)
{
    A z;
    MT(0)
    MF(I, F, *w < -CT ? (I)(*w * CT1) - 1 : (I)(*w * CT2), FALSE)
}
Z H1(cli)
{
    A z;
...
Z I rnd(I n)
{
    I d = random(), r = (unsigned)0x80000000 % n;
    R r > d ? rnd(n) : d % n;
}
#endif
H1(ran)
{
    A z;
    ND1 I1 MT(1) DO(
        a->n, if (a->p[i] < 1) {q=9;break; } z->p[i] = rnd(a->p[i])) R(I) z;
}
H2(dea)
{
    A z;
    ...
    W(gv(It, m))
    if (!m) R(I) z;
    p = z->p, t = k_tm(2 * m) - 1, j = 1 << (0xff & (h = *t));
#endif
    DO(j--, t[i] = -1);
    {
        long i, _i;
