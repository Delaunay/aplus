...
Z SBS allocNewBlock(int mw)
{
    ...
    while (blocksize < mw) {
        blocksize *= 2;
        if (SBS_MAXBLKSZ < blocksize)
            R 0;
    }
#else
    mzidx = SBS_DEFAULTMZIDX;
    while ((MZ[mzidx] - 1) < mw)
        if (SBS_MAXMZIDX < ++mzidx)
            R 0;
...
Z S newSymbol(const C* str, S prevsym)
{
    ...
    if (SymHashTable->nb * SBS_TRIGGERRATIO < ++SymHashTable->ni)
        growSymHashTable();
...
V sv(CX c, S s)
{
    C *t = (C*)strchr(s->n, '.'), b[99];
    if (t)
        *t = 0, strcpy(b, s->n), *t = '.', c = cxi(si(b)), s = si(t + 1);
    R vi(s, c);
}
V svlu(CX c, S s)
{
    C *t = (C*)strchr(s->n, '.'), b[99];
    if (t)
        *t = 0, strcpy(b, s->n), *t = '.', c = cxlu(si(b)), s = si(t + 1);
    R c ? vlu(s, c) : (V)0;
}
...
Z I rq(I c)
{
    while (*++s && ((c == '"' ? *s == '\\' : *s == c && s[1] == c) ? (I)++s : *s != c))
        ;
    R* s;
}
Z I acp(C* d, const C* s)
{
    C* t = d;
    for (; *d = *s; ++d, ++s)
        if (*s == '\'')
...
A gsv(I i, const C* s)
{
    ...
    if (n) {
        if (!i)
            ...
            n = z->n = *z->d = i == 2 ? ccp((C*)z->p, s) : acp((C*)z->p, s);
        if (n == 1)
            z->r = 0;
    }
    R z;
...
Z I rw()
{
    ...
    if ((n = c == '"') || c == '\'')
        R++ v, rq(c), *s = 0, n = (I)gsv(n + 1, v), *s++ = c, n;
    ...
    for (; readDoubleFromString(&s, d + n); s = dlb(s))
        if (++n == 9999)
            trr(1, "", 0, 0);
    ...
    if (n) {
        C c = *s;
        *s = 0;
        y = (I)strpbrk(v, ".Ee"), *s = c;
        if (!y)
            DO(
                n, if (y = d[i] != (t[i] = d[i])) {q=0;break; });
        R gvs(y ? Ft : It, n, y ? (I*)d : t);
    }

    /* look for a uni-mode compound (X.#) primitive */
    if (APMODE_UNI == APL && isal(*v) && '.' == v[1] && (n = rs_uni(v)))
        R n;
    ...
    if (*s == '.' && isal(s[1]) && (QS(n) || !n)) {
        ...
        y = u_rs(v, 2);
        if (QS(y))
            R MV(vi(XS(y), n ? cxi(XS(n)) : Rx));
        else
            trr(2, ".", 0, 0); /* trr does longjmp */
    }
    if (n)
        R n;
...
Z I bal(I f)
{
    ...
    for (; s = cl(s), c = *s; ++s)
        ...
        case ']':
            ++j;
            CS(
                '}', if (!i) {if(f)R -1;brr(*s); } else if (b[--i] != "{[("[j]) brr(b[i]));
        }
    if (i)
        brr(b[i - 1]);
    if (f)
...
I af(I n)
{
    ...
    if (h) {
        d = fnc->d;
        if (QN(*d)) {
            EQ(1, n != 2 && n != 3 ? (q = 5, 0) : (z = n > 2 ? Y[2] : 0, fnc->r > 2 ? rk(d[1], (A)d[2], (A)Y[1], (A)z) : ea(d[1], (A)Y[1], (A)z)));
            R z;
        }
        for (h = i = fnc->r; i--; *--Y = ic((A)d[i]))
            ;
    L0:
        fnc = (A)*Y;
    }
