...
#define vInUse(v) ((v->a || v->e || v->f || v->c || v->p || v->q || v->cd || v->rff || v->rfc || v->rpf || v->rpc || v->scd || v->atb) ? 1 : 0)
...
Z C* findAplusFileName(C* name)
{
    ...
    if (!r)
        t = findFileName(name, "+"), r = pfind("APATH", ap, t, R_OK);
    if (!r)
        t = findFileName(name, "a"), r = pfind("APATH", ap, t, R_OK);
    R r;
}
...
C* doloadafile(C* s, int u) /* silent version of loadafile */
{
    ...
    if (dbg_tl && !u)
        loadtrc(fn, 0);
    gwd(r), APL = 1, rf(u ? 0 : s, f), Cx = x, APL = m, fclose(f), chdir(r);
    setPWD();
    if (dbg_tl && !u)
        loadtrc(fn, 1);
    --dbg_depth;
...
Z void pipeOps(C* ss)
{
    ...
    for (i = 0; varname[i]; i++) {
        if (isal(varname[i]) || (i && isan(varname[i])))
            continue;
    ...
    case '|':
        if (h = popen(cmdstr, "w")) {
            ...
            PIPEPERR("%s error during pipe operation");
        break;
    case '>':
            ...
            while (0 < (n = buffread(bufp, fileno(h), PIPEOP_BUFCHUNKSIZE)))
                if (0 == n) {
                    if (PIPEOP_BLOCKS >= ++nblocks) {
                        H("%s error: pipe operation blocked", CC);
                        break;
                    } else
                        usleep(PIPEOP_BLOCKUSLEEP);
                }
            buffputc(bufp, '\0');
...
Z void listGlobals(I n, C* cxtname)
{
    ...
    for (i = 0; i < cxt->ht->nb; ++i)
        for (v = (V)cxt->ht->b[i]; v; v = v->v)
            if (-2 == n && vInUse(v) || -1 == n && v->e || v->t == n && v->a)
                H(" %s", v->s->n);
    NL;
}
...
Z void listContexts(void)
{
    I i;
    CX cx;
    for (i = 0; i < CxTable->nb; ++i)
        for (cx = (CX)CxTable->b[i]; cx; cx = cx->n)
            if ((cx->flag & 1) && cx != Rx)
                printCxt(cx);
    NL;
}
...
#endif
        CS(
            10, if ((chdir(*s ? s : getenv("HOME"))) == -1) {perr(s);break; } setPWD());
        CS(
            12, if (*s) {APL=(s[1]=='p')?APMODE_APL:('n'==s[1])?APMODE_UNI:
		   APMODE_ASCII;break; } H(APLpick("apl\n", "ascii\n", "uni\n")));
        CS(14, for (; *s; *w = d, w = bl(s = cl(w)), d = *w, *w = 0) expunge(sv(Cx, si(s))));
        CS(34, for (; *s; *w = d, w = bl(s = cl(w)), d = *w, *w = 0) rmd(sv(Cx, si(s))));
        CS(
            15, if (!*s || !isdi(*s) || s[1] && !isdi(s[1])) {H("%c%c\n",Fs[3],Fs[4]);break; } Fs[3] = s[1] ? *s++ : '0'; Fs[4] = *s);
        CF(17, sq);
        CS(18, H("%s\n", (C*)(a = versGet())->p); dc(a));
        CS(19, loadafile(s, 1));
        CS(21, f0(s));
...
Z H1(f_j)
{
    ...
    F f;
    F imaxmin[2];
    imaxmin[0] = ((F)LONG_MAX) + 1.0;
    imaxmin[1] = ((F)LONG_MIN) - 1.0;
    Q(a->t != Ft && a->n, ERR_TYPE)
    W(gd(It, a))
    DO(
        a->n,
...
A ep_cf(I i)
{
    ...
    A a = (A)Y[i];
    R(A)
    ((z = intToFloat(a)) ? (dc((A)a), Y[i] = z) : 0);
}

I cn(I i, I t)
{
    I a = Y[i], z = (I)gd(t, (A)a);
    R dc((A)a), Y[i] = z;
}

Z I dbgor(A a, A w, I i)
{
    ND2;
    I2;
    if (dbg_tbwc)
...
H2(castOr)
{
    ...
    switch (t) {
        ...
        CS(Et, castfn = (u == Ct ? symToChar : u == It ? (isNull ? charToInt : symToInt) : u == Ft ? (isNull ? intToFloat : symToFloat) : 0));
    default:
        castfn = 0;
        break;
    }
...
#define DDbw(x1, nx1, t, ttype, f)                  \
    I f(A w)                                        \
    {                                               \
        A z;                                        \
        I wd[MAXR], wr = w->r, wn = w->n;           \
        I x = MAX(1, ((nx1) / (x1)));               \
        F nx = (((F)x1) / ((F)nx1));                \
                                    ...
                                    H2(bwcv)
{
    ...
    if (u == t)
        ...
        R u == It ? bwc_i(w) : (u == Ft ? bwc_f(w) : (q = ERR_TYPE, 0));
    else if (t == It)
        R u == Ct ? bwi_c(w) : (u == Ft ? bwi_f(w) : (q = ERR_TYPE, 0));
    else if (t == Ft)
        R u == Ct ? bwf_c(w) : (u == It ? bwf_i(w) : (q = ERR_TYPE, 0));
    else if (t == Et)
        R u == Ct ? bws_c(w) : (u == It ? bws_i(w) : (u == Ft ? bws_f(w) : (q = ERR_TYPE, 0)));
    else
        R(q = ERR_TYPE, 0);
}
