    ...
    *s2;
{
    ...
    while (count-- && !result) {
        ...
        if (a < b)
            result = ((b <= CT * a) ? 0 : ((result) ? 1 : -1));
        else
            result = ((a <= CT * b) ? 0 : ((result) ? -1 : 1));
    }
    return (result);
}
...
#define RANGE_SEARCH_SET(LOW, HIGH) \
    {                               \
        Range_Setting.lo = (LOW);   \
        Range_Setting.hi = (HIGH);  \
        Range_Setting.flag = 1;     \
    }

#define RANGE_SEARCH_UNSET      \
    {                           \
        Range_Setting.flag = 0; \
    }

/* bsu - binary search: works for uniques only */
SUBROUTINE
int
    bsu(key, table, tabsize, itemsize, cmpfn) char *key,
    *table;
...
int (*cmpfn)();
{
    ...
    if (Range_Setting.flag) {
        lo = Range_Setting.lo;
        hi = result = Range_Setting.hi;
    }
...
int (*cmpfn)();
{
    ...
    while (lo < hi) {
        mid = lo + (hi - lo) / 2;
        if (0 > Indexes[mid] || Indexes[mid] >= tabsize)
            ERROUT(ERR_INDEX);
...
SUBROUTINE
int
    bspr(key, table, tabsize, itemsize, cmpfn) char *key,
    *table;
int tabsize, itemsize;
int (*cmpfn)();
{
...
int* pntarget;
{
    ...
    if (NULL != presult) {
        if (bothends) {
            ...
            (*presult) = ga(It, ti, res, target->d);
    }
    if (NULL != pntarget)
        (*pntarget) = res;
    if (NULL != pnsource)
        (*pnsource) = indexvector ? IdxCount : source->d[0];
    return (0);
...
I *loes, *hies;
{
    ...
    if (flag) {
        ...
        if (rc)
            return (rc);
        if (nexpected != ntarget)
            return (ERR_MISMATCH);
        if (nsexpected != nsource)
            return (ERR_MISMATCH);
    }
    ...
    for (i = 0; i < ntarget; ++i) {
        ...
        case Ct:
            loes[i] = (*bsfni)((char*)target->p + (i * Itemsize),
                (char*)source->p, source->d[0], Itemsize,
                bs_Ccmp);
            hies[i] = 1 + (*bsfne)((char*)target->p + (i * Itemsize), (char*)source->p, source->d[0], Itemsize, bs_Ccmp);
            break;
        case It:
            loes[i] = (*bsfni)((char*)target->p + (i * Itemsize),
                (char*)source->p, source->d[0], Itemsize,
                bs_Icmp);
            hies[i] = 1 + (*bsfne)((char*)target->p + (i * Itemsize), (char*)source->p, source->d[0], Itemsize, bs_Icmp);
            break;
        case Ft:
            loes[i] = (*bsfni)((char*)target->p + (i * Itemsize),
                (char*)source->p, source->d[0], Itemsize,
                bs_FcmpCT);
            hies[i] = 1 + (*bsfne)((char*)target->p + (i * Itemsize), (char*)source->p, source->d[0], Itemsize, bs_FcmpCT);
            break;
        }
        if ((!inequality) && loes[i] == Range_Setting.hi)
            loes[i] = hies[i] = -1;
...
int btype;
{
    ...
    if (source->t != Et || target->t != Et)
        ERROUT(ERR_TYPE);
    if (1 < source->r || 1 < target->r)
        ERROUT(ERR_RANK);
    ...
    target;
{
    ...
    bothends = FALSE;
    if (Et == target->t)
        return (nest_binary(source, target, BIN_I));
    else
        return (a_binary(source, target, bsi));
}
    ...
    target;
{
    ...
    bothends = FALSE;
    if (Et == target->t)
        return (nest_binary(source, target, BIN_E));
    else
        return (a_binary(source, target, bse));
}
    ...
    target;
{
    ...
    bothends = TRUE;
    if (Et == target->t)
        return (nest_binary(source, target, BIN_R));
    else
        return (a_binary(source, target, bsr));
}
    ...
    ind, target;
{
    ...
    if (Et == target->t)
        ...
        return (a_binary(source, target, bsp));
}
    ...
    ind, target;
{
    ...
    if (Et == target->t)
        ...
        return (a_binary(source, target, bspe));
}
    ...
    ind, target;
{
    ...
    if (Et == target->t)
        ...
        return (a_binary(source, target, bspr));
}
