...
S f;
{
    ...
    case TC_FLOAT:
        r->t = Ft;
        for (i = 0; i < j; i++)
            ((double*)r->p)[i] = ((float*)cp)[i];
        break;
    case TC_DOUBLE:
        r->t = Ft;
        for (i = 0; i < j; i++)
            ((double*)r->p)[i] = ((double*)cp)[i];
        break;
    default:
        dc(r);
        r = (A)0;
...
A v;
{
    ...
    case TC_CHAR:
        for (i = 0; i < j; i++)
            ((char*)cp)[i] = (char)v->p[i];
        break;
    case TC_U_CHAR:
        for (i = 0; i < j; i++)
            ((unsigned char*)cp)[i] = (unsigned char)v->p[i];
        break;
    case TC_SHORT:
        for (i = 0; i < j; i++)
            ((short*)cp)[i] = (short)v->p[i];
        break;
    case TC_U_SHORT:
        for (i = 0; i < j; i++)
            ((unsigned short*)cp)[i] = (unsigned short)v->p[i];
        break;
    case TC_INT:
        for (i = 0; i < j; i++)
            ((int*)cp)[i] = (int)v->p[i];
    ...
    case TC_U_INT:
        for (i = 0; i < j; i++)
            ((unsigned int*)cp)[i] = (unsigned int)v->p[i];
        break;
    case TC_LONG:
        for (i = 0; i < j; i++)
            ((long*)cp)[i] = (long)v->p[i];
        break;
    case TC_U_LONG:
        for (i = 0; i < j; i++)
            ((unsigned long*)cp)[i] = (unsigned long)v->p[i];
        break;
        /* these were ints */
    case TC_STRUCT1:
    ...
    case TC_FLOAT:
        for (i = 0; i < j; i++)
            ((float*)cp)[i] = (float)((double*)v->p)[i];
        break;
    case TC_DOUBLE:
        for (i = 0; i < j; i++)
            ((double*)cp)[i] = ((double*)v->p)[i];
        break;
    default:
        return -1; /* Failure */
    }
...
Z int PumpData(start, end, a) char *start, *end;
A a;
{
    char* cp;
    I i, len;
    ...
    case It:
        if (end - start < 2 * sizeof(char) + (1 + a->r) * sizeof(I) + a->n * sizeof(I)) {
            ...
            *cp++ = 'I';
            *cp++ = (char)a->r;
            (void)bcopy(&a->n, cp, sizeof(I));
            cp += sizeof(I);
            (void)bcopy(a->d, cp, (i = a->r * sizeof(I)));
            cp += i;
            (void)bcopy(a->p, cp, (i = a->n * sizeof(I)));
            cp += i;
            len = cp - start;
        }
        break;
    case Ft:
        if (end - start < 2 * sizeof(char) + (1 + a->r) * sizeof(I) + a->n * sizeof(F)) {
            ...
            *cp++ = (char)a->r;
            ...
            (void)bcopy(a->d, cp, (i = a->r * sizeof(I)));
            cp += i;
            (void)bcopy(a->p, cp, (i = a->n * sizeof(F)));
            cp += i;
            len = cp - start;
        }
        break;
    case Ct:
        if (end - start < 2 * sizeof(char) + (1 + a->r) * sizeof(I) + a->n * sizeof(C)) {
            ...
            *cp++ = (char)a->r;
            ...
            (void)bcopy(a->d, cp, (i = a->r * sizeof(I)));
            cp += i;
            (void)bcopy(a->p, cp, (i = a->n * sizeof(C)));
            cp += i;
            len = cp - start;
        }
        break;
            ...
            for (len = i = 0; i < a->n && len != -1; i++) {
                if (QA(((A*)a->p)[i]))
                    ...
                    if ((end - cp) < (len + sizeof(C) + sizeof(I))) {
                        ...
                        *cp++ = 'S';
                        (void)bcopy(&len, cp, sizeof(I));
                        cp += sizeof(I);
                        (void)bcopy(XS(((A*)a->p)[i])->n, cp, len);
                        cp += len;
                    }
                } else {
                    q = ERR_TYPE;
...
Z A SuckData(start, end) char **start, *end;
{
    char* cp;
    char type;
    int i;
    ...
    case 'C':
    case 'E':
        if (end - cp < sizeof(char) + sizeof(I) || (rank = (I)(unsigned char)*cp++) > MAXR) {
            q = ERR_LENGTH;
            R(A)
            0;
        }
        (void)bcopy(cp, &length, sizeof(I));
        cp += sizeof(I);
        ...
        (void)bcopy(cp, d, (i = rank * sizeof(I)));
        cp += i;
        switch (type) {
        case 'I':
            r = ga(It, rank, length, d);
            (void)bcopy(cp, r->p, (i = length * sizeof(I)));
            cp += i;
            break;
        case 'F':
            r = ga(Ft, rank, length, d);
            (void)bcopy(cp, r->p, (i = length * sizeof(F)));
            cp += i;
            break;
        case 'C':
            r = ga(Ct, rank, length, d);
            (void)bcopy(cp, r->p, (i = length * sizeof(C)));
            cp += i;
            break;
        case 'E':
    ...
    case 'S':
        ...
        s = gv(Ct, length);
        (void)bcopy(cp, s->p, (i = length * sizeof(C)));
        cp += i;
        r = (A)MS(si((char*)s->p));
        dc(s);
        break;
...
I index;
{
    ...
    if (table == NULL || index < 0 || index >= table->length || (ptr = table->ptr[index]) == (void*)(-1))
        ptr = (void*)(-1);
    R ptr;
}
            ...
            if (sTable[j].msymbol == k->p[i]) {
                ...
                case ST_INT:
                    ...
                    *((int*)(sp + sTable[j].offset)) = (int)*p->p;
                    break;
                case ST_INTENUM:
                    if (SymbolToEnum((EnumTable*)sTable[j].table, p, &value))
                        R 1;
                    *((int*)(sp + sTable[j].offset)) = (int)value;
                    break;
                case ST_LONGMASK:
                    if (SymbolsToMask((MaskTable*)sTable[j].table, p, &mask))
                        R 1;
