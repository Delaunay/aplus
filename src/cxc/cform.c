    ...
    VetteMachine(machine) A machine;
{
    ...
    if (Et != machine->t || 1 != machine->r || 4 != machine->n)
        return 1;
    ...
    if (Et != aobj->t || 1 < aobj->r || len != aobj->n)
        return 6;
    for (i = 0; i < aobj->n; ++i) {
        if (!QA(element = (A)aobj->p[i]))
            return 7;
        if (Et != element->t || 0 == element->n)
            return 8;
        for (j = 0; j < element->n; ++j) {
            if (!QA(subelement = (A)element->p[j]))
                return 81;
            if (Ct != subelement->t || 0 == subelement->n)
                return 82;
        }
    }
    ...
    for (i = 0; i < aobj->n; ++i) {
        if (!QA(element = (A)aobj->p[i]))
            return 10;
        if (It != element->t || 2 != element->r || 0 == element->n || element->d[1] != 1 + ((A)tokens->p[i])->n)
            return 11;
    }
...
SUBROUTINE
A
    MakeResult(sym, s, toktype, len) I sym;
char* s;
I *toktype, len;
{
...
SUBROUTINE
A
    RunMachine(s, forms, tokenobj, statesobj) char* s;
A forms, tokenobj, statesobj;
{
    ...
    for (formno = 0; NULL == result && formno < forms->n; ++formno) {
        ...
        for (i = 0; - 1 != state && i < slen; ++i) {
            if (NULL == (s1 = (char*)strchr((DEV_STRARG)tokenstr, s[i])))
                state = -1;
            else {
...
ENTRYPOINT
A
    ep_cform(s, machine) char* s;
A machine;
{
    A result;

    if (VetteMachine(machine))
        ERRMSG("machine");
    if (0 == strlen((DEV_STRARG)s))
        result = MakeEmptyResult(machine);
    else
        result = RunMachine(s, (A)machine->p[0], (A)machine->p[1], (A)machine->p[2]);
