...
A sysErrsym(n)
{
    A z = EnumToSymbol(ErrnoTable, n);
    R qz(z) ? (A)gsym("unknown") : z;
}
#endif
...
A pathfind(pathVariable, defaultPath, fileName, mode) char* pathVariable; /* environment variable containing path */
    ...
    C* a;
{
    ...
    bzero((C*)(d), sizeof(d));
    if ((f = opendir(a)) == (DIR*)0)
        R ga(Ct, 2, 0, d);
    hp = nodealloc();
    while (APLUS_READDIR(f, dirEntry, e) != 0) {
#if defined(HAVE_SVR4) || defined(linux)
        fs = (FS*)balloc(sizeof(FS));
        fs->l = strlen(e->d_name);
#else
        fs = (FS*)balloc(sizeof(FS));
        fs->l = e->d_namlen;
#endif
        if ((fs->l == 1) && (e->d_name[0] == '.'))
            continue;
        if ((fs->l == 2) && (e->d_name[0] == '.') && (e->d_name[1] == '.'))
            continue;
        bcopy(e->d_name, fs->n, fs->l);
        d[1] = (d[1] > fs->l) ? d[1] : fs->l;
        d[0]++;
        np = nodealloc();
        np->d = (void*)(fs);
        nodeinsert(hp, np);
...
#define S_ISLNK(m) (((m)&S_IFMT) == S_IFLNK)
    ...
    if ((i = readlink(a, b, st.st_size)) == -1)
        R gi(-1);
    b[i] = '\0';
    r = gv(Ct, i);
...
int initIoctlTable(void)
{
    ...
    EnumTablePtr->value = FIOCLEX;
    EnumTablePtr++;
    EnumTablePtr->value = FIONCLEX;
    EnumTablePtr++;
    EnumTablePtr->value = FIONREAD;
    EnumTablePtr++;
    EnumTablePtr->value = FIONBIO;
    EnumTablePtr++;
    EnumTablePtr->value = FIOASYNC;
    EnumTablePtr++;
    EnumTablePtr->value = FIOSETOWN;
    EnumTablePtr++;
    EnumTablePtr->value = FIOGETOWN;
    EnumTablePtr++;
    EnumTablePtr->value = TCGETS;
    EnumTablePtr++;
    EnumTablePtr->value = TCSETS;
    EnumTablePtr++;
    EnumTablePtr->value = TCSETSW;
    EnumTablePtr++;
    EnumTablePtr->value = TCSETSF;
    EnumTablePtr++;
    EnumTablePtr->value = TCXONC;
    EnumTablePtr++;
    EnumTablePtr->value = TCFLSH;
    EnumTablePtr++;
    EnumTablePtr->value = TIOCSCTTY;
    EnumTablePtr++;
    EnumTablePtr->value = TIOCGPGRP;
    EnumTablePtr++;
    EnumTablePtr->value = TIOCSPGRP;
    EnumTablePtr++;
    EnumTablePtr->value = TIOCOUTQ;
    EnumTablePtr++;
    EnumTablePtr->value = TIOCSTI;
    EnumTablePtr++;
    EnumTablePtr->value = TIOCGWINSZ;
    EnumTablePtr++;
    EnumTablePtr->value = TIOCSWINSZ;
    EnumTablePtr++;
    EnumTablePtr->value = TIOCMGET;
    EnumTablePtr++;
    EnumTablePtr->value = TIOCMBIS;
    EnumTablePtr++;
    EnumTablePtr->value = TIOCMBIC;
    EnumTablePtr++;
    EnumTablePtr->value = TIOCMSET;
    EnumTablePtr++;
    EnumTablePtr->value = TIOCGSOFTCAR;
    EnumTablePtr++;
    ...
    amode;
{
    ...
    if ((path = AToString(apath)) == (char*)(-1) || SymbolsToMask(AccessMasks, amode, &mode)) {
        q = ERR_DOMAIN;
        R 0;
    } else
        R(I)
        access(path, (int)mode);
}
...
I mode;
{
    ...
    if ((filename = AToString(afilename)) == (char*)(-1) || SymbolsToMask(OpenMasks, aflags, &flags)) {
        q = ERR_DOMAIN;
        R 0;
    } else
        R(I)
    ...
    A aobj;
{
    ...
    if (Et == aobj->t && 1 == aobj->n && QS(*aobj->p))
        name = XS(*aobj->p)->n;
    else if (Ct == aobj->t && 0 < aobj->n)
        name = (C*)aobj->p;
    else if (It == aobj->t && 1 == aobj->n) {
        isint = 1;
        uid = *aobj->p;
    } else
        ERROUT(ERR_DOMAIN);
...
{
    int statusp, count = 0;
    while (waitpid(-1, &statusp, WNOHANG) > 0)
        ++count;
    R count;
}
...
static I sysKill(pid_, signal_) int pid_;
A signal_;
{
    int aSignal;
