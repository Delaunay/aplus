 ...
 */
static A extractpass(hpp, dpp, endp, erp, trp, swap) char** hpp;
char** dpp;
char* endp;
I* erp;
char* trp;
...
I* plen;
{
    ...
    if ((*plen = sizepasscover(aobj, &hsz, &dsz, 4)) != 0)
        return (NULL);
    *plen = hsz + dsz;
...
I* plen;
{
    ...
    /* the constant x last parameter in sizepass and fillExportBuffer
     forces exported longs to be x bytes */
    if ((*plen = sizepasscover(aobj, &hsz, &dsz, 8)) != 0)
        return (NULL);
    *plen = hsz + dsz;
    cvp = (char*)malloc(*plen);
...
#endif
A ImportAObject(cvp, cvlen, trp) char* cvp;
I cvlen;
char* trp;
{
    ...
    flag = *hp;
    if ((flag & 0xf8) != CDRFLAGSMAGIC)
        return ((A)0); /* 0xf8 == 11111000 */
...
I for_a;
{
    ...
    if ((tr != (A)0) && tr->n != 0) {
        if ((tr->t != Ct) || (tr->r != 1) || (tr->n != 256))
            return (z);
        trp = (char*)(tr->p);
    } else
        trp = (char*)(0);

    if (NULL == (cv = AExportAObject(aobj, trp, for_a, &cvlen)))
        cc->p[0] = cvlen;
    else {
        z->n = z->d[0] = 2;
        cc->p[0] = 0;
    ...
    ep_exp64(aobj) A aobj;
{
    ...
    if (0 == (z = (A)AExport64AObject(aobj, (C*)0, 1, &cvlen)))
        ERROUT(ERR_NONCE);
    R z;
}
#endif /* 64BIT */
    ...
    ep_imp(acv) A acv;
{
    ...
    if (acv->n < 4)
        ERROUT(ERR_DOMAIN);
    if ((A)0 == (z = ImportAObject((char*)acv->p, acv->n, (C*)0)))
        ERROUT(ERR_DOMAIN);
    R z;
}
