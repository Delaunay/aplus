    ...
    while (cp && *cp != (char)NULL) {
        ...
        if (*cp == '%')
...
#endif
                for (z = str; *z != (char)NULL && p < &r[MAXERRSIZE - 1]; *p++ = *z++)
                    ;
                cp += 2;
                break;
            ...
            case 't':
                ...
                for (z = str + 4; *z != (char)NULL && p < &r[MAXERRSIZE - 1]; *p++ = *z++)
                    ;
                cp += 2;
                break;
...
void Exit(int exitcode, char* fmt, ...)
{
    char r[MAXERRSIZE];
    char _ErrBuf[2 * MAXERRSIZE];
    int errnum = errno;
...
void SetWarnFP(FILE* fp)
{
    _Warnfp = fp;
    return;
}

void SetWarnFunc(VPF f)
{
    _WarnFunc = f;
    return;
}
