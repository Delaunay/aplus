...
I* ptype;
{
    ...
    case Et:
        for (i = res = 0; (!q) && i < a->n; ++i)
            if (QA(a->p[i]))
                res += flat_onepass((A)a->p[i], ptype);
            else {
                if (-1 == *ptype)
                    *ptype = a->t;
                if (a->t != *ptype)
...
C* ptr;
{
    ...
    case Et:
        for (i = 0; (!q) && i < a->n; ++i)
            if (QA(a->p[i]))
                ptr = flat_twopass((A)a->p[i], type, ptr);
            else {
...
Z issf(a) A a;
{
    ...
    s = (A)*a->p;
    d = (A)a->p[1];
    R(!QF(s)) && (!QF(d)) && (qz(s) && qz(d) || sym(s) && s->n == d->n && 1 >= s->r && 1 >= d->r && purebox(d) && nodups(s));
}
...
I issfdups(a) A a;
{
    ...
    s = (A)*a->p;
    d = (A)a->p[1];
    R(!QF(s)) && (!QF(d)) && (qz(s) && qz(d) || sym(s) && s->n == d->n && 1 >= s->r && 1 >= d->r && purebox(d));
}

ep_issf(a) register A a;
{
    R issf(a) ? 1 : 0;
}
    ...
    ep_alsf(a) register A a;
{
    ...
    if (quasisf(a)) {
        ...
        if (1 == a->n && QS(*a->p)) {
            ...
            *((A)z->p[0])->p = *a->p;
            z->p[1] = (I)gv(Et, 1);
            *((A)z->p[1])->p = (I)aplus_nl;
            R z;
        }
        z = gv(Et, 2);
        if (1 >= a->n) {
            z->p[0] = z->p[1] = (I)aplus_nl;
            R z;
        }
