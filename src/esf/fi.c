...
SUBROUTINE
void
    strssub(str, target, repl) char *str,
    *target, *repl;
{
    int tlen = strlen((DEV_STRARG)target);
    while (NULL != (str = strstr(str, target)))
        strpaste(str, repl, 0, tlen);
}
#endif
...
SUBROUTINE
void
    matrix_quadfi(cm, nrows, ncol, outbool, outdata, olen, commas) char* cm;
I* outbool;
F* outdata;
...
SUBROUTINE
A scalar_quadfi(ostr, commas, dyna) char* ostr;
I commas;
{
    F num;
    I parin = FALSE, noerror = TRUE, coms = FALSE, hitnum = FALSE;
    ...
    while (noerror && (c = *s1++)) {
        ...
        case '\311':
            *s2++ = 'Q';
            break; /* iota--gives strtod trouble */
        case ',':
            coms = TRUE;
            if (!commas)
                noerror = FALSE;
