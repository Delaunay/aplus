...
#define TOKENTYPE(t) (((T_EOF == t) ? "End of file" : (T_NUM == t) ? "Number" : (T_STRING == t) ? "String" : (T_EDIT == t) ? "Edit" : (T_MODIFIER == t) ? "Modifier" : (T_PERIOD == t) ? "Period" : (T_COMMA == t) ? "Comma" : (T_LPAREN == t) ? "Left paren" : (T_RPAREN == t) ? "Right Paren" : "unknown"))
 ...
 */
static void freeforms(fmtlist) struct format* fmtlist;
{
    ...
    for (p = fmtlist; p != NULL; p = next) {
        if (p->type == 0) {
            ...
            if (p->text)
                (void)free((char*)p->text);
            if (p->gpattern)
                (void)free((char*)p->gpattern);
            if (p->nldecor)
                (void)free((char*)p->nldecor);
            if (p->nrdecor)
                (void)free((char*)p->nrdecor);
            if (p->pldecor)
                (void)free((char*)p->pldecor);
            if (p->prdecor)
                (void)free((char*)p->prdecor);
            if (p->odecor)
                (void)free((char*)p->odecor);
            if (p->backfill)
                (void)free((char*)p->backfill);
            if (p->symbpairs)
                (void)free((char*)p->symbpairs);
            if (p->symgpairs)
 ...
 */
static int datafind(indx, maxrow, d) int* indx;
int* maxrow;
A d;
{
    int i, rc = 0;
    ...
    if (d->t == Et && !QS(d->p[0])) {
        ...
        for (i = 0; i < cols; i++) {
            ...
            case Ct:
                dataList[(*indx)++].p = (I*)(cp++);
                break;
            case Ft:
                dataList[(*indx)++].p = (I*)(dp++);
                break;
            case Et: /* symbol */
            case It:
                dataList[(*indx)++].p = p++;
 ...
 */
static int gettoken(f) unsigned char* f;
{
   ...
   */
    if (f)
        tok.orig = tok.current = tok.last = p = f; /* Initial call */
    if (p == NULL) {
   ...
   */
    ...
    if (strchr((DEV_STRARG)edits, (int)*p)) {
        tok.type = T_EDIT;
        tok.ivalue = (int)*p++;
        if (ISlower(tok.ivalue))
            tok.ivalue = toupper(tok.ivalue);
        tok.current = p;
        return 0;
    }
 ...
 */
static int parsemodifiers(f) struct format* f;
{
    int c, rc = 0;
    ...
    while (tok.type == T_MODIFIER) {
        ...
        default:
            ...
            switch (c) {
            case 'M':
                f->nldecor = tok.svalue;
                break;
            case 'N':
                f->nrdecor = tok.svalue;
                break;
            case 'O':
                f->odecor = tok.svalue;
                break;
            case 'P':
                f->pldecor = tok.svalue;
                break;
            case 'Q':
                f->prdecor = tok.svalue;
                break;
            case 'R':
                f->backfill = tok.svalue;
                break;
            case 'S':
 ...
 */
static int parseedit(f) struct format* f;
{
    int rc = 0, i;
    ...
    case 'X': /* Relative tab negative value is OK */
        ...
        break;
    case 'E':
    case 'F': /* exponential: fixed point: */
        if (rc = gettoken((unsigned char*)0))
            return rc;
 ...
 */
static int parseform(fmtlist) struct format** fmtlist;
{
    int state = 0;
    int repeat = 0;
    int rc = 0;
 ...
 */
static void trav1(f, dcols, pcols, ncol) struct format* f; /* Next node of format tree to process */
 ...
 */
static int printcols(fmtlist, dcols, pcols) struct format* fmtlist;
int dcols;
int* pcols;
{
    int ncol = 0, rc = 0;
 ...
 */
static int textformat(cp, f, rows, cols) char* cp;
struct format* f;
int rows, cols;
{
    int i;
    ...
    if (TooBig(cp + f->width + cols * (rows - 1)))
        return ERR_DOMAIN;
 ...
 */
static int backfill(cp, f) char* cp;
struct format* f;
{
    int j, k;
    int rc = 0;
    if (f->backfill) {
        if (TooBig(cp + f->width))
            return ERR_DOMAIN;
        for (j = k = 0; j < f->width; j++) {
            if (cp[j] == ' ')
                cp[j] = f->backfill[k++];
            if (!f->backfill[k])
                k = 0; /* Backfill string short */
        }
    }
    return rc;
}
 ...
 */
static void docomma(o, i, w, l) char* o; /* cooked buffer */
char* i; /* raw buffer    */
int w; /* width         */
int l; /* leading comma */
{
 ...
 */
static void symbpairs(cp, sp) char *cp, *sp;
{
...
static void errorSet(cp, default_char, f) char *cp, *default_char;
struct format* f;
{
    char err_char[2];
...
static int aformat(cp, f, d, pcols) char* cp;
struct format* f;
struct column* d;
int pcols;
{
    ...
    if (TooBig(cp + f->width + pcols * (d->rows - 1)))
        return ERR_DOMAIN;
    ...
    case Et:
        for (i = 0; i < d->rows; i++, d->p += d->cols, cp += pcols) {
            if (!QS(*d->p) || (len = strlen((DEV_STRARG)(pp = XS(*d->p)->n))) > f->width) {
                errorSet(cp, "*", f);
                continue;
            }
            if (rc = backfill(cp, f))
                return rc;
            if (f->leftj)
                (void)bcopy(pp, cp, len);
            else
                (void)bcopy(pp, cp + f->width - len, len);
        }
        break;
    default:
...
#ifdef FUNCNOTUSED
static int roundnearest(d) double d;
{
    return (d < 0.0) ? (int)(d - .5) : (int)(d + .5);
}
#endif

static int iformat(cp, f, d, pcols) char* cp;
struct format* f;
struct column* d;
int pcols;
{
    ...
    for (i = 0; i < d->rows; i++, cp += pcols) {
        ...
        if (f->scale)
            val *= f->fscale; /* scale value if needed */
        ...
        if (val == 0.0 && f->odecor)
            (void)strncpy(final, f->odecor, FINALBUFSIZE);
        else
            sprintf(final, "%s%s%s", prefix, cooked, suffix);
        symbpairs(final, f->symbpairs);
        width = strlen((DEV_STRARG)final);
        if (width > f->width)
            errorSet(cp, "*", f);
        else if (f->leftj)
            strncpy(cp, final, width);
        else
            strncpy(cp + f->width - width, final, width);
    }
    return rc;
}

static int eformat(cp, f, d, pcols) char* cp;
struct format* f;
struct column* d;
int pcols;
{
    ...
    for (i = 0; i < d->rows; i++, cp += pcols) {
        ...
        if (width > 0 && econvert(val, f->precision, &decpt, &sign, raw)) {
#endif
            if ((!strcmp((DEV_STRARG)raw, (DEV_STRARG) "Inf")) || (!strcmp((DEV_STRARG)raw, (DEV_STRARG) "INF")) || (!strcmp((DEV_STRARG)raw, (DEV_STRARG) "NaN")))
                InfOrNaN = 1;
            else
                InfOrNaN = 0;
            ...
            if (!InfOrNaN) {
                ...
                if (f->precision <= 0)
                    (void)strncpy(cooked, expnt, COOKEDBUFSIZE);
                else if (f->precision == 1)
                    sprintf(cooked, "%c%s", raw[0], expnt);
                else {
                    cooked[0] = raw[0];
                    cooked[1] = '.';
                    cooked[2] = '\0';
...
static int fformat(cp, f, d, pcols) char* cp;
struct format* f;
struct column* d;
int pcols;
{
...
static int gformat(cp, f, d, pcols) char* cp;
struct format* f;
struct column* d;
int pcols;
{
    ...
    for (i = 0; i < d->rows; i++, cp += pcols) {
        ...
        reverse(final);
        ok = 1;
        trailing_zero = 1; /* reset indicator flags */
        for (fmt_p = (char*)final, num_p = (char*)raw; *fmt_p && ok; fmt_p++) {
            if (*fmt_p == stdsym[DIGIT] || toupper(*fmt_p) == stdsym[DIGIT]) {
                ...
                *fmt_p = *num_p;
                num_p++;
            } else if (*fmt_p == stdsym[NOZERO] || toupper(*fmt_p) == stdsym[NOZERO]) {

                if (!*num_p) { /* nothing left */
                    *fmt_p = (prefix_set) ? *prefix : ' ';
                    if (prefix_set)
                        prefix_set = 0; /* set only one */
                    continue;
                }
                if (!ISdigit(*num_p)) {
                    ok = 0;
        ...
        if (ok && !*num_p && !prefix_set) {
            ...
            for (j = 2; j < f->width; j++) {
                if ((!ISdigit(final[j - 2])) || (!ISdigit(final[j]))) {
                    ...
                    if ((toupper(f->gpattern[j - 2]) == stdsym[DIGIT] || toupper(f->gpattern[j - 2]) == stdsym[NOZERO]) && (toupper(f->gpattern[j]) == stdsym[DIGIT] || toupper(f->gpattern[j]) == stdsym[NOZERO]) && (toupper(f->gpattern[j - 1]) != stdsym[DIGIT] && toupper(f->gpattern[j - 1]) != stdsym[NOZERO])) {
...
static int trav2(f, d, rows, i, dcols, ncol, pcols, buf) struct format* f;
struct column* d;
    ...
    do {
        ...
        case 0: /* LOOP */
            for (j = 0; j < f->rcount; j++) {
                if (*i >= dcols)
                    return rc;
                if (rc = trav2(f->loop, d, rows, i, dcols, ncol, pcols, buf))
                    return rc;
            }
            break;
        case '<': /* String */
            if (rc = textformat(cp, f, rows, pcols))
                return rc;
            cp += f->width;
            break;
        case 'X':
...
static int dofmt(fmtlist, rows, dcols, pcols, buf) struct format* fmtlist;
...
char* ecvt(double Value, int NumberOfDigits, int* DecimalPointer, int* Sign)
{
    ...
    if (QPIDecvt_buffer == NULL)
        QPIDecvt_buffer = (char*)malloc(QPIDECVTBUFSIZE);
