...
#define G00(T, f)                           \
    Z void f(r, a, w, na, n) T *r, *w, *na; \
    I *a, n;
#define CI(T, f) G00(T, f) \
DO(n, if ((unsigned long)(n = *a++) >= e)* r++ = *na; else* r++ = w[n];)
...
A ep_from(aidx, asrc, ana) A aidx, asrc, ana;
{
    ...
    if (asrc->t != ana->t || Et == ana->t && asrc->n && sym(ana) != sym(asrc))
        ERROUT(ERR_TYPE);
    if (0 == asrc->r || 0 != ana->r && ana->r != asrc->r - 1)
        ERROUT(ERR_RANK);
    if (MAXR < aidx->r + asrc->r - 1)
        ERROUT(ERR_MAXRANK);
    for (i = 0; i < aidx->r; ++i)
        zn *= d[idx++] = aidx->d[i];
    for (i = 1; i < asrc->r; ++i) {
        zn *= d[idx++] = asrc->d[i];
        ic *= asrc->d[i];
        if (ana->r && ana->d[i - 1] != asrc->d[i])
            ERROUT(ERR_LENGTH);
    }
    if (0 == ana->r && 1 < asrc->r) {
        na = ga(asrc->t, asrc->r - 1, ic, asrc->d + 1);
        cp = (C*)na->p;
        for (i = 0; i < ic; ++i)
            tmv(asrc->t, (I*)(cp + i * SIZEOFITEM(asrc->t)), ana->p, 1);
    }

    z = ga(asrc->t, aidx->r + asrc->r - 1, zn, d);
    if (1 == asrc->r && Et > asrc->t) {
        e = asrc->n;
        (*indf[asrc->t])(z->p, aidx->p, asrc->p, ana->p, z->n);
    } else {
        is = ic * SIZEOFITEM(asrc->t);
        cp = (C*)asrc->p;
        zcp = (C*)z->p;
        for (i = 0; i < aidx->n; ++i) {
            idx = aidx->p[i];
