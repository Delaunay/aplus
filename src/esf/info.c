...
A ep_name(aobj) A aobj;
{
    ...
    if (QA(a)) {
        ...
        if (Xt >= a->t)
            ERROUT(ERR_DOMAIN);
        z = gs(Et);
        z->p[0] = MS(symjoin(((CX)(a->p[a->n + 2]))->s, XS(*a->d)));
    } else if (QX(a))
        z = gsym(xfs_name[U(a)]);
    else if (QP(a))
        z = gsym(pp(a));
...
A ep_locals(aobj) A aobj;
{
    ...
    V v;
    I *zp, ux, xpa;
    if (QX(aobj) || QA(aobj) && Xt + 1 == aobj->t)
        a = aobj;
    else {
        if (!QA(aobj) || Et != aobj->t)
            ERROUT(ERR_DOMAIN);
    ...
    if (QA(a)) {
        if (Xt + 1 == a->t) {
            ...
            za = (A)z->p[0];
            za->p[0] = MS(symjoin(((CX)a->p[2 + a->n])->s, XS(a->d[0])));
            za = (A)z->p[1];
            zp = za->p;
            DO(za->n, zp[i] = a->d[i + 1]);
            za = (A)z->p[2];
            zp = za->p;
            DO(za->n, zp[i] = a->p[i + 1]);
            R z;
        }
        if (Et == a->t || Xt < a->t)
            ERROUT(ERR_NONCE);
