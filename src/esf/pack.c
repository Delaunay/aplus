...
#define PK_ICABLE(t) (PK_AVAR == (t) || PK_AET == (t) || PK_AFUNC == (t) || PK_ABEAM == (t) || PK_ADF == (t))

#define PK_CODEDPTR(t) (PK_SYM == (t) || PK_VREF == (t) || PK_EXPR == (t) || PKE_VDAT == (t) || PKE_VDEP == (t))

#define PK_CODEDIDX(t) (PK_KWORD == (t) || PK_LOCAL == (t) || PK_PRIM == (t) || PK_XFUNC == (t))
...
typedef struct str_IdEntry {
    I p, type;
    long ftell, len;
} IdEntry;
typedef IdEntry* IDE;

typedef struct str_EgoEntry {
    I scx, vname, glob, cvname;
} EgoEntry;
...
typedef struct str_zhtn {
    I idx, obj, type;
    struct str_zhtn* n;
} * ZHTN;

SUBROUTINE
ZHTN hashget(ht, obj, type) HT ht;
I obj, type;
{
    ZHTN *zhtn = (ZHTN*)HTHASH(ht, obj ^ type), n;
    for (n = *zhtn; n; n = n->n)
        if (obj == n->obj && type == n->type)
            R n;
    R 0;
}

SUBROUTINE
I hashset(ht, obj, type, idx) HT ht;
I obj, type, idx;
{
...
void EgoHashCleanup()
{
    if (EHT_DYNAMIC == Eht.source || EHT_RUNTIME == Eht.source) {
        if (Eht.hashArray)
            mf(Eht.hashArray);
        if (Eht.bucketBuff)
            bufffree(Eht.bucketBuff);
    } else if (EHT_ICE == Eht.source) {
        if (Eht.bucketBuff)
            bfree((char*)Eht.bucketBuff);
...
SUBROUTINE
void EgoHashAdd(cvname, iego) S cvname;
I iego;
{
    I haidx;
...
void EgoHashBuildIfNeeded(type) I type;
{
    ...
    if (EHT_NONE == Eht.source) {
        if (Opts.verbose)
            H("\343 ZPK: No hash table on file.  Building it.\n");
...
I EgoHashLookup(cvname) S cvname;
{
    ...
    for (idx = Eht.hashArray[haidx]; idx; idx = Eht.bucketArray[idx - 1]) {
        ...
        if (cvname == Sthaw(Zpack.Ego[iego].cvname))
            R iego;
    }
    R - 1;
}
...
SUBROUTINE
I AddEgoEntry(cvname, tego) S cvname;
EGOE tego;
{
    I iego;
...
I zargvette(alist) A alist;
{
    ...
    if (qz(alist))
        ...
        R PKARG_SYMVEC;
    else if (issfdups(alist))
        R PKARG_SLOT;
    else if (alist && QA(alist) && Et == alist->t && 1 == alist->r && 2 == alist->n && sym(a0 = (A)alist->p[0]) && sym(a1 = (A)alist->p[1]) && a0->n == a1->n)
        R PKARG_SYMDEST;
    R PKARG_ERROR;
}
...
C* MakeFileName(afn) A afn;
{
    ...
    if (rootname == (C*)0)
        R(C*)
        0;
    if (strrchr((DEV_STRARG)rootname, '.') > strrchr((DEV_STRARG)rootname, '/'))
        R rootname;
...
C* getfilename(afn) A afn;
{
    ...
    I haveOpts = 0;
    if (Ct == afn->t)
        res = MakeFileName(afn);
    else if (Et != afn->t)
        res = (C*)0;
    else if (1 <= afn->n && QS(afn->p[0]))
        res = MakeFileName(afn);
...
{
    if (Zpack.fp)
        fwrite(ptr, sz, n, Zpack.fp);
    else
        buffstuff(Zpack.ResBuff, ptr, sz * n);
}
...
long iceTell()
{
    if (Zpack.fp)
        ...
        R Zpack.ResBuff->put - Zpack.ResBuff->get;
}
...
void writeIce(C* ptr, I sz, I n)
{
    ...
    cpos = 8L - iceTell() % 8L;
    if (8 > cpos)
        iceWrite((C*)IcePad, 1, cpos);
}

SUBROUTINE
long packInIceString(plen, str) long* plen;
C* str;
{
...
SUBROUTINE
void icerAvar(tid, aobj) IDE tid;
A aobj;
{
    tid->ftell = iceTell();
    tid->len = 0;
...
A aobj;
{
    ...
    iceCube[idx++] = aobj->n;
    for (i = 0; i < MAXR; ++i)
        iceCube[idx++] = aobj->d[i];
    for (i = 0; i < aobj->n; ++i)
        iceCube[idx++] = icedAtom(aobj->p[i]);
    tid->type = PK_AET;
    tid->p = (I)aobj;
    tid->ftell = packInIceArray(&tid->len, (C*)iceCube, sizeof(I), len);
    mf(iceCube);
...
SUBROUTINE
void icerAfunc(tid, aobj) IDE tid;
A aobj;
{
    ...
    for (i = 0; i < aobj->r; ++i)
        iceCube[idx + i] = (2 == i && (Xt + 2) == aobj->t || 3 == i && 5 == aobj->r && (Xt + 3 == aobj->t || Xt + 4 == aobj->t))
            ? iced((I)0, (I)PK_ZERO)
            : icedAtom(aobj->d[i]);
    idx += MAXR + 1;
    for (i = 0; i < aobj->n; ++i)
...
SUBROUTINE
void icerAbeam(tid, aobj) IDE tid;
A aobj;
{
    C* fname;
    I beammode, len = 0;
...
SUBROUTINE
void icerPrim(tid, pidx) IDE tid;
I pidx;
{
...
SUBROUTINE
void icerKword(tid, nidx) IDE tid;
I nidx;
{
...
extern C* xfs_name[];
SUBROUTINE
void icerXfunc(tid, xidx) IDE tid;
I xidx;
{
    tid->type = PK_XFUNC;
    tid->p = xidx;
...
SUBROUTINE
void icerExpr(tid, expr) IDE tid;
E expr;
{
    long len = 2 + expr->n;
    I* iceCube = ma(len);
...
SUBROUTINE
void icerLocal(tid, lidx) IDE tid;
I lidx;
{
    tid->type = PK_LOCAL;
    tid->p = lidx;
...
A aobj;
{
    ...
    iceCube[idx++] = aobj->r;
    for (i = 0; i < aobj->r; ++i)
        iceCube[idx + i] = icedAtom(aobj->d[i]);
    tid->type = PK_ADF;
    tid->p = (I)aobj;
    tid->ftell = packInIceArray(&tid->len, (C*)iceCube, sizeof(I), len);
    mf(iceCube);
...
V v;
{
    ...
    tid->type = (isdep ? PKE_VDEP : PKE_VDAT);
    tid->p = (I)v;
    tid->ftell = packInIceArray(&tid->len, (C*)iceCube, sizeof(I), len);
    mf(iceCube);
}
...
{
    IDE tid = &TempId;
    if (0 == obj && !(PK_PRIM == type || PK_KWORD == type || PK_LOCAL == type || PK_XFUNC == type || PK_ZERO == type))
        R iced((I)obj, (I)PK_ZERO);
    (*Icerfunc[type])(tid, obj);
    ++ZpkStats[type].refs;
    ++ZpkStats[type].objs;
    ZpkStats[type].size += tid->len;
    R AddIdEntry(tid);
}
...
I icedAtom(arg) I arg;
{
    ...
    case 0: /* A-object */
        ...
        if (0 == aobj)
            ...
            type = (aplus_nl == aobj) ? PK_NULL : mf_length(aobj) ? PK_ABEAM : -1;
        if (-1 == type)
            type = (Et > aobj->t) ? PK_AVAR : (Et == aobj->t) ? PK_AET : (Xt == aobj->t) ? PK_ADF : PK_AFUNC;
        break;
    case 1: /* global (V) */
        obj = (I)XV(arg);
        type = PK_VREF;
...
SUBROUTINE S Sthaw(iid) I iid;
{
    R XS(thaw(iid));
}

SUBROUTINE CX CXthaw(iid) I iid;
{
    R(CX)
    (thaw(iid));
}
...
#ifdef FUNCNOTUSED
SUBROUTINE E Ethaw(iid) I iid;
{
    R XE(thaw(iid));
}
#endif

SUBROUTINE void thawZero(tid) IDE tid;
{
    tid->p = 0;
}

SUBROUTINE void thawNull(tid) IDE tid;
{
    tid->p = (I)aplus_nl;
}

SUBROUTINE
void thawS(tid) IDE tid;
{
    tid->p = MS(si(Zpack.ice + tid->ftell));
}

SUBROUTINE
void thawCx(tid) IDE tid;
{
    tid->p = (I)cxi(Sthaw(tid->ftell));
}
...
SUBROUTINE
void thawPrim(tid) IDE tid;
{
    tid->p = aplus_pi(Zpack.ice + tid->ftell);
}
...
void thawExpr(tid) IDE tid;
{
    ...
    expr->f = thaw(ptr[1]);
    for (i = 0; i < expr->n; ++i)
        expr->a[i] = thaw(ptr[2 + i]);
    tid->p = ME(expr);
}

SUBROUTINE
void thawXfunc(tid) IDE tid;
{
    tid->p = xslu(Zpack.ice + tid->ftell);
}

SUBROUTINE
void thawLocal(tid) IDE tid;
{
    tid->p = ML(tid->ftell);
}
...
I thaw(iid) I iid;
{
    ...
    if (0 == tid->p && PK_ZERO != tid->type)
        (*Thawfunc[tid->type])(tid);
    R PK_ICABLE(tid->type) ? ic((A)tid->p) : tid->p;
}

SUBROUTINE
void thawGlobalVdat(tid, cxt, v) IDE tid;
CX cxt;
V v;
{
    I* ptr = (I*)(Zpack.ice + tid->ftell);
...
SUBROUTINE
void thawGlobalVdep(tid, cxt, v) IDE tid;
CX cxt;
V v;
{
    I* ptr = (I*)(Zpack.ice + tid->ftell);
...
SUBROUTINE
void thawGlobalData(iid, cxt, v) I iid;
CX cxt;
V v;
{
    I dat = thaw(iid);
...
SUBROUTINE
I thawGlobal(iid, scx, vname) I iid;
S scx;
S vname;
{
    IDE tid = Zpack.Id + iid;
...
I zstoreInitFromIce(astr) A astr;
{
   ...
   * stream if from file */
    if (Zpack.fp) {
        fseek(Zpack.fp, ZpkHeader.Ice.len, 0);
    } else
        buffstuff(Zpack.ResBuff, Zpack.ice, ZpkHeader.Ice.len);
    ...
    for (iid = 0; iid < Zpack.nId; ++iid) {
        ...
        if (PK_CODEDPTR(tid->type) && tid->p)
            tid->p = tid->p & ~aplusMask;
        else if (PK_CODEDIDX(tid->type) && tid->p)
            tid->p = U(tid->p);
...
A zstoreFinishUp()
{
    ...
    for (i = 0; i < Zpack.nId; ++i) {
        if (PK_ICABLE(tid[i].type))
            dc((A)tid[i].p);
        tid[i].p = 0;
    }
...
A zstoreCreate(I zarg, A alist)
{
    ...
    if (PKARG_SYMVEC == zarg)
        zstoreGlobals(alist);
    else if (PKARG_SLOT == zarg)
        zstoreSlot(alist);
    R zstoreFinishUp();
}
...
{
    I i, iego;
    S vname, cvname, scx, destscx, destname;
    CX saveCx = Cx;
    ...
    for (i = 0; i < asym->n; ++i) {
        ...
        if (-1 == iego)
            ...
            if (Opts.verbose)
                H("\343 PKG: Installing %s.%s (%s on file)\n",
...
SUBROUTINE
void
    zretrieveGlobals(zarg, alist) I zarg;
A alist;
{
    switch (zarg) {
    case PKARG_NULL:
        ...
        break;
    case PKARG_SYMVEC:
        zretrieveGlobalsSymvec(alist);
        break;
    case PKARG_SYMDEST:
        zretrieveGlobalsSymDest(alist);
        break;
    default:
        break;
    }
...
A zretrieveSlotSymvec(alist) A alist;
{
    ...
    for (i = 0; i < alist->n; ++i) {
        ...
        if (-1 == iego) {
            ...
            if (Opts.verbose)
                H("\343 PKG: %ld: Retrieving %s for slot\n", i, cvname->n);
...
SUBROUTINE
A zretrieveSlot(zarg, alist) I zarg;
A alist;
{
    switch (zarg) {
    case PKARG_NULL:
...
A zretrieveCatalog()
{
    ...
    z = gv(Et, Zpack.nEgo);
    for (iego = 0; iego < Zpack.nEgo; ++iego)
        z->p[iego] = thaw(Zpack.Ego[iego].cvname);
    R z;
}
...
A zretrievePieces()
{
    ...
    if (EHT_NONE == Eht.source)
        ...
        dim[1] = 2;
        dim[0] = Eht.nBuckets;
        aEgoHash = (A)gvi(Et, 5, gi(Eht.source), gi(Eht.nHash), gi(Eht.nBuckets),
            gc(It, 1, Eht.nHash, &Eht.nHash, Eht.hashArray),
            gc(It, 2, dim[0] * dim[1], dim, (I*)Eht.bucketBuff->min));
    }
    R(A)
    gvi(Et, 5, aHead, aEgo, aId, aIce, aEgoHash);
}

SUBROUTINE
int zretrieveInitFile(afn, cmdstr, pfname) A afn;
C* cmdstr;
C** pfname;
{
    ...
    /* vette arguments */
    if (fname == (C*)0) {
        H("\343 PKG: zretrieve: bad filename argument\n");
        R - 1;
    }
...
void zretrieveCleanup()
{
    ...
    for (iid = 0; iid < Zpack.nId; ++iid)
        if (Zpack.Id[iid].p && PK_ICABLE(Zpack.Id[iid].type))
            dc((A)Zpack.Id[iid].p);
...
A alist;
{
    ...
    /* vette file name */
    if (fname == (C*)0) {
        H("\343 PKG: zfpack: bad first argument\n");
        R gi(1);
    }
    ...
    if (dbg_tpack)
        packtrc(fname, cmdstr, 0);
    R z;
}

ENTRYPOINT
A ep_fadd(afn, alist) A afn;
A alist;
{
    C* fname = getfilename(afn);
    A z;
...
A alist;
{
    ...
    if (-1 == fd)
        R z;
    if (!Opts.pieces)
        zretrieveGlobals(zarg, alist);
    if (Opts.pieces || Opts.result)
        z = zretrievePieces();
    else
        z = (A)gz();
