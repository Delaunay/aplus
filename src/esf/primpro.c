...
I profileDyadic(A a, A w, I i)
{
    ...
    if (typeIdx != -1 && i < sizeP2) {
        (*cpuDyadic)[i][typeIdx][0] += (1000 * (tEnd.tms_utime - tStart.tms_utime)) / clockTicksPerSec;
        (*cpuDyadic)[i][typeIdx][1] += (1000 * (tEnd.tms_stime - tStart.tms_stime)) / clockTicksPerSec;
    }
...
I profileMonadic(A a, I i)
{
    ...
    if (typeIdx != -1 && i < sizeP1) {
        (*cpuMonadic)[i][typeIdx][0] += (1000 * (tEnd.tms_utime - tStart.tms_utime)) / clockTicksPerSec;
        (*cpuMonadic)[i][typeIdx][1] += (1000 * (tEnd.tms_stime - tStart.tms_stime)) / clockTicksPerSec;
    }
