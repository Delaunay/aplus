...
#define ISnamechar(X) ((UseAlnum && ISalnum(X)) || ((X) && NULL != strchr((DEV_STRARG)BeyondZebra, X)))

#define ISalnumA(X) (ISalnum(X) || '.' == (X) || '_' == (X) || '¢' == (X))
#define INCODE (!(incomment || inquote || indquote))
SUBROUTINE
UC* strname(UC* source, I slen, UC* target, I tlen)
{
    int inquote = FALSE;
    int indquote = FALSE;
    int incomment = FALSE;
    ...
    for (s = source; s <= stop; ++s) {
        ...
        default:
            if (INCODE && 0 == bcmp(s, target, tlen))
                if ((s == source || !ISnamechar(*(s - 1))) && !ISnamechar(*(s + tlen)))
                    return (s);
            break;
        }
    }
...
I *pcount, slen, tlen, rowlen;
{
    ...
    if (flags) {
        if (NULL != strchr((DEV_STRARG)flags, 'n'))
            namesonly = TRUE;
    }
    *pcount = 0;
    if (0 == tlen)
    ...
    while (NULL != s) {
        if (namesonly)
            s = strname(s, slen, target, tlen);
        else if (rowlen)
            s = bm_strmat(s, slen, target, tlen, t_bm, source, oslen, rowlen);
        else
            s = bm_strstr(s, slen, target, tlen, t_bm);
        if (q)
            goto OUT;
        if (s) {
            if (qarray == *pcount) {
...
int pos, len;
{
    ...
    if (0 < shift)
        for (i = strlen((DEV_STRARG)s); i >= pos + len; --i)
            *(s + (i + shift)) = *(s + i);
    else if (0 > shift)
        for (i = pos + len; i <= strlen((DEV_STRARG)s); ++i)
            *(s + (i + shift)) = *(s + i);
...
char* flags;
{
    ...
    if (Ct == targetarg->t) {
        if (Ct != replarg->t)
            ERROUT(ERR_TYPE);
        targets = gv(Et, 1);
        targets->p[0] = ic(targetarg);
        repls = gv(Et, 1);
        repls->p[0] = ic(replarg);
    } else {
        targets = (A)(ic(targetarg));
        repls = (A)(ic(replarg));
    }
    ...
    if (Et == targets->t) {
        ...
        if (nhits) {
            ...
            for (i = 0; i < slen; ++i)
                if (0 <= hits[i])
                    rlen += IAF(repls, hits[i], n) - IAF(targets, hits[i], n);
            r1 = (UC*)mab(1 + rlen);
            bzero(r1, 1 + rlen);
            do {
                if (0 <= hits[sidx]) {
...
OUT:
    ...
    if (shapepreserved && slen == rlen && 2 <= asource->r) {
        result->r = asource->r;
        DO(result->r, result->d[i] = asource->d[i]);
    }
    if (r1 != source && NULL != r1)
        mf((I*)r1);
    return (result);
}
...
A targets, repls;
{
    NDC2(targets, repls);
    UseAlnum = TRUE;
    BeyondZebra = DefaultBeyondZebra;
...
SUBROUTINE
A ss_matrix(A source, A hits, I tlen)
{
    A z;
    ...
    e[ld] = 1;
    for (i = ld; 0 <= --i;)
        e[i] = source->d[i + 1] * e[i + 1];
    for (i = count = 0; i < hits->n; ++i)
        if (good >= (hits->p[i] % source->d[ld]))
            ++count;
    z = gm(It, source->r, count);
    for (i = idx = 0; i < hits->n; ++i)
        if (good >= (hits->p[i] % source->d[ld])) {
            for (j = ld; 0 <= j; --j)
                z->p[j * count + idx] = (hits->p[i] / e[j]) % source->d[j];
            ++idx;
        }
...
C* options;
{
    ...
    if (2 <= source->r)
        result = ss_matrix(source, result, target->n);
    return (result ? result : gv(It, 0));
}

ENTRYPOINT
A
    ep_ss(source, target) A source;
A target;
{
    UseAlnum = TRUE;
    BeyondZebra = DefaultBeyondZebra;
...
A targets, repls, namechars;
{
    ...
    if (IsNull(namechars)) {
        ...
        if (Ct != namechars->t)
            ERROUT(ERR_TYPE);
        s = (char*)namechars->p;
        if (namechars->n && 'þ' == s[0])
            UseAlnum = FALSE, BeyondZebra = 1 + s;
        else
            UseAlnum = TRUE, BeyondZebra = s;
        if (IsNull(repls)) {
            if (Ct != targets->t)
                ERROUT(ERR_TYPE);
