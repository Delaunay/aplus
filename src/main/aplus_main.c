...
static void getm()
{
    ...
    if (-1 == select(fileno(stdin) + 1, &read_fd, 0, 0, 0) && Tf) {
        NL, sbi(), pr();
    } else if (FD_ISSET(fileno(stdin), &read_fd)) {
        tf();
    }
}
#endif
...
void aplus_main(long argc, char** argv)
{
    ...
    envinit();
    if (0 != _megsforheap)
        setk1(_megsforheap); /* must be before mem init! */
...
static void startupSyslog(C* version)
{
    ...
    if ((pwd = getpwuid(uid)) && pwd->pw_name)
        syslog(LOG_INFO, pwd->pw_name);
    else
        syslog(LOG_INFO, "user == %d", uid);
    closelog();
    bfree(s);
}
...
static void initReleaseData(C* rcode)
{
    if ('m' == *rcode) {
        ...
        if (!ISdigit(*rcode))
            goto breakout;
        if (0 >= (_majorRelease = (I)strtol(rcode, &rcode, 10)))
            goto breakout;
        if ('.' == *rcode) {
            _phaseOfRelease = POR_MAINTWORK;
            _whoseTree = strdup(1 + rcode);
        } else if (!*rcode)
            _phaseOfRelease = POR_MAINT;
        ...
        if (*rcode) {
            ...
            _phaseOfRelease = POR_DEV;
    } else if (ISdigit(*rcode)) {
        if (0 >= (_majorRelease = (I)strtol(rcode, &rcode, 10)))
            goto breakout;
        if ('a' == *rcode)
            _phaseOfRelease = POR_ALPHA;
        else if ('b' == *rcode)
            _phaseOfRelease = POR_BETA;
        else if ('.' == *rcode)
            _phaseOfRelease = POR_PROD;
        else
            goto breakout;
        ++rcode;
        if (POR_ALPHA == _phaseOfRelease && !*rcode)
            goto breakout;
        else {
            if (!ISdigit(*rcode)) {
                _phaseOfRelease = POR_UNSET;
                goto breakout;
            }
            if (0 > (_minorRelease = (I)strtol(rcode, &rcode, 10)))
                goto breakout;
        }
        if (*rcode) {
            _phaseOfRelease = POR_UNSET;
...
breakout:
    if (POR_UNSET == _phaseOfRelease)
        _majorRelease = _minorRelease = 0;
    return;
}
...
{
    if (_defaultATREE != (char*)(0))
        return;

    { /* Try to set ATREE by argv[0] */
        int i;
        C *s, *st, ss[] = { "/lib/s.+" };
        if (NULL == strchr(argv0, '/'))
            st = (C*)searchPATH(argv0);
        else
            st = strdup(argv0);
...
static I parseargs(argc, argv) register I argc;
register C* argv[];
{
    I isinvalid = 0;
